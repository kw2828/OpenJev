...
| Recurrent world model | Original GRU scaffold plus trained `PredictiveMemory`: recurrent PPO and action-conditioned 1/2/4-step prediction, reward/termination controls and reset diagnostics | Both the [native-start study](recurrent-world-model-study.md) and [cue-visible follow-up](cue-memory-study.md) failed their gates; useful memory, planning and independent transfer remain unestablished. Not Dreamer or an RSSM reproduction |
| Associative policy memory | `AssociativePolicy`: GRU, extra-capacity adapter, global-write and selective-write delta-rule stores; reset and causal replay tests | [Implemented controls and prior art](associative-memory.md); no trained result yet. Fast-weight memory and selective writes are established mechanisms |
