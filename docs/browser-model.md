...
The public GitHub Pages release was also tested in desktop Chrome: model setup took 8.2 seconds and the support example scored successfully and exported its original candidate IDs. The GPU library uses a pinned, integrity-checked CDN copy after the raw GitHub download failed from the public origin. These timings are one integration run. [Public deployment receipt](../evidence/browser-public-smoke.json) · [Public score export](../evidence/browser-public-support-smoke.json).
