...
\maketitle
\begin{abstract}
OpenJev exposes request-defined candidate decisions and a reproducible Doom control testbed. Early Bayesian and causal-memory studies did not establish their proposed mechanisms. Three engineering follow-ups improved a command-window utility, but failed broader continuation gates. Simple firing controls subsequently exposed a limitation of that utility: delayed hits during wait windows could go uncredited despite identical combat outcomes. A separately frozen reinforcement-learning study therefore evaluates kills and duration. Nine PPO/DQN fits received 294,912 Center training interactions; a selected history-PPO family then passed its predeclared combat gate on 1,536 fresh evaluation episodes. It gained 5.184 Center kills and 2.719 Line kills over event-memory rules. A secondary Center comparison favored history-PPO over current-only PPO by 2.188 kills, while all 288 Line confirmation episodes matched an always-fire baseline. The new combat result does not pass the earlier command-utility gates, establish a new RL algorithm, or isolate a general memory mechanism. We release protocols, checkpoints, unsuccessful methods, traces, figures and a browser-local decision playground.
\end{abstract}
...
The memory run completed 1,980 new episodes in 412.13 workstation seconds. Its preserved analysis records the failed continuation gate. The browser demo uses pinned Qwen3-0.6B weights through WebLLM and runs separately from these numerical controllers; it is not a live Doom port or a quality benchmark. The Python package remains \texttt{openjev}. The direct RL study additionally preserves all nine final checkpoints, 294,912 training transitions, and fresh development/confirmation traces. Training examples and optimization compute differ from the older supervised fits.\par
