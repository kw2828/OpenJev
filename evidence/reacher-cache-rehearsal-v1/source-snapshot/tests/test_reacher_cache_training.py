...
def settings(**kwargs):
    return training.CacheTrainingSettings(
...
def test_shared_gru_payload_and_independent_mlp_named_initialization_preserve_global_rng():
    ...
    for item in items[3:]:
        equal_tensors(item.student.state_dict(), initial["states"]["mlp"])
    with torch.no_grad():
...
@pytest.mark.parametrize(
    "original,wrong",
    [
        ("residual_gru", "encoded_current_gru"),
        ("encoded_current_gru", "cached_gru"),
        ("cached_gru", "residual_gru"),
        ("packet_mlp", "encoded_current_gru"),
        ("packet_mlp", "cached_mlp"),
        ("cached_mlp", "packet_mlp"),
    ],
)
...
@pytest.mark.parametrize("kind", ("encoded_current_gru", "cached_gru", "cached_mlp"))
def test_restore_binds_cache_feature_order_even_with_repaired_payload_checksum(kind):
    item = trainer(kind)
    payload = item.export_checkpoint()
    payload["model_configuration"]["public_memory_configuration"]["encoder_feature_order"][0] = "future_angle"
    reseal(payload)
    with pytest.raises(ValueError, match="semantics"):
        restore(item, payload)
...
@pytest.mark.parametrize("kind", ("encoded_current_gru", "cached_gru", "cached_mlp"))
def test_live_cache_mode_cannot_drift_inside_a_registered_class(kind):
    item = trainer(kind)
    item.student._cached = not item.student._cached
    with pytest.raises(ValueError):
        item.train_next()
    assert item.successful_updates == 0 and item.failed
...
@pytest.mark.parametrize("kind", ("cached_gru", "cached_mlp"))
def test_cache_retains_only_last_actual_angle_during_unchanged_loss(kind, monkeypatch):
    item = trainer(kind)
    seen = []
    ...
    packets = item._data["packets"][indices]
    cached = packets[:, 0, :4].clone()
    for step, state in enumerate(seen):
        cached = torch.where(packets[:, step, 6:7] == 1, packets[:, step, :4], cached)
        torch.testing.assert_close(state["cached_angles"], cached, rtol=0, atol=0)
        torch.testing.assert_close(state["packet"], packets[:, step], rtol=0, atol=0)



@pytest.mark.parametrize("kind", ("encoded_current_gru", "cached_gru", "packet_mlp", "cached_mlp"))
def test_current_packet_models_erase_poisoned_prior_prediction_in_training_prefix(kind, monkeypatch):
    ...
    def capture(state, packet):
        ...
        for key in ("packet", "hidden", "encoder_features"):
            if key in bad:
                bad[key] = torch.full_like(bad[key], float("nan"))
...
def test_operation_accounting_reports_all_five_models_without_compute_matching():
    ...
    for kind in ("encoded_current_gru", "cached_gru"):
        assert rows[kind]["work"]["operations"]["gru_cell_sample_calls"] == a + d
    assert rows["cached_mlp"]["work"]["operations"]["gru_cell_sample_calls"] == 0
