...
Four fresh engineering episodes, fifteen tiny fits and all 60 controller rows.
...
NAMESPACE = "reacher-cache-engineering-whole-tree-v1"
SOURCE_NAMESPACE = "reacher-cache-engineering-capacity-v1"
...
def prepare_fixture(folder, *, cap_seconds=600, audit_cap_seconds=600):
    ...
    memory = study.read(study.checked(study.ROOT / study.MEMORY_PLAN, study.MEMORY_PLAN_SHA))
    study.write(source / "memory-protocol-only.json", {"scope": SCOPE,
        "plan_path": study.MEMORY_PLAN, "plan_sha256": study.MEMORY_PLAN_SHA,
    ...
    plan.update(rng_namespace=NAMESPACE,
        ...
        objective_source=memory["objective_source"],
        memory_source={"plan_path": study.MEMORY_PLAN, "plan_sha256": study.MEMORY_PLAN_SHA,
            "audit_path": str(source / "memory-protocol-only.json"),
            "audit_receipt_sha256": study.sha(source / "memory-protocol-only.json"),
    ...
    study.write(folder / "fixture.json", {"scope": SCOPE, "plan_sha256": digest,
        "production_authentication": False, "new_models": 15, "control_rows": 60,
        "training_rows": 4, "namespace": NAMESPACE, "source_namespace": SOURCE_NAMESPACE,
        "inherited_checkpoint_calls": 0})
    return CacheFixture(folder, plan, path, digest, folder / "execution", source_plan, memory)
...
def fixture_audit_boundary(case):
    import audit_reacher_cache_study as auditor
