"""Prospective protocol helpers for the four-arm Reacher memory comparison.
...
SCORED_NAMESPACE = "reacher-memory-ablation-v1-scored"
ENGINEERING_NAMESPACES = (
    "reacher-memory-engineering-unit-v1",
    "reacher-memory-engineering-runner-v1",
    "reacher-memory-engineering-capacity-v1",
    "reacher-memory-engineering-whole-tree-v1",
)
...
CRITERION = {
    ...
    "persistent_vs_current": {"mean_improvement": .05, "every_pair_strictly_better": True},
    "persistent_vs_bounded": {"panel": "shift", "mean_improvement": .03, "every_pair_nonworse": True},
    "full_sensing_vs_current": {"maximum_mean_degradation": .02},
    "persistent_vs_mlp": {"gap_family_means_nonworse": True},
    "competence": {
...
def fit_manifest(plan):
    """Three paired quadruples, rotating arm order without dropping any fit."""
    require(plan["arms"] == list(ARMS) and plan["pairs"] == list(PAIRS), "All twelve paired fits required")
    rows = []
    for index, pair in enumerate(PAIRS):
        for arm in (*ARMS[index:], *ARMS[:index]):
            family = "mlp" if arm == "packet_mlp" else "gru"
...
def execution_order(plan):
    """Exactly 36 learned rows plus 15 reference rows; no reset intervention."""
...
def validate_settings(plan, *, engineering=False):
    ...
    require(isinstance(plan, dict) and set(expected) <= set(plan), "Missing memory-study settings")
    ...
    for key, value in expected.items():
        if key not in flexible:
            require(_identity(plan[key]) == _identity(value), f"Memory setting changed: {key}")
...
def _namespace(plan):
    ...
    require(value in (SCORED_NAMESPACE, *ENGINEERING_NAMESPACES), "Declared memory RNG namespace required")
    return value
...
def stream_contract(plan, prior_registries=(), prior_torch_registries=(), prior_plans=()):
    ...
    return {
        ...
        "intentional_reuse": [
            "The three GRU classes share initial parameter tensors per pair; the packet MLP has a separate named seed.",
            "All four arms in a pair share the same complete minibatch order; all inherited training episodes are shared.",
