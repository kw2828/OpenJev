...
import audit_reacher_memory_study as audit
...
def fit(arm):
    cfg = plan()
    settings = training.MemoryTrainingSettings(**{key: cfg[key] for key in audit.TRAINING_FIELDS})
...
def test_tensor_only_initialization_reconstruction_matches_actual_constructor(arm):
    ...
    settings = training.MemoryTrainingSettings(**{key: cfg[key] for key in audit.TRAINING_FIELDS})
    initial = training.make_initialization(410, 410, settings)
    expected = initial["states"]["mlp" if arm == "packet_mlp" else "gru"]
...
@pytest.mark.parametrize("original,wrong", [
    ("residual_gru", "current_gru"), ("current_gru", "bounded_gru"), ("bounded_gru", "residual_gru")])
...
def test_training_log_arithmetic_and_public_masks_are_independently_checked(mutation, tmp_path):
    cfg, trainer, initial, order, rows, data = fit("bounded_gru")
    body = check_checkpoint(cfg, trainer, initial, order)
    ...
    if mutation == "order":
        ...
        rows[0]["work"]["operations"]["replayed_transition_samples"] += 1
    elif mutation == "clip":
    ...
    write_logs(path, rows)
    with pytest.raises(ValueError):
        audit.audit_logs(cfg, path, "bounded_gru", order, body, packets=data["packets"].numpy())
...
def state_fixture(tmp_path, arm):
    ...
    cfg["steps"] = 50
    data = public_data(cfg)
    width = {"width": cfg["mlp_width"]} if arm == "packet_mlp" else {"hidden_size": cfg["hidden_size"]}
...
@pytest.mark.parametrize("mutation", ["stale_packet", "old_history", "wrong_action", "startup_mask", "imagined_root", "candidate_carry", "missing_reset", "missing_persistence", "state_hash", "work"])
def test_state_history_action_phase_and_cost_corruption_rejected(mutation, tmp_path):
    arm = "current_gru" if mutation == "missing_reset" else "residual_gru" if mutation == "missing_persistence" else "bounded_gru"
    ...
    if mutation == "stale_packet":
        ...
        arrays["root__real_packets"][:, 5, 0] = records[0]["policy"]["packets"][0]
    elif mutation == "wrong_action":
        arrays["carried__real_actions"][:, 5, 0] += .1
    elif mutation == "startup_mask":
        arrays["root__real_valid"][:, 0, 0] = True
...
def costs():
    rows = {}
    means = {"residual_gru": 8., "current_gru": 10., "bounded_gru": 9., "packet_mlp": 11.,
...
def test_all_twenty_five_checks_and_descriptive_kinematic_reference_retained():
...
def test_one_losing_seed_cannot_be_hidden_by_successful_family_mean():
    ...
    assert checks["ordinary: persistent mean improves current by 5%"]["passed"] is True
    assert checks["ordinary/pair2: persistent strictly beats current"]["passed"] is False
    assert gate["passed"] is False
...
@pytest.mark.parametrize("endpoint", ["current_five_percent", "bounded_three_percent", "full_two_percent"])
def test_inclusive_mean_thresholds_and_immediately_worse_values(endpoint):
    controls = costs()
    if endpoint == "current_five_percent":
        panel, limit, name = "ordinary", .95 * 10., "ordinary: persistent mean improves current by 5%"
    elif endpoint == "bounded_three_percent":
        panel, limit, name = "shift", .97 * 9., "shift: persistent mean improves bounded by 3%"
    else:
        panel, limit, name = "full", 1.02 * 10., "full: persistent mean degrades current at most 2%"
    ...
    worse = float(np.nextafter(limit, np.inf))
    for pair in protocol.PAIRS:
...
def test_auditor_source_does_not_import_or_construct_learned_components():
    ...
    for node in ast.walk(source):
        if isinstance(node, ast.Import):
            ...
            imported.extend(alias.name for alias in node.names)
    forbidden = ("memory_training", "memory_control", "world_models", "observation_baseline", "bounded_history")
