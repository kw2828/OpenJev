"""Prospective memory comparison with explicit model classes and fixed CEM.
...
import numpy as np
import reacher_objective_study as previous
import torch
...
OBJECTIVE_PLAN = "evidence/reacher-objective-ablation-v1/protocol/plan.json"
OBJECTIVE_PLAN_SHA = "dca778153230375a5a0e790390d328d69857dd4c2d3260b61fb9f214fa1d32f2"
OBJECTIVE_AUDIT = "evidence/reacher-objective-ablation-v1/audit/receipt.json"
OBJECTIVE_AUDIT_SHA = "ee5f68ba9c194d39695c16d257bb05e3f294488a14c025e50a9e881ce4280dc8"
NEW_SOURCES = (
    "src/openjev/research/reacher_observation_baseline.py",
    "tests/test_reacher_observation_baseline.py",
    "src/openjev/research/reacher_bounded_history.py",
    "tests/test_reacher_bounded_history.py",
    "src/openjev/research/reacher_memory_training.py",
    "tests/test_reacher_memory_training.py",
    "src/openjev/research/reacher_memory_control.py",
    "tests/test_reacher_memory_control.py",
    "src/openjev/research/reacher_memory_protocol.py",
    "tests/test_reacher_memory_protocol.py",
    "src/openjev/research/reacher_kinematic_control.py",
    "tests/test_reacher_kinematic_control.py",
    "scripts/reacher_memory_study.py",
    "tests/test_reacher_memory_study.py",
    "scripts/audit_reacher_memory_study.py",
    "tests/test_audit_reacher_memory_study.py",
)
SOURCES = previous.SOURCES + NEW_SOURCES


def settings_from_plan(plan):
    names = {field.name for field in fields(training.MemoryTrainingSettings)}
    require(names <= set(plan), "Complete memory training settings")
    return training.MemoryTrainingSettings(**{name: plan[name] for name in names})


def authenticated_sources():
    source, _ = previous.authenticated_sources()
    plan = read(checked(ROOT / OBJECTIVE_PLAN, OBJECTIVE_PLAN_SHA))
    receipt = read(checked(ROOT / OBJECTIVE_AUDIT, OBJECTIVE_AUDIT_SHA))
    require(receipt["status"] == "completed" and receipt["saved_output_only"] is True
            and receipt["plan_sha256"] == OBJECTIVE_PLAN_SHA
            and receipt["source_sha256"] == plan["sources"], "Completed objective lineage")
    for name, digest in plan["sources"].items():
        checked(ROOT / name, digest)
    for name, digest in receipt["files"].items():
        checked((ROOT / OBJECTIVE_AUDIT).parent / name, digest)
    return source, {
        "plan_path": OBJECTIVE_PLAN, "plan_sha256": OBJECTIVE_PLAN_SHA,
        "audit_path": OBJECTIVE_AUDIT, "audit_receipt_sha256": OBJECTIVE_AUDIT_SHA,
...
def prior_streams():
    ...
    parent = read(checked(ROOT / OBJECTIVE_PLAN, OBJECTIVE_PLAN_SHA))
    contract = parent["random_stream_contract"]
    numpy_prior.append(contract["registry"])
    torch_prior.append(contract["torch_registry"])
    descriptors.append({"plan_path": OBJECTIVE_PLAN, "plan_sha256": OBJECTIVE_PLAN_SHA,
    ...
    for excluded in contract["engineering_exclusions"]:
        ...
        descriptors.append({"plan_path": OBJECTIVE_PLAN, "plan_sha256": OBJECTIVE_PLAN_SHA,
                            "namespace": excluded["namespace"], "registry_kind": "objective-engineering"})
    # Literal engineering data/construction seeds. The new source tests are
    # frozen with the protocol; no scientific RNG is used to generate fixtures.
    numpy_prior.append({"memory/engineering/fixture": 410})
    torch_prior.append({"memory/engineering/fixture": 410, "memory/engineering/restore": 0})
    descriptors.append({"registry_kind": "memory-engineering-literal", "numpy_seeds": [410],
                        "torch_seeds": [0, 410]})
...
def prepare(out, *, cap_seconds, audit_cap_seconds):
    ...
    source, lineage = authenticated_sources()
    plan = protocol.settings()
    plan.update(training_source=source, objective_source=lineage, cap_seconds=cap_seconds,
...
def validate(path, expected):
    ...
    protocol.validate_settings(plan)
    require(set(plan["sources"]) == set(SOURCES), "Exact memory source membership")
    for name, digest in plan["sources"].items():
        checked(ROOT / name, digest)
    require(plan["runtime"] == base.runtime(), "Frozen runtime changed")
    source, lineage = authenticated_sources()
    require(plan["training_source"] == source and plan["objective_source"] == lineage, "Frozen lineage")
...
def run_validated(plan, expected, out, *, begin=None, final_validate=None, output_created=False):
    ...
    try:
        ...
        costs = {"training_preparation_seconds": read(out / "training-prepared.json")["wall_seconds"],
            ...
            "prior_costs": plan["objective_source"]["prior_costs"],
        ...
        write(out / "completed.json", {"status": "completed", "study": STUDY, "plan_sha256": expected,
            ...
            "cumulative_attempt_wall_seconds": plan["objective_source"]["prior_costs"]["cumulative_attempt_wall_seconds"]
                + total, "files": members})
        check_cap(deadline)
