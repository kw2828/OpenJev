"""Isolated actual-class control/prediction for four real-history comparators.
...
from openjev.research.reacher_bounded_history import parameter_and_operation_accounting
from openjev.research.reacher_memory_training import REGISTRY, SEMANTICS
...
VERSION = "reacher-memory-control-engineering-v1"
...
def model_identity(model, plan):
    ...
    width = model.width if kind == "packet_mlp" else model.hidden_size
    width_key = "mlp_width" if kind == "packet_mlp" else "hidden_size"
...
def work_accounting(model, *, assimilate_samples, advance_samples):
    ...
    if kind == "bounded_gru":
        result = parameter_and_operation_accounting(
            model, assimilate_samples=assimilate_samples, advance_samples=advance_samples
        )
    elif kind in ("current_gru", "packet_mlp"):
...
def learned_control(plan, model, panel, inputs_by_step, out, deadline=float("inf"), progress=None, *, cases):
    ...
    try:
        ...
        for step in range(HORIZON):
            ...
            # terminal state. This is the bounded model's one-advance real phase.
