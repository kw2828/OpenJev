"""Independent saved-output audit of the prospective Reacher explicit-cache comparison.
...
import audit_reacher_memory_study as inherited
...
VERSION = "reacher-cache-ablation-v1"
TRAINING_VERSION = "reacher-cache-training-engineering-v1"
CONTROL_VERSION = "reacher-cache-control-engineering-v1"
TRAINING_FIELDS = (
    "hidden_size",
    "mlp_width",
    "dt",
...
SEMANTICS = {
    ...
    "encoded_current_gru": "zero hidden and encode every sanitized real packet, including missing; recurrent imagined advances",
    "cached_gru": "zero hidden and encode last actual angles with current goal/validity/age; recurrent imagined advances",
    "packet_mlp": "overwrite state with every real packet; action-conditioned imagined packets only",
    "cached_mlp": "encode last actual angles with current goal/validity/age at real root; predicted packets only during imagination",
}
NEW_SOURCES = {
...
_DEADLINE = ContextVar("cache_audit_deadline", default=float("inf"))
...
def check_budget():
    if time.monotonic() > _DEADLINE.get():
        raise TimeoutError("Frozen cache audit cap exceeded")
...
def public_memory_configuration(plan, arm):
    if arm in ("residual_gru", "packet_mlp"):
        return None
    cached = arm in ("cached_gru", "cached_mlp")
    recurrent = arm != "cached_mlp"
    size = {"hidden_size": plan["hidden_size"]} if recurrent else {"width": plan["mlp_width"]}
    return {
        "version": "public-angle-cache-v1", "model_class": protocol.MODEL_CLASSES[arm],
        **size, "dt": plan["dt"], "noise_std": plan["noise_std"], "residual_reward": True,
        "encoder_feature_order": [
            ...
            "current_target_x", "current_target_y", "actual_validity", "actual_age_seconds"],
        ...
        "age_validation": {"rtol": 1e-5, "atol": 1e-6, "visible_age": 0.0},
        "imagined_rollout": "private action-conditioned state; no real-clock or cache updates",
        "state_keys": sorted(state_shapes(plan, arm)),
        "run_status_authority": "enclosing protocol and execution receipts",
    }
...
def model_configuration(plan, arm):
    return {
        ...
        "width": plan["mlp_width"] if arm in ("packet_mlp", "cached_mlp") else plan["hidden_size"],
        "dt": plan["dt"],
        "noise_std": plan["noise_std"],
        "residual_reward": True,
        "real_assimilation": SEMANTICS[arm],
        "public_memory_configuration": public_memory_configuration(plan, arm),
...
def shapes(plan, arm):
    if arm not in ("packet_mlp", "cached_mlp"):
...
def operations(plan, arm, a, d):
    require(arm in protocol.ARMS, "Declared cache arm")
    h, w = plan["hidden_size"], plan["mlp_width"]
    mlp = arm in ("packet_mlp", "cached_mlp")
    result = {
        "gru_cell_sample_calls": 0 if mlp else a + d,
        "linear_layer_sample_calls": (6 if mlp else 4) * d,
        "analytic_reward_sample_calls": d,
        "dense_affine_macs": d * (3 * w * w + 17 * w)
        if mlp else a * 3 * h * (h + 8) + d * (5 * h * h + 25 * h),
    ...
    if arm in ("encoded_current_gru", "cached_gru", "cached_mlp"):
        result.update(real_boundary_validation_samples=a,
                      cache_update_opportunity_samples=a if arm in ("cached_gru", "cached_mlp") else 0)
    return result
...
def state_shapes(plan, arm):
    require(arm in protocol.ARMS, "Declared cache state arm")
    result = {"packet": (8,)}
    if arm not in ("packet_mlp", "cached_mlp"):
        result["hidden"] = (plan["hidden_size"],)
    if arm in ("encoded_current_gru", "cached_gru", "cached_mlp"):
        result.update(real_index=(1,), last_visible_index=(1,), real_target=(2,), imagined_depth=(1,))
    if arm in ("cached_gru", "cached_mlp"):
        result["cached_angles"] = (4,)
    if arm == "cached_mlp":
        result["encoder_features"] = (8,)
    return result


def state_dtype(name):
    return np.int64 if name in ("real_index", "last_visible_index", "imagined_depth") else np.float32


def audit_cache_buffers(plan, values, public, next_packet, arm):
    """Pure public-prefix reconstruction; no learned encoder is replayed."""
    n, t = public.shape[:2]
    require(np.isin(public[..., 6], (0, 1)).all()
            and (public[:, 0, 6] == 1).all()
            and (public[..., :4][public[..., 6] == 0] == 0).all(),
            "Actually observed startup and sanitized public packets")
    observed = public[..., 6] == 1
    indices = np.broadcast_to(np.arange(t, dtype=np.int64), (n, t))
    latest = np.maximum.accumulate(np.where(observed, indices, -1), axis=1)
    cached = public[np.arange(n)[:, None], latest, :4]
    age = (indices - latest).astype(np.float32) * np.float32(plan["dt"])
    require(np.allclose(public[..., 7], age, rtol=1e-5, atol=1e-6)
            and (public[..., 7][observed] == 0).all(), "Public age from actual measured indices")
    require(np.array_equal(public[..., 4:6], np.broadcast_to(public[:, :1, 4:6], (n, t, 2))),
            "Static public target")
    for phase in ("root", "carried"):
        require(np.array_equal(values[f"{phase}__real_index"], indices[..., None])
                and np.array_equal(values[f"{phase}__last_visible_index"], latest[..., None])
                and np.array_equal(values[f"{phase}__real_target"], public[..., 4:6]),
                "Public real-clock and last-measurement index reconstruction")
        require(np.all(values[f"{phase}__imagined_depth"] == (0 if phase == "root" else 1)),
                "Real root and carried one-action phase")
        if arm in ("cached_gru", "cached_mlp"):
            require(np.array_equal(values[f"{phase}__cached_angles"], cached),
                    "Exact last actually observed angles; imagination cannot update cache")
    if arm == "cached_mlp":
        expected = np.concatenate((cached, public[..., 4:]), axis=-1)
        require(np.array_equal(values["root__encoder_features"], expected)
                and np.array_equal(values["carried__encoder_features"], next_packet),
                "Cached MLP root/imagined encoder features")
...
def audit_states(plan, folder, records, arm, fit):
    ...
    for phase in ("root", "carried"):
        for name, shape in schema.items():
            dtype = state_dtype(name)
            array(values[f"{phase}__{name}"], (n, t, *shape), dtype, "State schema")
    public = base.stack(records, "policy", "packets")
    ...
    if arm in ("encoded_current_gru", "cached_gru", "cached_mlp"):
        audit_cache_buffers(plan, values, public[:, :t], next_packet, arm)
...
def audit_prediction(plan, records, values, meta, arm, fit):
    ...
    require(plan["prediction_horizons"] == plan["auxiliary_horizons"], "Explicit matching prediction horizons")
    ...
    schema = state_shapes(plan, arm)
    root_bytes = n * sum(
        math.prod(shape) * np.dtype(state_dtype(name)).itemsize
        for name, shape in schema.items()
    )
    require(
...
def control_qualification(plan, controls):
    require(plan["criterion"] == protocol.CRITERION
            and plan["secondary_contrasts"] == protocol.SECONDARY_CONTRASTS
            and set(controls) == set(protocol.PANELS), "Frozen cache continuation criteria")
    ...
    for rows in controls.values():
        ...
        for row in rows.values():
            ...
            require(value.shape == (plan["control_episodes"],) and np.isfinite(value).all()
    ...
    def mean(panel, arm):
        return np.mean([controls[panel][f"{arm}-{pair}"]["episode_costs"]
                        for pair in protocol.PAIRS], axis=0)
    ...
    for panel in ("ordinary", "shift"):
        ...
        for comparator in ("cached_gru", "cached_mlp"):
            add(f"{panel}: persistent mean improves {comparator} by 3%",
                residual.mean(), .97 * mean(panel, comparator).mean())
            for pair in protocol.PAIRS:
                add(f"{panel}/{pair}: persistent nonworse than {comparator}",
                    controls[panel][f"residual_gru-{pair}"]["mean_cost"],
                    controls[panel][f"{comparator}-{pair}"]["mean_cost"])
        zero = controls[panel]["zero"]["mean_cost"]
        for pair in protocol.PAIRS:
            add(f"{panel}/{pair}: persistent beats zero by 10%",
                controls[panel][f"residual_gru-{pair}"]["mean_cost"], .90 * zero)
        for reference in ("known_state", "particle"):
            add(f"{panel}/{reference}: physics beats zero by 10%",
                controls[panel][reference]["mean_cost"], .90 * zero)
    for comparator in ("cached_gru", "cached_mlp"):
        add(f"full: persistent mean degrades {comparator} at most 2%",
            mean("full", "residual_gru").mean(), 1.02 * mean("full", comparator).mean())
    for panel in protocol.PANELS:
        for arm in protocol.ARMS:
            if arm != "residual_gru":
                differences[panel][f"persistent_minus_{arm}"] = (
                    mean(panel, "residual_gru") - mean(panel, arm)).tolist()
        for contrast in protocol.SECONDARY_CONTRASTS:
            differences[panel][contrast["name"]] = (
                mean(panel, contrast["treatment"]) - mean(panel, contrast["control"])).tolist()
        differences[panel]["persistent_minus_public_kinematic"] = (
            mean(panel, "residual_gru") - np.asarray(controls[panel]["public_kinematic"]["episode_costs"])).tolist()
    require(len(checks) == 28, "All28 prescribed checks")
    return {"passed": all(row["passed"] for row in checks), "checks": checks,
            "public_kinematic": "descriptive_only",
            "secondary_contrasts": "descriptive_only; never rescue a failed primary gate",
            "fail_interpretation": protocol.CRITERION["fail_interpretation"]}, differences
...
def validate_lineage(plan, execution):
    lineage = plan["memory_source"]
    require(set(lineage) == {"plan_path", "plan_sha256", "audit_path", "audit_receipt_sha256",
                             "prior_costs", "training_weights_reused", "previous_scientific_gate_passed"},
            "Memory lineage schema")
    require(lineage["plan_sha256"] == "05f09ee5425190f5d652aedb10af1641037035d85e906d04d86baf33552a5786"
            and lineage["audit_receipt_sha256"] == "2589383326a2312d7f738821da7fd11f82b9bee1b7168aba2e7f44aaa4946d5d",
            "Exact completed memory parent")
    ...
    require(receipt["status"] == "completed" and receipt["saved_output_only"] is True
            ...
            and lineage["previous_scientific_gate_passed"] is True,
            "Completed memory identity; no reused weights")
    for name, digest in receipt["files"].items():
        checked(receipt_path.parent / name, digest)
    summary = read(receipt_path.parent / "summary.json")
    require(summary["continuation_gate"]["passed"] is True
            and len(summary["continuation_gate"]["checks"]) == 25
            and all(row["passed"] for row in summary["continuation_gate"]["checks"]),
            "Authenticated parent continuation passed")
    for name, digest in parent["sources"].items():
        checked(ROOT / name, digest)
    require(plan["training_source"] == parent["training_source"]
            and plan["objective_source"] == parent["objective_source"],
            "Unchanged inherited training and objective source")
    cohort, _ = inherited.validate_lineage(parent, execution)
...
def inherited_memory_priors(parent, objective_lineage):
    ...
    return numpy_prior, torch_registries, descriptors


def validate_streams(plan, parent, execution):
    objective_lineage = parent["objective_source"]
    objective = read(checked(ROOT / objective_lineage["plan_path"], objective_lineage["plan_sha256"]))
    numpy_prior, torch_prior, descriptors = inherited_memory_priors(objective, objective_lineage)
    require(parent["random_stream_contract"] == memory_protocol.stream_contract(
        parent, numpy_prior, torch_prior, descriptors), "Authenticated memory prior registry reconstruction")
    contract, lineage = parent["random_stream_contract"], plan["memory_source"]
    numpy_prior.append(contract["registry"])
    ...
    descriptors.append({"registry_kind": "cache-engineering-literal", "numpy_seeds": [410],
                        "torch_seeds": [0, 410]})
    expected = protocol.stream_contract(plan, numpy_prior, torch_prior, descriptors)
    require(plan["random_stream_contract"] == expected
            and read(execution / "random-streams.json") == expected,
    ...
    return {"numpy_generators": len(expected["generators"]),
            ...
            "prior_numpy_registries": len(numpy_prior), "prior_torch_registries": len(torch_prior),
...
def validate_members(plan, expected, execution):
    ...
    require(
        ...
        "Complete cache execution identity",
    )
    for key, value in (
        ("fits", 15),
        ("control_rows", 60),
...
def audit_boundaries(plan, expected, execution, completed, fits):
    ...
    require(
        ...
        "All15 fits authenticated before restore",
    )
    require(set(restored["models"]) == set(plan["fit_order"]), "All15 actual models restored")
    ...
    return {
        "all_fifteen_fits_restored_before_evaluation": True,
...
def audit_costs(plan, execution, completed, initializations, fits, boundary, predictions, controls):
    ...
    exact_integer(saved["new_fits"], 15, "All15 fits charged")
    ...
    require(saved["prior_costs"] == plan["memory_source"]["prior_costs"], "Prior cumulative cost identity")
...
def audit_saved(plan, expected_plan_sha256, execution, out, *, engineering=False):
    ...
    require(not out.exists(), "Exclusive cache audit output required")
    ...
    try:
        ...
        (out / "README.md").write_text(
            "# Reacher explicit-cache saved-output audit\n\n"
            f"Continuation: **{'PASS' if gate['passed'] else 'FAIL'}** ({sum(row['passed'] for row in gate['checks'])}/28 checks).\n\n"
            f"All 15 fits and 60 control rows retained; {summary['native_transitions_checked']:,} native outcome transitions replayed. "
