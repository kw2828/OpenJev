"""Hand-built finite-world data, independent arithmetic, and fresh cost-readout replication gates."""
...
import audit_finite_cost_readout_replication as a


@pytest.fixture(autouse=True)
def engineering_namespace(monkeypatch):
    monkeypatch.setattr(a, 'NAMESPACE', 933001)
    monkeypatch.setattr(a, 'SEEDS', (933101, 933102, 933103))
    monkeypatch.setattr(a, 'CONFIG', {**a.CONFIG, 'seed_namespace': 933001, 'fit_seeds': [933101, 933102, 933103]})
...
def test_gates_require_each_seed_and_horizon_and_strictly_positive_reference(bad):
    ...
    if bad == 'short_seed':
        ...
        return
    result = a.gates(rows, baselines, counts)
    assert any(not criterion['passed'] for criterion in result['fixed_softened'].values())
...
def continuation_fixture():
    ...
    for row in rows:
        if row['arm'] == 'learned_readout':
            row['blind_regret'] = .01
    return rows, baselines, counts


def test_continuation_requires_all_six_pairs_and_does_not_rescue_blind_gate():
    rows, baselines, counts = continuation_fixture()
    # A blind MSE failure is still reported, while the narrower continuation
    # requires short/observed criteria and paired regret, not a blind pass.
    selected = next(row for row in rows if (row['arm'], row['seed'], row['horizon'])
                    == ('learned_readout', a.SEEDS[0], 4))
    selected['blind_cost_mse'] = .06
    result = a.replication_continuation(rows, baselines, counts)
    assert result['passed'] and result['status'] == 'REPLICATION_CONTINUATION_PASS'
    assert len(result['conditions']) == 8 and len(result['paired_regret']) == 6
    assert not result['blind_extrapolation_passed']
    assert result['requires_separate_followup_registration']
    assert result['exact_cost_anchor_repeated'] is False
    assert all(pair['difference'] == pytest.approx(-.01) for pair in result['paired_regret'])
    assert all(pair['relative_reduction'] == pytest.approx(.5) for pair in result['paired_regret'])


@pytest.mark.parametrize('bad', ['h4_tie', 'h8_worse', 'short', 'observed', 'support', 'zero_reference'])
def test_continuation_failure_cannot_be_hidden_by_means_or_other_seeds(bad):
    rows, baselines, counts = continuation_fixture()
    find = lambda arm, h: next(row for row in rows if (row['arm'], row['seed'], row['horizon'])
                              == (arm, a.SEEDS[1], h))
    if bad == 'h4_tie':
        find('learned_readout', 4)['blind_regret'] = .02
    elif bad == 'h8_worse':
        find('learned_readout', 8)['blind_regret'] = .021
    elif bad == 'short':
        find('learned_readout', 2)['observed_kl'] = .101
    elif bad == 'observed':
        find('learned_readout', 4)['observed_kl'] = .101
    elif bad == 'support':
        counts['base'] = 63
    else:
        find('learned_readout', 8)['blind_regret'] = 0
        find('fixed_softened', 8)['blind_regret'] = 0
    result = a.replication_continuation(rows, baselines, counts)
    assert not result['passed'] and result['status'] == 'REPLICATION_CONTINUATION_FAIL'
    assert not all(result['conditions'].values())
    if bad == 'zero_reference':
        pair = next(row for row in result['paired_regret'] if row['seed'] == a.SEEDS[1] and row['horizon'] == 8)
        assert pair['relative_reduction'] is None and not pair['passed']


def test_continuation_rejects_nonfinite_or_missing_pairs():
    rows, baselines, counts = continuation_fixture()
    row = next(row for row in rows if row['arm'] == 'learned_readout' and row['horizon'] == 4)
    row['blind_regret'] = float('nan')
    with pytest.raises(ValueError):
        a.replication_continuation(rows, baselines, counts)
    rows.pop()
    with pytest.raises(ValueError):
        a.replication_continuation(rows, baselines, counts)
...
def work_fixture():
    ...
    for arm in a.ARMS:
        ...
        for route in a.ROUTES:
            ...
            if arm == 'learned_readout' and not is_prefix:
...
def test_full_work_reconstruction_uses_attempt_orders_and_both_loss_populations(bad):
    ...
    if bad == 'blind_absorbed':
        ...
        work['learned_readout']['training_blind']['cost_head_softmax_calls'] = 0
    elif bad == 'head_in_prefix':
        work['learned_readout']['training_prefix']['cost_head_softmax_calls'] = 1
    elif bad == 'head_rows_as_batch':
        work['learned_readout']['evaluation_blind']['cost_head_probability_rows'] = 2 * 24
    if bad:
        with pytest.raises(ValueError):
...
def snapshot(matrix):
    """Independent serialization fixture using NumPy bytes, not the audit codec."""
...
def head_fixture(arm):
    ...
    # Explicit Gray-code decision labels, rather than using audit formula.
    ...
    return {'parameter_metadata': {'parameters': parameters, 'parameter_count': 1120 if learned else 1088,
        'parameter_bytes': 8960 if learned else 8704, 'buffer_bytes': 0 if learned else 256},
        'readout_initial': initial, 'readout_final': copy.deepcopy(initial)}


@pytest.mark.parametrize('arm', ('fixed_softened', 'learned_readout'))
def test_head_snapshots_preserve_cost_sign_gray_mapping_storage_and_endianness(arm):
    row = {'arm': arm, **head_fixture(arm)}
    a.validate_head_metadata(row)
    assert row['readout_initial']['matrix'][0][0] < 0
    assert np.asarray(row['readout_initial']['matrix']).sum(0) == pytest.approx(np.zeros(8), abs=1e-15)
    if arm == 'learned_readout':
        # A changed yet bounded readout is allowed. It is not evidence that
        # latent coordinates recover the true hidden-state labels.
        row['readout_final'] = snapshot(np.roll(row['readout_final']['matrix'], 1, axis=0))
        a.validate_head_metadata(row)


@pytest.mark.parametrize('bad', ['sign', 'hash', 'centering', 'bound', 'nonfinite', 'shape',
                               'fixed_changed', 'learned_wrong_initial', 'parameter_storage'])
def test_head_snapshot_guards_fail_closed(bad):
    row = {'arm': 'learned_readout', **head_fixture('learned_readout')}
    if bad == 'sign':
        row['readout_initial'] = snapshot(-np.asarray(row['readout_initial']['matrix']))
    elif bad == 'hash':
        row['readout_final']['sha256'] = '1' * 64
    elif bad == 'centering':
        value = np.asarray(row['readout_final']['matrix'])
        value[1, 0] -= .01
        row['readout_final'] = snapshot(value)
    elif bad == 'bound':
        value = np.asarray(row['readout_final']['matrix'])
        value[:, 0] = [-.8, .25, .25, .3]
        row['readout_final'] = snapshot(value)
        ...
        row['readout_final']['matrix'].pop()
    elif bad == 'fixed_changed':
        row = {'arm': 'fixed_softened', **head_fixture('fixed_softened')}
        row['readout_final'] = snapshot(np.roll(row['readout_final']['matrix'], 1, axis=0))
    elif bad == 'learned_wrong_initial':
        row['readout_initial'] = head_fixture('fixed_exact')['readout_initial']
    else:
        row['parameter_metadata']['parameter_count'] = 1088
    with pytest.raises(ValueError):
        a.validate_head_metadata(row)


def test_paired_head_contrasts_keep_only_softened_control_all_seeds_and_zero_denominators():
    ...
    rows = [{'arm': arm, 'seed': seed, 'horizon': h,
             **dict.fromkeys(metrics, {'fixed_softened': .5, 'learned_readout': .125}[arm])}
            for arm in a.ARMS for seed in a.SEEDS for h in a.HORIZONS]
    rows[0]['observed_kl'] = 0
    fits = [{'arm': arm, 'seed': seed, 'seconds': 2., **head_fixture(arm)} for arm in a.ARMS for seed in a.SEEDS]
    result = a.readout_comparisons(rows, fits)
    assert len(result['paired']) == 12 and len(result['means']) == 8 and len(result['matrices']) == 6
    assert all(row['control'] == 'fixed_softened' for row in result['paired'])
    first = result['paired'][0]['metrics']
    assert first['blind_regret']['relative_reduction'] == .75
    assert first['blind_regret']['difference'] == -.375
    assert first['observed_kl']['relative_reduction'] is None
    assert all(row['mean_absolute_change'] == 0 for row in result['matrices'])
...
def fabricated_metadata(folder):
    ...
    counts = {'train_generation_count': 1, 'dev_generation_count': 1, 'model_constructions': 6,
        ...
        'training_attempt_exposures': 1474560, 'training_prefix_rollouts': 23040,
        'training_prefix_event_exposures': 2989440, 'zero_endpoint_batches': 20160,
    ...
    return {'version': 'finite-cost-readout-replication-v1', 'config': copy.deepcopy(a.CONFIG),
