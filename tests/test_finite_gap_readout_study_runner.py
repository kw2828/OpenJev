...
import run_finite_gap_readout_study as r
from register_finite_gap_readout_study import CONFIG

ENGINEERING = {**CONFIG, 'dev_seed_namespace': 936001, 'dev_attempts': 8,
               'parent_seeds': [936101], 'train_namespace': 936001, 'batch_size': 3}


def upstream_fixture(tmp_path):
    folder = tmp_path / 'fabricated-parent'
    folder.mkdir()
    generated = r.generate_attempt_split(0, 8, 2, seed_namespace=936001)
    r._save(folder / 'train.npz', generated['data'])
    fits = []
    for parent in r.PARENTS:
        model = r.make_model(parent, 936101)
        values = r._state(model)
        name = f'{parent}-936101.npz'
        r._save(folder / name, values)
        with torch.no_grad():
            costs = model.readout_matrix().numpy().copy()
        fits.append({'arm': parent, 'seed': 936101, 'final_state_sha256': r._state_hash(values),
...
def test_all_heads_complete_before_fresh_dev_and_models_remain_unchanged(tmp_path, monkeypatch):
    ...
    def generation(split, attempts, horizon, **kwargs):
        ...
        assert kwargs == {'seed_namespace': 936001}
    ...
    for row in result['solves']:
        ...
        solve = row['solver_result']
        assert solve['version'] == 'finite-gap-readout-v1'
        assert solve['method'] == 'fixed-step-monotone-restarted-fista'
        assert solve['projection'] == {'applied': False, 'max_abs': 0., 'l2': 0.}
        assert solve['settings'] == {'max_iterations': 20000, 'certificate_every': 10,
            'fw_gap_tolerance': 1e-8, 'simplex_tolerance': 1e-12,
            'objective_increase_tolerance': 1e-12, 'monotonicity_roundoff': 1e-15}
        assert solve['certificate_history'][0]['iteration'] == 0
        assert solve['certificate_history'][-1]['qualifies']
        assert solve['certificate_history'][-1]['iteration'] == solve['work']['accepted_iterations']
        assert not any(check['qualifies'] for check in solve['certificate_history'][:-1])
        assert solve['raw_probabilities'] == solve['probabilities']
    count = result['counts']
    ...
    for parent in r.PARENTS:
        with np.load(output / f'states-base-{parent}-936101.npz', allow_pickle=False) as states:
...
def test_a_failed_head_prevents_any_dev_generation_and_is_preserved(tmp_path, monkeypatch):
    ...
    records = [json.loads(line) for line in (folder / 'solves.jsonl').read_text().splitlines()]
    assert failure['version'] == 'finite-gap-readout-study-v1'
    assert not (folder / 'base.npz').exists() and not (folder / 'summary.json').exists()
    for record in records:
        assert record == json.loads((folder / f"solve-{record['parent']}-{record['seed']}.json").read_text())
        assert record['solver_result']['complete'] is False
        assert record['solver_result']['failure_reasons'] == ['fabricated_qualification_failure']
        assert record['source_state_sha256'] == record['model_state_after']
