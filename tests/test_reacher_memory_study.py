...
def engineering_plan():
    ...
    namespace = "reacher-memory-engineering-runner-v1"
...
def test_all_twelve_restore_before_evaluation(fit_tree):
    ...
    assert all(type(models[row["name"]]) is training.REGISTRY[row["arm"]]
               for row in protocol.fit_manifest(plan))
    assert len(study.read(folder / "restored-models.json")["models"]) == 12
