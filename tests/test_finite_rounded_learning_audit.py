"""Independent rounded-study initialization, work, timing and comparison oracles."""
...
import audit_finite_rounded_learning as a

SPEC = a.StudyProfile('fabricated', 941001, (941101, 941102, 941103), 512, 128, 64, 10., 40.)
...
def allocation(arm='rounded', stop='late'):
...
def test_profile_whitelist_is_immutable_and_retains_scientific_support(tmp_path):
    ...
    spec = a.PROFILES['engineering-941001']
...
def test_incomplete_manifest_fails_before_any_array_decode(tmp_path, monkeypatch):
    ...
    del rows
    summary = {'version': 'finite-rounded-learning-v1', 'config': SPEC.config, 'fits': fits, 'files': {}}
...
def test_independent_separable_logit_balancing_has_uniform_solution():
    ...
    for arm in ('original_free', 'rounded'):
        arrays['initial', arm, 941101]['transition_logits'][:] = raw
    before = copy.deepcopy(arrays)
    ...
    assert not np.array_equal(arrays['initial', 'original_free', 941101]['transition_logits'],
                              arrays['initial', 'matched_free', 941101]['transition_logits'])
...
def test_initial_pairing_rejects_the_specific_incorrect_control(arm, name):
    _, arrays, _ = boundary_fixture()
    arrays['initial', arm, 941101][name].flat[0] += .01
    with pytest.raises(ValueError):
...
def test_raw_pairing_includes_signed_zero_bytes():
    _, arrays, _ = boundary_fixture()
    arrays['initial', 'rounded', 941101]['transition_logits'].flat[0] = -0.
...
def test_relative_gain_cannot_rescue_a_failed_absolute_criterion():
    ...
    assert not result['advance']['passed']
    assert result['advance']['name'] == 'ROUNDED_ADVANCE'
...
def test_boundary_transition_diagnostics_reject_incomplete_or_invalid_states(fault):
    _, arrays, _ = boundary_fixture()
    key = 'final', 'rounded', 941101
    if fault == 'missing':
        del arrays[key]
    elif fault == 'extra':
        arrays['unrecorded', 'rounded', 941101] = copy.deepcopy(arrays[key])
