...
W = load('finite_head_learning_worker_v2')
P = load('qualify_finite_head_exposure_v2')
...
def test_projection_charges_scaled_stages_and_unscaled_nonstage_work():
    allocations, fits = {}, []
    for arm in ('rounded_anchor', 'rounded_random', 'matched_free_random'):
        allocations[arm] = {'stages': [{'start_elapsed': .2, 'stopped_elapsed': .7},
                                      {'start_elapsed': .8, 'stopped_elapsed': 1.8}]}
        fits.append({'arm': arm, 'seed': 946301, 'seconds': 2.})
...
def test_projection_cannot_hide_unusable_timing(change):
    allocations = {arm: {'stages': [{'start_elapsed': 0., 'stopped_elapsed': 1.},
                   ...
                   for arm in ('rounded_anchor', 'rounded_random', 'matched_free_random')}
    fits = [{'arm': arm, 'seed': 946301, 'seconds': 3.} for arm in allocations]
    if change == 'missing_stage':
        allocations['rounded_random']['stages'].pop()
    elif change == 'negative_overhead':
        fits[0]['seconds'] = 1.
    else:
        allocations['rounded_random']['stages'][0]['stopped_elapsed'] = -1. if change == 'negative' else float('nan')
    with pytest.raises(ValueError):
...
def test_qualification_selects_new_probe_and_tests():
    ...
    assert commands[2] == ['/fixture/python', str(ROOT / 'scripts/qualify_finite_head_exposure_v2.py'),
