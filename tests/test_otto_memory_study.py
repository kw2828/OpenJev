"""Synthetic OTTO cohort scoring checks; never import or run its environment."""
...
SCRIPT = Path(__file__).resolve().parents[1] / "scripts/study_otto_memory.py"
...
def load_runner():
    spec = importlib.util.spec_from_file_location("synthetic_otto_study", SCRIPT)
...
def cohort():
    ...
    return [
        {"seed": 510001 + case, "arm": arm, "block": case // 8,
         "initial_hit": 1 + (case % 8) // 4,
        ...
        for case in range(64) for arm in ARMS
    ]


def score(runner, rows, weights=None, qualified=None):
    return runner.summarize(rows, {1: 0.8, 2: 0.2} if weights is None else weights,
...
def fail(rows, arm, seeds):
    for row in rows:
        if row["arm"] == arm and row["seed"] in seeds:
            row.update(found=False, steps=642, capped_time=642)
...
def test_import_cannot_load_simulator_or_neural_dependencies(monkeypatch):
    ...
    assert module.HORIZON == 642
    assert module.CONFIG == {"Ndim": 2, "lambda_over_dx": 1.0, "R_dt": 1.0,
...
def test_complete_passing_cohort_has_all_ten_conditions_and_no_input_mutation(runner):
    ...
    assert conditions(result)["complete_cohort"]["value"] == 512


def test_stratum_weights_apply_before_population_and_block_means(runner):
    rows = cohort()
    for row in rows:
        # Equal sampled counts must not turn the declared mixture into 50/50.
        steps = 10 + row["block"] if row["initial_hit"] == 1 else 50 + row["block"]
        row["steps"] = row["capped_time"] = steps
    result = score(runner, rows, weights={1: 0.75, 2: 0.25})
    assert result["strata"][1]["space_full"]["capped_time"] == 13.5
    assert result["strata"][2]["space_full"]["capped_time"] == 53.5
    assert result["means"]["space_full"]["capped_time"] == 23.5
    assert result["means"]["space_full"]["capped_time"] != 33.5
    assert [b["means"]["space_full"]["capped_time"] for b in result["blocks"]] == [
        20 + b for b in range(8)
    ]
    for arm in ARMS:
...
def test_step_642_found_and_censored_episodes_have_same_cost_different_success(runner, failed):
    rows = cohort()
    selected = next(r for r in rows if r["arm"] == "space_full" and r["seed"] == 510001)
    selected.update(steps=642, capped_time=642, found=not failed)
    result = score(runner, rows, weights={1: 0.5, 2: 0.5})
    assert result["means"]["space_full"]["capped_time"] == (63 * 18 + 642) / 64
...
def test_success_at_95pct_is_inclusive_and_one_additional_failure_rejects(runner):
    ...
    fail(rows, "space_full", {510001, 510002})
    condition = conditions(score(runner, rows))["full_success_at_least_95pct"]
    assert condition["value"] == 0.95 and condition["passes"] is True
    fail(rows, "space_full", {510003})
...
def test_infotaxis_outcomes_are_descriptive_and_cannot_rescue_space_failure(runner):
    ...
    for arm in ARMS:
        if arm.startswith("info_"):
            set_time(rows, arm, 1 if arm == "info_full" else 642)
    result = score(runner, rows)
...
def test_exact_case_ids_and_strata_are_required(runner, corruption):
    ...
    if corruption == "missing":
        ...
        key, value = {"wrong_seed": ("seed", 510065), "wrong_arm": ("arm", "space_latest"),
...
@pytest.mark.parametrize("weights", [
    {}, {1: 1.0}, {1: 0.8, 2: 0.2, 3: 0.0}, {1: 0.8, 2: 0.1},
    {1: 1.0, 2: 0.0}, {1: -0.2, 2: 1.2}, {1: math.nan, 2: 0.2},
    {1: math.inf, 2: 0.2},
])
...
@pytest.mark.parametrize("update", [
    {"steps": 0, "capped_time": 0}, {"steps": 643, "capped_time": 643},
...
def test_structural_qualification_is_required_explicitly(runner):
    with pytest.raises(TypeError, match="qualification"):
        runner.summarize(cohort(), {1: 0.8, 2: 0.2})


@pytest.mark.parametrize("update", [
    {"status": "failed"}, {"status": "started"}, {"checks": []},
    {"checks": [{"name": "broken", "passes": False}]},
    {"native_steps": 0}, {"native_steps": 1001}, {"native_steps": True},
    {"native_steps": 100.0},
])
