...
import run_finite_rounded_learning as runner
...
def test_three_transition_models_complete_before_fresh_dev_and_independent_audit(tmp_path, monkeypatch):
    ...
    def generate(split, attempts, horizon, **kwargs):
        assert kwargs == {'seed_namespace': 941001}
    ...
    def factory(arm, seed, *, check):
        assert arm in runner.ARMS and seed == 941101 and generations == [0]
    ...
    config = {'seed_namespace': 941001, 'fit_seeds': [941101], 'train_attempts': 8,
    ...
    for fit in fits:
        ...
        with np.load(folder / f"initial-{fit['arm']}-941101.npz", allow_pickle=False) as initial, \
                np.load(folder / f"boundary-{fit['arm']}-941101.npz", allow_pickle=False) as boundary:
    ...
    audited = audit(folder, profile='engineering-941001')
...
def test_failed_allocation_trace_is_saved_before_fit_rejection(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match='both original allocation stages'):
        runner.train('rounded', 941101, {'prefix': torch.zeros((1, 9, 31))}, prefixes,
                     {'batch_size': 1, 'stage1_seconds': 1., 'total_seconds': 2.}, tmp_path, {}, {}, lambda: None)
    assert json.loads((tmp_path / 'allocation-rounded-941101.json').read_text()) == result
...
def test_failed_normalization_preserves_partial_work_and_allocation(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match='failed fixed normalization'):
        runner.train('rounded', 941101, {}, {},
                     {'stage1_seconds': 1., 'total_seconds': 2.}, tmp_path, {}, {}, lambda: None)
    assert json.loads((tmp_path / 'allocation-rounded-941101.json').read_text()) == error.allocation_result
    assert json.loads((tmp_path / 'failed-normalization-rounded-941101.json').read_text()) == error.rounded_model_work
