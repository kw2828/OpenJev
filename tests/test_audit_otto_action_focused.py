...
SPEC = importlib.util.spec_from_file_location("_test_action_focused_audit", ROOT / "scripts/audit_otto_action_focused.py")
...
def test_complete_batch_work_includes_zero_target_lanes_and_full_tails(tmp_path):
    ...
    rows = obj.expected_batches(history,[list(range(54))]*80,'aux')
    ...
    assert rows[0] == {'epoch':1,'batch':0,'episode_indices':list(range(6)),
        ...
        'objective':'aux','prior_rows':46,'spo_loss_calls':0,'spo_weighted_rows':0}
...
def test_prior_only_chunks_keep_backward_geometry_but_charge_spo_calls(tmp_path):
    obj=audit.Audit(SimpleNamespace(output=tmp_path));obj.np=np
    ...
    aux=obj.expected_batches(history,orders,'aux')
    spo=obj.expected_batches(history,orders,'spo')
    assert len(aux)==len(spo)==720
    assert aux[0]['forward_chunks']==spo[0]['forward_chunks']==2
    assert aux[0]['backward_chunks']==spo[0]['backward_chunks']==1
    assert aux[0]['spo_loss_calls']==0 and spo[0]['spo_loss_calls']==1
    assert aux[0]['spo_weighted_rows']==spo[0]['spo_weighted_rows']==0
    assert aux[0]['prior_rows']==spo[0]['prior_rows']==8
    assert aux[-1]['optimizer_step']==spo[-1]['optimizer_step']==720
    weights[1]=.01
    changed=obj.expected_batches(history,orders,'spo')
    assert changed[0]['backward_chunks']==changed[0]['spo_loss_calls']==2
    assert changed[0]['spo_weighted_rows']==1
...
def test_scalar_spo_hand_scale_uniform_exact_ties_and_fixed_weights():
    prediction=np.array([[64,0,999,999],[0,64,0,999]],np.float32)
    teacher=np.array([[0,64,999,999],[0,0,64,999]],np.float32)
    legal=np.array([[True,True,False,False],[True,True,True,False]])
    weights=np.array([1/54,2/54],np.float64)
    value,bound=audit.scalar_spo_loss(prediction,teacher,weights,legal)
    assert value==3*float(np.float32(1/54))+2*float(np.float32(2/54))
    assert 0<bound<1e-4
    shifted,_=audit.scalar_spo_loss(prediction+128,teacher+256,weights,legal)
    assert shifted==value
    # The final whole-cohort sum keeps episode/IPW mass; multiplying a batch by
    # 54/B belongs only to its training objective, never to this rescore.
    one,_=audit.scalar_spo_loss(prediction[:1],teacher[:1],weights[:1],legal[:1])
    assert one==3*float(np.float32(1/54))
    assert one*(54/6)==pytest.approx(.5)


def test_scalar_spo_near_teacher_tie_is_not_an_exact_oracle_tie():
    prediction=np.array([[0,64,0,0]],np.float32)
    teacher=np.array([[0,5e-11,0,0]],np.float32)
    mask=np.array([[True,True,False,False]])
    assert audit.scalar_spo_loss(prediction,teacher,np.ones(1),mask)[0]==0
    teacher[0,1]=0
    assert audit.scalar_spo_loss(prediction,teacher,np.ones(1),mask)[0]==1
    # A sole eligible action has no decision loss, irrespective of score error.
    mask[0,1]=False
    assert audit.scalar_spo_loss(prediction,teacher,np.ones(1),mask)[0]==0


def test_scalar_spo_zero_weight_rows_are_unread_and_invalid_weight_rejected():
    prediction=np.array([[64,0,0,0],[np.nan]*4],np.float32)
    teacher=np.array([[0,64,0,0],[np.nan]*4],np.float32)
    masks=np.array([[True,True,False,False],[False]*4])
    value,_=audit.scalar_spo_loss(prediction,teacher,np.array([.5,0]),masks)
    assert value==1.5
    for weights in ([.5,-1],[.5,np.inf],[.5,1e-300]):
        with pytest.raises(ValueError,match='importance weight'):
            audit.scalar_spo_loss(prediction,teacher,weights,masks)
...
def reports():
    ...
    for scope in ('full','postcorrection','initial'):
        ...
        group['episode_weighted_raw_gap']=20.
        ...
        for seed in audit.SEEDS:
            ...
            for scope in ('full','postcorrection','initial'):
                for regime in audit.REGIMES:
                    ...
                    group['episode_weighted_agreement']=.6 if family.endswith('spo') else .5
                    group['episode_weighted_raw_gap']={'innovation_aux':10.,'innovation_spo':7.,'gru_aux':10.,'gru_spo':8.}[family]
            models.append({'family':family,'seed':seed,'metrics':metric})
    return models,one


def test_all41_records_have_three_distinct_memberships():
    models,hold=reports()
    rows=audit.continuation_rules(models,hold,technical_complete=True)
    assert len(rows)==41 and all(r['passes'] for r in rows)
    gates=audit.gate_decisions(rows)
    assert {k:v['total'] for k,v in gates.items()}=={'innovation_objective':23,'gru_objective':23,'architecture':29}
    for row in rows:
    ...
    assert all(not v['passes'] for v in audit.gate_decisions(audit.continuation_rules(models,hold,technical_complete=False)).values())
...
def test_objective_gate_rejects_one_regressing_seed_and_architecture_keeps_stronger_control():
    models,hold=reports()
    selected=next(r for r in models if r['family']=='innovation_spo' and r['seed']==audit.SEEDS[0])
    selected['metrics']['postcorrection']['by_regime']['lambda3']['episode_weighted_raw_gap']=10.1
    rules=audit.continuation_rules(models,hold,technical_complete=True)
    assert next(r for r in rules if r['name']==f'objective.innovation.lambda3.{audit.SEEDS[0]}.postcorrection.gap_nonregression')['passes'] is False
    assert next(r for r in rules if r['name']=='objective.innovation.lambda3.postcorrection.gap')['passes'] is True
    models,hold=reports()
    for r in models:
        if r['family']=='gru_aux':
            r['metrics']['postcorrection']['by_regime']['lambda3']['episode_weighted_raw_gap']=6.
            r['metrics']['initial']['by_regime']['lambda3']['episode_weighted_agreement']=.7
    rules=audit.continuation_rules(models,hold,technical_complete=True)
    gap=next(r for r in rules if r['name']=='architecture.lambda3.postcorrection.gap')
    assert gap['threshold']==.9*6 and gap['strict_upper_bound']==6 and gap['passes'] is False
    assert next(r for r in rules if r['name']=='architecture.lambda3.initial.agreement')['passes'] is False
    for r in models:
        for regime in audit.REGIMES:
            r['metrics']['postcorrection']['by_regime'][regime]['episode_weighted_raw_gap']=0.
    failed=[r for r in audit.continuation_rules(models,hold,technical_complete=True) if not r['passes'] and r['relation']=='<= and <']
    assert len(failed)==6
    hold['postcorrection']['by_regime']['lambda4']['episode_weighted_raw_gap']=0.
    assert next(r for r in audit.continuation_rules(models,hold,technical_complete=True)
                if r['name']=='common.lambda4.hold.postcorrection.positive_gap')['passes'] is False
...
def test_exact_new_cohort_seed_and_recipe_cardinality():
    ...
    assert sorted({r['seed'] for r in rows if r['stage'] == 'train' and r['regime'] == 'lambda3'}) == list(range(291000001,291000010))
    assert audit.SEEDS == (295000001,295000002,295000003) and audit.SELECTION_START == 296000001
    assert len(audit.FAMILIES)*len(audit.SEEDS)*80*9 == 8640
    assert 13+3*len(audit.FAMILIES)*len(audit.SEEDS) == 49
    assert audit.CONFIG['required_conditions'] == 41
