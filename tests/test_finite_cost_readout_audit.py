"""Hand-built finite-world data, independent arithmetic, and cost-readout gate rules."""
...
import audit_finite_cost_readout as a


@pytest.fixture(autouse=True)
def engineering_namespace(monkeypatch):
    monkeypatch.setattr(a, 'NAMESPACE', 932001)
    monkeypatch.setattr(a, 'SEEDS', (932101, 932102, 932103))
    monkeypatch.setattr(a, 'CONFIG', {**a.CONFIG, 'seed_namespace': 932001, 'fit_seeds': [932101, 932102, 932103]})
...
@pytest.mark.parametrize('arm', ('fixed_exact', 'fixed_softened', 'learned_readout'))
...
def test_paired_head_contrasts_keep_both_controls_all_seeds_and_zero_denominators():
    ...
    rows = [{'arm': arm, 'seed': seed, 'horizon': h,
             **dict.fromkeys(metrics, {'fixed_exact': .25, 'fixed_softened': .5, 'learned_readout': .125}[arm])}
            for arm in a.ARMS for seed in a.SEEDS for h in a.HORIZONS]
    ...
    assert len(result['paired']) == 24 and len(result['means']) == 12 and len(result['matrices']) == 9
    exact = result['paired'][0]['metrics']
    assert exact['blind_regret']['relative_reduction'] == .5
    assert exact['blind_regret']['difference'] == -.125
    assert exact['observed_kl']['relative_reduction'] is None
    assert result['paired'][12]['metrics']['blind_regret']['relative_reduction'] == .75
...
def fabricated_metadata(folder):
    ...
    counts = {'train_generation_count': 1, 'dev_generation_count': 1, 'model_constructions': 9,
        ...
        'readout_snapshot_evaluations': 18, 'readout_snapshot_softmax_evaluations': 6}
    ...
    return {'version': 'finite-cost-readout-v1', 'config': copy.deepcopy(a.CONFIG),
