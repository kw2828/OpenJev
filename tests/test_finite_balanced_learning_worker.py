...
PATH = Path(__file__).resolve().parents[1] / 'scripts/finite_balanced_learning_worker.py'
...
def qualification_fixture(tmp_path):
    ...
    plan = {'version': 'finite-balanced-learning-v1', 'mode': 'study',
    ...
    receipt['launch'].update(cwd=str(tmp_path), command=[
        '/fixture/python', str(tmp_path / 'scripts/finite_balanced_learning_worker.py'),
