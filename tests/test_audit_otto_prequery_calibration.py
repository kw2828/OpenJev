...
SPEC = importlib.util.spec_from_file_location("_test_prequery_audit", ROOT / "scripts/audit_otto_prequery_calibration.py")
...
def reports():
    ...
    for domain in ('full','postcorrection'):
        ...
        group['episode_weighted_raw_gap']=1.
        for age in group['by_age'].values():
            age['episode_weighted_raw_gap']=1.
            age['supported_case_count']=4
        one[domain]['by_regime']['lambda4']=copy.deepcopy(group)
    models=[{'family':f,'seed':s,'metrics':copy.deepcopy(one),
             'prior_metrics':{'by_regime':{r:{'episode_weighted_centered_mse':
                              8. if f=='innovation_aux' else 10.} for r in audit.REGIMES}}}
            for s in audit.SEEDS for f in audit.FAMILIES]
    for model in models:
        if model['family']=='innovation_aux':
            for r in audit.REGIMES:
                model['metrics']['postcorrection']['by_regime'][r]['episode_weighted_raw_gap']=.8
    return models,one


def test_all_55_rules_include_third_control_full_path_and_technical_gate():
    models,hold=reports()
    rules=audit.continuation_rules(models,hold,technical_complete=True)
    assert len(rules)==55 and all(r['passes'] for r in rules)
    models[0]['metrics']['postcorrection']['by_regime']['lambda3']['episode_weighted_raw_gap']=10.
    rules=audit.continuation_rules(models,hold,technical_complete=False)
    assert not rules[0]['passes']
    by={r['name']:r for r in rules}
    assert not by['lambda3.mean_gap_vs_innovation_gru_mse']['passes']
    assert not by['lambda3.mean_gap_vs_innovation_gru_aux']['passes']
    assert by['lambda3.full.mean_gap_vs_innovation_mse']['passes']
...
def test_all_six_zero_gap_ties_fail_and_prior_rule_is_independently_required():
    models,hold=reports()
    for model in models:
        for regime in audit.REGIMES:
            model['metrics']['postcorrection']['by_regime'][regime]['episode_weighted_raw_gap']=0
    failures=[r for r in audit.continuation_rules(models,hold,technical_complete=True) if not r['passes']]
    assert len(failures)==6 and all(r['relation']=='<= and <' for r in failures)
    models,hold=reports()
    for model in models:
        if model['family']=='innovation_aux':
            model['prior_metrics']['by_regime']['lambda3']['episode_weighted_centered_mse']=8.01
    failed=[r['name'] for r in audit.continuation_rules(models,hold,technical_complete=True) if not r['passes']]
    assert failed==['lambda3.mean_prior_mse_vs_innovation_mse']
