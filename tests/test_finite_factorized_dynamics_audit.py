"""Hand-built finite-world data, independent arithmetic, and factorized-dynamics gate rules."""
...
import audit_finite_factorized_dynamics as a


@pytest.fixture(autouse=True)
def engineering_namespace(monkeypatch):
    monkeypatch.setattr(a, 'NAMESPACE', 934001)
    monkeypatch.setattr(a, 'SEEDS', (934101, 934102, 934103))
    monkeypatch.setattr(a, 'CONFIG', {**a.CONFIG, 'seed_namespace': 934001, 'fit_seeds': [934101, 934102, 934103]})
...
def test_gates_require_each_seed_and_horizon_and_strictly_positive_reference(bad):
    ...
    if bad == 'short_seed':
        ...
        return
    result = a.gates(rows, baselines, counts)
    assert any(not criterion['passed'] for criterion in result['matched_free'].values())
...
def work_fixture():
    ...
    for arm in a.ARMS:
        ...
        for route in a.ROUTES:
            ...
            if arm == 'factorized':
                # One factorization per prefix-likelihood forward; two for
                # each endpoint forward (prefix filter and forecast).
                factorizations = (24 if training else 6) if is_prefix else 6
                values.update({key: count * factorizations for key, count in FACTOR_UNIT.items()})
                for key in ('reset_emission_softmax_calls', 'prefix_operator_softmax_calls', 'operator_observed_softmax_calls'):
                    values[key] = 0
...
@pytest.mark.parametrize('bad', [None, 'blind_absorbed', 'prefix_found_conditioned', 'prefix_padding', 'zero_batches',
                               'missing_head_normalization', 'head_in_prefix', 'head_rows_as_batch',
                               'missing_factor_products', 'factorized_old_softmax'])
def test_full_work_reconstruction_uses_attempt_orders_and_both_loss_populations(bad):
    ...
    elif bad == 'missing_head_normalization':
        work['dense_free']['training_blind']['cost_head_softmax_calls'] = 0
    elif bad == 'head_in_prefix':
        work['dense_free']['training_prefix']['cost_head_softmax_calls'] = 1
    elif bad == 'head_rows_as_batch':
        work['dense_free']['evaluation_blind']['cost_head_probability_rows'] = 2 * 24
    elif bad == 'missing_factor_products':
        work['factorized']['training_blind']['factor_product_entries'] -= 256
    elif bad == 'factorized_old_softmax':
        work['factorized']['evaluation_prefix']['reset_emission_softmax_calls'] = 6
    if bad:
        with pytest.raises(ValueError):
            a.validate_work(work, datasets, prefixes, orders, fits, np)
    else:
        result = a.validate_work(work, datasets, prefixes, orders, fits, np)
        assert result['zero_endpoint_batches'] == 63
        assert result['historical_forward_work_independently_replayed'] is False


FACTOR_UNIT = {'factorization_calls': 1, 'transition_softmax_calls': 1,
    ...
    'factor_product_entries': 256 + 1024 + 256, 'factor_found_reduction_columns': 32}


def snapshot(matrix):
    """NumPy bytes independently exercise the auditor's scalar struct codec."""
    value = np.asarray(matrix, dtype='<f8')
    return {'matrix': value.tolist(), 'sha256': hashlib.sha256(value.tobytes(order='C')).hexdigest()}


def head_fixture(arm):
    ...
    matrix = np.full((4, 8), .25, np.float64)
    matrix[preferred, np.arange(8)] = -.75
    matrix *= .9
    initial = snapshot(matrix)
    return {'parameter_metadata': {'parameters': parameters, 'parameter_count': sum(sizes.values()),
        'parameter_bytes': sum(sizes.values()) * 8, 'buffer_bytes': 0},
        'readout_initial': initial, 'readout_final': copy.deepcopy(initial)}


def dynamics_fixture(arm):
    """Known artificial factors, deliberately different from the finite world."""
    transition = np.full((4, 8, 8), .25 / 8, np.float64)
    emission = np.full((4, 8), .3 / 4, np.float64)
    hazard = np.empty((4, 8), np.float64)
    for action in range(4):
        for state in range(8):
            transition[action, (state + action) % 8, state] += .75
            hazard[action, state] = .02 + .002 * (action + state)
    emission[np.arange(8) % 4, np.arange(8)] += .7
    observed = emission[None, :, :, None] * (1 - hazard[:, None, :, None]) * transition[:, None]
    found = (hazard[:, :, None] * transition).sum(1)
    blind = observed.sum(1)
    work = dict.fromkeys(a.SNAPSHOT_WORK_KEYS, 0)
    work['operator_marginal_sum_calls'] = 1
    if arm == 'factorized':
        work.update(FACTOR_UNIT)
    else:
        work.update({'reset_emission_softmax_calls': 1, 'prefix_operator_softmax_calls': 1})
    return {'reset_emission': snapshot(emission), 'observed': snapshot(observed),
            'found': snapshot(found), 'blind': snapshot(blind), 'work': work}


def construction_fixture(arm):
    result = {**dict.fromkeys(FACTOR_UNIT, 0), 'matching_log_calls': 0, 'matching_log_entries': 0}
    if arm == 'matched_free':
        result.update({**FACTOR_UNIT, 'matching_log_calls': 2, 'matching_log_entries': 1088})
    return result


def fit_fixture(arm, seed):
    return {'arm': arm, 'seed': seed, **head_fixture(arm), 'construction_seconds': .01, 'seconds': 1.,
            'construction_work': construction_fixture(arm), 'dynamics_initial': dynamics_fixture(arm),
            'dynamics_final': dynamics_fixture(arm)}


@pytest.mark.parametrize('arm', ('factorized', 'matched_free', 'dense_free'))
def test_artificial_factors_normalize_snapshots_and_preserve_shared_learned_head(arm):
    row = fit_fixture(arm, a.SEEDS[0])
    a.validate_head_metadata(row)
    a.validate_dynamics_metadata(row)
    result = a.dynamics_snapshot(row['dynamics_initial'], arm)
    assert result['outgoing_mass_error'] <= 1e-12
    assert result['blind_marginal_error'] <= 1e-12
    assert row['parameter_metadata']['parameter_count'] == (352 if arm == 'factorized' else 1120)
    if arm == 'factorized':
        assert result['shared_emission_factorization_error'] <= 1e-12
    else:
        assert result['shared_emission_factorization_error'] is None


@pytest.mark.parametrize('bad', ['digest', 'shape', 'negative', 'reset_normalization', 'outgoing_mass',
                               'marginal', 'factor_emission', 'work', 'constructor', 'time', 'head_sign', 'parameters'])
def test_dynamics_and_storage_tampering_is_rejected(bad):
    row = fit_fixture('factorized', a.SEEDS[0])
    state = row['dynamics_final']
    if bad == 'digest':
        state['found']['sha256'] = '1' * 64
    elif bad == 'shape':
        state['found']['matrix'].pop()
    elif bad == 'negative':
        value = np.asarray(state['observed']['matrix'])
        value[0, 0, 0, 0] = -.001
        state['observed'] = snapshot(value)
    elif bad == 'reset_normalization':
        state['reset_emission'] = snapshot(np.asarray(state['reset_emission']['matrix']) * .9)
    elif bad == 'outgoing_mass':
        state['found'] = snapshot(np.asarray(state['found']['matrix']) + .01)
    elif bad == 'marginal':
        state['blind'] = snapshot(np.asarray(state['blind']['matrix']) + .01)
    elif bad == 'factor_emission':
        value = np.asarray(state['observed']['matrix'])
        value[0, 0, 0, 0] += .0001
        value[0, 1, 0, 0] -= .0001
        state['observed'] = snapshot(value)
    elif bad == 'work':
        state['work']['factor_product_entries'] -= 256
    elif bad == 'constructor':
        row['construction_work']['matching_log_calls'] = 2
    elif bad == 'time':
        row['construction_seconds'] = float('nan')
    elif bad == 'head_sign':
        row['readout_initial'] = snapshot(-np.asarray(row['readout_initial']['matrix']))
    else:
        row['parameter_metadata']['parameter_count'] = 1120
    with pytest.raises(ValueError):
        a.validate_head_metadata(row)
        a.validate_dynamics_metadata(row)


def test_initial_matching_is_on_functions_not_parameter_hashes():
    fits = [fit_fixture(arm, seed) for arm in a.ARMS for seed in a.SEEDS]
    assert len(a.initial_function_checks(fits)) == 3
    # A valid but different reset function in the matched control must fail
    # matching even though both controls still satisfy stochasticity.
    row = next(r for r in fits if r['arm'] == 'matched_free' and r['seed'] == a.SEEDS[1])
    value = np.asarray(row['dynamics_initial']['reset_emission']['matrix'])
    value[0] += .001
    value[1] -= .001
    row['dynamics_initial']['reset_emission'] = snapshot(value)
    a.validate_dynamics_metadata(row)
    with pytest.raises(ValueError, match='initial dynamics agree'):
        a.initial_function_checks(fits)


def test_paired_architecture_comparisons_include_both_controls_each_long_horizon():
    metrics = ('blind_regret', 'blind_cost_mse', 'observed_cost_mse', 'observed_kl',
               'blind_survival_mae', 'observed_survival_mae')
    rows = [{'arm': arm, 'seed': seed, 'horizon': h,
             **dict.fromkeys(metrics, {'factorized': .125, 'matched_free': .5, 'dense_free': .25}[arm])}
            for arm in a.ARMS for seed in a.SEEDS for h in a.HORIZONS]
    next(r for r in rows if r['arm'] == 'matched_free' and r['horizon'] == 4)['observed_kl'] = 0
    fits = [fit_fixture(arm, seed) for arm in a.ARMS for seed in a.SEEDS]
    result = a.dynamics_comparisons(rows, fits)
    assert len(result['paired']) == 12 and len(result['means']) == 12 and len(result['fits']) == 9
    assert {row['horizon'] for row in result['paired']} == {4, 8}
    assert result['paired'][0]['metrics']['blind_regret']['relative_reduction'] == .75
    assert result['paired'][0]['metrics']['blind_regret']['difference'] == -.375
    assert result['paired'][0]['metrics']['observed_kl']['relative_reduction'] is None
    assert result['paired'][6]['metrics']['blind_regret']['relative_reduction'] == .5


def fabricated_metadata(folder):
    """Complete nine-fit schedule and opaque payload bytes; no model or world."""
    def descriptor(path):
        ...
        for arm in a.ARMS[index:] + a.ARMS[:index]:
            ...
            fits.append({'arm': arm, 'seed': seed, 'epochs': 480, 'updates': 3840,
                ...
                **{key: '1' * 64 for key in ('initial_state_sha256', 'final_state_sha256', 'fixed_buffers_sha256')},
                'prefix_initial_sha256': None, 'operator_initial_sha256': None if arm == 'factorized' else '3' * 64,
                'reset_initial_sha256': None if arm == 'factorized' else '4' * 64,
                'prefix_operator_initial_sha256': None if arm == 'factorized' else '3' * 64,
                'oracle_prefix_input': False,
                **fit_fixture(arm, seed),
    ...
    counts = {'train_generation_count': 1, 'dev_generation_count': 1, 'model_constructions': 9,
        ...
        'optimizer_attempts': 34560, 'optimizer_steps': 34560, 'training_case_exposures': 8640,
        'training_attempt_exposures': 2211840, 'training_prefix_rollouts': 34560,
        'training_prefix_event_exposures': 4484160, 'zero_endpoint_batches': 30240,
        'evaluation_blind_rollouts': 9, 'evaluation_observed_rollouts': 9,
        'evaluation_shuffled_rollouts': 9, 'evaluation_case_views': 18,
        'evaluation_prefix_rollouts': 18, 'evaluation_prefix_event_views': 2430,
        ...
        'readout_snapshot_evaluations': 18, 'readout_snapshot_softmax_evaluations': 18, 'dynamics_snapshot_evaluations': 18}
    ...
    return {'version': 'finite-factorized-dynamics-v1', 'config': copy.deepcopy(a.CONFIG),
...
def test_metadata_authenticates_all_attempt_schedule_before_any_array_decode(tmp_path, monkeypatch):
    ...
    assert result['fits'] == result['checkpoint_files_hashed'] == 9
    assert result['epochs'] == 4320 and result['optimizer_steps_attested'] == 34560
    assert result['training_attempt_exposures_attested'] == 2211840
...
def test_metadata_tamper_rejected_before_decode(tmp_path, monkeypatch, bad):
    ...
    if bad == 'checkpoint':
        ...
        if bad == 'attempt_updates':
            ...
            row['prefix_loss_weight'] = 0
        else:
            row['operator_initial_sha256'] = '9' * 64
