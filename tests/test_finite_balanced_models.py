...
from openjev.research import finite_balanced_models as models
...
def custom_parameters():
    ...
    transition = ((3 * action + 5 * destination + 7 * origin + destination * origin) % 13 - 6) / 8
...
def numpy_fields(parameters, *, balanced):
    def softmax(value, axis):
        value = np.exp(value - value.max(axis=axis, keepdims=True))
        return value / value.sum(axis=axis, keepdims=True)

    log_transition = parameters["transition_logits"].copy()
    if balanced:
        for _ in range(64):
            for axis in (2, 1):
                maximum = log_transition.max(axis=axis, keepdims=True)
                normalizer = maximum + np.log(np.exp(log_transition - maximum).sum(axis=axis, keepdims=True))
                log_transition = log_transition - normalizer
        transition = np.exp(log_transition)
    else:
        transition = softmax(log_transition, 1)
...
def test_matched_free_and_balanced_have_equal_initial_functions_and_preserve_rng():
    rng = torch.random.get_rng_state().clone()
    free = models.make_model("matched_free", SEED)
    balanced = models.make_model("balanced", SEED)
    original = models.make_model("original_free", SEED)
    assert torch.equal(torch.random.get_rng_state(), rng)
    for name in ("emission_logits", "hazard_logits", "cost_logits"):
        assert torch.equal(getattr(free, name), getattr(balanced, name))
        assert torch.equal(getattr(free, name), getattr(original, name))
    a, b = models.probability_fields(free), models.probability_fields(balanced)
    for name in ("transition", "emission", "hazard"):
        torch.testing.assert_close(a[name], b[name], atol=1e-12, rtol=0)
    torch.testing.assert_close(free.readout_matrix(), balanced.readout_matrix(), atol=1e-12, rtol=0)
    first_snapshot, second_snapshot = free.dynamics_snapshot(), balanced.dynamics_snapshot()
    for name in ("reset_emission", "observed", "found", "blind"):
        torch.testing.assert_close(first_snapshot[name], second_snapshot[name],
                                   atol=1e-12, rtol=0)
    prefix, lengths = mixed_prefix()
    compare_outputs(models.prefix_predictions(free, prefix, lengths), models.prefix_predictions(balanced, prefix, lengths))
    ordinary, ordinary_lengths = public_prefix()
    actions, observations = blocks()
    for horizon in (1, 2, 8):
        compare_outputs(free.blind_rollout(ordinary, ordinary_lengths, actions[:, :horizon]),
                        balanced.blind_rollout(ordinary, ordinary_lengths, actions[:, :horizon]))
        compare_outputs(free.observed_rollout(ordinary, ordinary_lengths, actions[:, :horizon], observations[:, :horizon]),
                        balanced.observed_rollout(ordinary, ordinary_lengths, actions[:, :horizon], observations[:, :horizon]))


def test_balanced_prefix_and_forecast_match_independent_hidden_path_enumeration():
    model = models.make_model("balanced", SEED)
    parameters = custom_parameters()
    inject(model, parameters)
    transition, emission, hazard, head = numpy_fields(parameters, balanced=True)
    prefix, lengths = public_prefix()
...
def test_prefix_objective_gradient_matches_hidden_path_finite_difference_with_same_balanced_prior():
    model = models.make_model("balanced", SEED)
    parameters = custom_parameters()
    inject(model, parameters)
    ...
    def scalar_loss(logits):
        ...
        transition, emission, hazard, _head = numpy_fields(values, balanced=True)
...
def assert_kernel_work(work, calls):
    # Every construction uses the complete fixed64 primitive, never a cached T.
    expected = {"balanced_transition_calls": 1, "row_logsumexp_calls": 64,
                "column_logsumexp_calls": 64, "normalization_vectors": 4096,
                "normalization_entries": 32768, "completed_sweeps": 64,
                "exponential_calls": 1, "exponential_entries": 256,
                "residual_sum_calls": 2, "check_calls": 66}
...
def test_metadata_constructor_and_distinct_route_work_are_complete_and_stable(arm):
    ...
    assert metadata["transition_ideal_constraint_degrees"] == (196 if arm == "balanced" else 224)
    ...
    assert set(constructor) == set(models.WORK_KEYS)
    ...
    assert work["transition_softmax_calls"] == int(arm != "balanced")
    assert_kernel_work(work, int(arm == "balanced"))
    assert work["prior_transition_log_softmax_calls"] == int(arm != "balanced")
    assert work["prior_emission_log_softmax_calls"] == 1 and work["prior_hazard_logsigmoid_calls"] == 2
    assert work["prior_normalization_vectors"] == (8 if arm == "balanced" else 40)
    assert work["prior_normalization_entries"] == (32 if arm == "balanced" else 288)
    ...
    for observed in (False, True):
        ...
        assert_kernel_work(route, 2 * (arm == "balanced"))
...
def test_failed_balancing_and_external_stop_preserve_work_without_retry_or_parameter_mutation():
    model = models.make_model("balanced", SEED)
    with torch.no_grad():
    ...
    with pytest.raises(ValueError, match="strict transition probabilities") as caught:
        models.probability_fields(model)
    work = caught.value.balanced_model_work
    assert work == model.last_work and work is not model.last_work
    assert work["balanced_transition_calls"] == 1 and work["completed_sweeps"] == 64
    assert work["row_logsumexp_calls"] == work["column_logsumexp_calls"] == 64
    assert work["check_calls"] == 65 and work["exponential_calls"] == 1
    assert work["residual_sum_calls"] == work["emission_softmax_calls"] == 0
    assert torch.equal(model.transition_logits, before)
    model.last_work["completed_sweeps"] += 1
    assert work["completed_sweeps"] == 64
    ...
    work = error.balanced_model_work
    assert work["adapter_check_calls"] == work["balanced_transition_calls"] == 1
...
def test_matched_constructor_stop_exposes_only_performed_work():
    ...
    assert caught.value is error and calls == 3
    work = error.balanced_model_work
    assert work["adapter_check_calls"] == 1 and work["balanced_transition_calls"] == 1
