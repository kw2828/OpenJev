...
import run_finite_factorized_dynamics as r

CONFIG = {'seed_namespace': 934001, 'fit_seeds': [934101], 'train_attempts': 8,
...
def test_paired_fits_finish_before_dev_and_never_receive_oracle(tmp_path, monkeypatch):
    ...
    def checked_generation(split, attempts, horizon, **kwargs):
        assert kwargs == {'seed_namespace': 934001}
    ...
    fits = result['fits']
    assert fits[0]['case_order_sha256'] == fits[1]['case_order_sha256'] == fits[2]['case_order_sha256']
    assert fits[0]['operator_initial_sha256'] is None
    assert fits[0]['reset_initial_sha256'] is None
    assert [fit['parameter_metadata']['parameter_count'] for fit in fits] == [352, 1120, 1120]
    assert all(fit['readout_initial'] == fits[0]['readout_initial'] for fit in fits)
    for name in ('reset_emission', 'observed', 'found', 'blind'):
        np.testing.assert_allclose(fits[0]['dynamics_initial'][name]['matrix'],
                                   fits[1]['dynamics_initial'][name]['matrix'], atol=1e-12, rtol=0)
    assert all(fit['construction_seconds'] > 0 for fit in fits)
    assert fits[1]['construction_work']['factorization_calls'] == 1
    assert fits[0]['construction_work']['factorization_calls'] == fits[2]['construction_work']['factorization_calls'] == 0
    ...
    assert counts['readout_snapshot_evaluations'] == 6 and counts['readout_snapshot_softmax_evaluations'] == 6
    assert counts['dynamics_snapshot_evaluations'] == 6
    assert all(counts[key] == 0 for key in ('checkpoint_decodes', 'array_decodes', 'external_model_calls', 'native_calls', 'teacher_calls'))
    for arm in r.ARMS:
        with np.load(folder / f'prefix-{arm}-934101-base.npz', allow_pickle=False) as saved:
...
def test_global_denominators_partial_batch_and_empty_endpoint_step(tmp_path, monkeypatch, arm):
    ...
    for batch_size in (1, 2):
        ...
        model.construction_work = {}
        monkeypatch.setattr(r, 'dynamics_snapshot', lambda model, counts: {})
        recorded = []
        ...
        config = {**r.DEFAULT_CONFIG, 'fit_seeds': [934101], 'epochs': 1,
        ...
        fit = r.train(model, arm, 934101, data, prefixes, config, folder, counts, work, lambda: None)
        order = np.random.Generator(np.random.PCG64(np.random.SeedSequence([934101, 0, 818]))).permutation(3)
        expected = []
