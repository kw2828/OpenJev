...
SPEC = importlib.util.spec_from_file_location("readout_audit_tests", ROOT / "scripts/audit_otto_readout_ablation.py")
audit = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(audit)


def reports(joint=19., control=20., support=3):
    return [{"family": family, "seed": seed, "stage": "dev", "query_period": 4, "episodes": 18,
             "scopes": {scope: {"by_regime": {regime: {
                 "episodes": 9, "declared_case_count": 3, "supported_case_count": support,
...
def test_exact_five_percent_boundary_all_six_cells_and_order_independence():
    ...
    assert result["recurrent_weight_hypothesis_continues"] is True
    ...
    failed = audit.comparisons(rows)
    assert failed["passed_cells"] == 5 and failed["recurrent_weight_hypothesis_continues"] is False


@pytest.mark.parametrize("family", ("pretrained", "action_residual_only", "both_readouts"))
...
def test_each_control_and_each_gap_condition_is_required(family, scope):
    ...
    result = audit.comparisons(rows)
    assert result["passed_cells"] == 5 and result["recurrent_weight_hypothesis_continues"] is False
    cell = result["cells"][0]
...
def test_zero_support_retained_and_zero_against_zero_never_counts_as_gain():
    result = audit.comparisons(reports(joint=0., control=0., support=0))
    assert len(result["cells"]) == 6 and result["passed_cells"] == 0
    assert result["recurrent_weight_hypothesis_continues"] is False
...
def test_readout_proximity_is_descriptive_without_equivalence_or_continuation():
    result = audit.comparisons(reports(joint=100., control=104.))
...
@pytest.mark.parametrize("family,count,prefixes", (
    ...
    ("both_readouts", 232, ("slow.output.", "slow.action_residual.")),
...
def test_incomplete_producer_roster_rejected_before_any_array_decode(tmp_path):
    ...
    receipt = {"version": audit.PRODUCER_VERSION, "phase": "train", "status": "completed",
               "pending": None, "error": None, "fits_completed": 9, "optimizer_steps": 3240,
               "episode_exposures": 19440, "views_completed": 12, "array_decodes": 5,
...
def complete_journal(tmp_path, monkeypatch):
    spec = importlib.util.spec_from_file_location("readout_journal_reference_tests", ROOT / audit.REFERENCE)
    ref = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(ref)
    monkeypatch.setattr(audit, "ROOT", tmp_path)
    path = tmp_path / "work.jsonl"
    fits = [{"family": family, "seed": seed, "parameters": audit.parameter_metadata(family),
             "orders": [list(range(54)) for _ in range(40)]}
...
def test_complete_fabricated_3240_update_journal_is_counted(complete_journal):
    path, fits, ref = complete_journal
    checks = []
    result = audit.verify_journal(ref, audit.descriptor(path), fits, [6] * 54, lambda: checks.append(True))
    assert result["updates"] == 3240 and result["episode_exposures"] == 19440
    assert result["scheduled_training"]["forward_rows"] == 3240 * 36
    assert result["scheduled_training"]["forward_chunks"] == 3240
    assert len(checks) == 3240
