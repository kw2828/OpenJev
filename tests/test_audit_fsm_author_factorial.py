...
def test_five_percent_boundary_and_paired_seed_rule_are_distinct():
    ...
    for entry in refs:
        if entry['family'] == audit.old.CANDIDATE:
            # One ULP can round back to the boundary through record/seed means.
            entry['rows'] = rows(.950001)
    result = audit.decisions(refs, bla, cells, fit_complete=True, matrix_complete=True)
    assert result['family_means'][audit.old.CANDIDATE] > .95*result['family_means'][result['strongest_control']]
