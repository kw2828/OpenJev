...
spec = importlib.util.spec_from_file_location("runtime_calibration_report_tested", REPO / "scripts/report_dialogue_runtime_calibration.py")
...
def artifacts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(report.common, "CONTROL_PLAN", "old-control-plan.json")
    monkeypatch.setattr(report.common, "PREPARED", "synthetic/prepare")
    monkeypatch.setattr(report.common, "QUALIFIED", "synthetic/qualify")
    monkeypatch.setattr(report.common, "PREP_TERMINAL", "prepare-terminal.json")
    monkeypatch.setattr(report.common, "QUAL_TERMINAL", "qualify-terminal.json")
    sources = {}
    for name in report.SOURCES | {"scripts/dialogue_calibration_common.py", "scripts/dialogue_runtime_common.py",
    ...
    legacy_plan = {"sources": {"scripts/dialogue_calibration_common.py": sources["scripts/dialogue_calibration_common.py"]}}
    legacy_path = tmp_path / report.common.CONTROL_PLAN
    write_json(legacy_path, legacy_plan)
    control = {"version": report.common.VERSION,
               "sources": {"scripts/dialogue_runtime_common.py": sources["scripts/dialogue_runtime_common.py"]},
               "inputs": {"src/openjev/research/suspend_clock.py": sources["src/openjev/research/suspend_clock.py"],
                          report.common.CONTROL_PLAN: report.common.sha(legacy_path)},
    ...
    ctx = {"plan_path": legacy_path.resolve(), "plan": legacy_plan,
           "runtime_plan_path": control_path.resolve(), "runtime_plan": control,
    ...
    for phase in report.PHASES:
        ...
        if phase == "prepare":
            ...
            write_json(directory / "selection.json", {"selected_ids": [r["dialogue_id"] for r in calibration]})
                        ...
                        model_calls=0, neural_calls=0, service_exposure={"scope": "Synthetic services only"})
        else:
            ...
            count = {"qualify": 2, "pilot": 130, "infer": 4}[phase]
            for fit_id in report.FIT_ORDER:
                arm, seed = fit_id.rsplit("-", 1)
                ...
                if phase == "infer":
                    ...
                    fit.update(warmup={"dialogue_ids": ["warmup-0", "warmup-1"]},
                               estimator={"dialogue_ids": [f"estimator-{i}" for i in range(64)]},
                               verification={"dialogue_ids": [f"verification-{i}" for i in range(64)]},
            ...
            if phase == "qualify":
                            ...
                            projection={"admitted": False, "threshold_seconds": 1800, "total_seconds": 5081.543, "preparation_seconds": 3.})
                write_json(directory / "projection.json", done["projection"])
            elif phase == "pilot":
                done.update(parity_passed=True, completed_forwards=1560, timed_forwards=1536, warmup_forwards=24,
                            calibration_work={"dialogue_count": 4, "encoder_calls": 4,
                                              "padded_attention_positions": 400, "real_question_updates": 4},
                            projection={"admitted": True, "threshold_seconds": 1800, "total_seconds": 60.,
                                        "all_verifications_passed": True, "verifications_passed": 12, "verifications_total": 12,
                                        "components": {"preparation_parent_wall_seconds": 3.}})
                write_json(directory / "projection.json", done["projection"])
            else:
                done.update(pilot_sha256=phases["pilot"]["completed_sha256"],
                            pilot_terminal_sha256=phases["pilot"]["terminal"]["sha256"],
                            calibration_work=receipts["pilot"]["calibration_work"],
        ...
        if phase in ("prepare", "qualify"):
            control["inputs"][control["outputs"][phase] + "/completed.json"] = phases[phase]["completed_sha256"]
            control["inputs"][terminal_path.name] = phases[phase]["terminal"]["sha256"]
    control_path.write_text(json.dumps(control))
    monkeypatch.setattr(report.common, "PINS", dict(control["inputs"]))
    ...
    def common_auth(request, budget):
        ...
        assert request.pilot_sha256 == phases["pilot"]["completed_sha256"]
        assert request.pilot_terminal == terminal_paths["pilot"]
        assert request.pilot_terminal_sha256 == phases["pilot"]["terminal"]["sha256"]
        budget.check()
        return ctx
    ...
    monkeypatch.setattr(report.common.legacy, "authenticate_output", common_output)
    monkeypatch.setattr(report.common.legacy, "authenticate_terminal", common_terminal)
    monkeypatch.setattr(report.common, "execute", lambda *_: pytest.fail("Neural lifecycle must never execute"))
    monkeypatch.setattr(report.common.legacy, "execute", lambda *_: pytest.fail("Legacy neural lifecycle must never execute"))
...
def test_complete_artificial_report_preserves_raw_failure_and_full_fit_files(artifacts):
    ...
    assert receipt["new_control_total_seconds"] == 12 + receipt["wall_seconds"]
    assert receipt["legacy_v1_cost_failure_preserved"] and receipt["runtime_v2_admission_passed"]
    assert summary["legacy_v1_cost_failure"]["admitted"] is False
    assert summary["runtime_v2_admission"]["admitted"] is True
    assert set(summary["costs"]["separate_phases"]) == {"prepare", "qualify", "pilot", "infer"}
    assert summary["costs"]["nonnested_prior_phase_seconds"] == 12
    ...
    assert "V1 cost admission remains failed" in (f.args.out / "report.md").read_text()
    assert f.events.count("upstream-read-only") == 2 and f.events.count("terminal") == 8
...
@pytest.mark.parametrize("phase", report.PHASES)
def test_every_phase_binding_is_mandatory_before_upstream_or_decode(artifacts, monkeypatch, phase):
    f = artifacts
    del f.plan["phases"][phase]
    f.args.plan.write_text(json.dumps(f.plan))
    f.args.plan_sha256 = report.common.sha(f.args.plan)
    monkeypatch.setattr(report.np, "load", lambda *_a, **_k: pytest.fail("Prediction decoded without full lineage"))
    monkeypatch.setattr(report, "read_lines", lambda *_: pytest.fail("Rows decoded without full lineage"))
    with pytest.raises(ValueError, match="All four phase bindings"):
        report.execute(f.args)
    assert not f.events


@pytest.mark.parametrize("defect", ["v1_rewritten", "pilot_pin", "pilot_parent_pin", "work_geometry"])
def test_prior_denial_and_new_pilot_joins_cannot_be_rewritten(artifacts, monkeypatch, defect):
    f = artifacts
    reader = report.common.legacy if defect == "v1_rewritten" else report.common
    original = reader.authenticate_output

    def modified(path, digest, ctx, phase):
        value = copy.deepcopy(original(path, digest, ctx, phase))
        if defect == "v1_rewritten" and phase == "qualify":
            value["projection"].update(admitted=True, total_seconds=1799.)
        elif phase == "infer":
            if defect == "pilot_pin":
                value["pilot_sha256"] = "0" * 64
            elif defect == "pilot_parent_pin":
                value["pilot_terminal_sha256"] = "0" * 64
            elif defect == "work_geometry":
                value["calibration_work"]["encoder_calls"] += 1
        return value

    monkeypatch.setattr(reader, "authenticate_output", modified)
    monkeypatch.setattr(report.np, "load", lambda *_a, **_k: pytest.fail("Prediction decoded with false admission"))
    monkeypatch.setattr(report, "read_lines", lambda *_: pytest.fail("Rows decoded with false admission"))
    expected = "failed V1 cost admission" if defect == "v1_rewritten" else "Complete admitted calibration inference"
    with pytest.raises(ValueError, match=expected):
        report.execute(f.args)
    assert report.common.read(f.args.out / "failed.json")["progress"]["completed_fits"] == []


def test_new_pilot_parent_must_succeed_before_decode(artifacts, monkeypatch):
    f = artifacts
    f.terminals["pilot"].write_text(json.dumps({"status": "failed", "returncode": 1, "wall_seconds": 3.}))
    f.plan["phases"]["pilot"]["terminal"]["sha256"] = report.common.sha(f.terminals["pilot"])
    f.args.plan.write_text(json.dumps(f.plan))
    f.args.plan_sha256 = report.common.sha(f.args.plan)
    monkeypatch.setattr(report.np, "load", lambda *_a, **_k: pytest.fail("Prediction decoded before parent success"))
    monkeypatch.setattr(report, "read_lines", lambda *_: pytest.fail("Rows decoded before parent success"))
    with pytest.raises(ValueError, match="Successful parent required"):
        report.execute(f.args)
    assert "payloads-infer" not in f.events
