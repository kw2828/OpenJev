...
import run_finite_balanced_learning as runner
...
def test_three_transition_models_complete_before_fresh_dev_and_independent_audit(tmp_path, monkeypatch):
    ...
    def generate(split, attempts, horizon, **kwargs):
        assert kwargs == {'seed_namespace': 940001}
    ...
    def factory(arm, seed, *, check):
        assert arm in runner.ARMS and seed == 940101 and generations == [0]
    ...
    config = {'seed_namespace': 940001, 'fit_seeds': [940101], 'train_attempts': 8,
    ...
    assert hashes['original_free'] == hashes['balanced'] != hashes['matched_free']
    ...
    for fit in fits:
        ...
        with np.load(folder / f"initial-{fit['arm']}-940101.npz", allow_pickle=False) as initial, \
                np.load(folder / f"boundary-{fit['arm']}-940101.npz", allow_pickle=False) as boundary:
    ...
    from audit_finite_balanced_learning import audit

    generations_before_audit, model_calls_before_audit = list(generations), len(public_calls)
    audited = audit(folder, profile='engineering-940001')
...
def test_failed_allocation_trace_is_saved_before_fit_rejection(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match='both original allocation stages'):
        runner.train('balanced', 940101, {'prefix': torch.zeros((1, 9, 31))}, prefixes,
                     {'batch_size': 1, 'stage1_seconds': 1., 'total_seconds': 2.}, tmp_path, {}, {}, lambda: None)
    assert json.loads((tmp_path / 'allocation-balanced-940101.json').read_text()) == result
...
def test_failed_normalization_preserves_partial_work_and_allocation(tmp_path, monkeypatch):
    ...
    error.balanced_model_work = {'completed_sweeps': 7, 'balanced_transition_calls': 1}

    def fail(*args, **kwargs):
        raise error

    monkeypatch.setattr(runner, 'run_allocation', fail)
    with pytest.raises(ValueError, match='failed fixed normalization'):
        runner.train('balanced', 940101, {}, {},
                     {'stage1_seconds': 1., 'total_seconds': 2.}, tmp_path, {}, {}, lambda: None)
    assert json.loads((tmp_path / 'allocation-balanced-940101.json').read_text()) == error.allocation_result
    assert json.loads((tmp_path / 'failed-normalization-balanced-940101.json').read_text()) == error.balanced_model_work
