...
SPEC = importlib.util.spec_from_file_location(
    "readout_compute_collector_tests", Path(__file__).resolve().parents[1] / "scripts/collect_otto_readout_compute.py")
...
def test_master_roster_is36_fresh_dev_paths_without_any_confirmation():
    rows = M.cohort()
    assert M.VERSION == "otto-readout-compute-collection-v1"
    assert M.CASES == {"dev": 6}
    assert len(rows) == M.MASTER_EPISODES == M.EPISODES == 36
    assert [r["episode_index"] for r in rows] == list(range(36))
    assert len({r["episode_id"] for r in rows}) == 36
    assert M.execution_cohort("dev") == rows == M.cohort("dev")
    assert M.STAGE_EPISODES == {"dev": 36}
    assert M.CONFIGURATION["confirm_reserved_episodes"] == 0
    seeds = sorted({r["seed"] for r in rows})
    assert seeds == [*range(318000001, 318000007), *range(319000001, 319000007)]
    assert not set(seeds) & set(M.FIT_SEEDS)
    assert not set(seeds) & {*range(316000001, 316000007), *range(317000001, 317000007)}
    ...
    for regime in ("lambda3", "lambda4"):
        assert len([r for r in rows if r["regime"] == regime]) == 18
    with pytest.raises(ValueError, match="master cohort stage"):
        M.cohort("confirm")
...
def test_full_census_caps_include_every_split_and_forbid_replay():
    assert M.CALL_CAPS["teacher_score"] == M.CALL_CAPS["tensorflow_value"] == 78768
    for name in ("native_step", "actor_update", "analytic_score", "feature_build"):
        assert M.CALL_CAPS[name] == 36 * 2188
    for name in ("native_reset", "actor_construction", "backend_binding"):
        assert M.CALL_CAPS[name] == 36
...
def seed_review():
    ...
    hits = [{"path": f"source-{index}.py", "line": 1, "column": 9, "literal": "318966016",
             "context": "count = 318966016", "value": 318966016, "exact_seed": False} for index in range(5)]
    reviewed = [{**hit, "source_verified": True, "classification": "outside the exact selected environment seeds; whole-block freshness not claimed"} for hit in hits]
    ...
    review = {**common, "version": "otto-readout-compute-seed-review-v1", "status": "reserved_before_run",
              "exact_seed_hits": 0, "resolved_block_hits": reviewed}
    reservation = {**common, "version": "otto-readout-compute-seed-reservation-v1", "attempt": 1,
...
def test_seed_review_exact12_reserved_environment_seeds_excludes_reused_fit_seeds():
    review, reservation = seed_review()
    M.validate_seed_review(review, reservation)
    assert len(review["fresh_environment_seeds"]) == 12
...
@pytest.mark.parametrize("defect", ("exact_seed", "column", "literal", "unknown_path", "bool_value"))
def test_saved_lexical_evidence_is_checked_without_reading_original_files(defect):
    review, reservation = copy.deepcopy(seed_review())
    hit = reservation["block_hits"][0]
    if defect == "exact_seed":
        hit.update(value=318000001, literal="318000001", context="count = 318000001")
    elif defect == "column":
        hit["column"] = 1
    elif defect == "literal":
        hit["literal"] = "319966016"
    elif defect == "unknown_path":
        hit["path"] = "not-scanned.py"
    else:
        hit["value"] = True
    review["resolved_block_hits"][0].update(hit)
    with pytest.raises(ValueError):
        M.validate_seed_review(review, reservation)
...
def test_plan_cannot_change_master_roster_or_dev_only_recipe(defect):
    ...
    if defect == "phase":
        ...
        plan["execution_cohort"] = [{**row, "stage": "confirm"} for row in M.cohort()]
...
def test_metadata_freeze_contains_full_master_and_only_dev_execution(tmp_path, monkeypatch):
    ...
    assert len(plan["cohort"]) == 36 and len(plan["execution_cohort"]) == 36
...
def test_seed_authentication_binds_saved_inventory_without_freezing_other_studies(tmp_path, monkeypatch):
    path, _review, reservation = seed_files_fixture(tmp_path, monkeypatch)
    sources = {}
    M.authenticate_seed_review(path, sources)
    assert set(sources) == {M.SEED_REVIEW, M.RESERVATION}
    assert all(M.descriptor(name)["sha256"] == pin for name, pin in sources.items())
    for hit in reservation["block_hits"]:
        (tmp_path / hit["path"]).unlink()
    M.authenticate_seed_review(path, {})


@pytest.mark.parametrize("defect", ("review", "reservation", "conflict"))
def test_seed_authentication_rejects_changed_originals_or_reviewed_sources(defect, tmp_path, monkeypatch):
    path, _review, _reservation = seed_files_fixture(tmp_path, monkeypatch)
    sources = {}
    if defect == "conflict":
        sources[M.RESERVATION] = "0" * 64
    else:
        name = {"review": M.SEED_REVIEW, "reservation": M.RESERVATION}[defect]
...
def test_fake_native_complete_dev_body_has36_resets_no_confirmation_and_one_census_each(tmp_path, monkeypatch):
    ...
    assert len(run.rows) == 36 and set(run.datasets) == {"dev"}
    assert events == [value for _ in range(36) for value in ("step", "update")]
    for name in ("native_reset", "native_step", "teacher_score", "tensorflow_value", "actor_update"):
        assert run.ledger.calls[name]["attempted"] == run.ledger.calls[name]["returned"] == 36
    ...
    assert summary["teacher_calls"] == 36 and summary["deployed_queries"] == 24 and summary["annotation_only"] == 12
    with np.load(tmp_path / "dev.npz", allow_pickle=False) as archive:
        assert archive["episode_offsets"].tolist() == list(range(37))
    with (tmp_path / "episode-boundaries.jsonl").open() as stream:
        boundaries = [json.loads(line) for line in stream]
    assert [row["event"] for row in boundaries] == [value for _ in range(36) for value in ("attempt", "return")]
...
def test_native_arithmetic_and_ledger_are_exact_explicit_source_fork():
    """Independent syntax comparison protects the qualified mechanics."""
    root = Path(__file__).resolve().parents[1]
    old = ast.parse((root / "scripts/collect_otto_residual.py").read_text())
    new = ast.parse((root / M.SELF).read_text())
    old_top = {node.name: node for node in old.body if isinstance(node, (ast.FunctionDef, ast.ClassDef))}
    new_top = {node.name: node for node in new.body if isinstance(node, (ast.FunctionDef, ast.ClassDef))}
    for name in ("route_scores", "virtual_query", "episode_offsets", "paired_identity", "Ledger"):
        assert ast.dump(old_top[name], include_attributes=False) == ast.dump(new_top[name], include_attributes=False)
    old_run = {node.name: node for node in old_top["Run"].body if isinstance(node, ast.FunctionDef)}
    new_run = {node.name: node for node in new_top["Run"].body if isinstance(node, ast.FunctionDef)}
    for name in ("episode", "complete_episode", "save_stage", "body", "append_durable"):
        assert ast.dump(old_run[name], include_attributes=False) == ast.dump(new_run[name], include_attributes=False)
    imports = [node for node in new.body if isinstance(node, (ast.Import, ast.ImportFrom))]
    ...
    assert not names & {"numpy", "torch", "tensorflow", "tf_keras", "scipy", "openjev"}
