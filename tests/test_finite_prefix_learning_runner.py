...
import run_finite_prefix_learning as r

CONFIG = {'seed_namespace': 931001, 'fit_seeds': [931101], 'train_attempts': 8,
...
def test_paired_fits_finish_before_dev_and_never_receive_oracle(tmp_path, monkeypatch):
    ...
    def checked_generation(split, attempts, horizon, **kwargs):
        assert kwargs == {'seed_namespace': 931001}
        generation.append(split)
        if split:
            barrier = json.loads((folder / 'checkpoint-barrier.json').read_text())
            assert barrier['fit_count'] == 2 and barrier['dev_generation_count'] == 0
    ...
    assert len(result['rows']) == 8 and len(result['prefix_rows']) == 2
    fits = result['fits']
    for key in ('initial_state_sha256', 'operator_initial_sha256', 'reset_initial_sha256',
                'case_order_sha256', 'fixed_buffers_sha256'):
        assert fits[0][key] == fits[1][key]
    assert all(fit['parameter_metadata']['parameter_count'] == 1088 for fit in fits)
    assert all(fit['updates'] == 3 and fit['training_attempts'] == 8 for fit in fits)
    assert [fit['prefix_loss_weight'] for fit in fits] == [0, 1]
    counts = result['counts']
    assert counts['optimizer_steps'] == counts['optimizer_attempts'] == 6
    assert counts['training_attempt_exposures'] == 16
    assert counts['training_case_exposures'] == 2 * result['dataset_counts']['train']['retained']
    assert counts['training_prefix_event_exposures'] == result['dataset_counts']['train']['valid_prefix_events']
    assert counts['training_prefix_rollouts'] == 3 and counts['evaluation_prefix_rollouts'] == 6
    assert result['structural_work']['endpoint_only']['training_prefix'] == {}
    assert all(counts[key] == 0 for key in ('checkpoint_decodes', 'array_decodes', 'external_model_calls', 'native_calls', 'teacher_calls'))
    for arm in r.ARMS:
        with np.load(folder / f'prefix-{arm}-931101-base.npz', allow_pickle=False) as saved:
...
def test_global_denominators_partial_batch_and_empty_endpoint_step(tmp_path, monkeypatch, arm):
    ...
    for batch_size in (1, 2):
        ...
        config = {**r.DEFAULT_CONFIG, 'fit_seeds': [931101], 'epochs': 1,
        ...
        keys = ('training_blind_rollouts', 'training_observed_rollouts', 'zero_endpoint_batches',
                ...
                'checkpoint_writes', 'fit_count')
        counts = dict.fromkeys(keys, 0)
        work = {arm: {route: {} for route in r.ROUTES}}
        fit = r.train(model, arm, 931101, data, prefixes, config, folder, counts, work, lambda: None)
        order = np.random.Generator(np.random.PCG64(np.random.SeedSequence([931101, 0, 818]))).permutation(3)
        expected = []
        for start in range(0, 3, batch_size):
            ids = order[start:start + batch_size]
            endpoint = 7. if 1 in ids else 0.
            likelihood = sum([2., 3., 5.][i] for i in ids) / 3 if arm == 'endpoint_plus_prefix' else 0.
