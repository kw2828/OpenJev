...
from openjev.research import otto_cross_query_data as old_data
from openjev.research import otto_cross_query_scores as old_models
from openjev.research import otto_prequery_data as data
from openjev.research import otto_prequery_loss as losses
from openjev.research import otto_prequery_metrics as metrics
...
runner = module("_prequery_training_fixture", "scripts/train_otto_prequery_calibration.py")
old = module("_original_training_fixture", "scripts/train_otto_cross_query_forecasts.py")
...
def test_mse_matches_original_update_and_does_not_read_poisoned_prior_labels(kind):
    fixture = history()
    ...
    new, reference = nonzero(models.make_head(kind, 31)), nonzero(old_models.make_head(kind, 31))
    ...
    result = runner.batch_update(torch, models, data, losses, new, opt, fixture, [0, 1],
        objective="mse", check=lambda: None, stage=lambda *_: None)
    expected = old.batch_update(torch, old_models, old_data, reference, old_opt,
        {**fixture, "version": old_data.VERSION}, [0, 1], check=lambda: None, stage=lambda *_: None)
    assert result["loss"] == expected["loss"] == result["nonquery_loss"]
    assert result["prior_loss"] == 0 and result["backward_chunks"] == 1 and result["no_grad_chunks"] == 1
...
@pytest.mark.parametrize("kind", models.KINDS)
def test_auxiliary_activates_missing_label_chunks_and_takes_one_final_update(kind):
    fixture = history(positive=False)
    model = nonzero(models.make_head(kind, 71))
...
def test_query_only_zero_support_still_gets_defined_adam_step():
    ...
    model = models.make_head("innovation", 3)
    optimizer = torch.optim.Adam(model.parameters(), lr=.003)
...
def test_valid_barrier_complete_prior_support_and_real_window_projection(tmp_path):
    ...
    run.receipt["fits_completed"] = 12; run.valid_allowed = True; run.check = lambda: None
