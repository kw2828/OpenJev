"""Fabricated sensor primitives and collector only; no native backend or data."""
...
C = sys.modules.get("otto_belief_distillation_common") or module(
    "scripts/otto_belief_distillation_common.py", "otto_belief_distillation_common")
M = module("scripts/collect_otto_belief_distillation.py", "belief_distillation_collector_tests")
REFERENCE = module("scripts/study_otto_released_reference.py", "belief_distillation_reference_tests")
...
def test_roster_caps_and_random_action_stream_are_fixed_before_outcomes():
    roster = C.roster()
    assert len(roster) == len({r["seed"] for r in roster}) == len({r["id"] for r in roster}) == 1792
    assert sum(r["split"] == "train" for r in roster) == 1536
    for regime in ("lambda3", "lambda4"):
        assert sum(r["split"] == "dev" and r["regime"] == regime for r in roster) == 128
    block = 1536 * 4 + 256 * 8
    assert M.CALL_CAPS["native_step"] == 1792 * 8 + block == 22528
    assert M.CALL_CAPS["feature_build"] == 1792 * 9 + block == 24320
    assert M.CALL_CAPS["teacher_score"] == M.CALL_CAPS["normal_oracle"] == block == 8192
    assert M.CALL_CAPS["sampler_lookup"] == 1792 * 8 + block * 2 == 30720
    assert M.CALL_CAPS["committed_positions"] == 1792 * 2
    seed = roster[0]["seed"]
    np.testing.assert_array_equal(M.committed_actions(np, seed, 4), M.committed_actions(np, seed, 8)[:4])
    with pytest.raises(ValueError):
...
@pytest.mark.parametrize("split,horizon", [("train", 4), ("dev", 8)])
def test_fake_case_keeps_reconstructable_prefix_and_exact_oracle_work(fabricated_collector, split, horizon):
    run = fabricated_collector()
    identity = next(r for r in C.roster() if r["split"] == split)
...
def test_known_terminal_suffix_is_onehot_but_first_found_is_forecast(fabricated_collector, found_at):
    ...
    assert run.ledger.calls["normal_oracle"]["returned"] == 4
