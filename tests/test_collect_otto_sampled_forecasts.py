...
SPEC = importlib.util.spec_from_file_location(
    "score_forecast_collector_tests", Path(__file__).resolve().parents[1] / "scripts/collect_otto_sampled_forecasts.py")
...
def test_stage_export_retains_exact_float_bytes_and_all_episode_offsets(tmp_path, monkeypatch):
    ...
    for identity in MODULE.cohort("train"):
        ...
        run.selections.append({"episode_id": identity["episode_id"], **select_windows(1, 23600001+i)})
...
def deferred_fixture(tmp_path, failure=None):
    ...
    for step in range(3):
        ...
        after = packet(step+1, step == 2)
        view._observe(_packet(after, step+1))
        history.append({"public": packet(step), "posterior": before, "action": 0,
    ...
    run.label_mask = [False, True, False]
...
def test_declared_teacher_cap_preserves_complete_validation_and_all_paths():
    train = 18*2188 + 18*547 + 18*8*4 + 18*8*3
    valid = 36*2188
    assert train == 50238 and valid == 78768
