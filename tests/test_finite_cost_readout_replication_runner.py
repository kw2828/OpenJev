...
import run_finite_cost_readout_replication as r

CONFIG = {'seed_namespace': 933001, 'fit_seeds': [933101], 'train_attempts': 8,
...
def test_paired_fits_finish_before_dev_and_never_receive_oracle(tmp_path, monkeypatch):
    ...
    def checked_generation(split, attempts, horizon, **kwargs):
        assert kwargs == {'seed_namespace': 933001}
    ...
    assert [fit['parameter_metadata']['parameter_count'] for fit in fits] == [1088, 1120]
    np.testing.assert_allclose(fits[0]['readout_initial']['matrix'], fits[1]['readout_initial']['matrix'], atol=1e-12, rtol=0)
    assert fits[0]['readout_initial'] == fits[0]['readout_final']
    assert all(fit['updates'] == 3 and fit['training_attempts'] == 8 for fit in fits)
    assert [fit['prefix_loss_weight'] for fit in fits] == [1, 1]
    ...
    assert counts['training_prefix_event_exposures'] == 2 * result['dataset_counts']['train']['valid_prefix_events']
    assert counts['training_prefix_rollouts'] == 6 and counts['evaluation_prefix_rollouts'] == 6
    assert all(result['structural_work'][arm]['training_prefix'] for arm in r.ARMS)
    assert counts['readout_snapshot_evaluations'] == 4 and counts['readout_snapshot_softmax_evaluations'] == 2
    assert all(counts[key] == 0 for key in ('checkpoint_decodes', 'array_decodes', 'external_model_calls', 'native_calls', 'teacher_calls'))
    for arm in r.ARMS:
        with np.load(folder / f'prefix-{arm}-933101-base.npz', allow_pickle=False) as saved:
...
def test_global_denominators_partial_batch_and_empty_endpoint_step(tmp_path, monkeypatch, arm):
    ...
    for batch_size in (1, 2):
        ...
        config = {**r.DEFAULT_CONFIG, 'fit_seeds': [933101], 'epochs': 1,
        ...
        fit = r.train(model, arm, 933101, data, prefixes, config, folder, counts, work, lambda: None)
        order = np.random.Generator(np.random.PCG64(np.random.SeedSequence([933101, 0, 818]))).permutation(3)
        expected = []
