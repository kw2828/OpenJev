...
SPEC = importlib.util.spec_from_file_location("compute_native_bridge_tests",
                                             Path(__file__).parents[1] / "scripts/otto_readout_compute_native_bridge.py")
...
def metadata_fixture(tmp_path, monkeypatch):
    ...
    plan = {"version": "otto-readout-compute-collection-v1", "status": "frozen_before_collection", "phase": "dev",
    ...
    collection = {"version": plan["version"], "phase": "dev", "status": "completed", "complete": True,
        ...
        "native_inputs": native, "limits": plan["limits"], "completed_episodes": 36, "dev_episodes": 36,
        "confirm_episodes": 0, "master_episodes": 36, "training_updates": 0, "old_test_array_decodes": 0,
...
def test_opaque_mutations_rejected_without_decoding(tmp_path, monkeypatch, target):
    ...
    paths = {"source": tmp_path / M.COLLECTOR, "native": Path(receipt["native_inputs"]["weights"]["path"]),
