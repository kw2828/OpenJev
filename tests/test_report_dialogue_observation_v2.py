...
spec = importlib.util.spec_from_file_location("observation_report", ROOT / "scripts/report_dialogue_observation_v2.py")
r = importlib.util.module_from_spec(spec)
spec.loader.exec_module(r)


@pytest.fixture(autouse=True)
def fake_report_clock(monkeypatch):
    clock_type = r.SuspendClock
    ticks = itertools.count(0, 1_000_000)
    monkeypatch.setattr(r, "SuspendClock", lambda: clock_type(lambda: next(ticks)))
...
def study(tmp_path, monkeypatch):
    ...
    for name in ("scripts/report_dialogue_observation_v2.py", "tests/test_report_dialogue_observation_v2.py",
                 "src/openjev/research/dialogue_observation_metrics.py", "tests/test_dialogue_observation_metrics.py", r.WATCHDOG, r.CLOCK, r.CLOCK_TEST):
        path = root / name
        path.parent.mkdir(parents=True, exist_ok=True)
    ...
    freeze = root / "freeze"
    freeze.mkdir()
    sources = {name: r.sha(root / name) for name in ("scripts/report_dialogue_observation_v2.py", "tests/test_report_dialogue_observation_v2.py",
                "src/openjev/research/dialogue_observation_metrics.py", "tests/test_dialogue_observation_metrics.py", r.WATCHDOG, r.CLOCK, r.CLOCK_TEST)}
    ...
    command = [sys.executable, "-u", str(root / "scripts/study_dialogue_observation_v2.py"), "train", "--plan", str(args.plan),
               "--plan-sha256", args.plan_sha256, "--out", str(run), "--supervision", str(args.launch)]
    launch = {"version": "dialogue-observation-supervision-v2", "command": command, "parent_pid": 122,
              "pid": 123, "pgid": 123, "cwd": str(root), "started_unix": 100., "cap_seconds": 100,
              "watchdog_sha256": sources[r.WATCHDOG], "clock_source_sha256": sources[r.CLOCK],
              "clock_backend": "mach_continuous_time", "started_ns": 100_000_000_000, "deadline_ns": 200_000_000_000}
    dump(args.launch, launch)
    dump(args.terminal, {**launch, "returncode": 0, "timed_out": False, "error": None, "clock_error": None,
                         "group_absent": True, "wall_seconds": 8., "finished_unix": 106.5, "status": "completed",
                         "finished_ns": 108_000_000_000, "elapsed_ns": 8_000_000_000, "timing_available": True,
    ...
    timing = {"clock_backend": launch["clock_backend"], "started_ns": 101_000_000_000,
              "parent_started_ns": launch["started_ns"], "deadline_ns": launch["deadline_ns"], "timing_available": True,
              "supervision_sha256": args.launch_sha256}
    dump(run / "started.json", {"request": {"out": str(run), "command": "train", "plan": str(args.plan),
         "plan_sha256": args.plan_sha256, "supervision": str(args.launch)}, **timing,
         "finished_ns": None, "elapsed_ns": None, "wall_seconds": None})
    done.update(timing, finished_ns=107_000_000_000, elapsed_ns=6_000_000_000, files=files(run))
    dump(run / "completed.json", done)
    args.completed_sha256 = r.sha(run / "completed.json")
...
def native_timing():
    parent = {"clock_backend": "mach_continuous_time", "started_ns": 10_000_000_000,
...
def test_civil_backward_forward_or_no_jump_does_not_change_admission(civil_finish):
    parent, end, start, done = native_timing()
    end["finished_unix"] = civil_finish
    r.clock_envelope(parent, end, start, done, 100)


@pytest.mark.parametrize("mutation", ["suspend", "equal_deadline", "backend", "ns", "before_parent", "after_parent", "origin", "unavailable"])
def test_native_timing_corruptions_fail_closed(mutation):
    parent, end, start, done = native_timing()
    if mutation in ("suspend", "equal_deadline"):
        # Even exit zero, plausible small awake time and civil timestamps cannot admit late native time.
        end["returncode"] = 0
    ...
    elif mutation == "backend": done["clock_backend"] = "CLOCK_BOOTTIME"
...
def test_new_lifecycle_failures_precede_prediction_loading(study, monkeypatch, mutation):
    if mutation == "missing_terminal":
        study.args.terminal.unlink()
    elif mutation in ("worker_hash", "parent_origin"):
        study.done["supervision_sha256" if mutation == "worker_hash" else "parent_started_ns"] = "0" * 64 if mutation == "worker_hash" else 0
        reseal(study)
    else:
        end = r.read(study.args.terminal)
        if mutation == "cleanup": end["cleanup"]["errors"] = ["could not reap"]
        else: end["version"] = "v1"
        dump(study.args.terminal, end)
        study.args.terminal_sha256 = r.sha(study.args.terminal)
    monkeypatch.setattr(np, "load", lambda *_args, **_kwargs: pytest.fail("No predictions before lifecycle acceptance"))
    with pytest.raises((ValueError, FileNotFoundError)):
        r.execute(study.args)
    assert not (study.args.out / "summary.json").exists()


def test_standalone_freeze_deadline_identity_and_equality_boundary():
    _, _, start, done = native_timing()
    start["parent_started_ns"] = done["parent_started_ns"] = start["started_ns"]
    start["supervision_sha256"] = done["supervision_sha256"] = None
    start["deadline_ns"] = done["deadline_ns"] = start["started_ns"] + 300_000_000_000
    r.freeze_timing(start, done, 300)
    done.update(finished_ns=done["deadline_ns"], elapsed_ns=300_000_000_000, wall_seconds=300.)
    with pytest.raises(ValueError): r.freeze_timing(start, done, 300)


def test_clock_initialization_failure_never_claims_zero_elapsed(study, monkeypatch):
    error = RuntimeError("native clock unavailable")
    monkeypatch.setattr(r, "SuspendClock", lambda: (_ for _ in ()).throw(error))
    with pytest.raises(RuntimeError, match="native clock"):
        r.execute(study.args)
    failed = r.read(study.args.out / "failed.json")
    assert failed["wall_seconds"] is None and failed["elapsed_ns"] is None
    assert failed["last_successful_elapsed_ns"] is None and failed["timing_available"] is False


def test_post_receipt_suspend_expiry_demotes_success(study, monkeypatch):
    from openjev.research.suspend_clock import SuspendClock
    values = iter([0, 0, 0, 0, 300_000_000_000])
    monkeypatch.setattr(r, "SuspendClock", lambda: SuspendClock(lambda: next(values)))
    with pytest.raises(ValueError, match="wall/RSS"):
        r.execute(study.args)
    assert not (study.args.out / "receipt.json").exists()
    assert (study.args.out / "invalid-receipt.json").exists()
    assert r.read(study.args.out / "failed.json")["scientific_result_qualified"] is False


def test_preserved_failure_and_old_sources_are_fixed(tmp_path, monkeypatch):
    monkeypatch.setattr(r, "ROOT", tmp_path)
    old = {f"original/{i}": "a" * 64 for i in range(53)}
    dump(tmp_path / r.OLD_PLAN, {"source_sha256": old})
    monkeypatch.setattr(r, "OLD_PLAN_PIN", r.sha(tmp_path / r.OLD_PLAN))
    failed = tmp_path / "failed.json"
    dump(failed, {"status": "failed_technical_timing", "required_fits": 12, "quality_metrics_opened": False,
                  "individual_predictions_decoded": False, "weights_loaded": False, "resume_permitted": False,
                  "partial_scoring_permitted": False, "process_group_absent": True})
    monkeypatch.setattr(r, "FAILED_PIN", r.sha(failed))
    ref = {"path": str(failed), "sha256": r.sha(failed)}
    sources = {**old, **dict.fromkeys(r.V2_SOURCES, "b" * 64)}
    r.authenticate_failure(ref, sources)
    sources["original/0"] = "c" * 64
    with pytest.raises(ValueError, match="53 sources"): r.authenticate_failure(ref, sources)
    value = r.read(failed); value["resume_permitted"] = True; dump(failed, value)
    monkeypatch.setattr(r, "FAILED_PIN", r.sha(failed)); ref["sha256"] = r.sha(failed)
    with pytest.raises(ValueError, match="unresumed"): r.authenticate_failure(ref, {**old, **dict.fromkeys(r.V2_SOURCES, "b" * 64)})


def test_scoring_and_coverage_functions_are_identical_to_frozen_v1():
    import ast
    def functions(path):
        return {node.name: ast.dump(node, include_attributes=False) for node in ast.parse(path.read_text()).body
                if isinstance(node, ast.FunctionDef)}
    before = functions(ROOT / "scripts/report_dialogue_observation.py")
    after = functions(ROOT / "scripts/report_dialogue_observation_v2.py")
    for name in ("aggregate", "actor_work", "prepared_metadata", "expected_work", "state_counts", "work_sum",
                 "merge_invariants", "validate_invariants", "audit_fit"):
