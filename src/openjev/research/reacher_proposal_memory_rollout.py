"""Additive proposal-memory engineering rows with unchanged physics and policy.

The caller binds the original innovations before execution. Only causal proposal
centers change. The separately retained original tracking row remains frozen.
"""
...
from openjev.research.reacher_tracking_policy import TrackingPolicy
...
def save_decision(folder, trace, inputs):
    ...
    meta = {"step": step, "arm": trace["arm"], "input_identities": dict(inputs.identities()),
            ...
            "journalcallbackmetadata": [], "selectedmetadata": None, "proposal_memory": trace["proposal_memory"]}
...
def run_row(*, arm, proposal_mode, target_path, gain_schedule, noise, reset_seed, inputs_by_step,
    ...
    checked on load. Per-decision traces bind both these base innovations and
    the causally translated inputs scored by the unchanged policy.
    """
    ...
    settings = {"arm": arm, "proposal_mode": proposal_mode, "steps": steps, "reset_seed": reset_seed, "window": window,
    ...
    episode = controller = memory = None
    phase, current_step, closed = "setup", 0, False
    native_seconds = decision_seconds = observe_seconds = serialization_seconds = memory_seconds = 0.
    try:
        (out / "decisions").mkdir()
        ...
        current_packet = initial.copy()
        memory = CEMProposalMemory(mode=proposal_mode, steps=steps,
            planning_horizon=planning_horizon, action_block=action_block)
        write(out / "initial-proposal-memory.json", memory.snapshot())
        ...
        for step, stem in enumerate(stems):
            ...
            phase = "proposal_memory_prepare"
            tick = time.perf_counter()
            search_inputs, memory_trace = memory.prepare(search_inputs, current_packet[4:6].copy(), step=step)
            memory_seconds += time.perf_counter()-tick
            phase = "decision"
            tick = time.perf_counter()
            ...
            check_deadline(deadline)
            tick = time.perf_counter()
            trace["proposal_memory"] = memory_trace
            ...
            phase = "proposal_memory_commit"
            tick = time.perf_counter()
            memory.commit(trace["search"].selected_sequences[0].copy(), public["commands"][-1].copy())
            memory_seconds += time.perf_counter()-tick
            current_packet = returned.copy()
            check_deadline(deadline)
            tick = time.perf_counter()
        ...
        write(out / "proposal-memory.json", memory.snapshot())
        phase = "close"
        closed = True
        episode.close()
        check_deadline(deadline)
        ...
        receipt = {"status": "completed", "engineering": engineering, "arm": arm, "steps": steps,
            "proposal_mode": proposal_mode, "proposal_memory_seconds": memory_seconds,
        ...
        if memory is not None:
            try:
                write(out / "partial-proposal-memory.json", memory.snapshot())
            except BaseException as capture_error:  # noqa: BLE001 - preserve the primary failure
                cleanup_errors.append({"phase": "capture_proposal_memory", "error": repr(capture_error)})
