"""Matched, caller-supervised ordinary policy learning on fixed TRAIN targets.
...
def _bundle(bundle, rows):
    from openjev.research.otto_cost_regression import VERSION as TARGET_VERSION
    ...
    _require(all(np.array_equal(result["analytic"][key], result["continuation"][key])
...
def train_pair(features, target_bundle, seed, *, check, emit, checkpoint, recipe=DEFAULT_RECIPE):
    ...
    Production: 558 finite float32 TRAIN feature rows; seeds 10101..10103; 80
    ...
    try:
        ...
        _require(_same_weights(initial["analytic"], initial["continuation"]), "paired initial weights differ")
        for arm in ARMS:
