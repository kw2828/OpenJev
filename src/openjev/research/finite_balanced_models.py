"""Prospective balanced transport with original and matched free controls.
...
is column softmax for the free arms and the qualified fixed64 row/column
log-domain algorithm for balanced. Balancing T does not balance the surviving
...
log of the balanced initial T, then verifies its column softmax within1e-12.
...
columns; finite64-sweep output only satisfies its measured residual bound.

Prefix likelihood and its optional prior share one constructed set of fields.
The balanced prior uses the returned logT from that same graph, never a column
...
autograd, validation or scalar reductions. No tensors are cached between calls.

Failure never repairs probabilities or adds sweeps. The last route's mutable
work is available as model.last_work; raised exceptions receive an owned copy
as balanced_model_work (also on constructor failure). Constructor and metadata
...
from openjev.research.finite_balanced_transition import WORK_KEYS as KERNEL_WORK_KEYS
...
VERSION = 'finite-balanced-model-v1'
ARMS = ('original_free', 'matched_free', 'balanced')
SWEEPS, TOLERANCE = 64, 1e-12
...
def _failure(error, work):
    error.balanced_model_work = dict(work)


def _validate_model(model):
    require(type(model) is BalancedDynamicsModel, 'declared balanced-dynamics adapter only')
    model._parameters_valid()


class BalancedDynamicsModel(FactorizedDynamicsModel):
    """Same checkpoint tensor roster, with an explicit transport arm."""

    def __init__(self, arm, seed, *, check=lambda: None):
        require(type(arm) is str and arm in ARMS, 'declared balanced-transport arm')
        ...
        try:
            ...
            if arm == 'matched_free':
                with torch.no_grad():
                    fields = balanced_transition(self.transition_logits, sweeps=SWEEPS,
                                                 tolerance=TOLERANCE, check=check, work=constructor_work)
    ...
    def _parameters_valid(self):
        require(self.transport_arm in ARMS and callable(self.check) and self.study_arm == 'factorized',
                'unchanged balanced transport and inherited factorized configuration')
    ...
    def _fields(self, work):
        ...
        try:
            ...
            if self.transport_arm == 'balanced':
                result = balanced_transition(self.transition_logits, sweeps=SWEEPS, tolerance=TOLERANCE,
                                             check=self.check, work=work)
    ...
    def parameter_metadata(self):
        ...
        metadata.update({
            ...
            'transition_ideal_constraint_degrees': 196 if self.transport_arm == 'balanced' else 224,
            'transition_constraint': 'finite balanced row and column normalization'
                                     if self.transport_arm == 'balanced' else 'column softmax',
            'transition_degrees_scope': 'Ideal normalized manifolds; finite64-sweep balanced output is approximate.',
            'balanced_sweeps': SWEEPS if self.transport_arm == 'balanced' else None,
            'balanced_tolerance': TOLERANCE if self.transport_arm == 'balanced' else None,
            'matched_initialization': self.transport_arm == 'matched_free',
            'scope': 'Same352 stored parameters. Balanced T is a task-derived structural prior, not a retention guarantee. '
...
def make_model(arm, seed, *, check=lambda: None):
    return BalancedDynamicsModel(arm, seed, check=check)
...
def torch_objective(model, prefix, lengths, pseudocount):
    ...
    The balanced transition prior shares forward's normalized logT. Free-arm
    ...
    try:
        ...
        if model.transport_arm == 'balanced':
