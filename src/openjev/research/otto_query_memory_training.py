"""Pure TRAIN-only, whole-episode optimization for query-written memory.

This module performs no file IO, sampling, evaluation or scientific admission.
The caller authenticates the complete TRAIN census and its fixed 54-path roster,
sets the qualified CPU runtime, and witnesses frozen weights outside batches.
Smaller fabricated cohorts use the same explicit episode-count normalization.

Parameters remain fixed through all chronological 32-step chunks in a batch.
Carry is explicitly detached at every chunk edge. Missing *effective* gradients
become zero only after all chunks, followed by one clipped Adam update. Frozen
slow parameters, including the mandatory trainable-flag residual, never belong
to the optimizer. Guard/stage callbacks may observe or abort, not mutate state.
"""
...
def _effective(model):
    require(type(model) is models.QueryMemoryModel, "qualified composed training model")
