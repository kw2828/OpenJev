"""Prospective combined-event cadence development and confirmation."""
import argparse
import gzip
import hashlib
import json
...
PROTOCOL = ROOT/'research/protocols/event-cadence-doom-v1.json'
SOURCES = ['research/event_cadence_doom.py', 'research/analyze_event_cadence_doom.py',
...
def episode(scenario, seed, arm, protocol, settings=None, weights=None, replicate=None, trace=None):
    cadence = EventCadenceControl(**settings) if settings is not None else None
...
def select_development(rows, protocol):
    ...
    for scenario in protocol['development_scenarios']:
        ...
        c, r = means[scenario][candidate], means[scenario]['rules']
        checks.append(c['net_utility']-r['net_utility'] >= protocol['selection']['minimum_development_net_gain_vs_rules'])
        checks.append(c['kills']-r['kills'] >= protocol['selection']['minimum_development_kill_difference_vs_rules'])
    selected = candidate if all(checks) else None
    return {'selected':selected, 'eligible':[candidate] if selected else [], 'means':means,
...
def main():
    ...
    if args.stage == 'confirmation':
        ...
        arms = {'selected':selected, **{a:s for a,s in protocol['arms'].items() if a != selection['selected']}}
