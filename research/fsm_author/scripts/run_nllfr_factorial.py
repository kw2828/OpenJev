...
"""Supervise one registered four-cell evaluation; no restart, fitting or fallback."""
...
from evaluate_nllfr_factorial import (
    ENV,
    ROOT,
    metadata_admission,
    pin,
...
def supervise(registration, requested_output=None, requested_receipt=None):
    ...
    cfg, _, admission = metadata_admission(registration)
    ...
    parent_fits = {k: pin(relative(cfg['prerequisites'][k+'_process']['path'])) for k in ('old', 'new')}
    receipt_folder.mkdir(parents=True, exist_ok=False)
    producer = Path(__file__).with_name('evaluate_nllfr_factorial.py')
    ...
    receipt = {'phase': 'evaluate', 'command': command, 'status': 'running',
               ...
               'supervisor': pin(__file__), 'producer': pin(producer), 'parent_fit_processes': parent_fits}
    ...
    try:
        ...
        identity_error = None
        try:
            after_cfg, _, after_admission = metadata_admission(registration)
            require(after_cfg == cfg and after_admission == admission, 'parent admission drift')
            require(sha(registration) == registration_hash, 'registration changed during process')
            require({k: pin(relative(cfg['prerequisites'][k+'_process']['path'])) for k in ('old', 'new')} == parent_fits, 'parent fit closures changed')
        ...
        receipt['scope'] = 'Original four-cell evaluation process; no restart/rescue. A capped training fit remains an incomplete reference.'
        process_path.write_text(json.dumps(receipt, indent=2, allow_nan=False)+'\n')
    print(json.dumps({'status': receipt['status'], 'phase': 'evaluate', 'observed_exit_code': code,
