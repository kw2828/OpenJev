...
The pinned ChessFly model card declares `license: other` and says FlyWire noncommercial terms apply to graph derivatives; the Space declares GPL-3.0 for its code. The complete source-data terms need separate verification before releasing derivative graph assets or trained weights. Keep cached graph bytes, induced graphs and rewires outside OpenJev's MIT distribution. Independent graph-control code and synthetic tests can be published without packaging those external assets. This note makes no broader licensing determination.
