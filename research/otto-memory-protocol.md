# OTTO classical references and odor-memory opportunity pilot
...
RockSample failures remain unchanged.
...
Use the official isotropic-19x19 parameters: two dimensions,
`lambda_over_dx=1`, `R_dt=1`, Euclidean sensing. Keep automatic grid/hit-category
selection and require it to produce **N=19 and Nhits=3**. Failure of that check
...
the agent reaches it. The fixed horizon is **642 primitive moves**.
...
completed observations. Category 2 contains the saturated Poisson tail.
...
There are eight blocks `b=0..7`. Each block contains four cases with initial
hit 1 and four with initial hit 2. For `hit` in `{1,2}` and `j=0..3`:

```text
case_index = b*8 + (hit-1)*4 + j
seed = 510001 + case_index
```

All eight arms run all **64 cases, 512 episodes**. Rotate the base arm order
...
eight-case block. The aggregate is not the unweighted mean of the balanced
cohort. Report both strata, all blocks and every episode.
...
Qualification is bounded to **1,000 native steps** within the total allocation.
It must establish:

- Expected 19x19/three-category geometry, action IDs and legal boundary moves;
  both initial-hit priors and correct saturated-tail likelihoods.
- Source-found termination, terminal-hit handling, reset separation, and no
  action after termination. Source discovery on step 642 counts as success.
...
case/block/arm identity, and capped time `min(T,642)`. A source not found by the
horizon incurs **642**, not its last successful prefix length. Log
...
9. All **512** episodes and their action/process accounting are complete.
...
RSS, 256 MiB output and 340,000 native steps**, including qualification. The
complete cohort's worst-case move count is 328,704; qualification adds at most
1,000. A resource or qualification failure stops the run and preserves partial
