...
CAPTION = (
    ...
    '18 fresh initializer fits and 30 unchanged control fits; dots are individual fits, diamonds summarize their errors or latencies.\n'
    'Temporal/local models each have 962 parameters: 590 transition plus 372 initializer; last-two has 590.\n'
...
def extract(results, resources, fits, permutation):
    ...
    require(len(prows) == len({key(r) for r in prows}) == 36 and {key(r) for r in prows} == pexpected, 'all 36 diagnostic score attempts')
    for row in prows:
        validate_metric(row)
...
def tables(values):
    ...
    lines += ['', '## All 48 fits and request costs', '',
    ...
    lines += ['', 'Storage includes numeric parameters, state, buffers and normalizers. Request arrays are separate; temporary workspace and Python overhead are not measured. Legacy instant LPV retains 192 inactive recurrent weights. Model sizes and precision differ.', '',
              '## Five frozen criteria', '', '| Criterion | Passed |', '|---|---|']
    lines += [f"| {clean(c['name'])} | {c['passed']} |" for c in values['result']['conditions']]
    lines += ['', '## Older-order corruption: all 36 descriptive scores', '',
    ...
    lines += ['', '## All 18 diagnostic attempt receipts', '', '| Recording | Model | Rate | Seed | Status | Check / error |', '|---|---|---:|---:|---|---|']
