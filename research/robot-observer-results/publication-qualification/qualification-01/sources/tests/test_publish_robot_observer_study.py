...
def test_render_four_common_accuracy_axes_and_every_instance_cost(tmp_path, monkeypatch):
    _, _, value = fixture()
    class Axis:
        ...
        def set_yscale(self, scale): self.scale = scale
    ...
    class Figure:
        ...
        def savefig(self, path, **kw): path.write_bytes(b'fabricated figure')
    axes, fig = Axes(), Figure()
    ...
    assert [len(a.points) for a in axes.flat] == [52, 52, 52, 52, 43, 43]
    ...
    assert '3 of 5 criteria pass (all 5 required)' in fig.title
