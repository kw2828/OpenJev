...
The [proposed next performance experiment](robot-observer-next-direction.md) tests joint observer/dynamics training against a matched short-prefix control and continued conventional baselines. It is unregistered and unrun. A separate [initialization-by-norm factorial](robot-observer-norm-factorial-proposal.md) would clarify numerical attribution, but is deprioritized because fixing training completion has not produced useful forecasts.
