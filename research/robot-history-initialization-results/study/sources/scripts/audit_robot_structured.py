...
PLAN_SHA = '0dc6ff429e2b320844b9051f2756d2d1890039a05c7e840ac444f4f453f05880'
PARENT_SHA = '8e12dac5ef2cc6acf95bea68bd8551f3040c103e628fa52697dba0511ed1c52a'
COMMIT = 'cc8e6c81375891262c7bef54bb6aaf35006887ad'
FRESH_ARMS = ('householder', 'dense_bounded', 'dense_unbounded', 'dense_mlp', 'gru32')
ARMS = (*FRESH_ARMS, 'legacy_instant')
PARAMETERS = dict(zip(ARMS, (630, 806, 806, 590, 5916, 1014), strict=True))
...
PAYLOADS = ('normalizers.npz', 'linear.npz', 'causal_ridge_1.npz', 'causal_ridge_1.json',
            ...
            *(f'lpv_instant-{seed}-lr{ri}/{name}' for seed in (8101, 8102, 8103)
...
COMMAND = ['.venv/bin/python', '-u', 'scripts/robot_structured_study.py', '--registration',
           'research/robot-structured-registration.json', '--output', 'output/robot-structured-study-v1']
...
def authenticate(study, run_receipt):
    """Metadata and opaque hashing only; no raw MAT reads or array decoding."""
    ...
    require(study == ROOT / 'output/robot-structured-study-v1', 'fixed original study')
    ...
    require(plan['version'] == 'robot-structured-study-v1' and set(plan['sources']) == set(SOURCES), 'source roster')
    ...
    parent_registration = ROOT / 'research/robot-transition-registration.json'
    require(descriptor(parent_registration)['sha256'] == PARENT_SHA == plan['parent_registration_sha256'], 'closed parent registration')
    prior = read(parent_registration)
    require(set(prior['sources']) == set(SOURCES[:14]) and all(prior['sources'][n] == plan['sources'][n] for n in SOURCES[:14]), 'unchanged parent sources')
    cfg = dict(prior['config'])
    cfg.update(version='robot-structured-study-v1', arms=list(FRESH_ARMS), comparison_arms=list(ARMS),
               fit_cap_seconds=900., wall_cap_seconds=10800., mean_ratio=1.02, paired_ratio=1.05,
               latency_ratio=.8, storage_ratio=.8, legacy_refit=False, causal_ridge_refit=False, batch_reuse=True)
    del cfg['mean_reduction'], cfg['paired_reduction']
    ...
    require(env['python'] == sys.version and env['platform'] == platform.platform()
            and env['machine'] == platform.machine() and env['thread_env'] == dict.fromkeys(THREADS, '1'), 'same runtime/platform')
    for package in ('numpy', 'scipy', 'torch'):
        require(importlib.metadata.version(package) == env[package], 'same package: ' + package)
    launcher = ROOT / 'scripts/launch_robot_structured.py'
    ...
    require(Path(closure['manifest']['path']) == parent_folder / 'manifest.json'
            and Path(closure['receipt']['path']) == parent_folder / 'receipt.json', 'original parent paths')
    ...
    require(parent['audit']['inputs']['manifest'] == closure['manifest']
            and parent['audit']['inputs']['producer_receipt'] == closure['receipt']
            and parent['audit']['inputs']['run_receipt'] == closure['process']
    ...
    require(set(plan['data']) == expected_data and plan['data'] == prior['data']
            == parent['audit']['inputs']['parent_data'], 'exact saved parent data roster')
    for name, item in plan['data'].items():
        checked_pin(item, 'saved data/' + name)
    require(set(plan['parent_payloads']) == set(PAYLOADS) and len(PAYLOADS) == 40, '40 cached parent payloads')
    for name, item in plan['parent_payloads'].items():
        ...
        copied = ('parent-normalizers.npz' if name == 'normalizers.npz' else
                  'parent-fits.json' if name == 'fits.json' else
                  name.replace('lpv_instant-', 'legacy_instant-', 1))
    ...
    require(Path(qual['path']) == engineering / 'qualification-02.json'
            and qualification['status'] == 'PASS' and qualification['sources'] == plan['sources']
            and qualification['launcher'] == plan['launcher']
            and len(qualification['commands']) == 2, 'qualified exact sources')
    require(datetime.fromisoformat(qualification['timestamp']) <= datetime.fromisoformat(plan['created_utc'])
            <= datetime.fromisoformat(launch['started_utc']), 'qualification and registration precede original launch')
    commands = [['.venv/bin/ruff', 'check', *(name for name in SOURCES if name.endswith('.py')), 'scripts/launch_robot_structured.py'],
                ['.venv/bin/python', '-m', 'pytest', '-q', *(name for name in SOURCES if name.startswith('tests/'))]]
    qlogs = []
    for i, (row, command) in enumerate(zip(qualification['commands'], commands, strict=True)):
        require(row['command'] == command and row['returncode'] == 0
                and Path(row['log']) == engineering / f'qualification-02-{i}.log'
    ...
    diagnostic = plan['diagnostic_evidence']
    require(set(diagnostic) == {'definition.json', 'analyze.py', 'diagnostic.json', 'receipt.json',
                               'interpretation.md', 'launch-01.json', 'process-01.json', 'analysis-01.log'}, 'pre-fit diagnostic evidence')
    for key, item in diagnostic.items():
        require(Path(item['path']) == ROOT / 'output/robot-transition-mechanism-diagnostic-v1' / key, 'diagnostic exact path')
        checked_pin(item, 'diagnostic/' + key)
    inputs.update(parent_closure=closure, parent_data=plan['data'], parent_payloads=plan['parent_payloads'],
                  diagnostic_evidence=diagnostic, qualification=pin(qual['path']), qualification_logs=qlogs)
    ...
    require(receipt['status'] == 'PASS' and receipt['registration_sha256'] == PLAN_SHA
            and [receipt[k] for k in ('fits', 'fresh_fit_attempts', 'cached_fit_records', 'rows', 'raw_decodes',
                                     'reference_refits', 'legacy_refits', 'saved_fit_loads', 'saved_dev_loads', 'confirmation_access', 'official_test_access')]
            == [36, 30, 6, 160, 0, 0, 0, 7, 2, False, False]
            and 0 < receipt['seconds'] <= cfg['wall_cap_seconds']
            and receipt['seconds'] <= process['elapsed_seconds'], 'complete original closure')
    return plan, inputs
...
def decisions(rows, resources, cfg):
    """Independent complete-roster selection and all61 Boolean conditions."""
    dev = cfg['partitions']['dev']
    lookup = {(r['recording'], r['arm'], r['seed'], r['learning_rate'], r['horizon']): r for r in rows}
    require(len(lookup) == 160 == len(rows), 'complete unique metric roster')
    ...
    selection = {'selected_rates': chosen, 'options': options, 'ridge_options': ridge_options,
                 'selected_ridge': ridge, 'scope': 'same already-exposed DEV2; all three seeds per recipe; legacy rates reselected from six frozen fits, no refit'}
    conditions, details = [], {}
    def add(name, flag):
    ...
    for name in dev:
        ...
        for arm in ARMS[1:]:
            ...
            for i, seed in enumerate(SEEDS):
                left = neural['householder'][i] if neural['householder'] else None
        ...
        for j in range(6):
            available = 'householder' in means and 'gru32' in means
            add(f'{name}/joint{j}_no_10pct_harm', available and sum(v['per_joint_rmse_deg'][j] for v in neural['householder'])/3 <= 1.1*sum(v['per_joint_rmse_deg'][j] for v in neural['gru32'])/3)
    def cost(arm):
    ...
    left, right = cost('householder'), cost('dense_bounded')
    add('at_most_80pct_dense_bounded_latency', left is not None and right is not None and left[0] <= .8*right[0])
    add('at_most_80pct_dense_bounded_numeric_storage', left is not None and right is not None and left[1] <= .8*right[1])
    passed = sum(c['passed'] for c in conditions)
    require(len(conditions) == 61, '61 conditions')
    return selection, {'status': 'QUALIFIES_FOR_NEW_CONFIRMATION_PROTOCOL' if passed == 61 else 'DO_NOT_ADVANCE_STRUCTURED_TRANSITION',
                       'passed': passed, 'total': 61, 'conditions': conditions, 'details': details,
...
def audit(study, run_receipt):
    ...
    import numpy as np
    import robot_structured_study as replay
    import torch
    torch.set_num_threads(1)
    cfg = plan['config']
    counts = {'npz_decodes': 0, 'array_decodes': 0, 'fit_attempts': 36, 'fresh_fit_attempts': 30, 'cached_fit_records': 6, 'checkpoint_replays': 0,
              'prediction_files': 0, 'metric_rows': 160, 'saved_batches': 3, 'dev_target_windows': 0,
    ...
    roster += [('legacy_instant', seed, rate, f'legacy_instant-{seed}-lr{i}') for seed in SEEDS for i, rate in enumerate(RATES)]
    ...
    require(len(fits) == len(roster) == 36, 'all original attempts retained')
    ...
    for number, (fit, (arm, seed, rate, key)) in enumerate(zip(fits, roster, strict=True), 1):
        ...
        if arm == 'legacy_instant':
            fit_keys |= {'parent_key', 'parent_files'}
            original_key = key.replace('legacy_instant-', 'lpv_instant-', 1)
            originals = [row for row in parent_fits if row['key'] == original_key]
            require(len(originals) == 1 and fit['origin'] == 'cached_parent'
                    and fit['parent_key'] == original_key and originals[0]['arm'] == 'lpv_instant'
                    and originals[0]['seed'] == seed and originals[0]['learning_rate'] == rate
                    and fit['fit'] == originals[0]['fit'] and fit['fit']['status'] == 'PASS'
                    and fit['parent_files'] == {name: plan['parent_payloads'][original_key + '/' + name]
                                                for name in LEGACY_FILES}, 'unchanged cached fit lineage')
        else:
            require(fit['origin'] == 'fresh', 'new fit origin')
        ...
        require(type(receipt['completed_updates']) is int and len(trace) == receipt['completed_updates'] <= 4096
                and receipt['requested_updates'] == 4096 and receipt['learning_rate'] == rate, 'update ledger')
        for update, row in enumerate(trace, 1):
            require(set(row) == {'update', 'loss', 'gradient_norm_before_clip'} and type(row['update']) is int and row['update'] == update
        ...
        if receipt['status'] == 'PASS':
            require(len(trace) == 4096 and receipt['error'] is None and receipt['fit_seconds'] <= (600 if arm == 'legacy_instant' else 900), 'complete successful fit')
            ...
            require(receipt['error']['type'] == 'FitFailure' and receipt['error']['message'] in (
                ...
                'nonfinite autoregressive training loss', 'nonfinite training gradient norm',
    ...
    require(barrier == {'fresh_fit_attempts': 30, 'cached_fit_records': 6, 'fit_records': 36, 'dev_loads_this_run': 0, 'dev_exposed_prior': True,
    ...
    close(result, saved_result['result'], 'independent 61 conditions')
    ...
    counts.update(failed_fit_attempts=failure_count, passed_fit_attempts=36-failure_count,
                  failed_metric_rows=sum(r['status'] == 'FAILED' for r in rows), resource_rows=len(resources),
                  manifest_files=len(expected), condition_rows=61,
                  ...
                  initial_checkpoints=36, final_checkpoints=36, optimizer_checkpoints=36)
    require(authenticate(study, run_receipt) == (plan, inputs), 'unchanged inputs after replay')
    return {'version': 'robot-structured-audit-v1', 'status': 'PASS', 'agreement': True, 'inputs': inputs,
            ...
            'checks': {'complete_original_closure': True, 'saved_fit_normalization': True, 'paired_batches': True,
                       ...
                       'legacy_and_references_byte_identical': True, 'all36_checkpoint_barrier': True},
            'scope': ['No raw MAT decode, CONFIRM/TEST access or optimizer updates.',
                      'Metrics, batch reconstruction, target alignment, eight reference forecasts and 61 rules are independently recomputed.',
                      'All 30 fresh fit records and six archived fit records are retained; archived updates are not charged as new training.',
