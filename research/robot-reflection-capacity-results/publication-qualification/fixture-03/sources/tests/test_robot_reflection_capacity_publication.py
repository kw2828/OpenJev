...
def test_complete_negative_fixture_preserves_184_42_25_and_centers():
    ...
    assert 'CONFIRM' in table and 'not ensemble forecasts' in table
...
@pytest.mark.parametrize('first,second,scale', [
    ([1., 2.], [10., 20.], 'linear'),
    ([.001, .002], [10., 20.], 'log'),
    ([0., .001], [10., 20.], 'symlog'),
    ([0.], [0.], 'linear'),
...
def test_accuracy_axes_share_limits_and_scale_without_hiding_zero_or_failures(first, second, scale):
    def entries(points):
        return [{'points': [{'value': value} for value in points]}]
    values = {'panels': [{'entries': entries(first)}, {'entries': entries(second)}],
              'latency_ms': entries([100.]), 'storage_kib': entries([2.])}
    specs = plotter.axis_specs(values)
    assert specs[0] == specs[1] and specs[0]['scale'] == scale
    low, high = specs[0]['limits']
    assert low < high and all(low <= value <= high for value in first + second)
    assert specs[2]['limits'] != specs[3]['limits']
...
def test_package_opaque_roundtrip_keeps_all_cached_forecasts_and_exclusion_hashes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(packager, 'publication_fixture_files', lambda: (tmp_path, set()))
