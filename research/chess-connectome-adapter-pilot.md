...
Status: the [architecture prototype](../src/openjev/research/chess_connectome_adapter.py) is implemented and [51 synthetic CPU tests](../tests/test_chess_connectome_adapter.py) pass. No training protocol is frozen, no biological-topology model has been fitted, and no efficacy result is available. This proposal follows the failed candidate-conditioned comparison and remains separate from its continuation criteria and the frozen ChessBench transfer panel.
...
Synthetic tests verify exact initial outputs against the full direct backbone, independent backbone storage, frozen weights across optimizer steps, immediate output-projection gradients and later graph gradients. They independently reconstruct signed message direction, degree normalization, recurrence and occupancy pooling, including isolated, parallel and self-edge fixtures. They also check deterministic shared mappings, restored global RNG state, reset between calls, reordered and padded candidate menus, native board/history preservation, CPU float32/float64 behavior, invalid inputs and parameter/MAC counts against actual module calls. These checks do not measure chess quality. GPU execution and full-size resource feasibility have not yet been checked.

## Runtime and asset handling

The initial implementation may materialize dense matrices for every topology. A 1,409-by-1,409 FP32 matrix occupies 7,941,124 bytes. Report actual matrix operations, frozen-backbone work, slot projections, legal-move preparation, training time and full inference time. A sparse parameter mask is not a sparse-kernel speed result. Run a bounded synthetic resource preflight before freezing a training budget.
