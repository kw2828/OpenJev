...
The [memory comparison](reacher-geometry-memory.md) is now complete and independently audited: all 25 checks passed. Persistent GRU lowered cost by 31.86%/25.99% versus cached GRU on six-step/ten-step gaps, improving every paired fit. All learned and physics controllers now use the same geometry score and CEM256 search; supplied-physics controllers remain better. The [prospective design](../output/reacher-geometry-score-v1/next-experiment-design.md) records the original rationale. The next conventional control reconstructs state from two valid observations and intervening actions. A biological claim still requires a specific mechanism and matched controls.

The new workflow passed 189 component tests and a complete engineering rehearsal using twelve existing tiny checkpoints. All 51 rows completed; the independent auditor checked 2,550 executed transitions, 1,230,336 nominal candidate transitions and 450 selected advances, with zero replay discrepancy. A separate full-batch capacity profile and independent replay also completed. These establish engineering readiness, not scientific effectiveness. The [engineering receipt](../output/reacher-geometry-memory-v1/whole-tree-rehearsal-verification.json) and [capacity review](../output/reacher-geometry-memory-v1/readiness-review.md) record the evidence and fixed limits.
