"""Launch exactly one frozen history-initialization campaign with a terminal receipt."""
...
OUTPUT = ROOT / 'output/robot-history-initialization-study-v1'
COMMAND = ['.venv/bin/python', '-u', 'scripts/robot_history_initialization_study.py',
           '--registration', 'research/robot-history-initialization-registration.json',
           '--output', 'output/robot-history-initialization-study-v1']
...
def main():
    ...
    for name in (*plan['sources'], str(REGISTRATION.relative_to(ROOT)),
                 'scripts/launch_robot_history_initialization.py'):
    ...
    record = {'command': COMMAND, 'prefit_commit': commit,
              ...
              'scope': 'Eighteen fresh attempts and 30 cached fits; five fixed criteria; no restart or CONFIRM/TEST access'}
