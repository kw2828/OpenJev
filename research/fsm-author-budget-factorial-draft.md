# Training budget versus state-estimation budget

Status: prospective draft. No registration or new DEV evaluation exists for this
comparison. The larger-budget FIT-only process is separate and may not yet have
closed. Exact checkpoint/audit identities must be frozen before evaluation.
...
standardized targets. No new recording, 300 mV or official-test member is decoded.
...
Rotate the four-condition order across those 24 request slots using their index
modulo four. Each condition occupies each order position six times. The same
explicit order is used regardless of results.
...
complete new-checkpoint cells are controls. An unresolved required new reference
or an incomplete forecast cell blocks an affirmative survival claim. The original
capped fits remain diagnostic, not references required to become complete, and
cannot be promoted by improved inference alone.
