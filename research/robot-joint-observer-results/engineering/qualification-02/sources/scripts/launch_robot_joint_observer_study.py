"""Launch one jointly trained observer development study with an original terminal receipt."""
...
ENGINEERING = ROOT / 'output/robot-joint-observer-engineering-v1'
REGISTRATION = ROOT / 'research/robot-joint-observer-registration.json'
OUTPUT = ROOT / 'output/robot-joint-observer-study-v1'
COMMAND = ['.venv/bin/python', '-u', 'scripts/robot_joint_observer_study.py',
           '--registration', 'research/robot-joint-observer-registration.json',
           '--output', 'output/robot-joint-observer-study-v1']
...
def main():
    ...
    for name in (*plan['sources'], str(REGISTRATION.relative_to(ROOT)),
                 'scripts/launch_robot_joint_observer_study.py'):
    ...
    record = {'command': COMMAND, 'prefit_commit': commit,
              ...
              'scope': 'Twelve jointly trained observer/control fits with identical weight warm starts and complete retained controls; four exposed development recordings; no restart or official TEST access'}
