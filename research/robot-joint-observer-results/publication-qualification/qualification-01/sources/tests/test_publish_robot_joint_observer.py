...
import publish_robot_joint_observer as pub
...
def fixture():
    audit = pub.auditor
    rates = {'observer_position': .003, 'observer_position_fixed': None,
             'local_affine': .003, 'temporal_affine': .001,
             **dict.fromkeys(audit.parent_audit.parent_audit.FIXED),
             **audit.parent_audit.parent_audit.CACHED_RATES}
    ...
    selection = {'fixed_rates': dict.fromkeys(audit.ARMS, .001), 'inherited_rates': rates,
                 'alias': {'arm': 'frozen_position_lr001', 'source_arm': 'observer_position', 'learning_rate': .001},
                 'scope': cfg['selection_scope']}
    rows, attempts = [], []
    for identities in (audit.parent_audit.parent_audit.identities(), audit.parent_audit.identities(), audit.identities()):
    ...
    resources = [{**r, **audit.storage(r['arm']), 'status': 'PASS', 'error': None,
                 ...
                 for r in audit.resource_identities(rates)]
    conditions = [{'name': name, 'passed': i != 1} for i, name in enumerate(audit.CONDITIONS)]
    gate = {'status': 'JOINT_OBSERVER_DEVELOPMENT_FAIL', 'passed': 4, 'total': 5, 'conditions': conditions,
    ...
    counts = {'fresh_fits': 12, 'inherited_models': 45, 'final_checkpoints': 57,
              ...
              'resource_rows': 61, 'condition_rows': 5, 'parent_forecast_replays': 0,
              'optimizer_updates': 0, 'backward_replays': 0, 'native_clip_replays': 0, 'timing_replays': 0}
    value = {'results': {'version': audit.STUDY_VERSION, 'config': cfg, 'selection': selection, 'rows': rows, 'result': gate},
             ...
             'fit_diagnostics': {r['key']: {} for r in audit.identities()}, 'counts': counts}
    ...
    inputs.update({'parent/'+r['key']+'/final.npz': dict(pin) for r in audit.inherited_identities(rates)})
    ...
    # Literal inventory built separately from publication's roster helper.
    names = {'registration.json','runtime.json','fits.json','initialization-checks.json','checkpoint-barrier.json',
    ...
    names |= {r['key']+'/final.npz' for r in (*audit.identities(), *audit.inherited_identities(rates))}
    ...
    names |= {f'completed-fit-{i:02d}.json' for i in range(1,13)}
    names |= {f'completed-prediction-{i:03d}.json' for i in range(1,49)}
    names |= {f'completed-timing-{i:02d}.json' for i in range(1,62)}
    names |= {'dev-windows-'+n+'.npz' for n in audit.EXPOSED}
    names |= {r['prediction_file'] for r in attempts[244:] if r['prediction_file'] is not None}
...
def test_full_negative_roster_preserves_parent_failure_and_only_four_exclusions():
    ...
    assert len(inventory) == 444 and len(value['results']['rows']) == 584
    assert len(value['prediction_attempts']) == 292 and len(value['resources']) == 61
    assert len(pub.FAMILIES) == len(pub.LABELS) == 24
...
@pytest.mark.parametrize('damage', ['missing_initialization','missing_fit_diagnostic','old_prediction','measurement','traversal','lost_parent_row','promoted'])
def test_omissions_extra_banks_and_promotion_reject(damage):
    value, plan, inventory = fixture()
    if damage == 'missing_initialization': inventory.pop('initialization-checks.json')
    elif damage == 'missing_fit_diagnostic': inventory.pop('joint_observer_long-8101-lr0/diagnostic-summary.json')
    ...
    else: value['results']['result']['status'] = 'JOINT_OBSERVER_DEVELOPMENT_PASS'
...
def test_parent_rates_are_fixed_and_failed_observer_never_selected():
    ...
    for row in value['results']['rows']:
        ...
        if row['arm'] == 'observer_position': assert answer is True
        if row['arm'] in pub.auditor.ARMS: assert answer is True
...
def test_tables_retain_every_row_and_missing_cost_is_never_zero(tmp_path):
    ...
    assert len(scores) == 584 and len(costs) == 61
    ...
    assert 'no complete parent recipe' in text and 'JOINT_OBSERVER_DEVELOPMENT_FAIL' in text
    assert json.loads((tmp_path/'audited-summary.json').read_text())['fit_diagnostics'] == value['fit_diagnostics']
    assert not any(r['arm'] == pub.auditor.ALIAS for r in scores)
    assert [r['model_key'] for r in costs[-3:]] == [f'observer_position-{s}-lr0' for s in (8101,8102,8103)]
...
def test_render_full_and_detail_keep_every_outlier_and_shared_axes(tmp_path, monkeypatch):
    ...
    for axes in (full, detail):
        ...
        assert [len(a.points) for a in axes.flat] == [66, 67, 67, 67, 61, 61]
        assert sum(point[2]['marker'] == 'x' for axis in axes.flat[:4] for point in axis.points) == 24
...
def admitted_tree(root, monkeypatch):
    ...
    source = root/'scripts/audit_robot_joint_observer.py'; opaque(source)
    registration = root/'research/robot-joint-observer-registration.json'; write(registration, plan)
    ...
    for name, payload in (('registration.json', plan), ('results.json', value['results']), ('resources.json', value['resources']),
                          ('prediction-attempts.json', value['prediction_attempts']),
                          ('parent/results.json', {'rows': value['results']['rows'][:488]}),
                          ('parent/prediction-attempts.json', value['prediction_attempts'][:244])):
    ...
    write(eng/'registration-preflight-01.json', {'status': 'PASS', 'registration': inputs['registration'],
          'qualification': plan['qualification'], 'sources': 63, 'inputs': 69, 'model_calls': 0,
    ...
    launch = {'command': ['.venv/bin/python', 'scripts/audit_robot_joint_observer.py', '--study', pub.STUDY,
...
def test_closed_metadata_and_original_audit_bytes_join_before_saved_numbers(tmp_path, monkeypatch):
    ...
    assert len(auth['excluded']) == 4 and len(auth['inventory']) == 444
    assert auth['audit']['results']['result']['status'] == 'JOINT_OBSERVER_DEVELOPMENT_FAIL'


@pytest.mark.parametrize('damage', ['exit', 'timeout', 'overcap', 'command', 'log', 'closure', 'manifest',
                                   'source', 'results', 'parent_row', 'parent_attempt', 'fit_diagnostic', 'inputs', 'agreement', 'preflight'])
def test_original_audit_and_scalar_mismatches_fail_closed(tmp_path, monkeypatch, damage):
    ...
    elif damage == 'source': opaque(tmp_path/'scripts/audit_robot_joint_observer.py', b'changed')
    elif damage == 'results':
        data = pub.read(study/'results.json'); data['result']['passed'] = 5; write(study/'results.json', data)
    elif damage == 'parent_row': write(study/'parent/results.json', {'rows': []})
    elif damage == 'parent_attempt': write(study/'parent/prediction-attempts.json', [])
    elif damage == 'fit_diagnostic': write(study/'joint_observer_long-8101-lr0/diagnostic-summary.json', {'changed': True})
    elif damage == 'preflight':
        path = eng/'registration-preflight-01.json'; data = pub.read(path); data['inputs'] = 68; write(path, data)
    else:
        data = pub.read(audit)
...
def test_opaque_package_roundtrip_and_measurement_exclusions(tmp_path, monkeypatch):
    study, audit, eng, _, _ = admitted_tree(tmp_path, monkeypatch)
    monkeypatch.setattr(pub, 'render', lambda *args: None)
    receipt = pub.publish(study, audit, eng, tmp_path/pub.OUTPUT)
    output = tmp_path/pub.OUTPUT; manifest = pub.read(output/'manifest.json')
    assert receipt['status'] == 'PASS' and receipt['scientific_status'] == 'JOINT_OBSERVER_DEVELOPMENT_FAIL'
...
def test_changed_source_during_copy_prevents_success_receipt(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(pub, 'render', change)
...
def test_all_original_science_qualifications_preserved(tmp_path):
    ...
    assert len(files) == 142  # 63 source files, launcher, seven original receipt/log/helper files, per attempt.
...
def test_alias_moves_only_display_view_never_duplicates_or_relabels_scores():
    value, _, _ = fixture(); original = copy.deepcopy(value['results']['rows'])
    for recording in pub.auditor.EXPOSED:
        rows = [r for r in value['results']['rows'] if r['recording'] == recording and r['horizon'] == 128]
        groups = {a: [r for r in rows if pub.display_arm(r) == a] for a in pub.FAMILIES}
        assert sum(map(len, groups.values())) == len(rows) == 73
        assert len(groups['observer_position']) == len(groups['frozen_position_lr001']) == 3
        assert all(r['learning_rate'] == .003 for r in groups['observer_position'])
        assert all(r['arm'] == 'observer_position' and r['learning_rate'] == .001
                   for r in groups['frozen_position_lr001'])
    assert value['results']['rows'] == original


@pytest.mark.parametrize('damage', ['alias_key', 'alias_model', 'rate_selection', 'diagnostic_count', 'parent_replay', 'failed_cost'])
def test_fixed_alias_and_no_replay_scope_guards(damage):
    value, _, _ = fixture()
    if damage == 'alias_key': value['resources'][-1]['key'] = 'relabelled'
    elif damage == 'alias_model': value['resources'][-1]['model_key'] = 'observer_position-8103-lr1'
    elif damage == 'rate_selection': value['results']['selection']['fixed_rates'][pub.auditor.CANDIDATE] = .003
    elif damage == 'diagnostic_count': value['fit_diagnostics'].pop(next(iter(value['fit_diagnostics'])))
    elif damage == 'parent_replay': value['counts']['parent_forecast_replays'] = 1
    else: value['resources'][0]['status'] = 'FAILED'
    with pytest.raises(ValueError): pub.validate_scalars(value)


def test_complete_fresh_failure_stays_visible_and_cost_unavailable():
    value, plan, inventory = fixture()
    key = 'joint_observer_long-8101-lr0'; error = {'type': 'FailedTrainingAttempt'}
    for row in value['results']['rows']:
        if row['fit_key'] == key: row.update(status='FAILED', error=error, metrics=None)
    for row in value['prediction_attempts'][244:]:
        if row['fit_key'] == key:
            inventory.pop(row['prediction_file'])
            row.update(status='FAILED', errors=[error, error], prediction_file=None)
    cost = next(r for r in value['resources'] if r['key'] == key)
    cost.update(status='UNAVAILABLE', error=error, timing=None)
    gate = value['results']['result']
    gate['costs'][pub.auditor.CANDIDATE] = None
    gate['equal_file_means'].pop(pub.auditor.CANDIDATE)
    for detail in gate['details'].values(): detail['means'].pop(pub.auditor.CANDIDATE)
    for condition in gate['conditions']: condition['passed'] = False
    gate['passed'] = 0
    pub.validate_scalars(value)
    assert len(inventory) == 440
    pub.study_roster(inventory, plan, value['prediction_attempts'])


def test_all_five_saved_conditions_allow_positive_presentation_without_recomputation(tmp_path):
    value, _, _ = fixture()
    gate = value['results']['result']
    gate.update(status='JOINT_OBSERVER_DEVELOPMENT_PASS', passed=5)
    for row in gate['conditions']: row['passed'] = True
    pub.validate_scalars(value)
    pub.write_tables(tmp_path, value)
    assert '**JOINT_OBSERVER_DEVELOPMENT_PASS**, 5/5' in (tmp_path/'table.md').read_text()
    assert json.loads((tmp_path/'audited-summary.json').read_text())['results'] == value['results']
