...
class Run:
    ...
    def fit(self, kind, seed):
        ...
        for epoch in range(1,cfg['epochs']+1):
            order = rng.permutation(len(x)); order_sha = hashlib.sha256(order.tobytes()).hexdigest()
            ...
            for batch, offset in enumerate(range(0,len(x),cfg['batch_size'])):
                ...
                total_updates += 1; weighted.append(loss*len(ids))
        ...
        self.emit('fits.jsonl',record); self.fits.append(record); self.receipt['completed_fits'] += 1
    ...
    def execute(self):
        ...
        try:
            self.bind(); self.prepare(); self.fits = []
