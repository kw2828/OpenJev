...
def read(path):
    path = regular(path)
    require(path.stat().st_size <= 80 * 1024**2, "bounded saved JSON")
...
class Run:
    ...
    def __init__(self, args, kind="diagnosis"):
        ...
        try:
            ...
            write(self.out / "started.json", {"phase": kind, "launch": self.launch, "plan_sha256": args.plan_sha256})
    ...
    def finish(self, payloads, extra=None):
        for name, value in payloads.items():
            self.check(); require(Path(name).name == name and name.endswith(".json"), "flat JSON result")
...
def run(args):
    ...
    try:
        ...
        windows.update({key: meta[key] for key in ("episode_ids", "episode_regimes")})
