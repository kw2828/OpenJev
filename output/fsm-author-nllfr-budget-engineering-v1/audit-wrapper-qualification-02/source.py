    ...
    try:
        with LOG.open('xb') as handle:
            ...
            if record['state'] == 'running':
                record['state'] = 'EXITED'
    except BaseException as exc:  # noqa: BLE001 - preserve interruption/launch evidence and reap the child.
        ...
        except Exception as exc:  # noqa: BLE001 - any closure failure must invalidate the terminal receipt.
        ...
        for key, path in (('log', LOG), ('audit_output', OUTPUT)):
            ...
            try:
                if path.exists():
                    record[key] = pin(path)
            except Exception as exc:  # noqa: BLE001 - retain unreadable evidence as failure, never success.
        ...
        if OUTPUT.exists():
            ...
            except Exception as exc:  # noqa: BLE001 - malformed audit output must preserve failed closure.
