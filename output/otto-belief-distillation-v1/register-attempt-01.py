...
for name, descriptor in {**native['native_sources'], **native['native_inputs']}.items():
...
for name in ['seed-review-01.json','seed-review-02.json','source-review-01.json','register-01.py']:
    path = c.OUT / name
...
for directory in sorted(c.OUT.glob('engineering-*')) + sorted(c.OUT.glob('component-*')):
    for path in sorted(directory.iterdir()):
        if path.is_file():
            inputs[str(path.relative_to(c.ROOT))] = c.desc(path)
sources = {**native['native_sources'], **engineering['source_after']}
plan = {'version':c.VERSION, 'status':'frozen_before_collection', 'config':c.CONFIG, 'roster':c.roster(),
        'sources':sources, 'inputs':inputs, 'native_sources':native['native_sources'], 'runtime':native['native_runtime'],
