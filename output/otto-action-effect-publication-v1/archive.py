...
REGISTRATION_PIN = 'ef985361d0fe1593437a5dcfb56823dd8df79dab8f0dd88ed635487ad1994c2e'
...
def main(closure_sha):
    require(Path.cwd() == ROOT and re.fullmatch('[0-9a-f]{64}', REGISTRATION_PIN)
            and re.fullmatch('[0-9a-f]{64}', closure_sha), 'repository root and external closure pin')
    ...
    reference = plan['training_reference']
    parent = ROOT / 'output/otto-belief-distillation-v1'
    parent_train = parent / 'collection-01/train.npz'
    require(reference['train']['path'] == parent_train.relative_to(ROOT).as_posix()
            and reference['cases'] == 1046 and reference['training_horizon'] == 4
            and reference['prior_dev_decodes'] == 0
            and desc(parent_train) == {key: reference['train'][key] for key in ('sha256', 'bytes')}
            and parent_train in paths, 'exact registered reused TRAIN only')
    require({path for path in paths if path.is_relative_to(parent) and path.suffix in ('.npz', '.npy')}
            == {parent_train}, 'no parent DEV, predictions or checkpoints in registered inputs')
    ...
    required = ('README.md', 'LICENSE', 'research/experiment-index.md',
        'research/otto-action-effect-results.md', 'research/otto-action-effect-protocol.md',
        'research/otto-action-effect-next.md',
        'research/jev-architecture-source-review.md', 'tmp/otto-source-review-01/LICENSE',
        'output/otto-action-effect-v1/close-01.py',
        'output/otto-action-effect-publication-v1/render-01.py',
        'output/otto-action-effect-publication-v1/release-notes.md',
        'output/otto-action-effect-publication-v1/archive.py',
        'output/otto-action-effect-publication-v1/verify-remote-01.py',
    ...
    result_dir = ROOT / 'research/otto-action-effect-results'
    ...
    required_rendered = {'research/otto-action-effect-results.md',
                         'research/otto-action-effect-results/benchmark.png',
                         'research/otto-action-effect-results/summary.json'}
    ...
    require({path for path in paths if path.is_relative_to(parent) and path.suffix in ('.npz', '.npy')}
            == {parent_train}, 'publication additions preserve parent TRAIN-only array scope')
    files, by_name = {}, {}
    ...
    manifest = {'study': VERSION, 'files': files, 'registered_bindings': bindings,
        ...
        'scope': 'All registered source/direct input bytes including only the parent TRAIN array, exact new-study scientific payloads/checkpoints/journals, closed process metadata and completed publication/media. Parent DEV, old raw TEST and reserved confirmation arrays are not added.',
        'upstream_training': reference,
        'parent_evidence_release': 'https://github.com/kw2828/OpenJev/releases/tag/otto-belief-distillation-v1',
        ...
        'external_dependencies': ['Qualified Python/Torch/TensorFlow environments and executables are not bundled.',
            'Direct registered native weights/tensors and the parent TRAIN array are bundled. Parent closure and receipts refer to excluded parent DEV/fit/audit payloads and older native authentication chains; use the linked parent evidence release and repository for that separate evidence. No self-contained executable reproduction is claimed.',
