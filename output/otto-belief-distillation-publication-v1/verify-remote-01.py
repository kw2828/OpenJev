"""Verify released asset digests and exact README image blobs, without UI claims."""
...
TAG = 'otto-belief-distillation-v1'
...
def images(readme):
    ...
    require('research/otto-belief-distillation-results/benchmark.png' in names, 'new benchmark embedded in README')
    return names


def verify(commit):
    require(Path.cwd() == ROOT and re.fullmatch('[0-9a-f]{40}', commit), 'repository root and full publication commit')
    ...
    names = ['README.md', *images(local_readme.decode())]
    verified = []
    ...
    for name in names:
        ...
        require(len(entries) == 1, 'single committed README/image entry')
        ...
        require(sha(data) == expected['sha256'] and len(data) == expected['bytes'], 'image/README belongs to evidence archive')
    ...
    return {'status': 'passed', 'release': release['html_url'], 'commit': commit,
        'peeled_tag_commit': target['sha'], 'assets': asset_checks, 'readme_and_images': verified,
        'scope': 'GitHub server asset digests and exact commit blobs; every remote image frame decoded locally. This does not verify browser rendering, animation playback, or GitHub UI appearance.',
...
if __name__ == '__main__':
    ...
    try:
        result = verify(args.commit)
    except BaseException as error:
        with args.output.open('x') as stream:
            json.dump({'status': 'failed', 'commit': args.commit, 'error_type': type(error).__name__,
    ...
    print(json.dumps({'status': result['status'], 'release': result['release'], 'assets': len(result['assets']),
                      'readme_images': len(result['readme_and_images']) - 1}))
