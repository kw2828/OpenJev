...
def preserve_failure(path, value, error):
    try:
        write(path, value)
    except BaseException as secondary:  # noqa: BLE001 - preserve the original failure
        error.add_note(f'Failure receipt also failed: {secondary}')
...
def encode(args):
    ...
    try:
        ...
        needed_files = ['config.json', 'model.safetensors', 'tokenizer_config.json',
                        'special_tokens_map.json', 'tokenizer.json', 'vocab.txt']
        cached_files = {n: Path(hf_hub_download(ENCODER, n, revision=REVISION, local_files_only=True))
                        for n in needed_files}
        model_files = {n: sha(p) for n, p in cached_files.items()}
        ...
        # Verify exactly the model/tokenizer files used, not unrelated repository auxiliaries.
        for name, cached_path in cached_files.items():
            assert sha(cached_path) == model_files[name], name
        ...
        preserve_failure(out / 'failed.json', {'status': 'failed', 'error_type': type(e).__name__,
                         'error': str(e), 'wall_seconds': time.perf_counter() - started}, e)
        raise
...
def evaluate(model, ds, queries, features, destination):
    ...
    if getattr(model, 'method', None) in ('kalman', 'innovation_kalman'):
        original_update = model.update

        def observed_update(turn, valid, state):
            updated, values = original_update(turn, valid, state)
            active = values['valid']
            inflation = values['inflation'][active]
            ratio = values['normalized_innovation'][active]
            diagnostic.update({'real_turn_updates': int(active.sum()),
                               'inflated_updates': int((inflation > 0).sum()),
                               'inflation_sum': float(inflation.sum()),
                               'normalized_innovation_sum': float(ratio.sum())})
            diagnostic['maximum_inflation'] = max(diagnostic['maximum_inflation'],
                                                   float(inflation.max()) if inflation.numel() else 0.)
            return updated, values

        model.update = observed_update
    with torch.inference_mode():
    ...
    return {'queries': len(arrays['labels']), 'wall_seconds': time.perf_counter() - started,
            'innovation_diagnostics': dict(diagnostic),
            'diagnostic_scope': 'Internal learned-feature residuals, not semantic calibration; no extra passes'}


def reference_predictions(ds, queries, destination):
    """Deterministic references run once after freeze; no annotation-based parser."""
    from openjev.research.dialogue_carry import literal_mention_carry
    started = time.perf_counter()
    arrays = {k: [] for k in ['pred_none', 'pred_literal', 'labels', 'bin', 'unseen',
                              'dialogue', 'time', 'query']}
    for d in ds:
        tracks = {}
        for qi in sorted({r['query'] for r in d['queries']}):
            entry = queries[qi]
            candidates = [value if value is not None else entry['candidate_ids'][i]
                          for i, value in enumerate(entry['candidate_values'])]
            tracks[qi] = literal_mention_carry(d['user_text'], candidates, excluded_indices=(0, 1))
        for r in d['queries']:
            for k, value in [('pred_none', 0), ('pred_literal', tracks[r['query']][r['time']]),
                             ('labels', r['label']), ('bin', r['bin']), ('unseen', r['unseen']),
                             ('dialogue', d['id']), ('time', r['time']), ('query', r['query'])]:
                arrays[k].append(value)
    np.savez_compressed(destination, **{k: np.asarray(v) for k, v in arrays.items()})
    return {'file': Path(destination).name, 'sha256': sha(destination), 'queries': len(arrays['labels']),
            'wall_seconds': time.perf_counter() - started,
            'scope': 'Always NONE and longest unique literal USER mention with carry; ontology values only; '
                     'no negation, yes/no, DONTCARE interpretation, annotation spans or calibrated scores'}
...
def train(args):
    ...
    records, active, progress = [], None, {}
    started = time.perf_counter()
    try:
        references = reference_predictions(packet['cohorts']['dev'], queries,
                                            Path(args.out) / 'references.npz')
        for seed in SEEDS:
        ...
        preserve_failure(Path(args.out) / 'failed.json', {'status': 'failed', 'completed_fits': records,
              'active_fit': None if active is None else active.name,
              'active_progress': progress,
              'error_type': type(e).__name__, 'error': str(e),
              'wall_seconds': time.perf_counter() - started}, e)
        raise
