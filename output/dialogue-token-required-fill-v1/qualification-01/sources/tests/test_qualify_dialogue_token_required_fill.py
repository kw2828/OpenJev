...
import qualify_dialogue_token_required_fill as q
...
def fake_results():
    ...
    for case in q.CASES:
        ...
        paths = {"original": {"seconds": ratio}, "required_fill": {"seconds": 1.}}
...
def test_dense_cases_and_recipe_remain_unchanged():
    assert q.CASES == q.consolidation.CASES == q.consolidation.factoring.projection.CASES == q.packing.CASES
    assert q.consolidation.RECIPE["pair_orders"][0] == ["original", "consolidated"]
    assert q.consolidation.factoring.projection.RECIPE["pair_orders"][0] == ["original", "projected"]
...
def test_corrupt_membership_and_time_rejected(kind):
    ...
    if kind == "missing":
        ...
        rows[15]["measured"][0]["paths"]["required_fill"]["seconds"] = float("nan")
    else:
...
def test_tiny_geometry_actual_output_all_gradient_and_monitor_parity(mode, head):
    ...
    assert models["required_fill"].configuration()["packing_reference_only_fields"] == list(q.REFERENCE_FIELDS)
    ...
    assert [e["path"] for e in events] == ["original", "required_fill"]
    assert events[1]["packing"]["positions_and_scalars"] == expected
    assert work["parity_backward_returned"] == 2 and work["active"]["path"] == "required_fill"
    assert result["outputs"]["negative_infinity_elements"] > 0
    models["required_fill"].last_packing_work["packed_evidence_positions"] -= 1
    with pytest.raises(ValueError, match="packing work"):
        q.packing_record(models["required_fill"], expected)
...
def test_tiny_case_canonical_postwarm_adam_state_resets_and_complete_work(monkeypatch):
    ...
    assert all("packing" in e for e in events if e["path"] == "required_fill")
...
def frozen(tmp_path, monkeypatch):
    ...
    names = {q.PRIOR_SOURCE, q.GEOMETRY} | {f"prior/{i}.py" for i in range(59)}
    ...
    for name in ("training-plan.json", "batching-plan.json", "batching-completed.json", "packing-plan.json", "packing-completed.json", "factoring-plan.json", "factoring-completed.json", "projection-plan.json", "projection-completed.json"):
    ...
    args = SimpleNamespace(consolidation_plan=parent_dir/"plan.json", consolidation_plan_sha256=q.sha(parent_dir/"plan.json"),
        protocol=tmp_path/q.ADD_SOURCES[-1], protocol_sha256=q.sha(tmp_path/q.ADD_SOURCES[-1]), out=tmp_path/"freeze")
    digest = q.freeze(args)
    return args, digest


def test_synthetic_freeze_full66_closure_no_calls_and_exclusive(frozen):
    args, digest = frozen
    p = q.validate_plan(args.out/"plan.json", digest)
    assert len(p["source_sha256"]) == 66 and p["loss_weights"] == [2., 1., 2/3]
...
def test_corrupt_envelope_before_backend(frozen, kind):
    ...
    if kind == "source":
        ...
        (args.out/"consolidation-completed.json").write_text("{}")
    else:
        if kind == "runtime":
            ...
            p["consolidation_plan_sha256"] = "0"*64
        else:
...
def test_required_fill_counts_include_concat_schema_fill_state_and_reference_only_fields():
    ...
    assert len(work) == 70 and work["exogenous_width"] == 394
...
def test_multiple_attention_groups_keep_consolidated_calls_and_reduce_only_fill():
    valid = np.array([[True, True]], dtype=bool)
    candidate = np.array([[[True, True, False]]], dtype=bool)
    token = np.zeros((1, 2, 20), dtype=bool)
    token[0, 0, :3] = True
    token[0, 1, :19] = True
    previous = q.consolidation.mask_work(valid, candidate, token)
    ...
    assert previous["fill_feature_positions"] == 3 and work["fill_feature_positions"] == 1
    revised = {"fill_feature_positions", "fill_feature_calls", "fill_exogenous_input_scalars",
               "fill_hidden_scalars", "fill_feature_macs", "factored_feature_macs"}
    assert all(work[key] == value for key, value in previous.items() if key not in revised)


@pytest.mark.parametrize("kind,expected_fill", [("none", 0), ("masked_candidate", 1), ("padded_turn", 3), ("all", 6)])
def test_required_fill_predicate_and_typed_payloads(kind, expected_fill):
    ...
    actual = classes["required_fill"].packing_work(torch.from_numpy(valid), torch.from_numpy(candidate), torch.from_numpy(tokens))
    assert actual == work
    record = q.packing_record(SimpleNamespace(last_packing_work=actual), work)
    assert len(record["reference_only_fields"]) == 15
