"""Exclusive consolidated-pooling correctness/cost screen; no corpus or encoders.
...
import qualify_dialogue_token_factoring as factoring
import qualify_dialogue_token_packing as packing

ROOT = Path(__file__).resolve().parents[1]
VERSION = "dialogue-token-consolidation-qualification-v1"
PRIOR_SOURCE = "scripts/qualify_dialogue_token_factoring.py"
PRIOR_SHA256 = "2781d917d4dc4f14f3c77a54705227cddd9b21e6c44710514185d877c4dda1f2"
PARENT_PLAN_SHA256 = "c717e81f88e1c6c3e3fcc43b681fb8f5e6e4c8725b380d3b4ebc56ae9c71d536"
GEOMETRY, GEOMETRY_SHA256 = packing.GEOMETRY, packing.GEOMETRY_SHA256
ADD_SOURCES = ("src/openjev/research/dialogue_token_consolidated.py", "tests/test_dialogue_token_consolidated.py",
    "scripts/qualify_dialogue_token_consolidation.py", "tests/test_qualify_dialogue_token_consolidation.py",
    "research/dialogue-token-consolidation-protocol.md")
PATHS = ("original", "consolidated")
CASES = copy.deepcopy(factoring.CASES)
RECIPE = {**copy.deepcopy(factoring.RECIPE), "pair_orders": [list(PATHS), list(reversed(PATHS))]*2,
    "projection": "One biased turn projection/tanh over concatenated supported pooled rows",
    "feature_factoring": "One supported-row exogenous projection; one schema fill; two state columns at each ordered head update",
    "consolidation": "Attention groups retained; concatenate normalized pooled rows before shared projection and feature GEMMs"}
REFERENCE_FIELDS = (
...
def mask_work(valid, candidate, token):
    """Independent masks: unchanged row/MAC work, consolidated calls and payload."""
    work = factoring.mask_work(valid, candidate, token)
    rows = work["packed_evidence_positions"]
    return {**work, "turn_projection_calls": 1, "empty_turn_projection_calls": int(rows == 0),
        "group_feature_calls": int(rows > 0), "pooled_concat_scalars": rows*384,
        "pooled_concat_bytes_float32": 4*rows*384}


def parent_plan(path, expected, *, complete=True):
    bind(source_path(PRIOR_SOURCE), PRIOR_SHA256)
    require(expected == PARENT_PLAN_SHA256, "Exact frozen factoring plan required")
    return factoring.validate_plan(path, expected, complete=complete)


def freeze(args):
    with old.attempt(args.out, "freeze", {k: str(v) for k, v in vars(args).items()}) as (out, start, _, check):
        parent = parent_plan(args.factoring_plan, args.factoring_plan_sha256)
        require(Path(args.protocol).resolve() == source_path(ADD_SOURCES[-1]).resolve(), "Consolidation protocol path")
        bind(args.protocol, args.protocol_sha256)
        mapping = {**parent["source_sha256"], **{n: sha(source_path(n)) for n in ADD_SOURCES}}
        require(len(mapping) == 61 and len(parent["source_sha256"]) == 56, "Expected 61-source closure")
        snapshot(out, mapping)
        parent_dir = Path(args.factoring_plan).parent
        for name, origin in (("factoring-plan.json", args.factoring_plan), ("factoring-completed.json", parent_dir/"completed.json"),
                *((name, parent_dir/name) for name in ("projection-plan.json", "projection-completed.json", "packing-plan.json", "packing-completed.json", "batching-plan.json", "batching-completed.json", "training-plan.json"))):
            shutil.copyfile(origin, out/name)
        plan = {"version": VERSION, "recipe": RECIPE, "runtime": old.runtime(), "source_sha256": mapping,
            "factoring_plan_sha256": args.factoring_plan_sha256, "factoring_completed_sha256": sha(out/"factoring-completed.json"),
            ...
            "scope": "Same sixteen artificial workloads, consolidated pooled-row projection only; no scientific/full-training admission"}
...
def validate_plan(path, expected, *, complete=True):
    ...
    require(p["factoring_plan_sha256"] == PARENT_PLAN_SHA256, "Exact frozen factoring plan required")
    parent = parent_plan(path.parent/"factoring-plan.json", p["factoring_plan_sha256"], complete=False)
    bind(path.parent/"factoring-completed.json", p["factoring_completed_sha256"])
    done = read(path.parent/"factoring-completed.json")
    require(done["status"] == "completed" and done["phase"] == "freeze" and done["plan_sha256"] == p["factoring_plan_sha256"]
        and done["source_sha256"] == parent["source_sha256"] and done["model_calls"] == 0 and done["no_retry"] is True, "Parent freeze incomplete")
    require(len(p["source_sha256"]) == 61 and set(p["source_sha256"]) == set(parent["source_sha256"]) | set(ADD_SOURCES)
...
def backend():
    ...
    from openjev.research.dialogue_token_consolidated import DialogueTokenConsolidatedMemory
    class MonitoredConsolidated(study.v2.MonitoredCopyMemoryV2, DialogueTokenConsolidatedMemory):
        pass
    return study, torch, {"original": study.MonitoredTokenMemory, "consolidated": MonitoredConsolidated}
...
def packing_record(model, expected):
    ...
    return {"positions_and_scalars": dict(actual),
        ...
        "scope": "Grouped/pooled-concat/schema-fill/state and hidden-scatter payload; previous dense/projected fields reference-only; not traffic or peak memory"}
...
def parity(models, data, case, weight, study, torch, progress, emit, check, expected):
    ...
    aliased = {"original": models["original"], "batched": models["consolidated"]}
    def record(row):
        row = dict(row)
        row["path"] = "consolidated" if row["path"] == "batched" else row["path"]
        if row["path"] == "consolidated":
            row["packing"] = packing_record(models["consolidated"], expected)
        emit(row)
    try:
        return old.parity(aliased, data, case, weight, study, torch, progress, record, check)
    finally:
        if progress.get("active", {}).get("path") == "batched":
            progress["active"]["path"] = "consolidated"
...
def run_case(case, geometry, weight, study, torch, classes, progress, emit, check):
    ...
    for pair, order in enumerate(RECIPE["pair_orders"]):
        ...
        result["measured"].append({"pair": pair, "order": order, "paths": rows, "speed_ratio": rows["original"]["seconds"]/rows["consolidated"]["seconds"]})
...
def run(args):
    with old.attempt(args.out, "run", {**{k: str(v) for k, v in vars(args).items()}, "consolidation_harness_sha256": sha(__file__)}, capped=True) as (out, start, progress, check):
        ...
        write(out/"completed.json", {"status": "completed", "version": VERSION, "plan_sha256": args.plan_sha256,
            ...
            "scope": "Original assembler plus consolidated forward counters included in update wall; external comparison and receipt formatting outside update timing"})
...
if __name__ == "__main__":
    ...
    for key in ("factoring-plan", "factoring-plan-sha256", "protocol", "protocol-sha256", "out"):
