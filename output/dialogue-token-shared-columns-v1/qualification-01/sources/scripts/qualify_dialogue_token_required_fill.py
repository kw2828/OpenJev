"""Exclusive required-fill correctness/cost screen; no corpus or encoders.
...
import qualify_dialogue_token_consolidation as consolidation
import qualify_dialogue_token_packing as packing

ROOT = Path(__file__).resolve().parents[1]
VERSION = "dialogue-token-required-fill-qualification-v1"
PRIOR_SOURCE = "scripts/qualify_dialogue_token_consolidation.py"
PRIOR_SHA256 = "e031c87386c645d45caad5051a7b30519fd1d7e3a4d228eeef2d2c96bc221642"
PARENT_PLAN_SHA256 = "f2a1e01dbde82b180c1bca26df61e67756a952b308bc85a17123baf6c3c03ac8"
GEOMETRY, GEOMETRY_SHA256 = packing.GEOMETRY, packing.GEOMETRY_SHA256
ADD_SOURCES = ("src/openjev/research/dialogue_token_required_fill.py", "tests/test_dialogue_token_required_fill.py",
    "scripts/qualify_dialogue_token_required_fill.py", "tests/test_qualify_dialogue_token_required_fill.py",
    "research/dialogue-token-required-fill-protocol.md")
PATHS = ("original", "required_fill")
CASES = copy.deepcopy(consolidation.CASES)
RECIPE = {**copy.deepcopy(consolidation.RECIPE), "pair_orders": [list(PATHS), list(reversed(PATHS))]*2,
    "projection": "One biased turn projection/tanh over concatenated supported pooled rows",
    "feature_factoring": "One supported-row exogenous projection; only required schema fill; two state columns at each ordered head update",
    "required_fill": "Evaluate schema fill only where any turn is padded or a candidate is masked; no fill GEMM when no row needs fill"}
REFERENCE_FIELDS = (
    ...
    "dense_feature_macs", "full_fill_reference_positions", "full_fill_reference_calls",
    "full_fill_reference_exogenous_scalars", "full_fill_reference_hidden_scalars", "full_fill_reference_macs")
...
def mask_work(valid, candidate, token):
    """Independent public-mask fill rows, calls, MACs and typed logical payloads."""
    work = consolidation.mask_work(valid, candidate, token)
    b, t = valid.shape
    k = candidate.size
    needed = (~valid.all(axis=1))[:, None, None] | ~candidate
    f = int(needed.sum())
    partial = 0 < f < k
    p, h, e = 64, 64, 394
    rows, dense = work["packed_evidence_positions"], work["dense_evidence_positions"]
    gathered, lexical, zeros = 2*f*p if partial else 0, 10*f, k*h if f < k else 0
    return {**work, "fill_feature_positions": f, "fill_feature_calls": int(f > 0),
        "fill_exogenous_input_scalars": f*e, "fill_hidden_scalars": f*h,
        "fill_feature_macs": f*e*h, "factored_feature_macs": (rows+f)*e*h+2*dense*h,
        ...
        "fill_zero_storage_scalars": zeros, "fill_schema_scatter_positions": f if partial else 0,
        "fill_schema_scatter_scalars": f*h if partial else 0,
        "fill_auxiliary_float32_bytes": 4*(gathered+lexical+zeros), "fill_mask_predicate_calls": 1}


def parent_plan(path, expected, *, complete=True):
    bind(source_path(PRIOR_SOURCE), PRIOR_SHA256)
    require(expected == PARENT_PLAN_SHA256, "Exact frozen consolidation plan required")
    return consolidation.validate_plan(path, expected, complete=complete)


def freeze(args):
    with old.attempt(args.out, "freeze", {k: str(v) for k, v in vars(args).items()}) as (out, start, _, check):
        parent = parent_plan(args.consolidation_plan, args.consolidation_plan_sha256)
        require(Path(args.protocol).resolve() == source_path(ADD_SOURCES[-1]).resolve(), "Required-fill protocol path")
        bind(args.protocol, args.protocol_sha256)
        mapping = {**parent["source_sha256"], **{n: sha(source_path(n)) for n in ADD_SOURCES}}
        require(len(mapping) == 66 and len(parent["source_sha256"]) == 61, "Expected 66-source closure")
        snapshot(out, mapping)
        parent_dir = Path(args.consolidation_plan).parent
        for name, origin in (("consolidation-plan.json", args.consolidation_plan), ("consolidation-completed.json", parent_dir/"completed.json"),
                *((name, parent_dir/name) for name in ("factoring-plan.json", "factoring-completed.json", "projection-plan.json", "projection-completed.json", "packing-plan.json", "packing-completed.json", "batching-plan.json", "batching-completed.json", "training-plan.json"))):
            shutil.copyfile(origin, out/name)
        plan = {"version": VERSION, "recipe": RECIPE, "runtime": old.runtime(), "source_sha256": mapping,
            "consolidation_plan_sha256": args.consolidation_plan_sha256, "consolidation_completed_sha256": sha(out/"consolidation-completed.json"),
            ...
            "scope": "Same sixteen artificial workloads, required schema fill only; no scientific/full-training admission"}
...
def validate_plan(path, expected, *, complete=True):
    ...
    require(p["consolidation_plan_sha256"] == PARENT_PLAN_SHA256, "Exact frozen consolidation plan required")
    parent = parent_plan(path.parent/"consolidation-plan.json", p["consolidation_plan_sha256"], complete=False)
    bind(path.parent/"consolidation-completed.json", p["consolidation_completed_sha256"])
    done = read(path.parent/"consolidation-completed.json")
    require(done["status"] == "completed" and done["phase"] == "freeze" and done["plan_sha256"] == p["consolidation_plan_sha256"]
        and done["source_sha256"] == parent["source_sha256"] and done["model_calls"] == 0 and done["no_retry"] is True, "Parent freeze incomplete")
    require(len(p["source_sha256"]) == 66 and set(p["source_sha256"]) == set(parent["source_sha256"]) | set(ADD_SOURCES)
...
def backend():
    ...
    class MonitoredRequiredFill(study.v2.MonitoredCopyMemoryV2, DialogueTokenRequiredFillMemory):
        pass
    return study, torch, {"original": study.MonitoredTokenMemory, "required_fill": MonitoredRequiredFill}
...
def packing_record(model, expected):
    ...
    return {"positions_and_scalars": dict(actual),
        ...
        "float32_payload_bytes": {k: 4*v for k, v in actual.items() if k.endswith("_scalars")
            and k not in REFERENCE_FIELDS and k not in ("fill_mask_boolean_scalars", "fill_count_int64_scalars", "fill_index_int64_scalars")},
        "boolean_payload_bytes": {"fill_mask_boolean_scalars": actual["fill_mask_boolean_scalars"]},
        "int64_payload_bytes": {k: 8*actual[k] for k in ("fill_count_int64_scalars", "fill_index_int64_scalars")},
        "mac_scope": "First feature linear layer only; multiply-accumulates, not total model FLOPs or measured time",
        "scope": "Grouped/pooled-concat/required-fill/state and hidden-scatter logical payloads; mask bool/index int64 charged separately; full fill and earlier dense/projected fields reference-only; aliases are not additive, not traffic or peak memory"}
...
def parity(models, data, case, weight, study, torch, progress, emit, check, expected):
    ...
    aliased = {"original": models["original"], "batched": models["required_fill"]}
    def record(row):
        row = dict(row)
        row["path"] = "required_fill" if row["path"] == "batched" else row["path"]
        if row["path"] == "required_fill":
            row["packing"] = packing_record(models["required_fill"], expected)
        emit(row)
    try:
        return old.parity(aliased, data, case, weight, study, torch, progress, record, check)
    finally:
        if progress.get("active", {}).get("path") == "batched":
            progress["active"]["path"] = "required_fill"
...
def run_case(case, geometry, weight, study, torch, classes, progress, emit, check):
    ...
    for pair, order in enumerate(RECIPE["pair_orders"]):
        ...
        result["measured"].append({"pair": pair, "order": order, "paths": rows, "speed_ratio": rows["original"]["seconds"]/rows["required_fill"]["seconds"]})
...
def run(args):
    with old.attempt(args.out, "run", {**{k: str(v) for k, v in vars(args).items()}, "required_fill_harness_sha256": sha(__file__)}, capped=True) as (out, start, progress, check):
        ...
        write(out/"completed.json", {"status": "completed", "version": VERSION, "plan_sha256": args.plan_sha256,
            ...
            "scope": "Original assembler plus required-fill forward counters included in update wall; external comparison and receipt formatting outside update timing"})
...
if __name__ == "__main__":
    ...
    for key in ("consolidation-plan", "consolidation-plan-sha256", "protocol", "protocol-sha256", "out"):
