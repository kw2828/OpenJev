...
import qualify_dialogue_token_packing as q
...
def fake_results():
    ...
    for case in q.CASES:
        ...
        paths = {"original": {"seconds": ratio}, "packed": {"seconds": 1.}}
...
def test_corrupt_membership_and_time_rejected(kind):
    ...
    if kind == "missing":
        ...
        rows[15]["measured"][0]["paths"]["packed"]["seconds"] = float("nan")
    else:
...
def test_tiny_geometry_actual_output_all_gradient_and_monitor_parity(mode, head):
    ...
    assert [e["path"] for e in events] == ["original", "packed"]
    assert events[1]["packing"]["positions_and_scalars"] == expected
    assert work["parity_backward_returned"] == 2 and work["active"]["path"] == "packed"
    assert result["outputs"]["negative_infinity_elements"] > 0
    models["packed"].last_packing_work["packed_evidence_positions"] -= 1
    with pytest.raises(ValueError, match="packing work"):
        q.packing_record(models["packed"], expected)
...
def test_tiny_case_canonical_postwarm_adam_state_resets_and_complete_work(monkeypatch):
    ...
    assert all("packing" in e for e in events if e["path"] == "packed")
...
def frozen(tmp_path, monkeypatch):
    ...
    names = {q.PRIOR_SOURCE} | {f"prior/{i}.py" for i in range(39)}
    ...
    q.write(parent_dir/"training-plan.json", {"synthetic": True})
    ...
    args = SimpleNamespace(batching_plan=parent_dir/"plan.json", batching_plan_sha256=q.sha(parent_dir/"plan.json"),
        protocol=tmp_path/q.ADD_SOURCES[-2], protocol_sha256=q.sha(tmp_path/q.ADD_SOURCES[-2]), geometry=gp,
        geometry_sha256=q.sha(gp), out=tmp_path/"freeze")
    digest = q.freeze(args)
    return args, digest


def test_synthetic_freeze_full46_closure_no_calls_and_exclusive(frozen):
    args, digest = frozen
    p = q.validate_plan(args.out/"plan.json", digest)
    assert len(p["source_sha256"]) == 46 and p["loss_weights"] == [2., 1., 2/3]
    assert q.read(args.out/"completed.json")["model_calls"] == 0
    with pytest.raises(FileExistsError):
        q.freeze(args)


@pytest.mark.parametrize("kind", ["source", "snapshot", "runtime", "recipe", "loss", "parent_done", "geometry", "failed"])
def test_corrupt_envelope_before_backend(frozen, kind):
    ...
    if kind == "source":
        ...
        (args.out/"batching-completed.json").write_text("{}")
    else:
        if kind == "runtime":
