...
import qualify_dialogue_token_shared_columns as q
...
def fake_results():
    ...
    for case in q.CASES:
        ...
        paths = {"original": {"seconds": ratio}, "shared_columns": {"seconds": 1.}}
...
def test_dense_cases_and_recipe_remain_unchanged():
    assert q.CASES == q.required_fill.CASES == q.required_fill.consolidation.factoring.projection.CASES == q.packing.CASES
    assert q.required_fill.RECIPE["pair_orders"][0] == ["original", "required_fill"]
    assert q.required_fill.consolidation.factoring.projection.RECIPE["pair_orders"][0] == ["original", "projected"]
...
def test_corrupt_membership_and_time_rejected(kind):
    ...
    if kind == "missing":
        ...
        rows[15]["measured"][0]["paths"]["shared_columns"]["seconds"] = float("nan")
    else:
...
def test_tiny_geometry_actual_output_all_gradient_and_monitor_parity(mode, head):
    ...
    assert models["shared_columns"].configuration()["packing_reference_only_fields"] == list(q.REFERENCE_FIELDS)
    assert models["shared_columns"].configuration()["packing_view_only_fields"] == list(q.VIEW_FIELDS)
    ...
    assert [e["path"] for e in events] == ["original", "shared_columns"]
    assert events[1]["packing"]["positions_and_scalars"] == expected
    assert work["parity_backward_returned"] == 2 and work["active"]["path"] == "shared_columns"
    assert result["outputs"]["negative_infinity_elements"] > 0
    models["shared_columns"].last_packing_work["packed_evidence_positions"] -= 1
    with pytest.raises(ValueError, match="packing work"):
        q.packing_record(models["shared_columns"], expected)
...
def test_tiny_case_canonical_postwarm_adam_state_resets_and_complete_work(monkeypatch):
    ...
    assert all("packing" in e for e in events if e["path"] == "shared_columns")
...
def frozen(tmp_path, monkeypatch):
    ...
    names = {q.PRIOR_SOURCE, q.GEOMETRY} | {f"prior/{i}.py" for i in range(64)}
    ...
    for name in ("training-plan.json", "batching-plan.json", "batching-completed.json", "packing-plan.json", "packing-completed.json", "consolidation-plan.json", "consolidation-completed.json", "factoring-plan.json", "factoring-completed.json", "projection-plan.json", "projection-completed.json"):
    ...
    def fake_parent(path, digest, *, complete=True):
        q.bind(path, digest)
        result = q.read(path)
        assert len(result["source_sha256"]) == 66
        return result
    monkeypatch.setattr(q, "parent_plan", fake_parent)
    args = SimpleNamespace(required_fill_plan=parent_dir/"plan.json", required_fill_plan_sha256=q.sha(parent_dir/"plan.json"),
        protocol=tmp_path/q.ADD_SOURCES[-1], protocol_sha256=q.sha(tmp_path/q.ADD_SOURCES[-1]), out=tmp_path/"freeze")
    digest = q.freeze(args)
    return args, digest


def test_synthetic_freeze_full71_closure_no_calls_and_exclusive(frozen):
    args, digest = frozen
    p = q.validate_plan(args.out/"plan.json", digest)
    assert len(p["source_sha256"]) == 71 and p["loss_weights"] == [2., 1., 2/3]
...
def test_corrupt_envelope_before_backend(frozen, kind):
    ...
    if kind == "source":
        ...
        (args.out/"required-fill-completed.json").write_text("{}")
    else:
        if kind == "runtime":
            ...
            p["required_fill_plan_sha256"] = "0"*64
        else:
...
def test_shared_columns_counts_include_concat_schema_fill_state_and_reference_only_fields():
    ...
    assert len(work) == 74 and work["exogenous_width"] == 394
    assert work["pooled_concat_scalars"] == 3*384 and work["pooled_concat_bytes_float32"] == 3*384*4
    assert work["group_feature_positions"] == 3 and work["fill_feature_positions"] == 6
    assert work["state_feature_positions"] == 12
    assert (work["group_feature_calls"], work["fill_feature_calls"], work["state_feature_calls"]) == (1, 1, 2)
    assert work["group_feature_macs"] == 3*394*64 and work["fill_feature_macs"] == 6*394*64
    assert work["state_feature_macs"] == 2*12*64
    assert work["factored_feature_macs"] == (3+6)*394*64+2*12*64
    assert work["dense_feature_macs"] == 12*396*64
    assert work["group_exogenous_input_scalars"] == 3*394
    assert work["fill_exogenous_input_scalars"] == 6*394 and work["state_feature_input_scalars"] == 24
    assert work["grouped_head_schema_scalars"] == 2*3*64 and work["grouped_lexical_scalars"] == 30
    record = q.packing_record(SimpleNamespace(last_packing_work=work), work)
    assert set(record["reference_only_fields"]) == set(q.REFERENCE_FIELDS)
    assert not set(record["reference_only_fields"]) & set(record["float32_payload_bytes"])
    assert record["float32_payload_bytes"]["scatter_hidden_scalars"] == 3*64*4
    assert record["float32_payload_bytes"]["fill_hidden_scalars"] == 6*64*4
    valid[:] = False
    work = q.mask_work(valid, candidates, token)
    assert work["group_feature_positions"] == work["group_feature_calls"] == 0
    assert work["packed_turn_projection_positions"] == 0 and work["empty_turn_projection_calls"] == 1
    assert work["fill_feature_calls"] == 1 and work["fill_feature_macs"] == 6*394*64
    assert work["state_feature_calls"] == 2 and work["state_feature_macs"] == 2*12*64
    assert work["factored_feature_macs"] == 6*394*64+2*12*64
    assert work["scatter_hidden_bytes_float32"] == 0 and work["dense_hidden_bytes_float32"] == 12*64*4
    assert work["pooled_concat_scalars"] == work["pooled_concat_bytes_float32"] == 0
...
def test_changed_counter_width_is_rejected():
    ...
    wrong = dict(expected, scatter_hidden_scalars=2*32)
    with pytest.raises(ValueError, match="packing work"):
        q.packing_record(SimpleNamespace(last_packing_work=wrong), expected)


def test_all_parent_work_unchanged_with_one_view_across_ordered_linear_calls():
    valid = np.array([[True, True]], dtype=bool)
    candidate = np.array([[[True, True, False]]], dtype=bool)
    token = np.zeros((1, 2, 20), dtype=bool)
    token[0, 0, :3] = True
    token[0, 1, :19] = True
    previous = q.required_fill.mask_work(valid, candidate, token)
    work = q.mask_work(valid, candidate, token)
    assert work["pooling_groups"] == previous["pooling_groups"] == 2
    assert previous["turn_projection_calls"] == previous["group_feature_calls"] == 1
    assert work["turn_projection_calls"] == work["group_feature_calls"] == 1
    assert work["pooled_concat_scalars"] == 4*384
    assert work["pooled_concat_bytes_float32"] == 4*4*384
    assert previous["fill_feature_positions"] == work["fill_feature_positions"] == 1
    assert all(work[key] == value for key, value in previous.items())
    assert work["state_column_view_expressions"] == 1
    assert work["state_column_reference_expressions"] == work["state_column_linear_calls"] == 2
    assert work["state_column_view_scalars"] == 128


@pytest.mark.parametrize("kind,expected_fill", [("none", 0), ("masked_candidate", 1), ("padded_turn", 3), ("all", 6)])
def test_shared_columns_predicate_and_typed_payloads(kind, expected_fill):
    valid = np.ones((2, 2), dtype=bool)
    candidate = np.ones((2, 1, 3), dtype=bool)
    if kind == "masked_candidate":
        candidate[0, 0, 2] = False
    elif kind == "padded_turn":
        valid[0, 1] = False
    elif kind == "all":
        valid[:, 1] = False
    tokens = np.broadcast_to(valid[..., None], (2, 2, 4)).copy()
    work = q.mask_work(valid, candidate, tokens)
    partial = 0 < expected_fill < 6
    assert len(work) == 74
    assert work["fill_feature_positions"] == expected_fill
    assert work["fill_feature_calls"] == int(expected_fill > 0)
    assert work["full_fill_reference_positions"] == 6 and work["full_fill_reference_calls"] == 1
    assert work["omitted_fill_positions"] == 6-expected_fill
    assert work["fill_feature_macs"] == expected_fill*394*64
    assert work["full_fill_reference_macs"] == 6*394*64
    assert work["fill_mask_reduction_positions"] == 10
    assert work["fill_mask_boolean_scalars"] == work["fill_mask_boolean_bytes"] == 16
    assert work["fill_count_int64_scalars"] == 1 and work["fill_count_int64_bytes"] == 8
    assert work["fill_index_int64_scalars"] == (2*expected_fill if partial else 0)
    assert work["fill_index_int64_bytes"] == (16*expected_fill if partial else 0)
    assert work["fill_gathered_schema_scalars"] == (2*expected_fill*64 if partial else 0)
    assert work["fill_zero_lexical_scalars"] == 10*expected_fill
    assert work["fill_zero_storage_scalars"] == (6*64 if expected_fill < 6 else 0)
    assert work["fill_schema_scatter_positions"] == (expected_fill if partial else 0)
    assert work["fill_schema_scatter_scalars"] == (expected_fill*64 if partial else 0)
    assert work["fill_auxiliary_float32_bytes"] == 4*sum(work[k] for k in
        ("fill_gathered_schema_scalars", "fill_zero_lexical_scalars", "fill_zero_storage_scalars"))
    _, _, classes = q.backend()
    actual = classes["shared_columns"].packing_work(torch.from_numpy(valid), torch.from_numpy(candidate), torch.from_numpy(tokens))
    assert actual == work
    record = q.packing_record(SimpleNamespace(last_packing_work=actual), work)
    assert len(record["reference_only_fields"]) == 16
    assert record["boolean_payload_bytes"] == {"fill_mask_boolean_scalars": 16}
    assert record["int64_payload_bytes"] == {"fill_count_int64_scalars": 8,
        "fill_index_int64_scalars": 16*expected_fill if partial else 0}
    assert not (set(record["reference_only_fields"]) | set(record["boolean_payload_bytes"]) |
                set(record["int64_payload_bytes"])) & set(record["float32_payload_bytes"])
    assert record["float32_payload_bytes"]["fill_hidden_scalars"] == expected_fill*64*4


@pytest.mark.parametrize("turns", [1, 6, 15, 23])
def test_logical_parameter_view_never_reported_as_copied_float_payload(turns):
    valid = np.ones((1, turns), dtype=bool)
    candidate = np.ones((1, 1, 2), dtype=bool)
    tokens = np.ones((1, turns, 3), dtype=bool)
    work = q.mask_work(valid, candidate, tokens)
    assert work["state_column_view_expressions"] == 1
    assert work["state_column_reference_expressions"] == turns
    assert work["state_column_linear_calls"] == work["state_feature_calls"] == turns
    record = q.packing_record(SimpleNamespace(last_packing_work=work), work)
    assert record["view_only_fields"] == ["state_column_view_scalars"]
    assert record["logical_view_scalars"] == {"state_column_view_scalars": 128}
    assert "state_column_reference_expressions" in record["reference_only_fields"]
    assert all("state_column" not in key for field in ("float32_payload_bytes", "boolean_payload_bytes", "int64_payload_bytes")
               for key in record[field])
    wrong = dict(work, state_column_view_expressions=2)
    with pytest.raises(ValueError, match="packing work"):
        q.packing_record(SimpleNamespace(last_packing_work=wrong), work)
    valid[:] = False
    empty = q.mask_work(valid, candidate, tokens)
    assert empty["state_column_view_expressions"] == 1
    assert empty["state_column_linear_calls"] == turns
    assert empty["state_column_view_scalars"] == 128
