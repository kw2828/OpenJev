...
import qualify_dialogue_token_consolidation as q
...
def fake_results():
    ...
    for case in q.CASES:
        ...
        paths = {"original": {"seconds": ratio}, "consolidated": {"seconds": 1.}}
...
def test_dense_cases_and_recipe_remain_unchanged():
    assert q.CASES == q.factoring.CASES == q.factoring.projection.CASES == q.packing.CASES
    assert q.factoring.RECIPE["pair_orders"][0] == ["original", "factored"]
    assert q.factoring.projection.RECIPE["pair_orders"][0] == ["original", "projected"]
...
def test_corrupt_membership_and_time_rejected(kind):
    ...
    if kind == "missing":
        ...
        rows[15]["measured"][0]["paths"]["consolidated"]["seconds"] = float("nan")
    else:
...
def test_tiny_geometry_actual_output_all_gradient_and_monitor_parity(mode, head):
    ...
    assert models["consolidated"].configuration()["packing_reference_only_fields"] == list(q.REFERENCE_FIELDS)
    ...
    assert [e["path"] for e in events] == ["original", "consolidated"]
    assert events[1]["packing"]["positions_and_scalars"] == expected
    assert work["parity_backward_returned"] == 2 and work["active"]["path"] == "consolidated"
    assert result["outputs"]["negative_infinity_elements"] > 0
    models["consolidated"].last_packing_work["packed_evidence_positions"] -= 1
    with pytest.raises(ValueError, match="packing work"):
        q.packing_record(models["consolidated"], expected)
...
def test_tiny_case_canonical_postwarm_adam_state_resets_and_complete_work(monkeypatch):
    ...
    assert all("packing" in e for e in events if e["path"] == "consolidated")
...
def frozen(tmp_path, monkeypatch):
    ...
    names = {q.PRIOR_SOURCE, q.GEOMETRY} | {f"prior/{i}.py" for i in range(54)}
    ...
    for name in ("training-plan.json", "batching-plan.json", "batching-completed.json", "packing-plan.json", "packing-completed.json", "projection-plan.json", "projection-completed.json"):
    ...
    def fake_parent(path, digest, *, complete=True):
        q.bind(path, digest)
        result = q.read(path)
        assert len(result["source_sha256"]) == 56
        return result
    monkeypatch.setattr(q, "parent_plan", fake_parent)
    args = SimpleNamespace(factoring_plan=parent_dir/"plan.json", factoring_plan_sha256=q.sha(parent_dir/"plan.json"),
        protocol=tmp_path/q.ADD_SOURCES[-1], protocol_sha256=q.sha(tmp_path/q.ADD_SOURCES[-1]), out=tmp_path/"freeze")
    digest = q.freeze(args)
    return args, digest


def test_synthetic_freeze_full61_closure_no_calls_and_exclusive(frozen):
    args, digest = frozen
    p = q.validate_plan(args.out/"plan.json", digest)
    assert len(p["source_sha256"]) == 61 and p["loss_weights"] == [2., 1., 2/3]
...
def test_corrupt_envelope_before_backend(frozen, kind):
    ...
    if kind == "source":
        ...
        (args.out/"factoring-completed.json").write_text("{}")
    else:
        if kind == "runtime":
...
def test_consolidated_counts_include_concat_schema_fill_state_and_reference_only_fields():
    ...
    assert len(work) == 50 and work["exogenous_width"] == 394
...
def test_multiple_attention_groups_consolidate_only_the_two_following_gemms():
    ...
    revised = {"turn_projection_calls", "empty_turn_projection_calls", "group_feature_calls"}
