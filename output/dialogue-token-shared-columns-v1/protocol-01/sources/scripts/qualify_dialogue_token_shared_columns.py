"""Exclusive shared-columns correctness/cost screen; no corpus or encoders.
...
import qualify_dialogue_token_required_fill as required_fill

ROOT = Path(__file__).resolve().parents[1]
VERSION = "dialogue-token-shared-columns-qualification-v1"
PRIOR_SOURCE = "scripts/qualify_dialogue_token_required_fill.py"
PRIOR_SHA256 = "46d2bf4717af845da2de33fdf1b2688c54c3aa5a245027e4bd553b01e4eaf681"
PARENT_PLAN_SHA256 = "871d3e5633290f9015af02aaab7ec136adf3743b5dec1f0ff5e16b4c7498987e"
GEOMETRY, GEOMETRY_SHA256 = packing.GEOMETRY, packing.GEOMETRY_SHA256
ADD_SOURCES = ("src/openjev/research/dialogue_token_shared_columns.py", "tests/test_dialogue_token_shared_columns.py",
    "scripts/qualify_dialogue_token_shared_columns.py", "tests/test_qualify_dialogue_token_shared_columns.py",
    "research/dialogue-token-shared-columns-protocol.md")
PATHS = ("original", "shared_columns")
CASES = copy.deepcopy(required_fill.CASES)
RECIPE = {**copy.deepcopy(required_fill.RECIPE), "pair_orders": [list(PATHS), list(reversed(PATHS))]*2,
    "shared_columns": "One differentiable state-column view per forward, explicitly shared across ordered head updates; no persistent cache"}
REFERENCE_FIELDS = (
    ...
    "full_fill_reference_exogenous_scalars", "full_fill_reference_hidden_scalars", "full_fill_reference_macs", "state_column_reference_expressions")
VIEW_FIELDS = ("state_column_view_scalars",)
...
def mask_work(valid, candidate, token):
    """Independent mask work plus source-level view expressions, never allocation."""
    work = required_fill.mask_work(valid, candidate, token)
    turns = valid.shape[1]
    return {**work, "state_column_view_expressions": 1, "state_column_reference_expressions": turns,
        "state_column_view_scalars": 2*64, "state_column_linear_calls": turns}


def parent_plan(path, expected, *, complete=True):
    bind(source_path(PRIOR_SOURCE), PRIOR_SHA256)
    require(expected == PARENT_PLAN_SHA256, "Exact frozen required-fill plan required")
    return required_fill.validate_plan(path, expected, complete=complete)


def freeze(args):
    with old.attempt(args.out, "freeze", {k: str(v) for k, v in vars(args).items()}) as (out, start, _, check):
        parent = parent_plan(args.required_fill_plan, args.required_fill_plan_sha256)
        require(Path(args.protocol).resolve() == source_path(ADD_SOURCES[-1]).resolve(), "Shared-columns protocol path")
        bind(args.protocol, args.protocol_sha256)
        mapping = {**parent["source_sha256"], **{n: sha(source_path(n)) for n in ADD_SOURCES}}
        require(len(mapping) == 71 and len(parent["source_sha256"]) == 66, "Expected 71-source closure")
        snapshot(out, mapping)
        parent_dir = Path(args.required_fill_plan).parent
        for name, origin in (("required-fill-plan.json", args.required_fill_plan), ("required-fill-completed.json", parent_dir/"completed.json"),
                *((name, parent_dir/name) for name in ("consolidation-plan.json", "consolidation-completed.json", "factoring-plan.json", "factoring-completed.json", "projection-plan.json", "projection-completed.json", "packing-plan.json", "packing-completed.json", "batching-plan.json", "batching-completed.json", "training-plan.json"))):
            shutil.copyfile(origin, out/name)
        plan = {"version": VERSION, "recipe": RECIPE, "runtime": old.runtime(), "source_sha256": mapping,
            "required_fill_plan_sha256": args.required_fill_plan_sha256, "required_fill_completed_sha256": sha(out/"required-fill-completed.json"),
            ...
            "scope": "Same sixteen artificial workloads, shared per-forward state-column view only; no scientific/full-training admission"}
...
def validate_plan(path, expected, *, complete=True):
    ...
    require(p["required_fill_plan_sha256"] == PARENT_PLAN_SHA256, "Exact frozen required-fill plan required")
    parent = parent_plan(path.parent/"required-fill-plan.json", p["required_fill_plan_sha256"], complete=False)
    bind(path.parent/"required-fill-completed.json", p["required_fill_completed_sha256"])
    done = read(path.parent/"required-fill-completed.json")
    require(done["status"] == "completed" and done["phase"] == "freeze" and done["plan_sha256"] == p["required_fill_plan_sha256"]
        and done["source_sha256"] == parent["source_sha256"] and done["model_calls"] == 0 and done["no_retry"] is True, "Parent freeze incomplete")
    require(len(p["source_sha256"]) == 71 and set(p["source_sha256"]) == set(parent["source_sha256"]) | set(ADD_SOURCES)
...
def backend():
    ...
    from openjev.research.dialogue_token_shared_columns import DialogueTokenSharedColumnsMemory
    class MonitoredSharedColumns(study.v2.MonitoredCopyMemoryV2, DialogueTokenSharedColumnsMemory):
        pass
    return study, torch, {"original": study.MonitoredTokenMemory, "shared_columns": MonitoredSharedColumns}
...
def packing_record(model, expected):
    ...
    return {"positions_and_scalars": dict(actual),
        ...
        "view_only_fields": list(VIEW_FIELDS),
        "logical_view_scalars": {k: actual[k] for k in VIEW_FIELDS},
        "float32_payload_bytes": {k: 4*v for k, v in actual.items() if k.endswith("_scalars")
            and k not in REFERENCE_FIELDS and k not in VIEW_FIELDS and k not in ("fill_mask_boolean_scalars", "fill_count_int64_scalars", "fill_index_int64_scalars")},
        ...
        "view_scope": "Shared strided parameter view shape, not copied/allocated payload, backward nodes or memory traffic",
        "mac_scope": "First feature linear layer only; multiply-accumulates, not total model FLOPs or measured time",
        "scope": "Grouped/pooled-concat/required-fill/state and hidden-scatter logical payloads; mask bool/index int64 charged separately; full fill and earlier dense/projected fields reference-only; view-only fields excluded from payload estimates; aliases are not additive, not traffic or peak memory"}
...
def parity(models, data, case, weight, study, torch, progress, emit, check, expected):
    ...
    aliased = {"original": models["original"], "batched": models["shared_columns"]}
    def record(row):
        row = dict(row)
        row["path"] = "shared_columns" if row["path"] == "batched" else row["path"]
        if row["path"] == "shared_columns":
            row["packing"] = packing_record(models["shared_columns"], expected)
        emit(row)
    try:
        return old.parity(aliased, data, case, weight, study, torch, progress, record, check)
    finally:
        if progress.get("active", {}).get("path") == "batched":
            progress["active"]["path"] = "shared_columns"
...
def run_case(case, geometry, weight, study, torch, classes, progress, emit, check):
    ...
    for pair, order in enumerate(RECIPE["pair_orders"]):
        ...
        result["measured"].append({"pair": pair, "order": order, "paths": rows, "speed_ratio": rows["original"]["seconds"]/rows["shared_columns"]["seconds"]})
...
def run(args):
    with old.attempt(args.out, "run", {**{k: str(v) for k, v in vars(args).items()}, "shared_columns_harness_sha256": sha(__file__)}, capped=True) as (out, start, progress, check):
        ...
        write(out/"completed.json", {"status": "completed", "version": VERSION, "plan_sha256": args.plan_sha256,
            ...
            "scope": "Original assembler plus shared-columns forward counters included in update wall; external comparison and receipt formatting outside update timing"})
...
if __name__ == "__main__":
    ...
    for key in ("required-fill-plan", "required-fill-plan-sha256", "protocol", "protocol-sha256", "out"):
