...
def images(readme):
    ...
    require({name for name in names if name.lower().endswith('.gif')} == {
        'docs/assets/chess-candidate-game-001.gif', 'docs/assets/jepa-policy-181000-preview.gif',
        'evidence/reacher-geometry-memory-v1/report/fixed-case-replay.gif'}, 'all three existing animated README GIFs embedded')
    return names
...
def verify(commit, closure_sha):
    ...
    require(closure['version'] == TAG and closure['technical_complete'] is True
            ...
            and closure['architecture_claim'] is False and closure['status'] in ('DEV_PASS', 'DEV_FAIL'),
            'closed conditional-label pilot before publication verification')
    ...
    require(manifest['study'] == TAG and archive_check['status'] == 'passed'
            ...
            and manifest['registration'] == closure['registration']
            and manifest['recorded_processes'] == closure['processes']
            and manifest['recorded_work']['audit'] == closure['audit_counts']
    ...
    required = {'README.md', 'research/experiment-index.md', 'research/jev-architecture-source-review.md',
                'research/otto-conditional-label-protocol.md', 'research/otto-conditional-label-results.md',
                'research/otto-conditional-label-results/summary.json',
                'research/otto-conditional-label-results/benchmark.png',
                'research/otto-conditional-label-results/receipt.json', *media}
