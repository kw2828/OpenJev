...
learning objective. This weighting rule is fixed before fitting, with weights
computed only from admitted training counts.
...
The saved-output reader also reconstructs raw probability mass and Brier scores
in float64 from the saved float32 logs. This is an additional numerical check;
the scorer's forward check exponentiates in float32 before summation in float64.
Both must satisfy the same 2e-6 tolerance without repair.
