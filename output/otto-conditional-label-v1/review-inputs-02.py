...
pattern = re.compile(rb'(?<![A-Za-z0-9])(?:' + b'|'.join(str(x).encode() for x in blocks) + rb')[0-9]{6}(?![A-Za-z0-9])')
...
review = {'version': 'otto-conditional-label-seed-review-v1', 'created_unix_ns': time.time_ns(),
          ...
          'scanner_correction': 'Attempt01 matched digit substrings inside SHA256 hashes; attempt02 requires standalone alphanumeric boundaries. No seed matches or scientific runs occurred.',
...
c.write(c.OUT / 'seed-review-02.json', review)
...
c.require(c.read(c.OUT / 'source-review-01.json')['files'] == source['files'], 'preserved primary source review')
print(json.dumps({'seed_review_passed':review['passed'], 'seed_count':len(seeds),
                  ...
                  'seeds':c.desc(c.OUT / 'seed-review-02.json')}))
