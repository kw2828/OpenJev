...
def plan():
    return {
        "rng_namespace": "reacher-memory-control-engineering-unit-v1",
        "engineering": True,
        "steps": 50,
        "control_episodes": 1,
        "noise_std": 0.05,
...
def model(kind):
    width = {"width": 7} if kind == "packet_mlp" else {"hidden_size": 4}
    with torch.random.fork_rng(devices=[]):
        torch.manual_seed(410)
        ...
        for step in range(50):
            ...
            if kind == "bounded_gru":
                ...
                np.testing.assert_array_equal(
                    states["root__real_packets"][0, step, -width:],
                    record["policy"]["packets"][step + 1 - width : step + 1],
                )
                if width > 1:
                    np.testing.assert_array_equal(
                        states["root__real_actions"][0, step, -(width - 1) :],
                        record["policy"]["commands"][step + 1 - width : step],
                    )
                np.testing.assert_array_equal(
                    states["carried__pending_action"][0, step], record["policy"]["commands"][step]
                )
                np.testing.assert_array_equal(
                    states["carried__real_packets"][:, step], states["root__real_packets"][:, step]
                )
            if kind == "current_gru" and not record["policy"]["packets"][step, 6]:
                assert (states["root__hidden"][:, step] == 0).all()
...
def test_prediction_all_complete_windows_and_no_privileged_inputs(kind):
    ...
    changed["policy"]["packets"][20:, 4:6] += 0.1
...
def test_ragged_native_failure_preserves_all_completed_cases_and_original_interrupt(tmp_path, monkeypatch):
    ...
    with pytest.raises(KeyboardInterrupt, match="synthetic native failure"):
        control.learned_control(
            cfg, model("bounded_gru"), "ordinary", [innovations(2)] * 50, out, cases=cases(count=2)
        )
