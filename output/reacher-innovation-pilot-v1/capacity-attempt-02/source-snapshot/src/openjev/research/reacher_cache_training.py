"""Prospective anchor trainer for explicit public-cache controls.

Derived from the frozen memory trainer in a separate module; no historical
source or checkpoint semantics are changed.
...
open-loop windows use advance only. Thus cache phase semantics and
...
from openjev.research.reacher_cached_observation import (
    operation_counts as cache_operation_counts,
)
...
VERSION = "reacher-cache-training-engineering-v1"
...
def model_configuration(kind, settings):
    ...
    width = settings.mlp_width if kind in ("packet_mlp", "cached_mlp") else settings.hidden_size
    public_memory = None
    if kind in ("encoded_current_gru", "cached_gru", "cached_mlp"):
        public_memory = configuration_for(REGISTRY[kind], width=width, dt=settings.dt,
...
def _construct(kind, settings, seed):
    ...
    with torch.random.fork_rng(devices=[]):
        ...
        width = (
            {"width": settings.mlp_width} if kind in ("packet_mlp", "cached_mlp") else {"hidden_size": settings.hidden_size}
        )
        return (
            REGISTRY[kind](
                **width,
...
def make_initialization(gru_seed, mlp_seed, settings=None, *, role_names=None):
    ...
    settings = settings or CacheTrainingSettings()
...
class CacheTrainer:
    def __init__(self, kind, initialization, orders, public_data, *, source_sha256=None, runtime=None):
        started = time.perf_counter()
        self.settings = CacheTrainingSettings.from_configuration(initialization["settings"])
        ...
        family = "mlp" if kind in ("packet_mlp", "cached_mlp") else "gru"
    ...
    def _check_live(self):
        ...
        width = model.width if self.kind in ("packet_mlp", "cached_mlp") else model.hidden_size
        require(
        ...
        if self.model_configuration["public_memory_configuration"] is not None:
            require(model.configuration() == self.model_configuration["public_memory_configuration"],
                    "Live public-cache configuration drift")
        require(
    ...
    def _work(self, batch):
        ...
        if self.kind in ("encoded_current_gru", "cached_gru", "cached_mlp"):
            operations = cache_operation_counts(
                self.student, assimilate_samples=a, advance_samples=d
            )
        elif self.kind == "packet_mlp":
