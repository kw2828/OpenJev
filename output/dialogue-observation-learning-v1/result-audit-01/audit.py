...
Only stdlib and NumPy. No producer/reporter/metric/model imports. Full execution
...
VERSION = "dialogue-observation-independent-audit-v1"
STUDY = "dialogue-observation-scientific-v1"
REPORT = "dialogue-observation-report-v1"
...
REPORTER = "scripts/report_dialogue_observation.py"
METRICS = "src/openjev/research/dialogue_observation_metrics.py"
REPORTER_PIN = "36847018d165a3d41ab606bdfc2d93a5743eede011f32e591d873912ca4ad7a8"
...
def authenticate(args):
    ...
    need(done["plan_sha256"] == args.plan_sha256 == sha(args.run / "plan.json")
         and plan["version"] == STUDY and plan["fit_order"] == FIT_ORDER, "Frozen scientific plan identity")
    need(plan["source_sha256"] == done["source_sha256"] and len(plan["source_sha256"]) == 53
    ...
    need(len(tail) == 8 and (ROOT / tail[0]).resolve() == ROOT / "scripts/study_dialogue_observation.py"
    ...
    need(set(flags) == {"--plan", "--plan-sha256", "--out"} and flags["--plan-sha256"] == args.plan_sha256
    ...
    need(all(finite(end[k]) for k in ("wall_seconds", "finished_unix")) and finite(launch["started_unix"])
         and 0 <= end["finished_unix"] - launch["started_unix"] <= end["wall_seconds"]
         and done["wall_seconds"] <= end["wall_seconds"] <= limits["wall_seconds"], "Parent elapsed bounds")
    ...
    need(set(report["source_sha256"]) == {REPORTER, METRICS, "tests/test_report_dialogue_observation.py", "tests/test_dialogue_observation_metrics.py"}
...
def execute(args):
    ...
    def budget():
        need(time.perf_counter()-start <= CAPS["wall_seconds"] and peak_rss() <= CAPS["rss_bytes"], "Audit wall/RSS cap")
    ...
    try:
        ...
        receipt = {"version":VERSION,"status":"completed","agreement":True,"request":request,"source_sha256":sources,
                   ...
                   "wall_seconds":time.perf_counter()-start,"peak_rss_bytes":peak_rss(),"model_calls":0}
        ...
        try:
            ...
            write(args.out/"failed.json",{"version":VERSION,"status":"failed","request":request,"source_sha256":sources,
                  "error_type":type(error).__name__,"error":str(error),"wall_seconds":time.perf_counter()-start,
