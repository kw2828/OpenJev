...
NEW = [
    "src/openjev/research/otto_query_memory_training.py",
    "src/openjev/research/otto_query_memory_gate.py",
    "scripts/train_otto_query_memory.py",
    "scripts/qualify_otto_query_memory_capacity.py",
    "scripts/audit_otto_query_memory.py",
    "tests/test_otto_query_memory_training.py",
    "tests/test_otto_query_memory_gate.py",
    "tests/test_train_otto_query_memory.py",
    "tests/test_qualify_otto_query_memory_capacity.py",
    "tests/test_audit_otto_query_memory.py",
...
BOUND = [*NEW, "pyproject.toml", "uv.lock",
         ...
         "scripts/launch_otto_detached_phase.py",
         ...
         "output/otto-query-memory-training-engineering-v1/run_engineering.py"]
...
def main():
    ...
    before = pins()
    started = {"scope": "fabricated engineering only, including synthetic optimizer updates; no empirical arrays or checkpoints",
    ...
    probes = list((output / "pytest-temp").rglob("audit-capacity.json"))
    audit_capacity = None
    capacity_error = None
    try:
        if len(probes) != 1 or not probes[0].is_file() or probes[0].is_symlink():
            raise ValueError("exactly one fabricated scalar-audit capacity probe required")
        payload = probes[0].read_bytes()
        with (output / "audit-capacity.json").open("xb") as stream:
            stream.write(payload)
            stream.flush()
            os.fsync(stream.fileno())
        audit_capacity = json.loads(payload)
        if not (audit_capacity["admitted"] is True
                and audit_capacity["worst_case_rows"] == 3780864
                and audit_capacity["multiplier"] == 2
                and audit_capacity["reserve_seconds"] == 120
                and math.isfinite(audit_capacity["projected_seconds"])
                and 0 < audit_capacity["projected_seconds"] <= 480):
            raise ValueError("fabricated scalar-audit capacity did not admit the frozen audit cap")
    except (ValueError, KeyError, TypeError, OSError) as error:
        capacity_error = repr(error)
    after = pins()
    passed = (before == after and capacity_error is None
              and all(row["returncode"] == 0 and not row["timed_out"] for row in outcomes))
    receipt = {"scope": started["scope"], "status": "passed" if passed else "failed",
               ...
               "fabricated_optimizer_updates_exercised": True,
               "audit_capacity": audit_capacity, "audit_capacity_error": capacity_error,
