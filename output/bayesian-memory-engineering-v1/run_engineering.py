"""Check a separate Bayesian memory reference using fabricated unit tests only."""
import hashlib
import json
import os
import signal
...
NEW = ["src/openjev/research/bayesian_score_memory.py", "tests/test_bayesian_score_memory.py"]
...
def main():
    ...
    if set(NEW) & set(sources):
        raise ValueError("engineering must remain separate from the live study")
    bound = sorted(set(sources) | set(NEW) | {SELF})
    ...
    commands = [
        [str(ROOT / ".venv/bin/python"), "-m", "pytest", "-q", "-p", "no:cacheprovider",
         "--basetemp", str(output / "pytest-temp"), NEW[1]],
        [str(ROOT / ".venv/bin/ruff"), "check", "--no-cache", *NEW, SELF],
    ]
    scope = "standalone numerical reference; fabricated unit tests only; no current study integration"
    write(output / "started.json", {
        "scope": scope, "source_before": before, "commands": commands,
        "per_command_cap_seconds": 60, "environment_overrides": ENV,
