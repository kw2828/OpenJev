"""Supported-row consolidation correctness only, with tiny synthetic fixtures."""
import hashlib
...
from openjev.research.dialogue_token_consolidated import (
    ...
    FACTORED_SOURCE,
    VERSION,
    DialogueTokenConsolidatedMemory,
)
...
def model(head="scalar", mode="candidate", cls=DialogueTokenConsolidatedMemory):
...
def test_monitored_outputs_all_gradients_and_real_dummy_coverage(head, mode, monkeypatch):
    ...
    class MonitoredPacked(v2.MonitoredCopyMemoryV2, DialogueTokenConsolidatedMemory):
        pass
...
def test_parent_initialization_parameters_and_source_pin(head):
    ...
    packed = DialogueTokenConsolidatedMemory(head)
...
def test_original_validation_and_failed_forward_clears_work(kind):
    ...
    if kind == "nan_padding":
        ...
        a[5] = a[5][..., :9]
    for cls in (DialogueTokenMemory, DialogueTokenConsolidatedMemory):
...
def test_equal_input_projection_width_does_not_misdispatch_inherited_step(head):
    ...
    projected = DialogueTokenConsolidatedMemory(head, input_dim=6, projection_dim=6, hidden_dim=7).double()
...
def test_projected_work_rejects_invalid_projection_width(width):
    ...
    with pytest.raises(ValueError, match="Projection width"):
        DialogueTokenConsolidatedMemory.packing_work(a[1], a[4], a[6], 6, width)
...
def test_work_rejects_invalid_hidden_width():
    ...
    with pytest.raises(ValueError, match="Hidden width"):
        DialogueTokenConsolidatedMemory.packing_work(a[1], a[4], a[6], 6, 4, 0)
