...
Only ordinary/case0/pair0, all four arms, all50 executed steps are supported.
...
LABELS = ('Persistent GRU', 'Current-only GRU', 'Three-packet GRU', 'Packet MLP')
...
def authenticate(plan_path, expected_plan, audit, expected_audit, execution, engineering):
    ...
    if engineering:
        require(plan['rng_namespace'] == 'reacher-memory-engineering-whole-tree-v1'
                and plan['fixture_scope'] == 'synthetic_engineering_only_not_production_lineage_or_efficacy',
                'Explicit completed rehearsal only')
    else:
        require(plan['rng_namespace'] == 'reacher-memory-ablation-v1-scored', 'Scored namespace')
    require(receipt['source_sha256'] == plan['sources'] and len(plan['sources']) == 58,
            'Exact58 source bindings')
    ...
    require(complete['status'] == 'completed' and complete['plan_sha256'] == expected_plan
            and complete['files'] == receipt['execution_members'] and complete['fits'] == 12
            and complete['control_rows'] == 51, 'Complete12-fit51-row execution')
    ...
    require(summary['engineering'] is engineering and summary['plan_sha256'] == expected_plan
            ...
            and summary['coverage']['fits'] == 12 and summary['coverage']['control_rows'] == 51,
...
def fonts():
    ...
    return {size: ImageFont.truetype(str(path), size=size) for size in (14, 16, 18, 20, 28)}, {'path': str(path), 'sha256': sha(path)}


def frame(rows, step, font, engineering):
    image = Image.new('RGB', (1020, 860), COLORS['paper'])
    draw = ImageDraw.Draw(image)
    draw.rectangle((0, 0, 1020, 84), fill=COLORS['ink'])
    draw.text((26, 12), 'OpenJev | saved schematic replay', font=font[28], fill='white')
    draw.text((27, 51), 'Ordinary blackout | first case (0), first fit pair (0) | all four models', font=font[18], fill='#dbe5ef')
    if engineering:
        draw.rectangle((0, 84, 1020, 119), fill=COLORS['warning'])
        draw.text((26, 91), 'ENGINEERING FIXTURE - NOT A SCIENTIFIC RESULT', font=font[18], fill='white')
    else:
        draw.rectangle((0, 84, 1020, 119), fill='#dceaf4')
        draw.text((26, 91), 'One fixed episode, not a performance summary. Full study: 12 fits / 51 rows.', font=font[18], fill=COLORS['ink'])
    for index, (row, title) in enumerate(zip(rows, LABELS, strict=True)):
        x, y = 25+(index % 2)*500, 132+(index // 2)*315
        missing = not row['valid'][step-1]
        draw.rounded_rectangle((x, y, x+470, y+300), radius=12,
                               fill=COLORS['missing'] if missing else COLORS['visible'], outline='#d2dce5', width=1)
        draw.text((x+16, y+11), title, font=font[20], fill=COLORS['ink'])
        state = 'ANGLES MISSING' if missing else 'angles observed'
        draw.text((x+282, y+14), state, font=font[16], fill=COLORS['warning'] if missing else COLORS['muted'])
        cx, cy, scale = x+235, y+150, 450
        def point(value, cx=cx, cy=cy, scale=scale):
            return (round(cx+float(value[0])*scale), round(cy-float(value[1])*scale))
        for value in (-.2, -.1, 0, .1, .2):
            a, b = point((value, -.22)), point((value, .22))
            draw.line((a, b), fill=COLORS['grid'], width=1)
        ...
        draw.text((x+16, y+278), f'Saved cumulative native cost: {row["cost"][step]:.3f}', font=font[16], fill=COLORS['ink'])
    time_seconds = rows[0]['times'][step]-rows[0]['times'][0]
    draw.text((27, 776), f'Step {step:02d}/50  |  simulation {time_seconds:.2f} s  |  playback 5x slower  |  axes in meters', font=font[18], fill=COLORS['ink'])
    draw.text((27, 806), 'Shading: angles missing at the action\'s decision. Clean motion comes from saved audit state.', font=font[16], fill=COLORS['muted'])
    draw.text((27, 831), 'Red: target. Faint trail: fingertip path. Schematic centerlines, not simulator screenshots.', font=font[16], fill=COLORS['muted'])
    return image
...
def render(args):
    ...
    try:
        ...
        result = {'status': 'completed', 'scope': 'engineering_only_no_efficacy' if args.engineering else 'fixed_first_case_schematic_scored_replay',
                  ...
                  'selection': 'Fixed ordinary case0/pair0 for all four arms; all50 steps; no score-based selection.',
