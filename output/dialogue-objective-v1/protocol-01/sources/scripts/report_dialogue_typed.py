"""Saved-only metrics for the paired typed/support development experiment.

No model, producer, reporter helper, checkpoint deserializer or random generator
is imported. Logs are promoted to float64 without repairing probabilities.
"""
...
def authenticate_run(run, expected_sha):
    ...
    require(membership["admitted_row_indices"] == {"fit": plan["fit_row_indices"], "evaluation": plan["evaluation_row_indices"]}, "Original service split row identity")
    orders = {}
    for seed in SEEDS:
...
def execute(args):
    ...
    try:
        ...
        write(out/"receipt.json", {"status": "completed", "version": VERSION, "execution_completed_sha256": args.run_sha256,
              "source_sha256": digest(__file__), "plan_sha256": done["plan_sha256"], "technical_validity_passed": True,
