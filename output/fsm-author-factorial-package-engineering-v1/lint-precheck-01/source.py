...
def authenticate(study, audit_path, closure_path):
    ...
    # The wrapper's opaque input pins may include inherited archives. The held
    # authenticate API validates them; they are never admitted to this payload.
...
def gather(study, audit_path, closure_path, plots):
    ...
    for path in (audit_path, paths['new_audit'], ROOT/PARENT_MANIFEST, ROOT/PARENT_RECEIPT,
                 ROOT/LICENSE, Path(__file__), ROOT/AUDITOR):
        add(path)
