...
def fixture(root):
    ...
    fake_auditor = (
        b'import json\nfrom pathlib import Path\n'
        b'def authenticate(study, process, freeze):\n'
        b'    d=json.loads((Path(__file__).resolve().parents[1]/"admitted.json").read_text())\n'
        b'    return d["plan"],d["inputs"],{k:Path(v) for k,v in d["paths"].items()},{},{},{}\n'
    )
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Declared pytest basetemp trees may contain *current symlinks and
        # poison members. Pruning must happen before any descendant is read.
        for name in p.ENGINEERING_FIXTURE_TREES:
            base = root/name
            case = base/'test_case0'
            case.mkdir(parents=True)
            (case/'reserved_300mV_test.npy').write_bytes(b'poison: must never be opened')
            (base/'test_casecurrent').symlink_to(case, target_is_directory=True)
            save(str(base.parent.relative_to(root))+'/retained-receipt.json', {'status': 'PASS'})
            save(str(base.parent.relative_to(root))+'/source/retained.py', b'# source snapshot')
        revised = p.gather(*args)
        assert p.ENGINEERING_FIXTURE_TREES <= set(revised['omitted_optional_or_transient'])
        assert not any(any(name.startswith(base+'/') for base in p.ENGINEERING_FIXTURE_TREES)
                       for name in revised['files'])
        for name in p.ENGINEERING_FIXTURE_TREES:
            parent = str(Path(name).parent)
            assert parent+'/retained-receipt.json' in revised['files']
            assert parent+'/source/retained.py' in revised['files']
        assert all(name+'/qualification-01/receipt.json' in revised['files'] for name in p.ENGINEERING)
        p.run(*args, root/'release-with-pruned-fixtures')
        checks.append('only_declared_engineering_basetemp_pruned_with_receipts_retained')
        outside = root/'unscoped/fabricated-fixtures'
        outside.mkdir(parents=True)
        (outside/'casecurrent').symlink_to(bank.parent, target_is_directory=True)
        expect_failure(lambda: p.tree(outside.parent, omit_transient=True))
        checks.append('same_basename_outside_scoped_engineering_still_rejected')
        other = root/p.ENGINEERING[0]/'unlisted/fabricated-fixtures'
        other.mkdir(parents=True)
        (other/'casecurrent').symlink_to(bank.parent, target_is_directory=True)
        expect_failure(lambda: p.gather(*args))
        checks.append('unlisted_engineering_fixture_symlink_still_rejected')
