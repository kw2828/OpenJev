...
def add_tree(bundle, directory):
    ...
    for path in sorted(directory.rglob("*")):
        if path.is_symlink():
            require("engineering-01" in path.parts, "symlinks allowed only as recorded engineering aliases")
            SYMLINKS.append({"path": path.relative_to(ROOT).as_posix(), "target": os.readlink(path),
                "delivery": "metadata only; pytest convenience alias is not followed or recreated"})
            continue
        if path.is_file():
            add(bundle, path)
...
def main():
    ...
    try:
        ...
        for path in sorted(OUT.parent.iterdir()):
            if path.is_file():
                add("audit", path)
        ...
        dependency_manifest = save("historical-dependencies.json", {"dependencies": DEPS, "preserved_engineering_aliases": SYMLINKS,
