...
- [Token alignment comparison](research/dialogue-token-alignment-scientific-status.md): **nine-fit study launched September 20**, comparing token alignment with a matched token-mean model and the existing scorer. The data and behavioral criteria are fixed; quality will be reported only after all nine complete. No accuracy result yet.
