...
- [Shared-token dialogue experiment](research/dialogue-token-results.md): training stopped at **7/12 fits**. [Removing unused schema fill](research/dialogue-token-required-fill-results.md) passes **15/16 timing requirements** and all output/gradient checks. All twelve larger workloads pass, but one small workload reaches **0.845x**, below its 0.90 floor. Overall admission fails; no new accuracy result or training restart.
