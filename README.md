...
- [Shared-token dialogue experiment](research/dialogue-token-results.md): training stopped at **7/12 fits**. [Consolidated projections](research/dialogue-token-consolidation-results.md) pass **15/16 timing requirements**, with **1.61-1.87x** ratios on one real mask layout with artificial values. One small workload misses its floor, so overall admission fails. All output/gradient checks pass; no new accuracy result or training restart.
...
[![All sixteen consolidated-projection timing cells, including the failed small workload](output/dialogue-token-consolidation-v1/figure-02/timing.png)](research/dialogue-token-consolidation-results.md)
