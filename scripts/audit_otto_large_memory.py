"""Independent saved-output verification of the separate 53x53 OTTO memory screen.
...
import numpy as np
from scipy.special import gammainc, k0
...
N, NHITS, HORIZON, EPS = 53, 4, 2188, 1e-10
CENTER = (26, 26)
STRATA = (1, 2, 3)
INHERITED_PLAN_PIN = "1571012f6ad168436d30c53533e0db1d476300fbb3687242c9bd409c462ab430"
QUAL_NAMES = ["official_autoset_geometry", "all_likelihoods_include_tail_and_zero_origin", "initial_hit_mixture",
              ...
              "initial_prior_and_source_hit3", "seeded_and_forced_hit_replay_hit3", "global_numpy_rng_unchanged_hit3",
...
FILES |= {f"qualification-{i:02}.json" for i in range(1, 18)}
...
SOURCES |= {"scripts/study_otto_large_memory.py", "tests/test_otto_large_memory_study.py",
            "research/otto-large-memory-protocol.md", "output/otto-large-memory-v1/engineering-01/receipt.json"}
SOURCES |= {f"output/otto-large-memory-v1/engineering-01/check-{i:02}.log" for i in range(3)}
...
UPSTREAM_SOURCES |= {"isotropic/evaluate/parameters/isotropic-53x53.py"}
SCOPE = ("Independent saved source/process/payload bindings, 768 rotated cases, public movement/found/visited "
...
def authenticate(args, budget):
    ...
    for name, witness in done["files"].items():
        ...
        budget()
    require(sum(p.stat().st_size for p in run.iterdir()) <= 512 * 1024**2, "worker output cap")
    ...
    require(set(plan["sources"]) == SOURCES and set(plan["upstream_sources"]) == UPSTREAM_SOURCES
            and plan["upstream_commit"] == COMMIT, "exact twenty-one local and twelve upstream source closure")
    inherited_path = ROOT / "output/otto-memory-v1/plan-01.json"
    require(plan["inherited_plan_sha256"] == INHERITED_PLAN_PIN and sha(inherited_path) == INHERITED_PLAN_PIN,
            "unchanged small-study plan lineage")
    inherited = read(inherited_path)
    require(len(inherited["sources"]) == 14 and len(inherited["upstream_sources"]) == 11,
            "exact inherited source counts")
    for key in ("sources", "upstream_sources"):
        require(all(plan[key].get(name) == pin for name, pin in inherited[key].items()), "unchanged inherited source pins")
    require(plan["status"] == "frozen_before_native_execution" and plan["cohort"] == {
        "arms": list(ARMS), "seeds": list(range(610001, 610097)), "episodes": 768, "horizon": HORIZON,
        "weighting": "exact upstream initial-hit mixture, balanced 32 cases in each of three positive initial-hit strata",
        "case_assignment": "case=seed-610001, block=case//12, hit=1+(case%12)//4; rotate arm order left bycase%8"}, "fixed cohort")
    require(plan["limits"] == {"native_seconds": 1800, "rss_bytes": 4 * 1024**3, "output_bytes": 512 * 1024**2,
                               "native_steps": 1690000, "qualification_native_steps": 2000}, "fixed resource limits")
    require(plan["native_models"] == plan["training_updates"] == plan["external_models"] == 0, "no model allocation")
    require(plan["engineering_receipt"] == "output/otto-large-memory-v1/engineering-01/receipt.json"
    ...
    if command[1:2] == ["-u"]:
        command.pop(1)
    require(command[:2] == [imports["executable"], str(ROOT / "scripts/study_otto_large_memory.py")], "worker command")
...
def poisson_categories(mu):
    ...
    one = mu * zero
    two = mu * one / 2
    # Regularized lower gamma gives the >=3 Poisson tail without cancellation.
    return np.stack((zero, one, two, gammainc(3, mu)))


def analytic_kernel():
    coordinates = np.indices((2 * N + 1, 2 * N + 1)) - N
    radius = np.sqrt(np.sum(coordinates**2, axis=0))
    result = poisson_categories(2 * k0(np.where(radius == 0, 1, radius) / 3) / math.log(6))
    result[:, N, N] = 0
    r = np.arange(1, 3000, dtype=np.float64)
    likelihood = poisson_categories(2 * k0(r / 3) / math.log(6))
    shell = 2 * math.pi * r
    mass = [math.fsum((likelihood[h] * shell).tolist()) for h in STRATA]
    return result, np.array([0.0, *mass]) / math.fsum(mass)
...
def prior(kernel, hit):
    ...
    p[CENTER] = 0
    return normalized(p * likelihood_at(kernel, CENTER)[hit])
...
def source_position(value):
    ...
    require(value != list(CENTER), "source excludes initial visited cell")
    return tuple(value)
...
def qualify(run, kernel, weights, comparison):
    ...
    require(qualification["status"] == "completed" and [r["name"] for r in qualification["checks"]] == QUAL_NAMES,
            "all seventeen structural witnesses")
    ...
    for hit in STRATA:
        ...
        draws = Draws(420010 + hit, witness["draw_log"], comparison)
        ...
        for t, row in enumerate(trace, 1):
            ...
            require(row["position"] == list(position) and row["done"] is found
                    and (row["hit"] == -2 if found else type(row["hit"]) is int and 0 <= row["hit"] < NHITS), "qualification public semantics")
            if found:
    ...
    # Three calls per replay, then 1 saturated +30 north +1 blocked +78 to (52,52).
    expected_native = 3 * total + 110
    require(qualification["native_steps"] == checks["qualification_native_step_budget"]["steps"] == expected_native <= 2000,
...
def episode(key, trace, row, kernel, comparison, budget):
    ...
    for step, event in enumerate(trace[1:], 1):
        ...
        require(reading == -2 if found else type(reading) is int and 0 <= reading < NHITS, "found sentinel or categorical reading")
...
def summarize(rows, weights):
    def means(subset):
        answer = {}
        for arm in ARMS:
            strata = {h: [r for r in subset if r["arm"] == arm and r["initial_hit"] == h] for h in STRATA}
            require(all(strata.values()), "all three initial-hit strata")
            answer[arm] = {m: math.fsum(weights[h] * math.fsum(r[m] for r in strata[h]) / len(strata[h]) for h in STRATA)
                           for m in METRICS}
        return answer
    overall = means(rows)
    strata = {str(h): {arm: {m: math.fsum(r[m] for r in rows if r["initial_hit"] == h and r["arm"] == arm) / 32
                            for m in METRICS} for arm in ARMS} for h in STRATA}
    ...
    rules.extend(({"name": "complete_cohort", "value": 768, "passes": True},
    ...
    return {"scope": "classical odor-memory opportunity screen; no trained architecture or official evaluator replication",
            "episodes": 768, "initial_hit_weights": {str(h): weights[h] for h in STRATA}, "means": overall,
...
def execute(args):
    ...
    try:
        ...
        with np.load(run / "public-kernel.npz", allow_pickle=False) as saved:
            ...
            require(kernel.dtype == weights.dtype == np.dtype("float64") and kernel.shape == (NHITS, 2 * N + 1, 2 * N + 1)
                    and weights.shape == (NHITS,), "kernel shape and precision")
        ...
        comparison.close(float(np.max(np.abs(kernel - analytic))), 0.0, "all 45796 kernel cells", 1e-12)
        ...
        require(weights[0] == 0 and all(0 < w < 1 for w in weights[1:]) and abs(float(weights.sum()) - 1) <= 1e-12,
                "positive three-stratum weights")
        ...
        for case in range(96):
            ...
            for arm in ARMS[shift:] + ARMS[:shift]:
                ...
                require(eg is not None and tg is not None and eg[0] == tg[0] == key and len(eg[1]) == 1,
                        "all 768 cases exactly once in fixed rotation")
                row = eg[1][0]
        ...
        require(done["native_steps_attempted"] == done["native_steps_returned"] == steps + qualification["native_steps"] <= 1690000,
                "complete qualification plus cohort native-call accounting")
        require(done["qualification_steps"] == qualification["native_steps"] and done["completed_episodes"] == 768,
        ...
        result = summarize(rows, {h: float(weights[h]) for h in STRATA})
        ...
        budget()
        write(output / "summary.json", {"agreement": True, "scope": SCOPE, "episodes": 768, "cohort_steps": steps,
        ...
        budget()
        print(json.dumps({"status": "completed", "agreement": True, "episodes": 768,
