"""Plot every saved factored-head timing cell without model imports or calls."""
...
def main():
    ...
    assert receipt["status"] == "completed" and receipt["version"] == "dialogue-token-factoring-qualification-v1"
    ...
    ax.set_xlabel("Original time / factored time: above 1 means faster")
    ax.xaxis.grid(True, alpha=.15)
    ax.set_axisbelow(True)
    ax.spines[["top", "right", "left"]].set_visible(False)
    ax.tick_params(axis="y", length=0)
    outcome = "passed" if summary["engineering_admission"] else "failed"
    fig.suptitle(f"Factored observation/state head: fixed speed qualification {outcome}", x=.03, ha="left", fontsize=17, weight="bold")
