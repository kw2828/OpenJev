...
VERSION = "otto-query-advantage-study-v2"
SELF, TEST = "scripts/study_otto_query_advantage_v2.py", "tests/test_study_otto_query_advantage_v2.py"
PROTOCOL = "research/otto-query-advantage-v2-protocol.md"
...
LABEL_SEED, LABEL_HORIZON, REPLICAS = 19700001, 32, tuple(range(16))
FAILURE_RESERVE = 1024**2
CONFIGURATION = {"schedules": list(SCHEDULES), "regimes": {"base": 3., "shift": 4.},
    "first_seeds": {"base": 19500001, "shift": 19600001}, "cases_per_regime": 12,
...
ROLES = {"collection_plan", "sampler_plan", "sampler_receipt", "sampler_terminal", "sampler_review",
         "seed_review", "engineering_receipt", "logging_admission"}
NEW_COMPONENTS = {SELF, TEST, "src/openjev/research/otto_query_pair_rollouts.py",
    ...
    "tests/test_otto_query_advantage.py", "scripts/audit_otto_query_advantage_v2.py",
    "tests/test_audit_otto_query_advantage_v2.py"}
NEW_COMPONENTS |= {"src/openjev/research/durable_jsonl.py", "tests/test_durable_jsonl.py"}
NEW_SOURCES = NEW_COMPONENTS | {PROTOCOL, COLLECTOR, "scripts/qualify_otto_query_logging.py",
    "research/otto-query-logging-qualification-protocol.md"}
...
def authenticate_inputs(inputs):
    ...
    require(seeds["version"] == "otto-query-advantage-seeds-v2" and seeds["status"] == "reserved_before_collection"
            and seeds["reserved_seeds"] == [*range(19500001, 19500013), *range(19600001, 19600013)]
            and seeds["label_seed"] == LABEL_SEED and seeds["bootstrap_seed"] == 19800001
    ...
    require(engineering["status"] == "passed" and engineering["results"]
            ...
            and engineering["logger_throughput_admitted"] is True
            and engineering["sources_before"] == engineering["sources_after"]
            and set(engineering["sources_after"]) == NEW_COMPONENTS, "all qualified new source/test files")
    ...
    logging_admission = read(paths["logging_admission"])
    require(logging_admission["status"] == "passed" and logging_admission["admitted"] is True,
            "separately closed logger qualification")
    bound = {}
    for role in ("plan", "receipt", "terminal", "launch"):
        record = logging_admission[role]
        bound[role] = regular(record["path"])
        require(descriptor(bound[role]) == {k: record[k] for k in ("sha256", "bytes")}, "logging external pin")
    logger_plan, logger_receipt, logger_terminal = (read(bound[k]) for k in ("plan", "receipt", "terminal"))
    logger_launch = read(bound["launch"])
    require(logger_plan["version"] == logger_receipt["version"] == "otto-query-logging-qualification-v1"
            and logger_receipt["status"] == "completed" and logger_receipt["admitted"] is True
            and logger_receipt["plan_sha256"] == descriptor(bound["plan"])["sha256"]
            and logger_receipt["sources"] == logger_plan["sources"]
            and logger_receipt["supervision_sha256"] == descriptor(bound["launch"])["sha256"]
            and logger_receipt["launch"] == logger_launch
            and all(logger_terminal[k] == v for k, v in logger_launch.items())
            and logger_terminal["started_ns"] <= logger_receipt["started_ns"] < logger_receipt["finished_ns"]
            <= logger_terminal["finished_ns"], "completed unchanged fabricated logger comparison")
    require(logger_terminal["status"] == "completed" and logger_terminal["returncode"] == 0
            and logger_terminal["timed_out"] is False and logger_terminal["group_absent"] is True
            and logger_terminal["cleanup"]["group_absent"] is True and logger_terminal["cleanup"]["reaped"] is True
            and logger_terminal["cleanup"]["errors"] == [] and logger_terminal["cap_seconds"] == 120
            and logger_terminal["error"] is logger_terminal["clock_error"] is None
            and logger_terminal["finished_ns"] <= logger_terminal["deadline_ns"]
            and logger_terminal["command"] == [str(ROOT / INTERPRETER),
                str(ROOT / "scripts/qualify_otto_query_logging.py"), "--plan", str(bound["plan"]),
                "--plan-sha256", descriptor(bound["plan"])["sha256"], "--output", str(bound["receipt"].parent),
                "--supervision", str(bound["launch"])],
            "original successful logger supervisor")
    for name, desc in logger_plan["sources"].items():
        require(descriptor(ROOT / name) == desc, "qualified current logging source")
    directory = bound["receipt"].parent
    require({str(p.relative_to(directory)) for p in directory.rglob("*") if p.is_file()}
            == set(logger_receipt["files"]) | {"receipt.json"}, "complete logger qualification evidence")
    for name, desc in logger_receipt["files"].items():
        require(descriptor(directory / name) == desc, "logger qualification payload")
    results = logger_receipt["results"]
    require(len(results) == 12 and len({(r["block"], r["implementation"]) for r in results}) == 12,
            "six complete paired logging blocks")
    ratios = []
    for block in range(6):
        rows = {r["implementation"]: r for r in results if r["block"] == block}
        require(set(rows) == {"old", "new"}, "both logging implementations")
        require(all(r["acknowledged"] == r["append_fsyncs"] == 4096 and r["total_seconds"] > 0
                    and r["journal"] == logger_receipt["expected_journal"] for r in rows.values()),
                "equivalent durable append contract")
        ratios.append(rows["old"]["total_seconds"] / rows["new"]["total_seconds"])
    require(statistics.median(ratios) >= 2 and sum(r > 1 for r in ratios) >= 5,
            "prospective fabricated throughput admission")
...
class Ledger:
    ...
    def emit(self, name, value):
        ...
        try:
            self.run.check()
            require(self.run.journal is not None, "durable journal initialized before event")
            self.run.journal.append(name, value, context=intent)
...
class Run:
    ...
    def check(self):
        ...
        if self.journal is not None:
            require(not self.journal.poisoned, "durable journal not poisoned")
            require(self.journal.total_bytes <= LIMITS["output_bytes"]
                    and self.journal.non_sampler_bytes <= LIMITS["non_sampler_output_bytes"], "journal output caps")
            return
    ...
    def guard_write(self, name, size):
        self.check()
        if self.journal is not None:
            self.journal.reserve(name, size)
            return
        sizes = {p.name: p.stat().st_size for p in self.out.iterdir()}
    ...
    def bind(self):
        ...
        if command[1:2] == ["-u"]:
            command.pop(1)
        require(sys.argv[0] == str(ROOT / SELF), "canonical absolute producer launch")
    ...
    def sampler_event(self, anchor_id, event):
        ...
        require(self.sampler_events < 3019680, "fixed sampler event bound")
        kind = event["event"]
    ...
    def execute(self):
        ...
        try:
            ...
            for name in PAYLOADS:
                if name.endswith(".jsonl"):
                    with (self.out / name).open("x"):
                        pass
            tick = time.perf_counter()
            journal_module = load(ROOT / "src/openjev/research/durable_jsonl.py", "_query_v2_durable_journal")
            self.journal = journal_module.DurableJSONL(self.out,
                total_limit=LIMITS["output_bytes"], non_sampler_limit=LIMITS["non_sampler_output_bytes"],
                failure_reserve=FAILURE_RESERVE,
                per_file_record_limits={"sampler-events.jsonl": LIMITS["sampler_event_bytes"]})
            # Reserve the original combined 1 MiB setup allowance before the
            # unchanged setup publishes its two small external JSON artifacts.
            require(len(json.dumps(self.plan["runtime"]).encode()) < 256 * 1024, "bounded setup metadata")
            self.journal.reserve("runtime.json", 512 * 1024)
            self.journal.reserve("setup.json", 512 * 1024)
            ...
            require(self.journal.close() is True and not self.journal.pending
                    and not self.journal.errors and not self.journal.poisoned, "successful journal closure")
            self.journal_close_seconds = time.perf_counter() - close_tick
            self.receipt["journal_closure"] = {"pending": self.journal.pending, "errors": self.journal.errors,
                "poisoned": self.journal.poisoned, "close_seconds": self.journal_close_seconds}
            self.journal = None
            ...
            if self.journal is not None:
                try:
                    self.journal.close(suppress=True)
                except BaseException as close_error:  # noqa: BLE001 - preserve primary failure through cleanup
                    error.add_note(f"Journal cleanup: {close_error!r}")
                self.receipt["journal_failure"] = {"pending": self.journal.pending,
                    "errors": self.journal.errors, "poisoned": self.journal.poisoned}
                self.journal = None
