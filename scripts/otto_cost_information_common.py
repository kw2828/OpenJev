"""Prospective conditional-cost diagnostic metadata and bounded-process helpers."""
...
PARENT_VERSION = 'otto-action-effect-v1'
PARENT = ROOT / 'output' / PARENT_VERSION
PARENT_CLOSURE_PIN = '63973e02c90224a7224f4626af3ddc17509d5ee18e6f5aa0625dd0b5863f8d2d'
FAMILIES = ('effect_recurrent', 'paired_recurrent', 'paired_blind', 'paired_direct')
FIT_SEEDS = (330000001, 330000002, 330000003)
CONFIG = {'prefix_transitions': 8, 'attempted_per_regime': 32, 'min_cases': 16,
          'exact_horizon': 4, 'forecast_horizon': 8, 'mc_draws': 128, 'min_mc_survivors': 32,
          'base_first': 331000001, 'shift_first': 332000001,
          'select_first': 333000001, 'eval_first': 334000001, 'bootstrap_seed': 335000001,
          'bootstrap_replicates': 2000, 'quantile': .025, 'shift_headroom_fraction': .10,
          'base_headroom_fraction': 0., 'uniform_bits': 53, 'root_grid_max_tv': 1e-10,
          'primary_family': 'paired_recurrent', 'families': list(FAMILIES), 'fit_seeds': list(FIT_SEEDS)}
CAPS = {'collect': 1800, 'predict': 1800, 'audit': 1800}
SOURCES = ['scripts/qualify_otto_cost_information.py', 'scripts/otto_cost_information_common.py',
           'scripts/collect_otto_cost_information.py', 'scripts/run_otto_cost_information.py',
           'scripts/audit_otto_cost_information.py',
           'src/openjev/research/otto_conditional_cost_tree.py',
           'src/openjev/research/otto_conditional_cost_metrics.py',
           'tests/test_otto_conditional_cost_tree.py', 'tests/test_otto_conditional_cost_metrics.py',
           'tests/test_run_otto_cost_information.py', 'tests/test_audit_otto_cost_information.py',
           'research/otto-cost-information-protocol.md',
...
def roster():
    result = []
    for regime, first in (('lambda3', CONFIG['base_first']), ('lambda4', CONFIG['shift_first'])):
        for i in range(CONFIG['attempted_per_regime']):
            ordinal = len(result)
            result.append({'regime': regime, 'seed': first + i, 'initial_hit': 1 + i % 3,
                           'select_seed': CONFIG['select_first'] + ordinal,
                           'eval_seed': CONFIG['eval_first'] + ordinal,
                           'id': f'diagnostic:{regime}:{first + i}'})
    return result


def parent_reference():
    """Authenticate frozen parent checkpoints by opaque hashes, never parent data."""
    closure_path = PARENT / 'closure-01.json'
    require(desc(closure_path)['sha256'] == PARENT_CLOSURE_PIN, 'published parent closure pin')
    ...
    receipt_path, terminal_path = PARENT / 'fit-01/receipt.json', PARENT / 'fit-native-01.terminal.json'
    binding = closure['processes']['fit']
    require(desc(receipt_path) == binding['receipt'] and desc(terminal_path) == binding['terminal'], 'parent fit closure')
    ...
    require(receipt['status'] == terminal['status'] == 'completed' and terminal['returncode'] == 0
            and terminal['group_absent'] and terminal['cleanup']['reaped'] and not terminal['timed_out']
            and terminal['error'] is None and terminal['clock_error'] is None, 'completed parent fit')
    summary_path = PARENT / 'fit-01/summary.json'
    require(desc(summary_path) == receipt['files']['summary.json'], 'closed parent fit metadata')
    checkpoints = {}
    for family in FAMILIES:
        for seed in FIT_SEEDS:
            name = f'{family}-{seed}.npz'
            path = PARENT / 'fit-01' / name
            require(desc(path) == receipt['files'][name], 'unchanged frozen checkpoint')
            checkpoints[f'{family}__{seed}'] = {'path': str(path.relative_to(ROOT)), **desc(path)}
    return {'parent_closure': desc(closure_path), 'fit_receipt': desc(receipt_path),
            'fit_terminal': desc(terminal_path), 'fit_summary': desc(summary_path),
            'checkpoints': checkpoints, 'parent_data_array_decodes': 0,
            'historical_parent_worker_seconds': {name: x['worker_seconds'] for name, x in closure['processes'].items()},
            'scope': 'Frozen existing policies only; no parent TRAIN or DEV arrays admitted. Historical model creation is not a new training cost.'}
