"""Fresh three-arm comparison with exact, paired optimizer-update exposure.

The frozen rounded models, numerical objectives, data and optimizer recipe are
unchanged. A new explicit controller requires all prefix and joint updates and
uses one safety cap; elapsed time cannot choose the successful update count.
"""
...
VERSION = 'finite-update-learning-v1'
DEFAULT_CONFIG = {'seed_namespace': 433260924, 'train_attempts': 512, 'dev_attempts': 128,
...
def _config(config):
    ...
    values = dict(config)
    prefix, joint = values.pop('prefix_updates'), values.pop('joint_updates')
    cap = values.pop('fit_cap_seconds')
    require(type(prefix) is int and type(joint) is int and prefix > 0 and joint > 0
            and prefix + joint <= 100000, 'positive bounded exact update counts')
    require(type(cap) is float and math.isfinite(cap) and 0 < cap <= 120.,
            'positive bounded controller safety cap')
    checked = _base_config({**values, 'epochs': 1})
    del checked['epochs']
    return {**checked, 'prefix_updates': prefix, 'joint_updates': joint, 'fit_cap_seconds': cap}
...
def run(folder: Path, config: dict, check):
    ...
    try:
        ...
        summary = {'version': VERSION, 'config': config, 'counts': counts, 'rows': rows,
            'counts_scope': 'Successful exact-update runs contain only accepted attempts; failed runs retain attempted work and partial states separately. Accepted counts remain explicit. Checkpoint writes count 27 parameter and 27 optimizer boundary files; detailed snapshot/rollback/work counters and every attempt reside in each fit allocation file.',
