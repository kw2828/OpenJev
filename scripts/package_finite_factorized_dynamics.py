"""Package authenticated closed factorized-dynamics evidence as opaque bytes.
...
from render_finite_factorized_dynamics import (
    ROOT,
    authenticate,
    descriptor,
    inventory,
...
VERSION = 'finite-factorized-dynamics-package-v1'
...
def package(studyfolder, reportfolder, outputfolder, *, registration_sha256, commit=None, publication_paths=()):
    ...
    require(render_receipt['version'] == 'finite-factorized-dynamics-report-v1'
    ...
    renderer_path = regular(ROOT / 'scripts/render_finite_factorized_dynamics.py')
    ...
    require(qualification_plan in engineering_plans, 'qualifying attempt is present in named engineering roster')
    ...
    for engineering_plan in engineering_plans:
        ...
        if snapshot.exists():
            require(inventory(snapshot) == sources, 'complete attempt-specific engineering source snapshot')
        ...
        if engineering_plan == qualification_plan:
            ...
            require(terminal['cwd'] == str(ROOT) and command[0] == record['runtime']['executable']
                    and (ROOT / command[1]).resolve() == ROOT / 'scripts/finite_factorized_dynamics_worker.py'
    ...
    publication_attempt = ROOT / 'output/finite-factorized-dynamics-publication-attempt-01'
    publication_attempt_files = inventory(publication_attempt)
    require(set(publication_attempt_files) == {'failure.json', 'render_finite_factorized_dynamics.py',
                                               'package_finite_factorized_dynamics.py'},
            'exact preserved first publication-failure roster')
    for name, pin in publication_attempt_files.items():
        add('publication/attempt-01/' + name, publication_attempt / name, pin)
    for name in (*sorted(expected_outputs), 'receipt.json'):
        add('publication/report/' + name, reportfolder / name)
    for name in ('README.md', 'LICENSE', 'scripts/render_finite_factorized_dynamics.py', 'scripts/package_finite_factorized_dynamics.py'):
    ...
    manifest = {'version': VERSION, 'study': STUDY_NAME, 'registration': descriptor(plan_path),
        ...
        'initial_function_checks': audit['dynamics_comparisons']['initial_function_checks'],
        'fit_costs': [{'arm': row['arm'], 'seed': row['seed'], 'construction_seconds': row['construction_seconds'],
                       'construction_work': row['construction_work'], 'fit_seconds': row['seconds'],
                       'parameter_count': row['parameter_metadata']['parameter_count']}
                      for row in saved['fits']],
        'structural_work': saved['structural_work'],
        'structural_work_scope': saved['structural_work_scope'],
        ...
        'publication_attempts': [{'directory': str(publication_attempt), 'status': 'FAILED',
            'files': publication_attempt_files,
            'scope': 'Original pre-output renderer failure and its two helper source snapshots; publication provenance only, not a scientific rerun.'}],
        'scope': 'All registered sources, original closed phase payloads including nine checkpoints, fresh endpoint and all-attempt prefix arrays, oracle controls, journals with all initial/final dynamics and head snapshots plus separate construction work/timing, original receipts/launches/terminals/logs, every named engineering attempt with its source snapshot, the first publication failure with both helper snapshots, and completed report. Failed attempts remain failures. No prior scientific datasets or checkpoints.',
        'member_mapping': 'study/ preserves study-relative names; sources/ preserves repository source paths; publication/ contains named presentation artifacts. Absolute original paths remain provenance metadata only.',
