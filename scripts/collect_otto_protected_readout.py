"""Complete public collection for the fresh protected-readout study.
...
VERSION = "otto-protected-readout-collection-v1"
SELF, TEST = "scripts/collect_otto_protected_readout.py", "tests/test_collect_otto_protected_readout.py"
...
FIRST = {"train": {"lambda3": 297000001, "lambda4": 298000001},
         "valid": {"lambda3": 299000001, "lambda4": 300000001}}
...
CONFIGURATION = {"arms": list(ARMS), "first_seeds": FIRST, "cases": CASES, "horizon": HORIZON,
    ...
    "selection_seed_first": 302000001, "validation_annotation": "complete census",
    "training_annotation": "union of all period-four query rows and selected windows; query anchors do not enlarge sampled nonquery loss windows; later-query auxiliary targets are projected separately",
...
NEW_COMPONENTS = {SELF, TEST,
    'src/openjev/research/otto_prequery_scores.py', 'src/openjev/research/otto_prequery_data.py',
    'src/openjev/research/otto_prequery_loss.py', 'src/openjev/research/otto_spo_plus_loss.py',
    'src/openjev/research/otto_action_focused_loss.py', 'src/openjev/research/otto_score_forecast_data.py',
    'src/openjev/research/otto_sampled_forecast_data.py', 'src/openjev/research/otto_protected_readout_metrics.py',
    ...
    'tests/test_otto_action_focused_loss.py', 'tests/test_otto_score_forecast_data.py',
    'tests/test_otto_sampled_forecast_data.py', 'tests/test_otto_protected_readout_metrics.py',
    'scripts/train_otto_protected_readout.py', 'tests/test_train_otto_protected_readout.py',
    'scripts/audit_otto_protected_readout.py', 'tests/test_audit_otto_protected_readout.py',
    'scripts/qualify_otto_prequery_capacity.py', 'tests/test_qualify_otto_prequery_capacity.py'}
NEW_COMPONENTS |= {
    "src/openjev/research/otto_protected_readout.py", "tests/test_otto_protected_readout.py",
    "src/openjev/research/otto_protected_training_model.py", "tests/test_otto_protected_training_model.py",
    "scripts/launch_otto_detached_phase.py", "tests/test_launch_otto_detached_phase.py"}
NEW_SOURCES = NEW_COMPONENTS | {PROTOCOL,
    ...
    "src/openjev/research/otto_prequery_metrics.py", "src/openjev/research/otto_cross_query_metrics.py",
    "scripts/train_otto_prequery_calibration.py", "scripts/train_otto_cross_query_forecasts.py",
    "scripts/qualify_otto_prequery_capacity.py", "scripts/audit_otto_score_forecasts.py",
    "src/openjev/research/otto_separate_prior_metrics.py", "tests/test_otto_separate_prior_metrics.py",
    "src/openjev/__init__.py", "src/openjev/research/__init__.py",
    "scripts/audit_otto_action_focused.py", "scripts/train_otto_action_focused.py",
    "src/openjev/research/otto_action_focused_metrics.py"}
...
def train_annotation_steps(selection):
    ...
    model inputs for chronological scans; they do not enlarge sampled nonquery
    loss windows. The separately qualified projector owns later-query targets.
    """
...
def authenticate_inputs(inputs):
    ...
    expected_seeds = [first + case for split, regimes in FIRST.items()
                      for first in regimes.values() for case in range(CASES[split])] + [301000001, 301000002, 301000003] + list(range(302000001, 302000055))
...
class Run:
    ...
    def setup(self):
        ...
        write(self.out / "deployment.json", {"original_setup_wall_seconds": self.original_setup_wall,
            ...
            "scope": "All full paths; TRAIN retains every query anchor and samples at most eight nonquery-loss windows after termination. VALID annotates every state. Deployed and deferred scores are distinct. No deployment-cost claim."})
    ...
    def annotate_train(self, identity, history, start_row, held):
        ...
        selection = self.sample_windows(len(history), 302000001 + identity["episode_index"])
