"""Launch one frozen-dynamics observer development study with an original terminal receipt."""
...
ENGINEERING = ROOT / 'output/robot-observer-engineering-v1'
REGISTRATION = ROOT / 'research/robot-observer-registration.json'
OUTPUT = ROOT / 'output/robot-observer-study-v1'
COMMAND = ['.venv/bin/python', '-u', 'scripts/robot_observer_study.py',
           '--registration', 'research/robot-observer-registration.json',
           '--output', 'output/robot-observer-study-v1']
...
def main():
    ...
    for name in (*plan['sources'], str(REGISTRATION.relative_to(ROOT)),
                 'scripts/launch_robot_observer_study.py'):
    ...
    if plan['config']['wall_cap_seconds'] != 14400.:
        raise ValueError('Fixed 14400-second native and 14460-second external caps required')
    if OUTPUT.exists():
    ...
    record = {'command': COMMAND, 'prefit_commit': commit,
              ...
              'scope': 'Eighteen initializer fits with frozen dynamics, complete retained controls; four exposed development recordings; no restart or official TEST access'}
