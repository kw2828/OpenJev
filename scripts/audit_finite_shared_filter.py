"""Independent saved-output audit for the fresh shared-filter finite-world diagnostic.
...
VERSION = 'finite-shared-filter-audit-v1'
NAMESPACE = 422260924
...
COMMON_WORK_KEYS = {'prefix_assimilation_calls', 'prefix_assimilation_rows', 'projection_calls', 'projection_rows',
    ...
    'cost_readout_calls', 'cost_readout_rows', 'absorbed_rows', 'privileged_prefix_rows', 'event_probability_rows'}
...
def validate_structural_work(work, retained):
    """Check source-derived fixed geometry, not backward work or actual FLOPs."""
    require(type(work) is dict and set(work) == set(ARMS), 'structural work has all three learned arms')
    for arm in ARMS:
        require(type(work[arm]) is dict and set(work[arm]) == set(ROUTES), 'all five structural work routes')
        is_gru = arm == 'gru_prefix'
        expected_keys = COMMON_WORK_KEYS if is_gru else COMMON_WORK_KEYS | FILTER_WORK_KEYS
        for route in ROUTES:
            values = work[arm][route]
            require(type(values) is dict and set(values) == expected_keys
                    and all(type(value) is int and value >= 0 for value in values.values()),
                    'exact nonnegative integer work roster, including zero keys')
            training, observed = route.startswith('training_'), route.endswith('_observed')
            n, horizon, repetitions = (retained['train'], 2, 3 * 480) if training else (retained['base'], 8, 3)
            calls, exposures = repetitions * math.ceil(n / 64), repetitions * n
            expected = {'prefix_assimilation_calls': 9 * calls if is_gru else 0,
                'prefix_assimilation_rows': 9 * exposures if is_gru else 0,
                'projection_calls': calls if is_gru else 0, 'projection_rows': exposures if is_gru else 0,
                'prefix_softmax_calls': calls if is_gru else 0, 'operator_observed_softmax_calls': calls,
                'operator_blind_softmax_calls': 0, 'operator_marginal_sum_calls': calls,
                'blind_transition_calls': horizon * calls, 'blind_transition_rows': horizon * exposures,
                'cost_readout_calls': horizon * calls, 'cost_readout_rows': horizon * exposures,
                'privileged_prefix_rows': 0, 'event_probability_rows': horizon * exposures if observed else 0}
            if not is_gru:
                expected.update({'prefix_filter_calls': 8 * calls, 'prefix_filter_rows': 8 * exposures,
                    'reset_emission_softmax_calls': calls, 'reset_emission_rows': exposures,
                    'prefix_operator_softmax_calls': calls})
            require(all(values[key] == value for key, value in expected.items()), 'fixed prefix/forecast work geometry: ' + arm + '/' + route)
            require(values['observed_branch_rows'] == values['observed_conditioning_rows'] <= horizon * exposures
                    and math.ceil(values['observed_branch_rows'] / 64) <= values['observed_branch_calls']
                    <= min(horizon * calls, values['observed_branch_rows']), 'bounded observed branch call/row totals')
            require(values['absorbed_rows'] <= (horizon - 1) * exposures, 'bounded absorbed forecast rows')
            if not observed:
                require(values['observed_branch_calls'] == values['observed_branch_rows']
                        == values['observed_conditioning_rows'] == values['absorbed_rows'] == 0,
                        'blind route has no observed conditioning or absorbed rows')
    return {'arms': 3, 'routes': 15, 'fixed_geometry_checked': True,
            'historical_forward_work_independently_replayed': False,
            'scope': 'Saved forward call/row totals constrained by fixed input geometry; not FLOPs, backward work or matched compute.'}


def validate_observed_work(work, datasets, np):
    """Derive observed conditioning and absorbing rows from public labels."""
    for split, route, repetitions in (('train', 'training_observed', 3 * 480), ('base', 'evaluation_observed', 3)):
        observations = datasets[split]['observations']
        ...
        ordinary_rows = int((observations < 4).sum()) * repetitions
        absorbed_rows = (observations.size - int(event_draws.sum())) * repetitions
        for arm in ARMS:
            values = work[arm][route]
            require(values['observed_conditioning_rows'] == values['observed_branch_rows'] == ordinary_rows
                    and values['absorbed_rows'] == absorbed_rows, 'observed work rows reconstructed from saved labels')
...
def validate_metadata(folder, summary, *, check=lambda: None):
    ...
    def read(name, *, lines=False):
        path = folder / name
        require(path.is_file() and not path.is_symlink(), 'regular saved shared-filter metadata')
        return [json.loads(line) for line in path.read_text().splitlines()] if lines else json.loads(path.read_text())

    require(summary['version'] == 'finite-shared-filter-v1', 'shared-filter producer version')
    compare(summary['config'], CONFIG, path='registered shared-filter configuration')
    fits = summary['fits']
    expected_pairs = [(arm, seed) for index, seed in enumerate(SEEDS) for arm in ARMS[index:] + ARMS[:index]]
    require([(row['arm'], row['seed']) for row in fits] == expected_pairs, 'nine completed shared-filter fits')
    ...
    require(set(summary['files']) == expected_files, 'complete shared-filter producer payload roster')
    for name, descriptor in summary['files'].items():
        check()
        path = folder / name
        require(path.is_file() and not path.is_symlink(), 'regular original shared-filter payload')
    ...
    for row in fits:
        ...
        for key, present in (('prefix_initial_sha256', arm == 'gru_prefix'),
                             ('operator_initial_sha256', True),
                             ('reset_initial_sha256', arm in ('shared_filter', 'untied_filter')),
                             ('prefix_operator_initial_sha256', arm in ('shared_filter', 'untied_filter'))):
    ...
    for seed in SEEDS:
        operator = fit_map['shared_filter', seed]['operator_initial_sha256']
        require(all(fit_map[arm, seed]['operator_initial_sha256'] == operator for arm in ARMS),
                'paired forecast-operator initialization across all arms')
        require(fit_map['shared_filter', seed]['reset_initial_sha256']
                == fit_map['untied_filter', seed]['reset_initial_sha256'], 'paired filter-reset initialization')
        require(all(fit_map[arm, seed]['prefix_operator_initial_sha256'] == operator
                    for arm in ('shared_filter', 'untied_filter')), 'paired canonical prefix/forecast operator value hashes')
    ...
    expected_counts = {'train_generation_count': 1, 'dev_generation_count': 1, 'model_constructions': 9,
        ...
        'training_blind_rollouts': 9 * updates, 'training_observed_rollouts': 9 * updates,
    ...
    work_record = validate_structural_work(summary['structural_work'], retained)
    return {'fits': 9, 'checkpoint_files_hashed': 9, 'epochs': 4320, 'evaluation_views': 9,
            'retained': retained, 'optimizer_steps_attested': 9 * updates,
            'training_case_exposures_attested': 9 * 480 * n,
            'structural_work_validation': work_record,
...
def audit(folder, *, check=lambda: None):
    ...
    compare(sorted(summary['rows'], key=row_key), sorted(rows, key=row_key), path='independent shared-filter metrics')
    ...
    validate_observed_work(summary['structural_work'], datasets, np)
    return {'version': VERSION, 'agreement': True, 'technical_complete': False,
            ...
            'structural_work': summary['structural_work'],
            ...
            'limitations': [
                ...
                'Only the untrained exact control receives a privileged posterior and known operators; every learned arm receives public inputs.',
                'Checkpoint bytes and saved work records do not replay training or prove historical execution order.',
                'Forward call/row counters are source-constrained attestations; observed row totals are reconstructed from labels. They exclude autograd and validation FLOPs.',
                'Shuffled outputs are scored as saved; public-prefix routing and absence of privileged inputs remain source-reviewed.',
