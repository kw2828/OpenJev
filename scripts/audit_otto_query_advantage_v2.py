...
SELF = "scripts/audit_otto_query_advantage_v2.py"
PRODUCER = "scripts/study_otto_query_advantage_v2.py"
FIRST_SEEDS = {"base": 19500001, "shift": 19600001}
LABEL_SEED = 19700001
...
VERSION = "otto-query-advantage-audit-v2"
...
BOOTSTRAP_SEED, BOOTSTRAP_DRAWS = 19800001, 2000
...
def validate_producer_process(worker, terminal, launch, started, *, root, plan_path,
                              plan_sha256, run_path, launch_path, launch_sha256):
    """Pure original-process join, after the caller authenticates these bytes.

    V2 deliberately freezes an absolute script path. Equivalent relative paths
    are refused prospectively rather than silently accepted after a run. This
    helper performs no filesystem access, runtime import, policy call or retry.
    """
    root, plan_path, run_path, launch_path = map(Path, (root, plan_path, run_path, launch_path))
    require(all(p.is_absolute() and ".." not in p.parts for p in (root, plan_path, run_path, launch_path))
            and all(p.is_relative_to(root) for p in (plan_path, run_path, launch_path)), "absolute contained process paths")
    command = list(terminal["command"])
    if command[1:2] == ["-u"]:
        command.pop(1)
    require(command[:3] == [str(root / ".venv-otto-released-native/bin/python"), str(root / PRODUCER), "run"]
            and len(command) == 11, "canonical absolute producer invocation")
    options = dict(zip(command[3::2], command[4::2], strict=True))
    require(options == {"--plan": str(plan_path), "--plan-sha256": plan_sha256,
                        "--supervision": str(launch_path), "--output": str(run_path)}, "complete original option joins")
    require(launch["command"] == terminal["command"] and all(terminal[k] == v for k, v in launch.items())
            and started["launch"] == launch, "same original launch in worker and terminal")
    require(worker["status"] == "completed" and worker["complete"] is True
            ...
            and worker["plan_sha256"] == plan_sha256 and worker["supervision_sha256"] == launch_sha256,
            "complete worker bound to original plan and launch")
    closure = worker["journal_closure"]
    require(closure["pending"] == closure["errors"] == [] and closure["poisoned"] is False
            and type(closure["close_seconds"]) in (int, float) and math.isfinite(closure["close_seconds"])
            and 0 <= closure["close_seconds"] <= worker["wall_seconds"], "durable journal fully closed")
    require(started["request"] == {"mode": "run", "plan": str(plan_path), "plan_sha256": plan_sha256,
                                  "supervision": str(launch_path), "output": str(run_path)}
            and started["started_ns"] == worker["started_ns"], "exact worker-start request")
    require(terminal["status"] == "completed" and type(terminal["returncode"]) is int and terminal["returncode"] == 0
            ...
            and terminal["cleanup"]["errors"] == [], "successful original cleanup")
    require(all(type(terminal[k]) is int and terminal[k] > 0 for k in ("pid", "pgid", "parent_pid"))
            and terminal["pid"] == terminal["pgid"] != terminal["parent_pid"]
            and Path(terminal["cwd"]) == root and terminal["cap_seconds"] == 900
            and terminal["clock_source_sha256"] == CLOCK_PIN and terminal["watchdog_sha256"] == SUPERVISOR_PIN,
            "original PID/group/runtime identity")
    require(all(type(v) is int for v in (terminal["started_ns"], terminal["finished_ns"], terminal["deadline_ns"],
                                        worker["started_ns"], worker["finished_ns"], terminal["elapsed_ns"]))
            ...
            and terminal["elapsed_ns"] == terminal["finished_ns"] - terminal["started_ns"], "complete original deadline interval")
    require(worker["wall_seconds"] == (worker["finished_ns"] - worker["started_ns"]) / 1e9
            and terminal["wall_seconds"] == terminal["elapsed_ns"] / 1e9, "actual worker/parent physical time")
    return {"producer": PRODUCER, "canonical_command": command, "cap_seconds": 900,
            "journal_close_seconds": closure["close_seconds"],
...
class Audit:
    ...
    def authenticate(self):
        ...
        self.require(p["version"] == w["version"] == "otto-query-advantage-study-v2"
        ...
        self.require(c["schedules"] == list(SCHEDULES) and c["prefixes"] == list(PREFIXES)
            and c["label_seed"] == LABEL_SEED and c["label_horizon"] == 32 and c["replicate_ids"] == list(range(16))
        ...
        self.require(w["sources"] == p["sources"] and w["inputs"] == p["inputs"]
            ...
            and SELF in p["sources"] and "tests/test_audit_otto_query_advantage_v2.py" in p["sources"], "exact frozen provenance")
        ...
        command = t["command"]
        self.require(type(command) is list and command.count("--supervision") == 1,
                     "one original launch option")
        ...
        self.receipt["producer_process"] = validate_producer_process(w, t, launch, started, root=ROOT,
            plan_path=self.args.plan, plan_sha256=self.args.plan_sha256, run_path=self.run,
            launch_path=launch_path, launch_sha256=w["supervision_sha256"])
    ...
    def sampler(self):
        ...
        for anchor in self.available:
            base = {"version": "otto-teacher-rollouts-v1", "seed": LABEL_SEED, "anchor_id": anchor["anchor_id"]}
            operation_id = 0
            ...
            for rep in range(16):
                ...
                rng = np.random.Generator(np.random.PCG64(np.random.SeedSequence([0x4F54544F, LABEL_SEED, anchor["anchor_id"], rep, 0])))
                ...
                for first in actions:
                    ...
                    rng = np.random.Generator(np.random.PCG64(np.random.SeedSequence([0x4F54544F, LABEL_SEED, anchor["anchor_id"], rep, 1])))
    ...
    def journals(self):
        ...
        self.require(sum(costs[k] for k in ("setup_wall_seconds", "collection_wall_seconds", "paired_labels_wall_seconds", "anchor_save_seconds"))
            + self.worker["journal_closure"]["close_seconds"]
    ...
    def execute(self):
        ...
        try:
            ...
            with (self.out / "bootstrap.jsonl").open("x") as stream:
                ...
                stream.flush(); os.fsync(stream.fileno())
            result.update(costs=costs, counts=dict(self.counts), journal_closure=self.worker["journal_closure"], worker_seconds=self.worker["wall_seconds"],
