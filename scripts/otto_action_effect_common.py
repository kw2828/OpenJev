"""Prospective paired action-effect pilot metadata and bounded-process helpers."""
...
PARENT_VERSION = 'otto-belief-distillation-v1'
PARENT = ROOT / 'output' / PARENT_VERSION
PARENT_CLOSURE_PIN = '6a72021165661ab60a33809ba857e7f304ba64849160374c713597e1c73e6eaf'
PARENT_TRAIN = PARENT / 'collection-01/train.npz'
CONFIG = {'prefix_transitions': 8, 'train_horizon': 4, 'dev_horizon': 8,
          'train_cases': 1046, 'dev_cases_per_regime': 128, 'min_train': 1046, 'min_dev_per_regime': 64,
          'minimum_supported_cases': 48, 'dev_base_first': 328000001, 'dev_shift_first': 329000001,
          'fit_seeds': [330000001, 330000002, 330000003], 'epochs': 80, 'batch': 32,
          'lr': .003, 'clip': 5., 'aux_weight': .1, 'cost_weight': 1., 'effect_weight': .1,
          'effect_variance_floor': 1e-6, 'candidate': 'effect_recurrent',
          'controls': ['paired_recurrent', 'paired_blind', 'paired_direct'],
          'long_effect_improvement': .1, 'long_gap_improvement': .05,
          'long_log_tolerance': .01, 'normal_relative_tolerance': .01,
          'legacy_long_logscore_improvement': .01, 'uniform_bits': 53,
          'known_terminal_policy': 'exact onehot only after previously observed found'}
CAPS = {'collect': 1800, 'fit': 1800, 'audit': 1800}
SOURCES = ['scripts/qualify_otto_action_effect.py', 'scripts/otto_action_effect_common.py',
           'scripts/collect_otto_action_effect.py', 'scripts/run_otto_action_effect.py', 'scripts/audit_otto_action_effect.py',
           'src/openjev/research/otto_action_effect_metrics.py',
           'tests/test_otto_action_effect_metrics.py', 'tests/test_collect_otto_action_effect.py',
           'tests/test_run_otto_action_effect.py', 'tests/test_audit_otto_action_effect.py',
           'research/otto-action-effect-protocol.md',
           'scripts/otto_belief_distillation_common.py', 'scripts/run_otto_belief_distillation.py',
           'src/openjev/research/otto_action_latent_model.py', 'src/openjev/research/otto_action_latent_metrics.py',
           'src/openjev/research/otto_predictive_belief.py', 'src/openjev/research/otto_sampler_law.py',
           'src/openjev/research/otto_belief_distillation_metrics.py',
           'tests/test_otto_action_latent_model.py', 'tests/test_otto_action_latent_metrics.py',
           'tests/test_run_otto_belief_distillation.py',
           'tests/test_otto_predictive_belief.py', 'tests/test_otto_belief_distillation_metrics.py',
...
def original_native():
    ...
    collector = load(OLD_COLLECTOR, '_action_effect_native_lineage')
...
def training_reference():
    """Bind only the parent's TRAIN split and its closed acquisition cost."""
    require(desc(PARENT / 'closure-01.json')['sha256'] == PARENT_CLOSURE_PIN, 'published parent closure pin')
    closure = read(PARENT / 'closure-01.json')
    receipt_path = PARENT / 'collection-01/receipt.json'
    terminal_path = PARENT / 'collection-native-01.terminal.json'
    require(closure['version'] == PARENT_VERSION and closure['technical_complete']
            and closure['independent_audit_passed'] and not closure['old_test_admitted'], 'closed parent study')
    binding = closure['processes']['collection']
    require(desc(receipt_path) == binding['receipt'] and desc(terminal_path) == binding['terminal'], 'parent acquisition closure')
    ...
    require(receipt['status'] == terminal['status'] == 'completed' and terminal['returncode'] == 0
            and terminal['group_absent'] and terminal['cleanup']['reaped'] and not terminal['timed_out'], 'completed parent acquisition')
    require(desc(PARENT_TRAIN) == receipt['files']['train.npz'], 'unchanged reused TRAIN bytes')
    summary_path = PARENT / 'collection-01/summary.json'
    require(desc(summary_path) == receipt['files']['summary.json'], 'parent acquisition accounting')
    summary = read(summary_path)
    require(summary['counts']['train'] == {'lambda3': 1046, 'lambda4': 0}, 'exact reused TRAIN cases')
    return {'train': {'path': str(PARENT_TRAIN.relative_to(ROOT)), **desc(PARENT_TRAIN)},
            'parent_closure': desc(PARENT / 'closure-01.json'), 'parent_collection_receipt': desc(receipt_path),
            'cases': 1046, 'training_horizon': 4, 'prior_dev_decodes': 0,
            'teacher_cost_linkage': {'whole_parent_collection_seconds': receipt['wall_seconds'],
                'whole_parent_native_steps': summary['calls']['native_step']['returned'],
                'whole_parent_teacher_annotations': summary['calls']['teacher_score']['returned'],
                'scope': 'Full parent acquisition charged as shared upstream data cost, includes its unused DEV acquisition. No free-label claim.'}}
...
class Run:
    def __init__(self, args, phase):
        ...
        self.clock = load('src/openjev/research/suspend_clock.py', '_action_effect_clock').SuspendClock()
