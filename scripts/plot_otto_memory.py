"""Render all saved OTTO arms and blocks without new policy/environment calls."""
...
def main(args):
    ...
    s = json.loads(raw)
    if s["episodes"] != 512 or [b["block"] for b in s["blocks"]] != list(range(8)) or len(s["criteria"]) != 10:
    ...
    for ax, metric, title, xlabel, scale in zip(axes[:2], ("capped_time", "controller_seconds"),
            ...
            ("Moves to source, capped at 642", "Milliseconds per episode"), (1, 1000), strict=True):
    ...
    fig.suptitle("Olfactory search: recent observations explain most of the benefit", x=.025, ha="left", y=.98,
                 weight="bold", fontsize=19)
    fig.text(.025, .90, f"512 sampled-source searches | 64 paired cases | 8 controllers | Continuation {decision}: {sum(c['passes'] for c in s['criteria'])}/10", fontsize=12)
    fig.text(.025, .055,
             "Dots: eight fixed blocks; bars: means. Initial-hit strata weighted by the upstream mixture. No confidence intervals shown.\n"
    ...
    for extension in ("png", "pdf"):
        fig.savefig(args.output / f"otto-memory.{extension}", dpi=180, facecolor="white")
    plt.close(fig)
