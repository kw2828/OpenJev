...
until all of its journals have been flushed/fsynced. Exactly one original teacher score per state; no fitting.
"""
...
VERSION = "otto-score-forecast-collection-v1"
SELF, TEST = "scripts/collect_otto_score_forecasts.py", "tests/test_collect_otto_score_forecasts.py"
...
FIRST = {"train": {"lambda3": 22100001, "lambda4": 22200001},
         "valid": {"lambda3": 22300001, "lambda4": 22400001}}
CASES = {"train": 9, "valid": 6}
HORIZON, EPISODES = 2188, 90
LIMITS = {"native_seconds": 900, "rss_bytes": 4 * 1024**3, "output_bytes": 2 * 1024**3}
CALL_CAPS = {"tensorflow_construction": 1, "tensorflow_build": 1, "tensorflow_load": 1,
    "tensorflow_value": EPISODES * HORIZON, "native_reset": EPISODES, "native_step": EPISODES * HORIZON,
    "actor_construction": EPISODES, "backend_binding": EPISODES, "analytic_score": EPISODES * HORIZON,
    "feature_build": EPISODES * HORIZON, "teacher_score": EPISODES * HORIZON,
    "actor_update": EPISODES * HORIZON}
CONFIGURATION = {"arms": list(ARMS), "first_seeds": FIRST, "cases": CASES, "horizon": HORIZON,
    ...
    "period": 4, "teacher_calls_per_state": 1,
...
NEW_COMPONENTS = {SELF, TEST, "src/openjev/research/otto_recurrent_scores.py",
    "tests/test_otto_recurrent_scores.py", "src/openjev/research/otto_score_forecast_data.py",
    "tests/test_otto_score_forecast_data.py"}
...
PAYLOADS = {f"{name}.gz" for name in JOURNALS} | {"started.json", "runtime.json", "setup.json",
    ...
    "valid.npz", "costs.json", "summary.json"}
...
def route_scores(arm, step, allowed, analytic_action, teacher, select, cache):
    """One teacher call, with skipped actions fixed first and an owned held cache."""
...
def authenticate_inputs(inputs):
    ...
    expected_seeds = [first + case for split, regimes in FIRST.items()
                      for first in regimes.values() for case in range(CASES[split])] + [225001, 225002, 225003]
...
class Run:
    def __init__(self, args):
        ...
        self.receipt = {"version": VERSION, "status": "started", "training_updates": 0,
            "scope": "Complete fixed TRAIN/VALID trajectories and one original teacher score per preaction state; no fitting."}
    ...
    def setup(self):
        ...
        write(self.out / "deployment.json", {"original_setup_wall_seconds": self.original_setup_wall,
            ...
            "scope": "All paths physically annotate every preaction state. Deployed queries and annotation-only calls are distinct; no deployment-cost claim."})
    ...
    def episode(self, identity):
        ...
        for step in range(HORIZON):
            ...
            require(self.ledger.calls["tensorflow_value"]["returned"] - tf_before == 1, "exactly one physical teacher forward per state")
            deployed += int(queried); annotations += int(not queried); corrections += int(correction)
            ...
            self.ledger.emit("samples.jsonl", {**identity, "row_index": row_index, "step": step,
                "public": current, "posterior": witness, "features": features.tolist(), "raw_q": raw.tolist(),
        ...
        require(deployed == (steps if arm == "neural" else (steps + 3) // 4 if arm == "period4_hold" else 0)
            and deployed + annotations == steps and corrections == (steps + 3) // 4, "controller and annotation accounting")
        ...
        return {**identity, "steps": steps, "rows": steps, "start_row": start_row,
            ...
            "deployed_queries": deployed, "annotation_only": annotations, "teacher_calls": steps,
    ...
    def save_stage(self, stage):
        ...
        with np.load(path, allow_pickle=False) as saved:
            require(set(saved.files) == set(ARRAY_KEYS) and all(saved[k].dtype == arrays[k].dtype
                and saved[k].shape == arrays[k].shape and saved[k].tobytes() == arrays[k].tobytes() for k in ARRAY_KEYS),
    ...
    def body(self):
        ...
        for channel in ("tensorflow_value", "native_step", "actor_update", "analytic_score", "feature_build", "teacher_score"):
            require(self.ledger.calls[channel]["returned"] == n, "one operation per retained state: " + channel)
        for channel in ("native_reset", "actor_construction", "backend_binding"):
        ...
        summary = {"version": VERSION, "complete": True, "episodes": EPISODES, "rows": n,
            "datasets": self.datasets, "teacher_calls": n, "deployed_queries": sum(r["deployed_queries"] for r in self.rows),
