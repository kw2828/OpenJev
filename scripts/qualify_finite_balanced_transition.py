"""Source-bound fabricated-input qualification; never trains or reads weights."""
...
VERSION = 'finite-balanced-transition-qualification-v1'
...
def register(attempt):
    ...
    plan = {'version': VERSION, 'root': str(ROOT), 'runtime': runtime(), 'sources': sources(),
            'environment': ENV, 'commands': commands(), 'cap_seconds': 90, 'output_limit_bytes': 8 * 1024**2,
            'output': str(output), 'supervision': str(prefix) + '.launch.json', 'snapshot': str(snapshot),
            'base_commit': subprocess.check_output(['git', 'rev-parse', 'HEAD'], text=True).strip(),
            'scope': 'Fabricated numerical primitive only; no checkpoint decoding, training or empirical evaluation.'}
...
def worker(args):
    ...
    record = {'version': VERSION, 'status': 'FAILED', 'plan': str(plan_path),
              ...
              'commands': [], 'scope': 'Fabricated primitive checks only', 'started_unix': time.time()}
    try:
        ...
        require(plan['cap_seconds'] == 90 and plan['output_limit_bytes'] == 8 * 1024**2
        ...
        require(launch['command'] == expected and launch['pid'] == os.getpid() and launch['pgid'] == os.getpgrp()
                and launch['cwd'] == str(ROOT) and launch['cap_seconds'] == 90, 'original native launch identity')
