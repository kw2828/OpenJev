...
def validate_launch_binding(plan, receipt):
    ...
    expected = [plan['runtime']['executable'],
                str(Path(plan['root']) / 'scripts/finite_update_learning_worker.py'),
...
def admit_qualification(plan, receipt_path):
    ...
    for key in ('version', 'root', 'runtime', 'sources', 'config', 'phases', 'tests',
                'lint_sources', 'rss_limit_bytes', 'output_limit_bytes', 'kernel_qualification', 'exposure_probe', 'parent_evidence'):
    ...
    require(probe['audit'] == descriptor(probe_folder / 'audit.json')
            and probe_audit['version'] == 'finite-update-learning-audit-v1'
            and probe_audit['profile'] == 'engineering-942201'
    ...
    require(probe['status'] == 'PASS' and probe['config'] == plan['exposure_probe']['config']
            ...
            and {(row['arm'], row['seed']) for row in probe['projections']} ==
                {(arm, 942301) for arm in ('original_free', 'matched_free', 'rounded')}
...
def main():
    ...
    try:
        ...
        require(plan['version'] == 'finite-update-learning-v1', 'registered study version')
        ...
        for name, pin in plan['parent_evidence'].items():
            require(descriptor(ROOT / name) == pin, 'unchanged closed parent evidence')
        ...
        for name, pin in plan['parent_evidence'].items():
            require(descriptor(ROOT / name) == pin, 'closed parent evidence unchanged after phase')
