...
def validate_registered_plan(plan_path, plan):
    from register_finite_head_learning import (
        CONFIG,
        EXPOSURE,
        SOURCES,
        TESTS,
    ...
    require(Path(plan_path).resolve() == expected_path and plan['version'] == 'finite-head-learning-v1'
...
def qualification_commands(plan):
    ...
    return [[str(root / '.venv/bin/ruff'), 'check', *plan['lint_sources']],
            ...
            [plan['runtime']['executable'], str(root / 'scripts/qualify_finite_head_exposure.py'),
...
def validate_launch_binding(plan, receipt):
    ...
    expected = [plan['runtime']['executable'],
                str(Path(plan['root']) / 'scripts/finite_head_learning_worker.py'),
...
def main():
    ...
    try:
        ...
        require(plan['version'] == 'finite-head-learning-v1', 'registered study version')
