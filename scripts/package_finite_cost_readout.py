...
from render_finite_cost_readout import (
    ROOT,
    authenticate,
    descriptor,
    inventory,
...
VERSION = 'finite-cost-readout-package-v1'
STUDY_NAME = 'finite-cost-readout-v1'
...
def package(studyfolder, reportfolder, outputfolder, *, registration_sha256, commit=None, publication_paths=()):
    ...
    require(render_receipt['version'] == 'finite-cost-readout-report-v1'
    ...
    renderer_path = regular(ROOT / 'scripts/render_finite_cost_readout.py')
    ...
    for engineering_plan in engineering_plans:
        ...
        if engineering_plan == qualification_plan:
            ...
            require(terminal['cwd'] == str(ROOT) and command[0] == record['runtime']['executable']
                    and (ROOT / command[1]).resolve() == ROOT / 'scripts/finite_cost_readout_worker.py'
    ...
    for name in ('README.md', 'LICENSE', 'scripts/render_finite_cost_readout.py', 'scripts/package_finite_cost_readout.py'):
    ...
    manifest = {'version': VERSION, 'study': STUDY_NAME, 'registration': descriptor(plan_path),
        ...
        'scope': 'All registered sources, original closed phase payloads including nine checkpoints, fresh endpoint and all-attempt prefix arrays, oracle controls, journals, original receipts/launches/terminals/logs, every named engineering attempt with its source snapshot, and completed report. Failed attempts remain failures. No prior scientific datasets or checkpoints.',
