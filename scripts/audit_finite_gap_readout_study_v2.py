...
VERSION = 'finite-gap-readout-study-audit-v1'
NAMESPACE = 428260924
...
def scalar_lipschitz(states, np):
    """Safe Hessian row-sum bound from raw states, without solver statistics."""
    xb, xo = states['blind_states'], states['observed_states']
    n, h = xb.shape[:2]
    denominator = 4 * n * h
    gram = [[math.fsum(float(x[i, t, s]) * float(x[i, t, k])
                        for x in (xb, xo) for i in range(n) for t in range(h)) / denominator
             for k in range(8)] for s in range(8)]
    value = 2 * max(math.fsum(abs(entry) for entry in row) for row in gram)
    require(math.isfinite(value) and value >= 0, 'finite independent curvature bound')
    return value


SETTINGS = {'max_iterations': 20000, 'certificate_every': 10, 'fw_gap_tolerance': 1e-8,
            'simplex_tolerance': 1e-12, 'objective_increase_tolerance': 1e-12,
            'monotonicity_roundoff': 1e-15}
GAP_WORK_KEYS = {'gradient_calls', 'quadratic_value_calls', 'projection_calls',
                 'direct_objective_gradient_passes', 'check_calls', 'monotonicity_restarts',
                 'iterations', 'accepted_iterations'}
HISTORY_KEYS = {'iteration', 'objective', 'fw_gap', 'simplex_violation',
                'certificate_passed', 'objective_nonincreasing', 'qualifies'}


def verify_history(record, initial_check, final_check):
    """Reconcile saved stopping history and work; do not replay unsaved iterates."""
    require(record['settings'] == SETTINGS and record['method'] == 'fixed-step-monotone-restarted-fista',
            'exact registered projected-solver settings and method')
    work, solver, history = record['work'], record['solver'], record['certificate_history']
    require(set(work) == GAP_WORK_KEYS and all(type(value) is int and value >= 0 for value in work.values()),
            'exact nonnegative projected-solver work')
    iterations, restarts = work['iterations'], work['monotonicity_restarts']
    require(iterations == work['accepted_iterations'] and 0 <= iterations <= 20000
            and iterations % 10 == 0 and restarts <= iterations,
            'successful retained iterate and fixed certificate stopping schedule')
    expected_schedule = list(range(0, iterations + 1, 10))
    require(type(history) is list and [row['iteration'] for row in history] == expected_schedule,
            'complete initial/every-ten/final certificate history without duplicates')
    for index, row in enumerate(history):
        require(type(row) is dict and set(row) == HISTORY_KEYS and type(row['iteration']) is int,
                'exact certificate-history fields')
        require(all(type(row[key]) in (int, float) and math.isfinite(row[key])
                    for key in ('objective', 'fw_gap', 'simplex_violation'))
                and row['objective'] >= 0 and row['simplex_violation'] >= 0,
                'finite nonnegative history objective and feasibility')
        certified = row['simplex_violation'] <= 1e-12 and -1e-12 <= row['fw_gap'] <= 1e-8
        nonincreasing = row['objective'] <= initial_check['objective'] + 1e-12
        require(row['certificate_passed'] is certified and row['objective_nonincreasing'] is nonincreasing
                and row['qualifies'] is (certified and nonincreasing), 'recorded certificate stop conditions')
        require(row['qualifies'] is (index == len(history) - 1), 'single solve stops at first qualified certificate')
    for row, independent in ((history[0], initial_check), (history[-1], final_check)):
        for key in ('objective', 'fw_gap', 'simplex_violation'):
            compare(row[key], independent[key], path='independent endpoint history ' + key)
    gradients = iterations + restarts
    values = 1 + gradients if iterations else 0
    passes = len(history)
    expected = {'gradient_calls': gradients, 'quadratic_value_calls': values,
                'projection_calls': gradients, 'direct_objective_gradient_passes': passes,
                'check_calls': 2 + passes + values + iterations + 2 * gradients,
                'monotonicity_restarts': restarts, 'iterations': iterations, 'accepted_iterations': iterations}
    compare(work, expected, path='complete projected-solver executed-operation accounting')
    termination = 'initial_certificate' if not iterations else 'direct_certificate'
    require(record['termination'] == termination and solver['message'] == termination
            and solver['success'] is True and type(solver['status']) is int and solver['status'] == 0
            and solver['iterations'] == iterations and solver['nfev'] == values and solver['njev'] == gradients,
            'qualified projected-solver termination and exact work joins')
    return {'iterations': iterations, 'restarts': restarts, 'certificate_checks': passes,
            'intermediate_iterates_independently_replayed': False}


def verify_solve(record, states, data, np, *, check=lambda: None):
    """Certify original/final raw-state loss, curvature and simplex dual gap."""
    ...
    require(simplex_violation(initial) <= 1e-12 and np.all(initial >= 0), 'feasible original softmax head')
    require(raw.tobytes() == final.tobytes(), 'no export repair: raw and final probability bytes match')
    projection = record['projection']
    require(type(projection) is dict and set(projection) == {'applied', 'max_abs', 'l2'}
            and projection['applied'] is False
            and all(type(projection[key]) in (int, float) and math.isfinite(projection[key])
                    and projection[key] == 0 for key in ('max_abs', 'l2')),
            'exactly zero post-stop projection')
    ...
    lipschitz = scalar_lipschitz(states, np)
    require(type(record['lipschitz']) in (int, float) and math.isfinite(record['lipschitz'])
            and math.isclose(record['lipschitz'], lipschitz, rel_tol=1e-12, abs_tol=0.),
            'independent Hessian row-sum Lipschitz bound')
    compare(record['objective_initial'], initial_check['objective'], path='original TRAIN cost objective')
    compare(record['objective_raw'], final_check['objective'], path='retained TRAIN cost objective')
    compare(record['raw_feasibility'], final_check['simplex_violation'], path='retained simplex feasibility')
    ...
    require(record['version'] == 'finite-gap-readout-v1' and record['status'] == 'SOLVE_PASS'
            and record['complete'] is True and record['failure_reasons'] == [], 'successful single solve without fallback')
    history = verify_history(record, initial_check, final_check)
    ...
    return {'objective_initial': initial_check['objective'], 'objective_final': final_check['objective'],
            ...
            'raw_feasibility': final_check['simplex_violation'], 'projection_max_abs': 0.,
            'lipschitz': lipschitz, 'history': history, 'independently_certified': True}
...
CONFIG = {'dev_seed_namespace': NAMESPACE, 'dev_attempts': 128, 'dev_horizon': 8,
          ...
          'solver_maxiter': 20000, 'solver_certificate_every': 10, 'solver_monotonicity_roundoff': 1e-15,
          'solver_gap_tolerance': 1e-8, 'solver_nonincrease_tolerance': 1e-12}
...
def validate_metadata(folder, summary, *, check=lambda: None):
    ...
    require(summary['version'] == 'finite-gap-readout-study-v1' and summary['config'] == CONFIG, 'exact frozen diagnostic configuration')
...
def audit(folder, *, check=lambda: None):
    ...
    return {'version': VERSION, 'agreement': True, 'technical_complete': False,
            ...
            'limitations': [
                ...
                'Initial/final certificates and the curvature bound are independently reconstructed; intermediate history/work attestations are not replays of unsaved iterates.',
