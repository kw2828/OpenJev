...
ARMS = ("exit", "factorized_full", "factorized_recent128", "factorized_latest", "particle_full", "quality", "known_map", "privileged")
LABELS = ("Exit immediately", "Full-support full history", "Full-support recent 128", "Full-support latest",
          "Fixed particles full", "Quality only", "Map supplied, quality inferred*", "Map + qualities supplied*")
COLORS = ("#858d96", "#1c6b48", "#527bba", "#ab6a1c", "#8b5d9c", "#ae5a5a", "#17647c", "#53616b")
...
def main(args):
    ...
    if summary["episodes"] != 256 or len(maps) != 8 or set(maps) != set(range(13001, 13009)):
        raise ValueError("figure requires all eight maps and 256 episodes")
    ...
    expected_conditions.update(("factorized_full_gain_vs_exit", "known_map_gain_vs_exit", "privileged_gain_vs_exit", "complete_cohort", "factorized_observed_forecast_bounds"))
    criteria = summary["criteria"]
    if (len(criteria) != 13 or {c["name"] for c in criteria} != expected_conditions
    ...
    figure, axes = plt.subplots(1, 3, figsize=(17.5, 7.2), gridspec_kw={"width_ratios": [1.1, 1, 1]})
    ...
    for index, control in enumerate(controls):
        per_map = [m["scores"]["factorized_full"]["raw_return"] - m["scores"][control]["raw_return"]
                   for m in summary["by_map"]]
        values["paired_map_gains"][control] = per_map
        axes[2].scatter(per_map, index + np.linspace(-.16, .16, 8), color=COLORS[ARMS.index(control)], s=30, alpha=.7)
        axes[2].scatter([np.mean(per_map)], [index], color=COLORS[ARMS.index(control)], marker="|", s=500, linewidths=2.7)
    axes[2].axvline(0, color="#777777", linewidth=1)
    axes[2].axvline(5, color="#1c6b48", linestyle="--", linewidth=1, label="Required mean gain: +5")
    axes[2].set_yticks(range(4), ("vs Recent 128", "vs Latest", "vs Quality", "vs Particles"))
    axes[2].set_ylim(3.65, -.65)
    axes[2].set_title("Full-support full-history difference", loc="left", weight="bold", pad=13)
    ...
    figure.suptitle("RockSample: does full location support improve control?", x=.025, ha="left", y=.98,
                    fontsize=19, weight="bold")
    figure.text(.025, .902, f"256 episodes | 8 maps x 4 resets x 8 controllers | Continuation {decision}: {passed}/13 conditions",
    ...
    figure.text(.025, .055, "Each dot is one map's four-reset mean; bars mark equal-map means. No confidence intervals shown.\n"
                "*References receive the map; only the last also receives qualities. Controller time includes construction, filtering and planning; excludes simulator/I/O.\n"
    ...
    figure.subplots_adjust(left=.18, right=.985, top=.79, bottom=.24, wspace=.47)
    for suffix in ("png", "pdf"):
        figure.savefig(args.output / f"representations.{suffix}", dpi=180, facecolor="white")
    plt.close(figure)
