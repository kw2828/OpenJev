"""V2 saved-record audit with explicit floating-point centering verification.
...
This is a narrow versioned copy of pinned V1, whose failed run stays immutable.
V2 adds its failure lineage and replaces the ill-conditioned normalized-target
tolerance with byte-exact qualified arithmetic plus rational roundoff checks in
original cost units. All scientific criteria and other audit stages are intact.
"""
...
VERSION = 'otto-teacher-learning-saved-audit-v2'
SELF = 'scripts/audit_otto_teacher_learning_v2.py'
BASE_AUDITOR = 'scripts/audit_otto_teacher_learning.py'
BASE_PIN = 'a048af6bc8b702532f2b9248a86a2c8d916347ab3972dbb2764544a530e24cd4'
PRESERVED = {
...
PRODUCER = 'scripts/study_otto_teacher_learning.py'
PLAN_PIN = '7c155b10b630ee6fb7b16e302accfc0883b4a39ff7a0cb6f94fe4b6ffb0bec90'
...
H, LABEL_SEED, SEEDS = 2188, 19000002, (10101, 10102, 10103)
...
ROOT_FILES = {'started.json', 'panels.jsonl', 'collection.json', 'training-data.npz', 'training-data.json',
...
class Audit:
    ...
    def admission(self):
        ...
        self.receipt.update(base_auditor_sha256=BASE_PIN, preserved_inputs=PRESERVED,
                            repair_scope='Exact qualified target arithmetic and rational original-unit roundoff checks only.')
    ...
    def authenticate(self):
        ...
        self.require(plan['version'] == worker['version'] == 'otto-teacher-learning-v1'
        ...
        self.require(set(worker['files']) == PAYLOADS
                     and {p.name for p in self.run.iterdir()} == PAYLOADS | {'receipt.json'}, 'exact 583 payload closure')
        ...
        self.counts.update(payloads=583, sources=len(plan['sources']), inputs=len(plan['inputs']))
    ...
    def panels(self):
        ...
        for row, belief in zip(self.rows, self.beliefs, strict=True):
            ...
            self.equal([(r['replicate_id'], r['first_action']) for r in records],
                       list(itertools.product(range(16), actions)), 'complete canonical continuation panel')
            ...
            events = iter(self.lines(self.run / filename, maximum=512, count_limit=565615818))
            ...
            self.require(event_count == 67 + 4 * len(records) + 8 * steps - 2 * found, 'panel event identity')
            self.reduction(panel['summary'], records, actions, anchor)
        ...
        self.require(totals['records'] == 32304, 'all continuation costs')
        expected = {'anchor_snapshot': 558, 'source_generator': 8928, 'source_draw': 8928,
                    'teacher_snapshot': 32304, 'hit_generator': 32304, 'teacher_choose': totals['steps'] - 32304,
        ...
        for key, value in {'panels': 558, 'records': 32304, 'steps': totals['steps'], 'found': totals['found'],
                           'censored': 32304 - totals['found'], 'sampler_events': totals['events']}.items():
        ...
        self.counts.update(panels=558, continuations=32304, sampler_events=totals['events'], continuation_moves=totals['steps'])

    def reduction(self, summary, records, actions, anchor):
        values = {(r['replicate_id'], r['first_action']): r for r in records}
        n, k = 16, len(actions)
        totals = {a: sum(values[r, a]['steps'] for r in range(n)) for a in actions}
        ...
        for row in summary['actions']:
            action = row['action']
            successes = sum(values[r, action]['found'] for r in range(n))
            mean = totals[action] / n
            self.label_means[anchor, action] = mean
        ...
        for row, (first, second) in zip(summary['pairs'], pair_ids, strict=True):
            delta = [values[r, first]['steps'] - values[r, second]['steps'] for r in range(n)]
            total = sum(delta)
    ...
    def targets(self):
        np = self.np
        selected = {row['row_index']: row for row in self.rows}
        analytic = np.full((558, 4), np.inf, dtype=np.float64)
        found = set()
        with relative(f'{SYMM}/dagger-rows.jsonl').open('rb') as stream:
            for index, raw in enumerate(stream):
                self.check()
                self.require(len(raw) <= 8192, 'bounded historical metadata line')
                lexical = json.loads(raw, parse_int=str, parse_float=str)
                self.require(lexical['row_index'] == str(index), 'original TRAIN row order')
                if index not in selected:
                    continue
                row, original = selected[index], decode(raw)
                self.equal({k: v for k, v in original.items() if k != 'teacher_costs'},
                           {k: v for k, v in row.items() if k != 'anchor_id'}, 'selected analytic label provenance')
                self.require(len(original['teacher_costs']) == 4, 'four historical costs')
                for action, cost in enumerate(original['teacher_costs']):
                    if self.mask[row['anchor_id'], action]:
                        self.finite(cost, 'finite analytic cost', nonnegative=False)
                        analytic[row['anchor_id'], action] = cost
                    else:
                        self.require(cost is None, 'blocked analytic null')
                found.add(index)
        self.require(index + 1 == 4596 and found == set(selected), 'all selected TRAIN labels')
        ...
        self.target_hashes = {}
        for kind, costs in (('analytic', analytic), ('continuation', self.label_means)):
            self.context = {'phase': 'targets', 'kind': kind}
            saved = self.data['analytic_costs' if kind == 'analytic' else 'continuation_means']
            self.require(saved.dtype == np.float64 and saved.shape == (558, 4)
                         and np.array_equal(saved, costs), 'saved raw targets match authentic labels/records')
            centered = np.zeros((558, 4), dtype=np.float64)
            ...
            qualified_centered = np.where(self.mask, finite_costs - qualified_means[:, None], 0.0)
            denominators = (0.25 * np.maximum(
                np.max(np.where(self.mask, costs, -np.inf), axis=1)
                - np.min(np.where(self.mask, costs, np.inf), axis=1), 1e-8)
                if kind == 'analytic' else np.ones(558, dtype=np.float64))
            qualified_centered /= denominators[:, None]
            ...
            self.require(original.dtype == np.float64 and original.shape == (558, 4)
                         ...
                         and original.tobytes() == qualified_centered.tobytes(),
                         'byte-exact qualified float64 centering recipe')
            row_moments = []
            for i, row in enumerate(self.rows):
                actions = row['public']['valid_actions']
                values = [float(costs[i, a]) for a in actions]
                mean = math.fsum(values) / len(values)
                denominator = .25 * max(max(values) - min(values), 1e-8) if kind == 'analytic' else 1
                for action in actions:
                    centered[i, action] = (costs[i, action] - mean) / denominator
                self.target_roundoff_checks(values, [original[i, a] for a in actions], qualified_means[i], mean, denominator)
                row_moments.append(math.fsum(float(centered[i, a]) ** 2 for a in actions) / len(actions))
            scale = max(1e-8, math.sqrt(math.fsum(float(w) * v for w, v in zip(weights, row_moments, strict=True)) / 558))
            cast = self.data[f'{kind}_scaled_float32']
            qualified_row_squares = np.sum(qualified_centered * qualified_centered, axis=1, dtype=np.float64) / counts
            qualified_scale = max(1e-8, float(np.sqrt(np.sum(weights * qualified_row_squares, dtype=np.float64) / 558)))
            self.require(float(self.data_info['scales'][kind]).hex() == qualified_scale.hex(),
                         'exact qualified global RMS scale')
            self.near(self.data_info['scales'][kind], scale, 'single weighted TRAIN RMS')
            self.require(cast.dtype == np.float32 and cast.shape == (558, 4) and np.isfinite(cast).all()
                         and cast.tobytes() == (qualified_centered / qualified_scale).astype(np.float32).tobytes(),
                         'float64 centering precedes float32 training cast')
            for key, expected in (('weights_float64', weights), ('weights_float32', weights.astype(np.float32)),
                ...
                self.require(value.dtype == expected.dtype and value.shape == expected.shape
                             and np.array_equal(value, expected), 'common episode weights/mask')
    ...
    def training(self):
        ...
        for seed in SEEDS:
            for kind in KINDS:
                ...
                self.near(fit['costs']['label_allocation_seconds'], self.collection['seconds'] / 3 if kind == 'continuation' else 0,
                          'new label cost belongs to continuation family')
    ...
    def conclusions(self):
        ...
        for regime in FIRST:
            ...
            for seed in SEEDS:
                value = means[f'continuation@{seed}']
            ...
            candidate, baseline = families['continuation'], families['analytic']
            gains = [math.fsum(block[f'analytic@{seed}'] - block[f'continuation@{seed}'] for seed in SEEDS) / 3
                     for block in blocks]
        ...
        expected = {'regimes': regimes, 'positive_control_checks': positive, 'competence_checks': competence,
                    ...
                    'inherited_results_revised': False, 'version': 'otto-teacher-learning-v1'}
        ...
        self.near(summary['costs']['continuation_collection_seconds'], self.collection['seconds'], 'complete collection interval')
        ...
        self.result = {**expected, 'version': VERSION, 'producer_version': 'otto-teacher-learning-v1',
