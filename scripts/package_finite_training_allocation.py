"""Package closed training-allocation evidence with an opaque-byte roundtrip.
...
from render_finite_training_allocation import (
    REGISTRATION_SHA256,
    ROOT,
    STUDY_NAME,
    authenticate,
...
VERSION = 'finite-training-allocation-package-v1'
...
def package(study, report, output, *, registration_sha256, commit, publication):
    ...
    require(report == ROOT / 'research/finite-training-allocation-results',
    ...
    require(receipt['renderer'] == {'path': str(ROOT / 'scripts/render_finite_training_allocation.py'),
             **descriptor(ROOT / 'scripts/render_finite_training_allocation.py')},
            'renderer source matches receipt')
    require(receipt['version'] == 'finite-training-allocation-report-v1'
    ...
    draft = ROOT / 'output/finite-training-allocation-visual-draft-01'
    review = read(draft / 'visual-review.json')
    draft_names = {'renderer.py', 'original-report/summary.json', 'original-report/report.md',
                   'original-report/benchmark.png', 'original-report/receipt.json'}
    require(review['status'] == 'PRESENTATION_REVISION' and set(review['files']) == draft_names,
            'exact retained visual draft')
    for name in sorted(draft_names):
        add('publication/visual-draft-01/' + name, draft / name, review['files'][name])
    add('publication/visual-draft-01/visual-review.json', draft / 'visual-review.json')
    named = {'README.md', 'LICENSE', 'scripts/render_finite_training_allocation.py',
             'scripts/package_finite_training_allocation.py', *publication,
    ...
    manifest = {
        ...
        'source_pins': plan['sources'], 'kernel_qualification': plan['kernel_qualification'],
        'scope': 'Every authenticated study and numerical-prerequisite artifact, all 47 pinned sources, 27 model and 27 optimizer checkpoints, training orders, histories, predictions, timings, update traces, original phase records and named publication files. No directory is included wholesale.',
        ...
        'counts': dict.fromkeys(('array_decodes', 'checkpoint_decodes', 'model_calls',
                                'optimizer_calls', 'generator_calls'), 0),
    }
