"""Render audited saved OTTO results and two prospectively selected case-zero replays.
...
VERSION = 'otto-released-reference-presentation-v1'
CLOCK = 'src/openjev/research/suspend_clock.py'
CLOCK_PIN = 'cac9077db3c66f5ef43d9412b732f5b869c1004c4bac98589816780c120f6124'
AUDITOR = 'scripts/audit_otto_released_reference.py'
RUNNER = 'scripts/study_otto_released_reference.py'
ARMS = ('released_tf', 'analytic_all4', 'analytic_inbounds')
REGIMES = {'base': {'seed': 850001, 'lambda': 3}, 'shift': {'seed': 860001, 'lambda': 4}}
LABELS = {'released_tf': 'Released TF', 'analytic_all4': 'Analytic: all 4', 'analytic_inbounds': 'Analytic: in bounds'}
COLORS = {'released_tf': '#2563eb', 'analytic_all4': '#0f8b73', 'analytic_inbounds': '#c76a15'}
...
SCOPE = ('Saved, independently audited aggregates and prospectively selected case zero in each regime. '
         'Three arms and both regimes retained. GIF playback follows evenly spaced decision steps, '
         'not measured wall time. Source markers expose evaluator truth for the viewer only. '
         'No model, simulator, actor or new scientific scoring calls.')
...
class Render:
    ...
    def authenticate(self):
        ...
        require(worker['version'] == 'otto-released-reference-v1' and worker['completed_episodes'] == 576
                and worker['training_updates'] == worker['external_model_calls'] == 0
                and worker['numpy_port_admitted'] is False, 'complete nontraining reference run')
        ...
        require(plan['version'] == 'otto-released-reference-v1' and plan['status'] == 'frozen_before_native_run'
        ...
        require(result['agreement'] is True and result['episodes'] == 576 and set(result['cohorts']) == set(REGIMES), 'complete independently reconstructed aggregates')
    ...
    def chart(self, summary):
        ...
        for row, (regime, specification) in enumerate(REGIMES.items()):
            ...
            require(set(data['means']) == set(ARMS) and len(data['blocks']) == 8, 'complete plotted arm and block coverage')
            for col, (metric, title, scale) in enumerate(metrics):
                ...
                ax.bar(range(3), values, width=.62, color=[COLORS[a] for a in ARMS], alpha=.9, zorder=2)
                ...
                ax.set_xticks(range(3), ['Released\nTF', 'Analytic\nall 4', 'Analytic\nin bounds'])
        ...
        competence = sum(c['passes'] for d in summary['cohorts'].values() for c in d['competence_checks'])
        teacher = sum(c['passes'] for d in summary['cohorts'].values() for c in d['stronger_teacher_checks'])
        fig.suptitle('Released OTTO policy versus analytic controls', x=.065, y=.99, ha='left', fontsize=22, weight='bold')
        fig.text(.065, .94, f'Reference competence: {competence}/6 conditions | Teacher candidate: {teacher}/12 conditions | 576 episodes', fontsize=13)
        fig.legend(handles=[Patch(facecolor=COLORS[a], label=LABELS[a]) for a in ARMS], loc='lower center',
                   bbox_to_anchor=(.5, .047), ncol=3, frameon=False)
        ...
        fig.subplots_adjust(left=.085, right=.985, bottom=.15, top=.855, hspace=.40, wspace=.28)
        fig.savefig(self.out / 'reference-comparison.png', dpi=200, facecolor='white')
        fig.savefig(self.out / 'reference-comparison.svg', facecolor='white', metadata={'Date': None})
        plt.close(fig)
        values = {'scope': SCOPE, 'points': points, 'competence_passed': competence, 'competence_total': 6,
                  'teacher_passed': teacher, 'teacher_total': 12,
                  'competent_reference': summary['competent_reference'], 'promising_teacher_candidate': summary['stronger_value_teacher'],
                  'utility_compute_advantage': summary['utility_compute_advantage'],
    ...
    def replay_cases(self):
        ...
        require(episode_count == 576 and all('episode' in r for r in selected.values()), 'all fixed replay episodes present')
    ...
    def gif(self, regime, record):
        ...
        for global_step in record['frame_steps']:
            self.check()
            frame = Image.new('RGB', (1120, 500), '#f7f9fc')
            ...
            for column, arm in enumerate(ARMS):
                ...
                x0, y0, scale = 24 + column * 369, 110, 5.85

                def xy(position, x0=x0, y0=y0, scale=scale):
                    return x0 + position[1] * scale, y0 + position[0] * scale

                draw.text((x0, 78), LABELS[arm], fill=COLORS[arm], font=font)
                ...
                draw.text((x0, 428), f"Move {step}/{data['steps']} | {status}", fill='#14243d', font=font)
                draw.text((x0, 454), f"Last public hit: {packet['hit']} | row {packet['position'][0]}, col {packet['position'][1]}", fill='#405168', font=small)
    ...
    def execute(self):
        require(self.out.is_absolute(), 'absolute exclusive presentation output')
        self.out.mkdir(parents=True, exist_ok=False)
        previous = signal.getsignal(signal.SIGALRM)
        try:
            require(digest(ROOT / CLOCK)['sha256'] == CLOCK_PIN, 'registered clock before evidence reads')
            self.clock = load(ROOT / CLOCK, '_reference_presentation_clock').SuspendClock()
