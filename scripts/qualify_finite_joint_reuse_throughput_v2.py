...
import qualify_finite_joint_reuse_throughput as failed_v1
...
VERSION = 'finite-joint-reuse-throughput-v2'
...
FAILED = ROOT / 'output/finite-joint-reuse-throughput-v1'
FAILED_PLAN_SHA256 = 'ebebb49b85c09f8ed51a58f78113bfae4df71611e60e0d66c07d5fb458ec6b89'
FAILED_CLOSURE_SHA256 = {
    'engineering-01.receipt.json': 'b8d73e5b5f164fbede1337fcb8273a72a2f949c43485b3dc6c5b020cbe0acb78',
    'qualify-native-01.launch.json': 'eb7239715b545b985ce5bacea9293d3e4ea1e0ef999196062dd821542f2f7903',
    'qualify-native-01.terminal.json': '951a224f649d2f1486bad6bf166a2b1defbfcf8425eaa6780d63dc623905ac2c',
    'qualify-native-01.log': '6b287a023904203d8f07ad816538599b1deac92b06d3e89f718516e5da2d054f',
}
NEW_SOURCES = (
    'scripts/audit_finite_joint_reuse_throughput_v2.py',
    'tests/test_finite_joint_reuse_throughput_audit_v2.py',
    'scripts/qualify_finite_joint_reuse_throughput_v2.py',
    'research/finite-joint-reuse-throughput-v2-protocol.md',
)
SOURCES = tuple(dict.fromkeys((*failed_v1.SOURCES, *NEW_SOURCES)))
LINT_SOURCES = tuple(dict.fromkeys((*failed_v1.NEW_SOURCES, *NEW_SOURCES)))
...
def commands(phase):
    ...
    if phase == 'qualify':
        return [[str(ROOT / '.venv/bin/ruff'), 'check',
                 *[name for name in LINT_SOURCES if name.endswith('.py')]],
    ...
    return [[sys.executable, str(ROOT / 'scripts/audit_finite_joint_reuse_throughput_v2.py'),
...
def failed_attempt():
    """Authenticate the sole v1 test failure without promoting or replaying it."""
    plan_path = FAILED / 'registration.json'
    require(descriptor(plan_path)['sha256'] == FAILED_PLAN_SHA256, 'original failed registration pin')
    original = read(plan_path)
    failed_v1.validate_plan(original)
    require(len(original['sources']) == 88 and set(original['sources']) == set(failed_v1.SOURCES),
            'all88 failed-attempt sources remain frozen')
    for name, digest in FAILED_CLOSURE_SHA256.items():
        require(descriptor(FAILED / name)['sha256'] == digest, 'original failed closure bytes')
    receipt = read(FAILED / 'engineering-01.receipt.json')
    launch = read(FAILED / 'qualify-native-01.launch.json')
    terminal = read(FAILED / 'qualify-native-01.terminal.json')
    failed_v1.validate_launch(original, 'qualify', launch, FAILED_PLAN_SHA256)
    require(receipt['version'] == failed_v1.VERSION and receipt['phase'] == 'qualify'
            and receipt['status'] == 'FAILED'
            and receipt['error'] == "ValueError('registered phase command passed: 1')"
            and receipt['plan'] == str(plan_path) and receipt['plan_sha256'] == FAILED_PLAN_SHA256
            and receipt['output'] == str(FAILED / 'engineering-01')
            and receipt['supervision'] == str(FAILED / 'qualify-native-01.launch.json')
            and receipt['launch'] == launch and receipt['predecessors'] == {}
            and receipt['sources_before'] == original['sources']
            and files(FAILED / 'engineering-01') == receipt['files'],
            'exact original failed test receipt and source closure')
    require(all(terminal.get(key) == value for key, value in launch.items())
            ...
            'original ordinary test failure, not timeout or incomplete cleanup')
    require(len(receipt['commands']) == 2, 'original two-command failure roster')
    for index, row in enumerate(receipt['commands']):
        require(row['command'] == original['commands']['qualify'][index]
                and row['returncode'] == (0, 1)[index]
                and row['log'] == descriptor(FAILED / f'engineering-01/command-{index}.log')
                and type(row['elapsed_ns']) is int and 0 <= row['elapsed_ns'] <= terminal['elapsed_ns'],
                'original lint PASS and test FAIL command/log joins')
    require(sum(row['elapsed_ns'] for row in receipt['commands']) <= terminal['elapsed_ns'],
            'failed attempt nested command timing')
    test_log = (FAILED / 'engineering-01/command-1.log').read_text()
    require('1 failed, 77 passed in 2.45s' in test_log
            and 'test_exact_median_threshold_is_not_success' in test_log,
            'preserved exact test assertion failure')
    expected_names = {'registration.json', *FAILED_CLOSURE_SHA256,
                      'engineering-01/command-0.log', 'engineering-01/command-1.log',
                      'source-snapshot/manifest.json',
                      *('source-snapshot/' + name for name in original['sources'])}
    inventory = files(FAILED)
    require(set(inventory) == expected_names and len(inventory) == 96,
            'complete original failure inventory, no run or audit artifacts')
    return {'folder': str(FAILED), 'registration': descriptor(plan_path),
            'files': inventory, 'status': 'FAILED', 'timed_benchmark_fits': 0,
            'correction': 'Test key selector matched rounded_median as a round condition; production timing gate is unchanged.'}
...
def validate_plan(plan):
    ...
    require(failed_attempt() == plan['failed_attempt'], 'original failed qualification unchanged')
...
def register():
    ...
    plan = {'version': VERSION, 'root': str(ROOT), 'runtime': runtime(), 'environment': ENV,
            ...
            'prerequisite': prerequisite(), 'failed_attempt': failed_attempt(),
            'base_commit': subprocess.check_output(['git', 'rev-parse', 'HEAD'], text=True).strip(),
            'scope': 'Correct only the v1 test assertion selecting timing condition keys. The original failed qualification is retained; zero timed benchmark fits preceded v2. Unchanged engineering throughput comparison; no DEV, task evaluation or scientific admission.'}
