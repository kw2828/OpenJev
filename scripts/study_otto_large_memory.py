"""Qualify the official 53x53 OTTO setting and run a separate fixed memory screen."""
...
CASES = 96
FIRST_SEED = 610001
EPISODES = CASES * len(ARMS)
CASES_PER_BLOCK = 12
QUALIFICATION_STEP_CAP = 2000
NATIVE_STEP_CAP = 1690000
CONFIG = {"Ndim": 2, "lambda_over_dx": 3.0, "R_dt": 2.0,
...
def summarize(rows, weights, *, qualification):
    structural = (qualification.get("status") == "completed"
                  ...
                  and 0 < qualification["native_steps"] <= QUALIFICATION_STEP_CAP)
    require(set(weights) == set(STRATA) and all(math.isfinite(w) and 0 < w < 1 for w in weights.values())
            and abs(sum(weights.values()) - 1) <= 1e-12, "invalid initial-hit mixture")
    expected = {(seed, arm) for seed in range(FIRST_SEED, FIRST_SEED + CASES) for arm in ARMS}
    require(len(rows) == EPISODES and {(r["seed"], r["arm"]) for r in rows} == expected,
            "incomplete or duplicated cohort")
    for r in rows:
        case = r["seed"] - FIRST_SEED
        require(r["block"] == case // CASES_PER_BLOCK and r["initial_hit"] == 1 + (case % CASES_PER_BLOCK) // 4,
    ...
    def group(subset):
        ...
        for arm in ARMS:
            ...
            means[arm] = {m: sum(weights[h] * sum(r[m] for r in strata[h]) / len(strata[h]) for h in STRATA)
                          for m in metrics}
        return means
...
class Run:
    ...
    def check(self):
        ...
        require(sum(p.stat().st_size for p in self.out.iterdir() if p.is_file()) <= 512 * 1024**2,
                "output cap exceeded")
        require(self.receipt["native_steps_attempted"] <= NATIVE_STEP_CAP, "native step cap exceeded")

    def step(self, env, action, hit=None):
        self.check()
        require(self.receipt["native_steps_attempted"] < NATIVE_STEP_CAP, "native step allocation exhausted")
        if self.receipt.get("phase") == "qualification":
            require(self.receipt["native_steps_attempted"] < QUALIFICATION_STEP_CAP, "qualification step allocation exhausted")
    ...
    def qualify(self, SourceTracking, HeuristicPolicy, seeded_environment, observation, OdorMemory):
        ...
        record("official_autoset_geometry", template.N == GRID_SIZE and template.Nhits == NHITS)
        # Independent analytic likelihood formula for the frozen 2D configuration.
        cells = np.indices((2 * GRID_SIZE + 1, 2 * GRID_SIZE + 1)) - GRID_SIZE
        distance = np.sqrt(np.sum(cells**2, axis=0))
        mu = 2 * k0(np.where(distance == 0, 1, distance) / 3) / np.log(6)
        analytic = np.stack([poisson.pmf(0, mu), poisson.pmf(1, mu), poisson.pmf(2, mu), poisson.sf(2, mu)])
        analytic[:, GRID_SIZE, GRID_SIZE] = 0
        ...
        radius = np.arange(1, 3000)
        shell = np.pi * ((radius + .5)**2 - (radius - .5)**2)
        mus = 2 * k0(radius / 3) / np.log(6)
        masses = np.array([0, np.sum(poisson.pmf(1, mus) * shell),
                           np.sum(poisson.pmf(2, mus) * shell), np.sum(poisson.sf(2, mus) * shell)])
        weights = masses / masses.sum()
        # Check adapter's actual initial-hit distribution against the independent formula.
        random_initial = seeded_environment(SourceTracking, 420001, CONFIG)
        ...
        for h in STRATA:
            first = seeded_environment(SourceTracking, 420010 + h, CONFIG, initial_hit=h)
            second = seeded_environment(SourceTracking, 420010 + h, CONFIG, initial_hit=h)
        ...
        forced_source = np.array([GRID_SIZE - 1, GRID_SIZE - 1])
        fixture = forced_environment(forced_source, 1)
        start = tuple(fixture.agent)
        got = self.step(fixture, 0, hit=NHITS - 1)
        record("saturated_hit_public_update", got == (NHITS - 1, 0, False) and fixture.obs["hit"] == NHITS - 1
        ...
        record("source_found_terminal", fixture.obs == {"hit": -2, "done": True}
               and fixture.p_source[GRID_SIZE - 1, GRID_SIZE - 1] == 1 and fixture.p_source.sum() == 1)
        record("qualification_native_step_budget", self.receipt["native_steps_attempted"] <= QUALIFICATION_STEP_CAP,
        ...
        return {h: float(weights[h]) for h in STRATA}
    ...
    def cohort(self, SourceTracking, HeuristicPolicy, seeded_environment, observation, OdorMemory, weights):
        ...
        with (self.out / "transitions.jsonl").open("x") as transitions, (self.out / "episodes.jsonl").open("x") as episodes:
            ...
            for case in range(CASES):
                self.check()
                seed, hit, block = FIRST_SEED + case, 1 + (case % CASES_PER_BLOCK) // 4, case // CASES_PER_BLOCK
                shift = case % len(ARMS)
