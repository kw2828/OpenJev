"""Explicit per-fit separate/reuse integration; no admission or dataset generation.

The model, prefix stage, Adam ownership/reset, exact-update controller, orders,
checkpoint callbacks and global objective are inherited unchanged. Separate
mode retains the frozen joint numerical statements. Reuse mode calls the
per-call helper. Logical rollout counts remain comparable; actual
shared work belongs only to five joint_reuse_* routes, never to legacy blind,
observed or prefix routes. training_prefix contains prefix-only stage work in
reuse mode. No timing or performance assertion follows from those call counts.

The caller provides distinct, empty existing fit folders and the original
all-attempt tensors with their endpoint-row mapping. This module performs no
registration, generation, DEV evaluation, implicit retry or persistent cache.
Full wrapper time includes durable allocation output; the exact controller
charges setup, updates, three boundaries and final summary to its safety cap.
"""
...
from openjev.research.finite_rounded_models import ARMS, DYNAMICS, prefix_predictions, torch_objective
from openjev.research.finite_rounded_models import make_model as learned_model
from openjev.research.finite_update_allocation import Hooks, Snapshot, run_update_allocation

VERSION = 'finite-joint-reuse-training-v1'
IMPLEMENTATIONS = ('separate', 'reuse')
...
def train(arm, seed, data, prefixes, config, folder, counts, structural_work, check, *, implementation):
    ...
    require(type(implementation) is str and implementation in IMPLEMENTATIONS, 'explicit separate or reuse implementation')
    context, orders = {}, {}
