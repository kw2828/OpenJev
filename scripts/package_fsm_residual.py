"""Opaque evidence delivery after a closed FSM residual study and agreeing audit.
...
REGISTRATION = 'research/fsm-residual-registration.json'
REGISTRATION_SHA256 = '1ac959d74756df7ae6e78c070056ec55b97955e3de0ba070490df61bf9cf90ea'
LICENSE = 'research/fsm-correction-results/DATA_LICENSE.txt'
LICENSE_SHA256 = 'a1227b74fe74c9595e030fe6560874f3258d46a308943a44468be4fac3332b14'
ENGINEERING = ('output/fsm-residual-engineering-v1',
               'output/fsm-residual-core-engineering-v1',
               'output/fsm-residual-audit-engineering-v1',
               'output/fsm-residual-package-engineering-v1')
EXTRAS = (REGISTRATION, 'research/fsm-residual-protocol.md',
          'research/fsm-residual-prior-art.md', 'scripts/audit_fsm_residual.py',
          'scripts/audit_fsm_correction.py', 'scripts/plot_fsm_residual.py',
          'scripts/package_fsm_residual.py', 'tests/test_audit_fsm_residual.py')
SOURCES = {f'src/openjev/research/{name}.py' for name in
           ('fsm_residual_study', 'fsm_residual', 'fsm_study', 'fsm_data',
            'fsm_linear', 'fsm_gru', 'predictive_state_correction')}
COMMAND = ['.venv/bin/python', '-u', '-m', 'openjev.research.fsm_residual_study',
           '--protocol', REGISTRATION, '--data',
           'output/fsm-correction-engineering-v1/admission-01/combined_data.npz',
           '--output', 'output/fsm-residual-study-v1']
AUDIT_COMMAND = ['.venv/bin/python', 'scripts/audit_fsm_residual.py', '--study',
                 'output/fsm-residual-study-v1', '--process',
                 'output/fsm-residual-engineering-v1/original-process.json', '--output',
                 'output/fsm-residual-audit-v1/audit.json']
...
def authenticate(study, audit_path, plots, root=ROOT):
    ...
    require(study == root/'output/fsm-residual-study-v1', 'exact original study required')
    ...
    require(plan['protocol_markdown_path'] == 'research/fsm-residual-protocol.md'
    ...
    require(audit['auditor'] == pin(root/'scripts/audit_fsm_residual.py')
            and audit['independent_helper'] == pin(root/'scripts/audit_fsm_correction.py'),
    ...
    require(plot['study'] == str(study)
            and set(plot['inputs']) == {'summary.json', 'evaluations.json', 'fits.json'}, 'plot study identity')
    for name, item in plot['inputs'].items():
        require(descriptor_matches(item, study/name), 'plot input changed')
    require(descriptor_matches(plot['renderer'], root/'scripts/plot_fsm_residual.py'),
...
def archive_payloads(files, output, metadata):
    ...
    for name in files:
        ...
        require(parts and not PurePosixPath(name).is_absolute() and '..' not in parts
                and '\\' not in name and name != 'manifest.json', 'unsafe archive name')
...
def package(study, audit_path, plots, output, *, root=ROOT):
    ...
    metadata = {'scope': 'Complete original residual study, all declared engineering attempts, audit and plots; opaque copies, no scientific replay.',
