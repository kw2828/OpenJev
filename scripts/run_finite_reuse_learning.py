"""Fresh exact-update study using the qualified per-call joint computation reuse.

Only the training implementation and its disjoint structural-work routes change
from the frozen exact-update runner. Data, initializers, prefix objective, Adam
reset, batch orders, exact reference, checkpoint barrier and evaluation remain
unchanged. Every arm explicitly uses reuse; no global mutation or persistent
cache is introduced. Admission and resource supervision belong to the caller.
"""
...
from openjev.research.finite_observation_models import metadata
...
VERSION = 'finite-reuse-learning-v1'
DEFAULT_CONFIG = {'seed_namespace': 434260924, 'train_attempts': 512, 'dev_attempts': 128,
    'batch_size': 64, 'learning_rate': .003, 'fit_seeds': [434261001, 434261002, 434261003],
