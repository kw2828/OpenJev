...
SELF = "scripts/otto_readout_compute_native_bridge.py"
COLLECTOR = "scripts/collect_otto_readout_compute.py"
CLOCK = "src/openjev/research/suspend_clock.py"
SUPERVISOR = "scripts/supervise_dialogue_observation_v2.py"
INTERPRETER = ".venv-otto-released-native/bin/python"
VERSION = "otto-readout-compute-native-bridge-v1"
CAP_SECONDS = 60
PINS = {
    COLLECTOR: "108d6cbfc7d6edecbae064ac03b820fc0637dd464de7ad8cc03ba170197bd47e",
...
def collection_closure(inputs, sources):
    ...
    require(plan["version"] == receipt["version"] == "otto-readout-compute-collection-v1"
            ...
            and receipt["completed_episodes"] == receipt["dev_episodes"] == 36
            and receipt["confirm_episodes"] == 0 and receipt["master_episodes"] == 36
...
def execute(args):
    ...
    try:
        ...
        with metadata_imports_only():
            ...
            collector = load(COLLECTOR, "_compute_bridge_native_collector")
            plan = read(inputs["collection_plan"]["path"])
            collector.validate_plan(plan)
            sources, runtime, native_paths, _, _ = collector.authenticate_inputs(plan["inputs"])
            require(plan["sources"] == sources and plan["runtime"] == runtime
                    and plan["native_inputs"] == {k: {"path": str(p.relative_to(ROOT)), **descriptor(p)}
                                                  for k, p in native_paths.items()},
                    "same qualified native collection runtime and inputs")
            _, collected, directory, _ = collection_closure(inputs, sources)
            require(collected["peak_rss_bytes"] <= collector.LIMITS["rss_bytes"]
                    and set(collected["calls"]) == set(collector.CALL_CAPS)
                    and all(type(row["returned"]) is int
                            and row["attempted"] == row["returned"]
                            and 0 <= row["returned"] <= collector.CALL_CAPS[key]
                            for key, row in collected["calls"].items()),
                    "bounded complete collection work")
