...
from finite_observation_worker import ROOT, closed_producer, descriptor, publish, require, runtime

SOURCES = (
    'scripts/register_finite_observation_learning.py',
    'scripts/finite_observation_worker.py',
    ...
    'tests/test_finite_observation_world.py',
    'tests/test_finite_observation_models.py',
    'tests/test_finite_observation_learning.py',
    'tests/test_finite_observation_worker.py',
    'tests/test_otto_observation_operator_model.py',
    'research/finite-observation-learning-protocol.md',
    'pyproject.toml',
    'uv.lock',
)
TESTS = [name for name in SOURCES if name.startswith('tests/')]
LINT = [name for name in SOURCES if name.endswith('.py')]
CONFIG = {'seed_namespace': 420260924, 'train_attempts': 512, 'dev_attempts': 128,
          'epochs': 48, 'batch_size': 64, 'learning_rate': .003,
          'fit_seeds': [420261001, 420261002, 420261003],
...
def main():
    ...
    folder = ROOT / 'output/finite-observation-learning-v1'
    ...
    plan = {'version': 'finite-observation-learning-v1', 'mode': args.mode,
