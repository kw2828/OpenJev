"""Matched prequery calibration fitting; no teacher or environment calls.
...
SELF = "scripts/train_otto_prequery_calibration.py"
TEST = "tests/test_train_otto_prequery_calibration.py"
...
METRICS = "src/openjev/research/otto_prequery_metrics.py"
WINDOWS = "src/openjev/research/otto_score_forecast_data.py"
SAMPLING = "src/openjev/research/otto_sampled_forecast_data.py"
PROTOCOL = "research/otto-prequery-calibration-protocol.md"
AUDIT = "scripts/audit_otto_prequery_calibration.py"
AUDIT_TEST = "tests/test_audit_otto_prequery_calibration.py"
...
VERSION = "otto-prequery-calibration-training-v1"
COLLECTION_VERSION = "otto-prequery-calibration-collection-v1"
...
LIMITS = {"seconds": 10800, "rss_bytes": 4 * 1024**3, "output_bytes": 2 * 1024**3}
...
COMPONENTS = {SELF, TEST, MODELS, DATA, LOSS, METRICS, AUDIT, AUDIT_TEST,
    "tests/test_otto_prequery_scores.py", "tests/test_otto_prequery_data.py",
    "tests/test_otto_prequery_loss.py", "tests/test_otto_prequery_metrics.py"}
NEW = COMPONENTS | {PROTOCOL, WINDOWS, SAMPLING, CAPACITY, CLOCK, SUPERVISOR,
                   "scripts/train_otto_cross_query_forecasts.py",
                   "tests/test_qualify_otto_prequery_capacity.py", BASE_MODELS,
...
def cohort():
    ...
    for stage, starts, count in (("train", (271000001, 272000001), 9),
                                 ("valid", (273000001, 274000001), 6)):
...
def authenticate_inputs(inputs):
    ...
    require(capsummary["admitted"] is True and capsummary["technical_complete"] is True
        ...
        and capacity["configuration"]["admission_seconds"] == 8100., "fixed capacity gate")
...
class Run:
    ...
    def admit(self):
        ...
        self.clock = load(ROOT / CLOCK, "_prequery_training_clock").SuspendClock()
    ...
    def batch(self, model, optimizer, data, indices, kind, seed, epoch, batch):
        ...
        result = batch_update(self.torch, self.models, self.data, self.losses, model, optimizer, data, indices,
                              objective=CELLS[kind][1], check=self.check, stage=stage)
    ...
    def fit(self, kind, seed, history):
        ...
        model = self.models.make_head(architecture, seed)
        ...
        fit = {"family": kind, "architecture": architecture, "objective": objective,
            "seed": seed, "parameter_count": self.models.parameter_count(architecture),
    ...
    def body(self):
        ...
        self.models = load(ROOT / MODELS, "_prequery_training_models")
        self.data = load(ROOT / DATA, "_prequery_training_data")
        self.losses = load(ROOT / LOSS, "_prequery_training_loss")
        self.windows = load(ROOT / WINDOWS, "_prequery_census_windows")
        self.metrics = load(ROOT / METRICS, "_prequery_metrics")
        ...
        for seed in SEEDS:
            ...
            for architecture in self.models.KINDS:
                paired = [r for r in subset if r["architecture"] == architecture]
                require(len(paired) == 2 and paired[0]["initial_tensors"] == paired[1]["initial_tensors"]
        ...
        for model, fit in fitted:
            ...
            records.append({"family": fit["family"], "architecture": fit["architecture"],
                "objective": fit["objective"], "seed": fit["seed"], "prediction_work": work,
        ...
        summary = {"version": VERSION, "models": records, "hold": hold,
            ...
            "required_conditions": 55, "forecast_continuation": all(c["passes"] for c in conditions),
            "technical_complete_pending_saved_audit": True,
            "scientific_conditions": 54, "scientific_passed": sum(c["passes"] for c in conditions[1:]),
