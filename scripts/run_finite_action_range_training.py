"""Six loss arms through the unchanged fixed-update random-head training path.

Only the external factory/metadata mapping and blind-loss wrapper differ from
the frozen head trainer. All extra scalar calculations/counters occur inside
charged joint-update callbacks. Construction, Adam/reset, snapshots, cursor
lookup, clipping, losses outside blind cost and boundary hashes are unchanged.
No dataset generation, evaluation, registration or hidden-state input occurs.
"""
...
from openjev.research.finite_action_range_loss import (
    ARMS,
    LOSS_KINDS,
    LOSS_WORK_KEYS,
    TARGET_FIELDS,
...
from openjev.research.finite_action_range_loss import make_model as learned_model
...
VERSION = 'finite-action-range-training-v1'
...
def train(arm, seed, data, prefixes, config, folder, counts, structural_work, check, *, implementation):
    ...
    def validate():
        ...
        return {'parameter_metadata': context['parameter_metadata'],
                ...
                'head_initialization_work': context['head_initialization_work'],
                'loss_kind': LOSS_KINDS[arm], 'loss_work': dict(scalar_work), 'attempts': n,
        ...
        if kind == 'prefix':
            ...
            if implementation == 'separate':
                ...
                result = joint_objective(model, prefixes['prefix'][indices], prefixes['lengths'][indices],
                    ...
                    total_attempts=n, total_survivors=survivors, total_events=events, loss_kind=LOSS_KINDS[arm])
                for key in LOSS_WORK_KEYS:
                    scalar_work[key] += result['loss_work'][key]
                loss = result['loss']
            ...
            diagnostics = {'kind': kind, 'cursor': cursor, 'epoch': epoch, 'batch_offset': offset,
                           ...
                           'batch_size': b, 'loss_kind': LOSS_KINDS[arm],
                           'loss_work': dict(result['loss_work']), 'order_sha256': hashlib.sha256(orders[epoch].astype('<i8').tobytes()).hexdigest()}
            counts['training_prefix_rollouts'] += 1
    ...
    hooks = Hooks(validate=validate, new_optimizer=optimizer, snapshot_model=snapshot_model,
        ...
        final_summary=lambda opt, cursor: {'parameter_metadata': metadata(context['model']),
                                           ...
                                           'loss_kind': LOSS_KINDS[arm], 'loss_work': dict(scalar_work),
    ...
    try:
        ...
        if hasattr(error, 'action_range_loss_work'):
            _write(folder / f'failed-action-range-loss-{arm}-{seed}.json', error.action_range_loss_work)
