"""Independent saved-output audit for a task-independent cost-head intervention.
...
VERSION = 'finite-head-learning-audit-v1'
NAMESPACE = 436260924
ARMS = ('rounded_anchor', 'rounded_random', 'matched_free_random')
TRANSPORT = MappingProxyType({'rounded_anchor': 'rounded', 'rounded_random': 'rounded',
...
PROFILES = MappingProxyType({'science': SCIENCE,
    'engineering-946001': StudyProfile('engineering-946001', 946001, (946101,), 8, 8, 3, 3, 4, 30.),
    'engineering-946201': StudyProfile('engineering-946201', 946201, (946301,), 512, 8, 64, 32, 64, 30.)})
...
def boundary_hashes(arrays, np):
    ...
    full, restricted, head_state = hashlib.sha256(), hashlib.sha256(), hashlib.sha256()
    for name, value in sorted(arrays.items()):
        ...
        if name in DYNAMIC_NAMES:
            restricted.update(len(header).to_bytes(8, 'little'))
            restricted.update(header)
            restricted.update(value.tobytes(order='C'))
        else:
            head_state.update(len(header).to_bytes(8, 'little'))
            head_state.update(header)
            head_state.update(value.tobytes(order='C'))
    ...
    return {'full': full.hexdigest(), 'dynamics': dynamic.hexdigest(),
            'dynamics_state': restricted.hexdigest(), 'head_state': head_state.hexdigest(), 'head': head}
...
def allocation_comparisons(rows, fits, original_gates, *, spec=SCIENCE):
    ...
    paired, means, times = [], [], []
    for control in ('matched_free_random',):
        for horizon in (4, 8):
    ...
    require(len(conditions) == 3 + 2 * len(spec.seeds) + 2, 'complete registered allocation advance conditions')
    anchor = []
    for seed in spec.seeds:
        for horizon in HORIZONS:
            first, second = keyed['rounded_random', seed, horizon], keyed['rounded_anchor', seed, horizon]
            anchor.append({'candidate': 'rounded_random', 'anchor': 'rounded_anchor', 'seed': seed, 'horizon': horizon,
                **{name: {'candidate': first[name], 'anchor': second[name], 'difference': first[name] - second[name]}
                   for name in metrics}})
    passed = all(conditions.values())
    return {'paired': paired, 'mean_regret': means, 'mean_fit_time': times,
            'anchor_comparisons': anchor, 'anchor_is_descriptive_only': True,
            'advance': {'name': 'HEAD_INDEPENDENT_ADVANCE', 'passed': passed,
                        'status': 'HEAD_INDEPENDENT_ADVANCE_PASS' if passed else 'HEAD_INDEPENDENT_ADVANCE_FAIL',
                        'conditions': conditions},
            'interpretation': 'The random-head rounded candidate must meet every absolute criterion and paired requirement against random-head matched-free. The privileged-head rounded anchor is descriptive and cannot rescue failure. The same exact updates and paired minibatches do not equalize gradients or compute. Generic random logits change the initialization scale as well as removing the cost-template prior.'}
...
def initial_transition_checks(checkpoints, np):
    """Independently reconstruct each public seed's one task-independent head."""
    ...
    for seed in seeds:
        ...
        for name in ('emission_logits', 'hazard_logits'):
            require(len({initial[arm][name].tobytes(order='C') for arm in ARMS}) == 1,
                    'all initial emission and hazard parameters agree exactly')
        require(initial['rounded_anchor']['transition_logits'].tobytes(order='C') ==
                initial['rounded_random']['transition_logits'].tobytes(order='C'),
                'both rounded raw transition initializations agree exactly')
        random = np.random.Generator(np.random.PCG64(np.random.SeedSequence([seed, 436, 1]))).standard_normal((4, 8))
        require(random.dtype == np.float64 and all(initial[arm]['cost_logits'].tobytes(order='C') == random.tobytes(order='C')
                for arm in ('rounded_random', 'matched_free_random')),
                'both random heads equal the declared independent standard-normal stream exactly')
        anchor_probabilities = np.array([[.025 + (.9 if action == ((state ^ (state >> 1)) & 3) else 0.)
            for state in range(8)] for action in range(4)], dtype=np.float64)
        require(float(np.max(np.abs(initial['rounded_anchor']['cost_logits'] - np.log(anchor_probabilities)))) <= 1e-12,
                'anchor retains the original privileged .9-cost-template log probabilities')
        rounded, diagnostics = rounded_transition_from_logits(initial['rounded_random']['transition_logits'], np)
        matched = column_transition(initial['matched_free_random']['transition_logits'], np)
        match_error = float(np.max(np.abs(rounded - matched)))
        require(diagnostics['admitted'] and match_error <= 1e-12,
                'independent rounded versus matched-free initial transition')
        result.append({'seed': seed, **diagnostics, 'matched_max_abs': match_error,
                       'shared_initial_emission_hazard': True, 'shared_rounded_raw_transition': True,
                       'random_head_seed_entropy': [seed, 436, 1],
                       'random_head_sha256': hashlib.sha256(random.astype('<f8', copy=False).tobytes()).hexdigest(),
                       'initializer_reconstructions': 1, 'random_heads_bitwise_paired': True,
                       'anchor_privileged_template_verified': True})
    return result
        ...
        for arm in ARMS:
            for label in ('initial', 'boundary', 'final'):
                ...
                result.append({'arm': arm, 'seed': seed, 'boundary': label,
                    'transition_map': 'four_sweep_slack_rounding' if TRANSPORT[arm] == 'rounded' else 'column_softmax',
                    ...
                    'correction_status': 'reconstructed' if TRANSPORT[arm] == 'rounded' else 'not_executed',
...
def verify_boundaries(allocations, checkpoints, optimizer_payloads, np):
    ...
    prefix_pairs = {}
    for seed in initialization:
        hashes = {}
        for label in ('initial', 'boundary'):
            left, right = checkpoints[label, 'rounded_anchor', seed], checkpoints[label, 'rounded_random', seed]
            require(all(left[name].tobytes(order='C') == right[name].tobytes(order='C') for name in DYNAMIC_NAMES),
                    'rounded anchor and random dynamics match bitwise before and after prefix training')
            left_optimizer = optimizer_payloads[label, 'rounded_anchor', seed]
            right_optimizer = optimizer_payloads[label, 'rounded_random', seed]
            require(left_optimizer == right_optimizer and json.dumps(left_optimizer, sort_keys=True, separators=(',', ':'), allow_nan=False)
                    == json.dumps(right_optimizer, sort_keys=True, separators=(',', ':'), allow_nan=False),
                    'rounded prefix-only Adam states match exactly')
            hashes[label] = {'dynamics_state_sha256': boundary_hashes(left, np)['dynamics_state'],
                'optimizer_state_sha256': hashlib.sha256(json.dumps(left_optimizer, sort_keys=True, separators=(',', ':'), allow_nan=False).encode()).hexdigest()}
        prefix_pairs[seed] = {'seed': seed, 'boundaries': hashes, 'bitwise_equal': True}
    result = []
    for (arm, seed), allocation in allocations.items():
        ...
        for index, label in enumerate(('initial', 'boundary', 'final')):
            ...
            require(hashes['full'] == record['model_sha256'] and opt['sha256'] == record['optimizer_sha256']
                    and hashes['dynamics_state'] == record['metadata']['dynamics_state_sha256']
                    and hashes['head_state'] == record['metadata']['head_state_sha256'],
        ...
        result.append({'arm': arm, 'seed': seed, 'boundaries': checked,
                       'initial_transition_check': initialization[seed], 'prefix_pair_check': prefix_pairs[seed],
...
INITIALIZER_WORK_KEYS = ('rng_constructions', 'standard_normal_calls', 'standard_normal_entries',
                  'parameter_copy_calls', 'parameter_copy_entries', 'parameter_copy_bytes')


def head_metadata(row):
    """Check effective policy metadata, not inherited model head annotations."""
    arm, seed = row['arm'], row['seed']
    require(arm in ARMS, 'declared head study arm')
    random = arm != 'rounded_anchor'
    work = dict(zip(INITIALIZER_WORK_KEYS, (1, 1, 32, 1, 32, 256) if random else (0,) * 6, strict=True))
    integer_work(row['head_initialization_work'], set(INITIALIZER_WORK_KEYS))
    require(row['head_initialization_work'] == work, 'exact separately charged head initialization')
    pm = row['model_metadata']
    records = {name: {'shape': list(shape), 'count': math.prod(shape), 'dtype': 'torch.float64',
                     'bytes': 8 * math.prod(shape), 'requires_grad': True}
               for name, shape in PARAMETER_SHAPES.items()}
    require(pm['version'] == 'finite-head-initialization-v1'
            and pm['transport_model_version'] == 'finite-rounded-model-v1'
            and pm['arm'] == arm and pm['transport_arm'] == TRANSPORT[arm] and pm['seed'] == seed
            and pm['parameters'] == records and pm['count'] == pm['trainable_count'] == 352
            and pm['parameter_bytes'] == 2816 and pm['buffer_bytes'] == 0 and pm['buffers'] == {}
            and pm['mass_width'] == 8 and pm['privileged_prefix'] is False
            and pm['known_operators'] is False and pm['fixed_cost_readout'] is False
            and pm['privileged_readout_initialization'] is (not random)
            and pm['readout_delta'] == (None if random else .1), 'effective public-input model and head metadata')
    expected = {'policy': 'task_independent_standard_normal' if random else 'inherited_cost_template',
                'generator': 'numpy.random.Generator(PCG64)' if random else None,
                'seed_sequence_entropy': [seed, 436, 1] if random else None,
                'shape': [4, 8], 'dtype': 'float64', 'inherited_template_is_constructed': True}
    require(pm['head_initialization'] == expected and pm['head_initialization_work'] == work,
            'declared domain-separated task-independent head policy')
    rounded = TRANSPORT[arm] == 'rounded'
    require(pm['transition_stored_parameters'] == 256
            and pm['transition_ideal_constraint_degrees'] == (196 if rounded else 224)
            and pm['rounded_sweeps'] == (4 if rounded else None)
            and pm['rounded_slack'] == (1e-8 if rounded else None)
            and pm['rounded_tolerance'] == (1e-12 if rounded else None)
            and pm['matched_initialization'] is (not rounded), 'unchanged transport architecture metadata')
    compare(pm['construction_work'], construction_work(arm), path='inherited constructor accounting')


def validate_reference_metadata(summary):
    require(summary['regimes'] == {'train': {'split_id': 0, 'epsilon': .12},
                                  'base': {'split_id': 1, 'epsilon': .12}}, 'BASE-only declared regimes')
    shapes = {'costs': [4, 8], 'known_observed': [4, 4, 8, 8], 'known_found': [4, 8], 'regime_epsilon': []}
    ...
    require(pm['version'] == 'finite-regime-reference-v1' and pm['arm'] == 'exact_exact'
            ...
            and type(pm['epsilon']) is float and pm['epsilon'] == .12
            and pm['regime_state_buffer'] == 'regime_epsilon'
            and pm['known_operator_source'] == 'finite_observation_world.world(epsilon=0.12)'
            and digest_value(summary['oracle_state_sha256']), 'fixed zero-parameter BASE reference')


def declared_prefix_pairs(allocations, *, spec=SCIENCE):
    """Match the producer's pre-DEV record; saved numerical bytes are checked later."""
    records = []
    for seed in spec.seeds:
        saved = {arm: {p['label']: p['metadata'] for p in allocations[arm, seed]['checkpoints']} for arm in ARMS}
        anchor, random = saved['rounded_anchor'], saved['rounded_random']
        for label in ('initial', 'boundary'):
            require(anchor[label]['dynamics_state_sha256'] == random[label]['dynamics_state_sha256']
                    and anchor[label]['optimizer_state_sha256'] == random[label]['optimizer_state_sha256'],
                    'declared paired rounded prefix dynamics and Adam')
        head_hashes = {arm: saved[arm]['initial']['head_state_sha256'] for arm in ARMS}
        require(all(head_hashes[arm] == saved[arm]['boundary']['head_state_sha256'] for arm in ARMS),
                'all saved heads preserved before DEV')
        records.append({'seed': seed, 'arms': ['rounded_anchor', 'rounded_random'],
            ...
            'head_initial_boundary_sha256': head_hashes, 'all_heads_unchanged_after_prefix': True,
...
def validate_metadata(folder, summary, *, check=lambda: None, spec=SCIENCE):
    ...
    require(summary['version'] == 'finite-head-learning-v1' and summary['implementation'] == 'reuse',
            'registered reuse-only allocation producer')
    require(summary['transport_arms'] == dict(TRANSPORT), 'explicit study-arm transport mapping')
    ...
    for row in fits:
        ...
        require(row['implementation'] == 'reuse'
                and row['integration_version'] == 'finite-head-training-v1'
        ...
        compare(allocation['metadata'], {'parameter_metadata': row['parameter_metadata'],
            'model_metadata': row['model_metadata'], 'head_initialization_work': row['head_initialization_work'],
            'attempts': spec.train_attempts,
        ...
        for point in allocation['checkpoints']:
            ...
            require(type(record) is dict and set(record) == {'label', 'model', 'optimizer', 'model_state_sha256',
                'optimizer_state_sha256', 'joint_cursor', 'dynamics_state_sha256', 'head_state_sha256'} and record['label'] == label
                and record['model_state_sha256'] == point['model_sha256']
                and record['optimizer_state_sha256'] == point['optimizer_sha256']
                and record['joint_cursor'] == point['joint_cursor']
                and digest_value(record['dynamics_state_sha256']) and digest_value(record['head_state_sha256']),
                'complete boundary artifact joins')
        ...
        for kind in ('dynamics', 'head'):
            expected_hashes = {p['label']: p['metadata'][kind + '_state_sha256'] for p in allocation['checkpoints']}
            require(row[kind + '_boundary_sha256'] == expected_hashes
                    and type(row[kind + '_boundary_hash_evaluations']) is int
                    and row[kind + '_boundary_hash_evaluations'] == 3, 'all charged restricted state hash exports')
    paired_checks = paired_batch_checks(allocations, spec=spec)
    paired_batches = {str(seed): hashlib.sha256(json.dumps([
        p['result']['diagnostics']['indices'] for p in allocations[ARMS[0], seed]['trace'] if p['kind'] == 'joint'],
        separators=(',', ':'), allow_nan=False).encode()).hexdigest() for seed in spec.seeds}
    prefix_pairs = declared_prefix_pairs(allocations, spec=spec)
    require(summary['paired_batch_sha256'] == paired_batches, 'same-seed batch hashes match original trace indices')
    compare(summary['prefix_pair_checks'], prefix_pairs, path='all prefix pairing and frozen-head checks')
    barrier = {'checkpoints': [{'arm': row['arm'], 'seed': row['seed'], **row['checkpoint']} for row in fits],
               ...
               'paired_batch_sha256': paired_batches, 'prefix_pair_checks': prefix_pairs}
    compare(summary['checkpoint_barrier'], barrier, path='all-finals-before-DEV attestation')
    compare(read('checkpoint-barrier.json'), barrier, path='durable generation barrier')
    times = summary['prediction_times']
    require([(r['arm'], r['seed']) for r in times] == [(arm, seed) for arm in ARMS for seed in spec.seeds], 'all endpoint views')
    ...
    require([(r['arm'], r['seed']) for r in summary['prefix_prediction_times']]
            == [(arm, seed) for arm in ARMS for seed in spec.seeds], 'all attempted-prefix evaluations')
    for row in summary['prefix_prediction_times']:
        require(row['regime'] == 'base' and row['attempts'] == spec.dev_attempts
                and row['valid_events'] == summary['dataset_counts']['base']['valid_prefix_events']
    ...
    return {'paired_batch_checks': paired_checks, 'prefix_pair_checks': prefix_pairs,
            'head_initialization_work': {str(seed): {arm: next(r['head_initialization_work'] for r in fits
                if r['arm'] == arm and r['seed'] == seed) for arm in ARMS} for seed in spec.seeds},
            'dynamics_boundary_hash_exports': 3 * len(fits), 'head_boundary_hash_exports': 3 * len(fits),
            'fits': len(fits), 'payloads': len(expected_files), 'checkpoint_files_hashed': 3 * len(fits), 'optimizer_files_hashed': 3 * len(fits),
...
def audit(folder, *, check=lambda: None, profile='science'):
    ...
    counters = {'array_decodes': 0, 'checkpoint_decodes': 0, 'model_calls': 0, 'optimizer_calls': 0,
                'world_or_generator_calls': 0, 'native_calls': 0, 'initializer_reconstructions': 0}
    ...
    counters['initializer_reconstructions'] = len(spec.seeds)
    ...
    require(counters['array_decodes'] == 9 + 4 * len(ARMS) * len(spec.seeds) and counters['checkpoint_decodes'] == 3 * len(ARMS) * len(spec.seeds)
            and counters['optimizer_json_decodes'] == 3 * len(ARMS) * len(spec.seeds)
            and counters['initializer_reconstructions'] == len(spec.seeds), 'exact saved data/model/optimizer decode roster')
    ...
    return {'version': VERSION, 'profile': profile, 'agreement': True, 'technical_complete': False,
            ...
            'limitations': [
                ...
                'All saved transitions are independently reconstructed from raw boundary parameters. Both rounded arms receive four-sweep/slack boundary correction diagnostics; the matched-free control uses its actual column softmax.',
                ...
                'The random-head candidate must pass all three absolute criteria and every comparison against random-head matched-free. The privileged-head anchor cannot rescue a failed condition.',
                'The declared random initializer is reconstructed once per fit seed, separately counted from model/world calls; all actual checkpoint head bytes and paired dynamics-only prefix boundaries are checked.',
                'Removing the cost-template initialization also changes its scale and shape. Supervised cost labels and the favorable doubly stochastic world remain; this does not isolate arbitrary latent identification or prove transfer.',
