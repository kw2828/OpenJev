"""Render all saved teacher-learning fits; no models or environments are called."""
...
def main():
    ...
    receipt = read(args.run / "receipt.json")
    ...
    colors = {"analytic": "#2878A8", "continuation": "#CA641C"}
    points = []
    for ax, (metric, label, multiplier) in zip(axes, metrics, strict=True):
        for family, color in colors.items():
            for seed, marker in zip((10101, 10102, 10103), ("o", "s", "^"), strict=True):
                arm = f"{family}@{seed}"
    ...
    fig.suptitle("Can longer-term teacher costs improve a small search policy?", fontsize=17, y=.98)
    ...
    handles = [Line2D([], [], color=colors[k], linewidth=2.4, marker="D", label=label)
               for k, label in (("analytic", "Analytic-target family mean"),
                                ("continuation", "Continuation-target family mean"))]
    ...
    for suffix in ("png", "svg"):
        fig.savefig(args.output / f"teacher-learning.{suffix}", dpi=180, facecolor="white")
    plt.close(fig)
