"""DEV-only full-census collector for the fresh residual-estimator screen.

This explicit source fork preserves the qualified query-memory collector's
...
The master allocation reserves all54 fresh paths, but this worker can execute
only DEV18. Confirmation collection and even its serialization roundtrip require
a separately implemented and admitted future phase; no confirmation bypass is
provided. No learner, fitting, estimator selection or old-study TEST access.
"""
...
VERSION = "otto-residual-collection-v1"
SELF, TEST = "scripts/collect_otto_residual.py", "tests/test_collect_otto_residual.py"
PROTOCOL = "research/otto-residual-estimator-protocol.md"
DESIGN = "research/otto-residual-estimator-design.md"
ENGINEERING = "research/otto-residual-estimator-engineering.md"
FORK_SOURCE = "scripts/collect_otto_query_memory.py"
FORK_PIN = "e1cd73b7e2389c208955ce5dd46e7f7238444482d5ae06d7ee73fc640a579c8f"
...
FIRST = {"dev": {"lambda3": 314000001, "lambda4": 315000001},
         "confirm": {"lambda3": 316000001, "lambda4": 317000001}}
CASES = {"dev": 3, "confirm": 6}
STAGE_EPISODES = {"dev": 18, "confirm": 36}
PHASE = "dev"
HORIZON, EPISODES, MASTER_EPISODES = 2188, 18, 54
...
CONFIGURATION = {"phase": PHASE, "arms": list(ARMS), "first_seeds": FIRST, "cases": CASES,
    ...
    "confirm_reserved_episodes": 36, "confirm_collected_episodes": 0,
    "rotation": "global master case index modulo three; indices continue across phases",
    ...
    "confirmation_boundary": "not supported; requires separately admitted future collection phase after DEV pass",
...
NEW_SOURCES = NEW_COMPONENTS | {PROTOCOL, DESIGN, ENGINEERING, FORK_SOURCE,
    "output/otto-residual-runner-engineering-v1/run_engineering.py"}
SEED_REVIEW = "output/otto-residual-estimator-v1/seed-review-01.json"
SEED_REVIEW_PIN = "09164f7a013e6a03c879316ba75c3bd7e809a14d9ab338cb52d3836c2b75731c"
RESERVATION = "output/otto-residual-estimator-v1/seed-reservation-02.json"
RESERVATION_PIN = "00e0f1a8f864c3b3c53e5c89844518724112414c2773fd5d391e00d4d54d86d9"
PREVIOUS_RESERVATION = "output/otto-residual-estimator-v1/seed-reservation-01.json"
PREVIOUS_RESERVATION_PIN = "7d7ff7c16b1e5e2c77260a603d4202efd54e6ff7555e8bb8a1ecdf6e8ddf56ee"
...
def execution_cohort(phase):
    require(phase == PHASE,
            "only DEV collection is implemented; confirmation needs a separately admitted future phase")
    return cohort(PHASE)
...
def validate_seed_review(seeds, reservation):
    ...
    files are not all assumed unchanged forever. The original receipts and the
    five actual reviewed match sources are separately pinned by admission.
    """
    ...
    require(seeds["version"] == "otto-residual-seed-review-v1" and seeds["status"] == "reserved_before_run"
            and seeds["admits_execution"] is False and seeds["allocations"] == allocations
            and seeds["fresh_environment_seeds"] == expected_seeds and len(set(expected_seeds)) == 18
    ...
    require(reservation["version"] == "otto-residual-estimator-seed-reservation-v1"
            and reservation["attempt"] == 2 and reservation["status"] == "candidate_blocks_require_review"
            ...
            "original scoped seed reservation 02")
    hits, resolved = reservation["block_hits"], seeds["resolved_block_hits"]
    require(isinstance(hits, list) and isinstance(resolved, list) and len(hits) == len(resolved) == 5,
            "all five conservative block matches resolved")
    for original, reviewed in zip(hits, resolved, strict=True):
        require(set(reviewed) == set(original) | {"classification", "source_verified"}
                ...
                and reviewed["source_verified"] is True and reviewed["classification"] in (
                    "non-seed operation or sample count",
                    "fractional digits of a reported floating-point metric, not an integer seed"),
...
def authenticate_seed_review(path, sources):
    ...
    require(set(record) == {"path", "sha256", "bytes"} and record["path"] == RESERVATION
            ...
            "review binds unchanged original reservation 02")
    reservation = read(RESERVATION)
    validate_seed_review(seeds, reservation)
    previous = reservation["supersedes"]
    require(previous["path"] == PREVIOUS_RESERVATION and previous["sha256"] == PREVIOUS_RESERVATION_PIN
            and descriptor(previous["path"]) == {key: previous[key] for key in ("sha256", "bytes")},
            "unsuccessful original reservation 01 is preserved")
    verified = {SEED_REVIEW: SEED_REVIEW_PIN, RESERVATION: RESERVATION_PIN,
                PREVIOUS_RESERVATION: PREVIOUS_RESERVATION_PIN}
    for hit in reservation["block_hits"]:
        name = hit["path"]
        expected = reservation["files"][name]
        require(descriptor(name) == expected, "unchanged reviewed non-seed match source")
        lines = regular(name).read_text().splitlines()
        require(type(hit["line"]) is int and 1 <= hit["line"] <= len(lines)
                and lines[hit["line"] - 1] == hit["context"], "original reviewed match context")
        verified[name] = expected["sha256"]
...
def authenticate_inputs(inputs):
    require(set(inputs) == ROLES, "exact residual collection roles")
    paths = {}
    ...
    collector = load(ROOT / COLLECTOR, "_residual_qualified_collection")
...
class Run:
    def __init__(self, args):
        ...
        self.receipt = {"version": VERSION, "status": "started", "training_updates": 0,
            ...
            "scope": "DEV18 full fixed paths and teacher census. Confirmation unstarted; no fitting or autonomous efficacy."}

    def check(self):
        require(self.clock.now_ns() < self.launch["deadline_ns"], "original shared residual DEV collection deadline")
        rss = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss * (1 if sys.platform == "darwin" else 1024)
        self.receipt["peak_rss_bytes"] = rss
        require(rss <= LIMITS["rss_bytes"], "residual DEV collection RSS cap")
    ...
    def bind(self):
        ...
        self.clock = load(ROOT / CLOCK, "_residual_clock").SuspendClock()
    ...
    def setup(self):
        ...
        write(self.out / "deployment.json", {"original_setup_wall_seconds": self.original_setup_wall,
            ...
            "scope": "DEV18 full paths and every teacher score. Confirmation is reserved but unstarted. Deployed and annotation-only scores are distinguished. All collector schedules remain period four. No deployment-cost claim."})
    ...
    def execute(self):
        ...
        def interrupt(_signum, _frame):
            raise InterruptedError("original residual DEV collection supervisor stopped worker")
        ...
        try:
            ...
            self.receipt.update(status="completed", complete=True, files=files, calls=self.ledger.calls,
                completed_episodes=len(self.rows), dev_episodes=18, confirm_episodes=0, master_episodes=54,
