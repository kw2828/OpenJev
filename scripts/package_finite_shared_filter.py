"""Package an authenticated closed shared-filter study by opaque, deterministic bytes.
...
from render_finite_shared_filter import (
    ROOT,
    authenticate,
    descriptor,
    inventory,
...
VERSION = 'finite-shared-filter-package-v1'
STUDY_NAME = 'finite-shared-filter-v1'
...
def package(studyfolder, reportfolder, outputfolder, *, registration_sha256, commit=None, publication_paths=()):
    ...
    require(descriptor(studyfolder / 'registration-01.json')['sha256'] == registration_sha256, 'owner-supplied registration identity')
    ...
    require(render_receipt['version'] == 'finite-shared-filter-report-v1'
    ...
    renderer_path = regular(ROOT / 'scripts/render_finite_shared_filter.py')
    ...
    require(qualification_plan.name == 'engineering-registration-01.json'
            and sorted(studyfolder.glob('engineering-registration-[0-9][0-9].json')) == [qualification_plan],
            'this first-qualification package cannot silently omit another engineering attempt')
    study_file(qualification_plan)
    qualification_sources = read(qualification_plan)['sources']
    snapshot = studyfolder / 'engineering-source-01'
    if snapshot.exists():
        require(inventory(snapshot) == qualification_sources, 'complete preserved qualification source snapshot')
    for name, pin in qualification_sources.items():
        source = snapshot / name if snapshot.exists() else ROOT / name
        add('study/engineering-source-01/' + safe_relative(name), source, pin)
    ...
    for name in ('README.md', 'LICENSE', 'scripts/render_finite_shared_filter.py', 'scripts/package_finite_shared_filter.py'):
    ...
    manifest = {'version': VERSION, 'study': STUDY_NAME, 'registration': descriptor(plan_path),
        ...
        'engineering_attempts': [{'registration': qualification_plan.name,
                                 'status': phases['qualify']['receipt']['status'], 'qualifying': True}],
        'scope': 'All registered sources, original closed phase payloads including nine checkpoints and the fresh TRAIN/DEV arrays, oracle controls, journals, original receipts/launches/terminals/logs, the original first qualification and its source snapshot, and completed report. No prior scientific datasets or checkpoints.',
