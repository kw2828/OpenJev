...
def expected_command(plan, plan_path, worker_path, supervision, runner):
    command = [plan['python_executable'], str(ROOT / runner), '--plan', str(plan_path),
               '--plan-sha256', sha(plan_path)]
    # Preserve the literal argument order used by each already-defined runner.
    paths = [('--supervision', supervision), ('--output', str(worker_path.parent))]
    if runner != OLD:
        paths.reverse()
    return command + [value for pair in paths for value in pair]
...
def terminal_identity(worker_path, plan_path, terminal_path, runner):
    ...
    if command[1:2] == ['-u']:
        command.pop(1)
    require(command == expected_command(plan, plan_path, worker_path, request['supervision'], runner),
