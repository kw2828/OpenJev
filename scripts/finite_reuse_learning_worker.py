...
def validate_registered_plan(plan_path, plan):
    from register_finite_reuse_learning import (
        CONFIG,
        EXPOSURE,
        SOURCES,
        TESTS,
        authenticate_throughput,
        phase_specs,
    )
    ...
    require(Path(plan_path).resolve() == expected_path and plan['version'] == 'finite-reuse-learning-v1'
    ...
    require(authenticate_throughput() == plan['throughput_qualification'],
            'complete original throughput prerequisite unchanged')
...
def qualification_commands(plan):
    ...
    return [[str(root / '.venv/bin/ruff'), 'check', *plan['lint_sources']],
            ...
            [plan['runtime']['executable'], str(root / 'scripts/qualify_finite_reuse_exposure.py'),
...
def validate_launch_binding(plan, receipt):
    ...
    expected = [plan['runtime']['executable'],
                str(Path(plan['root']) / 'scripts/finite_reuse_learning_worker.py'),
...
def admit_qualification(plan, receipt_path):
    ...
    for key in ('version', 'root', 'runtime', 'sources', 'config', 'phases', 'tests',
                'lint_sources', 'rss_limit_bytes', 'output_limit_bytes', 'throughput_qualification', 'exposure_probe'):
    ...
    require(probe['audit'] == descriptor(probe_folder / 'audit.json')
            and probe_audit['version'] == 'finite-reuse-learning-audit-v1'
            and probe_audit['profile'] == 'engineering-944201'
    ...
    require(probe['status'] == 'PASS' and probe['config'] == plan['exposure_probe']['config']
            ...
            and {(row['arm'], row['seed']) for row in probe['projections']} ==
                {(arm, 944301) for arm in ('original_free', 'matched_free', 'rounded')}
...
def main():
    ...
    try:
        ...
        require(plan['version'] == 'finite-reuse-learning-v1', 'registered study version')
        ...
        if args.phase == 'qualify':
            ...
            from run_finite_reuse_learning import run
