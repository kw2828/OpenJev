"""Prospective belief-distillation pilot metadata and bounded-process helpers."""
...
CONFIG = {'prefix_transitions': 8, 'train_horizon': 4, 'dev_horizon': 8,
          'train_cases': 1536, 'dev_cases_per_regime': 128, 'min_train': 800, 'min_dev_per_regime': 64,
          'minimum_supported_cases': 48, 'train_first': 324000001,
          'dev_base_first': 325000001, 'dev_shift_first': 326000001,
          'fit_seeds': [327000001, 327000002, 327000003], 'epochs': 80, 'batch': 32,
          'lr': .003, 'clip': 5., 'aux_weight': .1, 'cost_weight': 1.,
          'candidate': 'recurrent_soft', 'controls': ['recurrent_sampled', 'action_blind_soft', 'direct_soft'],
          'long_logscore_improvement': .01, 'long_gap_improvement': .05,
          'normal_relative_tolerance': .01, 'uniform_bits': 53,
          'known_terminal_policy': 'exact onehot only after previously observed found'}
CAPS = {'collect': 1800, 'fit': 1800, 'audit': 1800}
SOURCES = ['scripts/qualify_otto_belief_distillation.py', 'scripts/otto_belief_distillation_common.py',
           'scripts/collect_otto_belief_distillation.py', 'scripts/run_otto_belief_distillation.py',
           'scripts/audit_otto_belief_distillation.py',
           'src/openjev/research/otto_action_latent_model.py',
           'src/openjev/research/otto_action_latent_metrics.py',
           'src/openjev/research/otto_predictive_belief.py',
           'src/openjev/research/otto_sampler_law.py',
           ...
           'tests/test_collect_otto_belief_distillation.py', 'tests/test_run_otto_belief_distillation.py',
           'tests/test_audit_otto_belief_distillation.py',
           'research/otto-belief-distillation-protocol.md',
...
def original_native():
    ...
    collector = load(OLD_COLLECTOR, '_belief_distillation_native_lineage')
...
class Run:
    def __init__(self, args, phase):
        ...
        self.clock = load('src/openjev/research/suspend_clock.py', '_belief_distillation_clock').SuspendClock()
