"""Complete public collection for the matched separate-prior study.
...
VERSION = "otto-separate-prior-collection-v1"
SELF, TEST = "scripts/collect_otto_separate_prior.py", "tests/test_collect_otto_separate_prior.py"
...
FIRST = {"train": {"lambda3": 281000001, "lambda4": 282000001},
...
CONFIGURATION = {"arms": list(ARMS), "first_seeds": FIRST, "cases": CASES, "horizon": HORIZON,
    ...
    "selection_seed_first": 286000001, "validation_annotation": "complete census",
...
NEW_COMPONENTS = {SELF, TEST, "src/openjev/research/otto_separate_prior_scores.py",
    "tests/test_otto_separate_prior_scores.py", "src/openjev/research/otto_prequery_scores.py",
    "tests/test_otto_prequery_scores.py", "src/openjev/research/otto_prequery_data.py",
    "tests/test_otto_prequery_data.py", "src/openjev/research/otto_prequery_loss.py",
    "tests/test_otto_prequery_loss.py", "src/openjev/research/otto_score_forecast_data.py",
    ...
    "scripts/train_otto_separate_prior.py", "tests/test_train_otto_separate_prior.py",
    "scripts/qualify_otto_separate_prior_capacity.py", "tests/test_qualify_otto_separate_prior_capacity.py",
    "scripts/audit_otto_separate_prior.py", "tests/test_audit_otto_separate_prior.py"}
...
def authenticate_inputs(inputs):
    ...
    expected_seeds = [first + case for split, regimes in FIRST.items()
                      for first in regimes.values() for case in range(CASES[split])] + [285000001, 285000002, 285000003] + list(range(286000001, 286000055))
...
class Run:
    ...
    def annotate_train(self, identity, history, start_row, held):
        ...
        selection = self.sample_windows(len(history), 286000001 + identity["episode_index"])
