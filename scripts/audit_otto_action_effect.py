...
VERSION = 'otto-action-effect-saved-audit-v1'
...
def mechanism_thresholds():
    cfg = c.CONFIG
    return {'long_effect_relative_gain': cfg['long_effect_improvement'],
            'long_gap_relative_gain': cfg['long_gap_improvement'],
            'long_log_relative_tolerance': cfg['long_log_tolerance'],
...
def mechanism_gate(reports, sensitivity, *, candidate, controls, fit_seeds, regimes, thresholds):
    """Independent saved-scalar aggregation/comparisons, never producer gate calls."""
    from openjev.research import otto_belief_distillation_metrics as legacy

    keys = {'long_effect_relative_gain', 'long_gap_relative_gain', 'long_log_relative_tolerance',
            'normal_log_relative_tolerance', 'normal_gap_relative_tolerance', 'minimum_supported_cases'}
    c.require(type(thresholds) is dict and set(thresholds) == keys, 'exact independent mechanism thresholds')
    for key in keys - {'minimum_supported_cases'}:
        c.require(type(thresholds[key]) in (int, float) and math.isfinite(thresholds[key])
                  and 0 <= thresholds[key] <= 1, 'finite prospective relative threshold')
    c.require(type(thresholds['minimum_supported_cases']) is int and thresholds['minimum_supported_cases'] > 0,
              'positive independent support threshold')
    legacy.evaluate_reports(reports, candidate=candidate, controls=controls, fit_seeds=fit_seeds, regimes=regimes,
    ...
    c.require(type(sensitivity) is list and len(sensitivity) == len(wanted)
              and all(type(row) is dict and set(row) == {'family', 'fit_seed', 'report'} for row in sensitivity),
              'complete independent sensitivity roster')
    indexed = {(row['family'], row['fit_seed']): row['report'] for row in sensitivity}
    c.require(set(indexed) == wanted, 'exact unique independent effect identities')
    by = {(r['family'], r['fit_seed'], r['condition']): r for r in reports}
    manifest = reports[0]['identity_manifest']
    c.require(all(row['identity_manifest'] == manifest and row['target_sha256'] == reports[0]['target_sha256']
                  for row in reports), 'same factual cohort across conditions')
    effects, expected_signal = {}, {}
    for family in families:
        for seed in fit_seeds:
            source = indexed[family, seed]
            c.require(source['version'] == 'otto-belief-action-sensitivity-v1' and source['condition'] == 'gap'
                      and source['horizons'] == list(range(1, 9)) and source['blocks'] == len(manifest)
                      and source['case_filtering'] is False and source['model_effect_included'] is True
                      and source['actions_verified_opposite'] is True and source['action_mapping'] == 'a XOR1'
                      and set(source['per_horizon']) == set(map(str, range(1, 9))), 'complete fixed opposite effect evidence')
            for regime in regimes:
                blocks = [r for r in manifest if r['regime'] == regime]
                ids = sorted({r['case_id'] for r in blocks})
                cases = []
                for identity in ids:
                    errors, signals = [], []
                    count = sum(row['case_id'] == identity for row in blocks)
                    for h in range(1, 9):
                        group = source['per_horizon'][str(h)]
                        c.require(set(group['by_regime']) == set(regimes), 'complete effect regime roster')
                        leaf = group['by_regime'][regime]
                        c.require(leaf['horizons'] == [h] and leaf['declared_cases'] == leaf['supported_cases'] == len(ids)
                                  and leaf['blocks'] == leaf['available_rows'] == leaf['scored_rows'] == len(blocks)
                                  and leaf['unsupported_case_ids'] == [] and len(leaf['cases']) == len(ids),
                                  'all independent case and row denominators retained')
                        rows = {row['case_id']: row for row in leaf['cases']}
                        c.require(set(rows) == set(ids), 'exact effect originating cases')
                        value = rows[identity]
                        c.require(value['blocks'] == value['rows'] == count, 'repeated block denominator')
                        for name in ('jensen_shannon', 'total_variation', 'oracle_signal', 'model_effect_error'):
                            number = value[name]
                            c.require(type(number) in (int, float) and math.isfinite(number) and number >= 0,
                                      'finite nonnegative independently read effect')
                            average = math.fsum(float(row[name]) / len(ids) for row in rows.values())
                            c.require(abs(average - leaf['case_weighted_' + name]) <= 1e-12 + 1e-12 * abs(average),
                                      'complete equal-case sensitivity aggregate')
                        signal = (value['jensen_shannon'], value['total_variation'], value['oracle_signal'])
                        label = regime, h, identity
                        if label not in expected_signal:
                            expected_signal[label] = signal
                        c.require(signal == expected_signal[label], 'all views share the same oracle effect')
                        if h >= 5:
                            errors.append(float(value['model_effect_error']))
                            signals.append(float(value['oracle_signal']))
                    cases.append({'case_id': identity, 'horizons': 4,
                                  'model_effect_error': math.fsum(v / 4 for v in errors),
                                  'oracle_signal': math.fsum(v / 4 for v in signals)})
                effects[family, seed, regime] = {'declared_cases': len(ids), 'blocks': len(blocks),
                    'rows': 4 * len(blocks), 'horizons': [5, 6, 7, 8], 'cases': cases,
                    'case_weighted_model_effect_error': math.fsum(row['model_effect_error'] / len(ids) for row in cases),
                    'case_weighted_oracle_signal': math.fsum(row['oracle_signal'] / len(ids) for row in cases)}
    cells = []
    for seed in fit_seeds:
        for regime in regimes:
            for control in controls:
                cg = by[candidate, seed, 'gap']['groups']['long']['by_regime'][regime]
                bg = by[control, seed, 'gap']['groups']['long']['by_regime'][regime]
                cn = by[candidate, seed, 'normal']['groups']['all']['by_regime'][regime]
                bn = by[control, seed, 'normal']['groups']['all']['by_regime'][regime]
                comparisons = [
                    ('long_effect_gain', effects[candidate, seed, regime]['case_weighted_model_effect_error'],
                     effects[control, seed, regime]['case_weighted_model_effect_error'], 'long_effect_relative_gain', 'gain'),
                    ('long_gap_gain', cg['case_weighted_decision_gap'], bg['case_weighted_decision_gap'], 'long_gap_relative_gain', 'gain'),
                    ('long_log_nonregression', cg['case_weighted_log_score'], bg['case_weighted_log_score'], 'long_log_relative_tolerance', 'bound'),
                    ('normal_log_nonregression', cn['case_weighted_log_score'], bn['case_weighted_log_score'], 'normal_log_relative_tolerance', 'bound'),
                    ('normal_gap_nonregression', cn['case_weighted_decision_gap'], bn['case_weighted_decision_gap'], 'normal_gap_relative_tolerance', 'bound')]
                conditions = []
                for name, left, right, key, relation in comparisons:
                    good = False
                    if left is not None and right is not None:
                        if relation == 'gain':
                            good = left < right and left <= right * (1 - thresholds[key])
                        else:
                            good = left <= right * (1 + thresholds[key])
                    conditions.append({'name': name, 'passed': good, 'candidate': left, 'control': right,
                                       'relative_threshold': thresholds[key]})
                support = {'long_gap': cg['supported_cases'], 'normal': cn['supported_cases'],
                           'long_effect': effects[candidate, seed, regime]['declared_cases']}
                conditions.append({'name': 'paired_support', 'passed': all(v >= thresholds['minimum_supported_cases'] for v in support.values()),
    ...
    return {'version': 'otto-action-effect-gate-v1', 'candidate': candidate, 'controls': list(controls),
        'fit_seeds': list(fit_seeds), 'regimes': list(regimes), 'thresholds': dict(thresholds), 'cells': cells,
...
def thresholds():
    cfg = c.CONFIG
    return {'long_log_relative_gain': cfg['legacy_long_logscore_improvement'],
        'long_gap_relative_gain': cfg['long_gap_improvement'],
        'normal_log_relative_tolerance': cfg['normal_relative_tolerance'],
        'normal_gap_relative_tolerance': cfg['normal_relative_tolerance'],
        'minimum_supported_cases': cfg['minimum_supported_cases']}
...
def verify_training(np, directory, receipt, training_reference, summary, *, check=lambda: None):
    ...
    normalization_fields = {'cost_scale', 'rms_squared_before_floor', 'variance_floor', 'source',
                            'teacher_units_divisor', 'dev_decodes', 'effect_variance', 'effect_variance_before_floor',
                            'effect_variance_floor', 'effect_source'}
    ...
    effect = normalization['effect_variance']
    effect_before = normalization['effect_variance_before_floor']
    c.require(type(effect) is float and math.isfinite(effect) and effect > 0
              and type(effect_before) is float and math.isfinite(effect_before) and effect_before >= 0
              and normalization['effect_variance_floor'] == c.CONFIG['effect_variance_floor']
              and effect == max(effect_before, c.CONFIG['effect_variance_floor'])
              and normalization['effect_source'] == 'TRAIN only, mean squared signed oracle effect, equal cases and all four horizons',
              'fixed TRAIN-only effect variance matches recorded mean and floor')
    n = training_reference['cases']
    cfg = c.CONFIG
    c.require(type(n) is int and n == cfg['train_cases'] == cfg['min_train'] and training_reference['prior_dev_decodes'] == 0,
    ...
    for row in fits:
        ...
        c.require(row['model_kind'] == MODEL_KINDS[row['family']]
                  and row['soft_targets'] is True
                  and type(row['training_rollouts_per_batch']) is int and row['training_rollouts_per_batch'] == 3
                  and type(row['effect_weight']) is float
                  and row['effect_weight'] == (cfg['effect_weight'] if row['family'] == cfg['candidate'] else 0.)
                  and row['blind_input_keys'] == ['prefix', 'prefix_lengths', 'actions']
                  and row['privileged_targets_only'] == ['raw_costs', 'gap_oracle', 'normal_oracle', 'opposite_oracle'],
    ...
    calls = {'optimizer_steps': 12 * per_fit_updates, 'fit_count': 12, 'dev_array_decodes': 1,
             'train_array_decodes': 1, 'training_rollouts': 36 * per_fit_updates}
    ...
    return {'fits_checked': 12, 'normalization_records_checked': 1,
            ...
            'training_rollouts_declared': calls['training_rollouts']}
...
def admit(run):
    ...
    c.require({'dev.npz', 'sensor-laws.npz', 'summary.json', 'training-reference.json'} <= set(receipts['collect']['files'])
...
def run_audit(run):
    ...
    c.require(training_reference == c.read(collection / 'training-reference.json') == fit_summary['training_reference'],
              'registered unchanged reused TRAIN provenance')
    counts.update(verify_training(np, fitted, receipts['fit'], training_reference, fit_summary, check=run.check))
    c.require(fit_summary['collection_receipt'] == c.desc(collection / 'receipt.json')
              and fit_summary['train_data'] == {k: training_reference['train'][k] for k in ('sha256', 'bytes')}
    ...
    c.require(set(saved) == {'reports', 'gate', 'legacy_gate', 'sensitivity'} and len(saved['reports']) == 24, 'complete saved report and gate payload')
    ...
    for index, (family, seed) in enumerate((f, s) for f in FAMILIES for s in seeds):
        ...
        run.receipt['pending'] = None
    legacy_gate = metrics.evaluate_reports(reports, candidate=c.CONFIG['candidate'], controls=c.CONFIG['controls'],
        fit_seeds=seeds, regimes=('lambda3', 'lambda4'), thresholds=thresholds())
    close_equal(saved['legacy_gate'], legacy_gate, 'separately retained legacy gate')
    gate = mechanism_gate(reports, sensitivity, candidate=c.CONFIG['candidate'], controls=c.CONFIG['controls'],
        fit_seeds=seeds, regimes=('lambda3', 'lambda4'), thresholds=mechanism_thresholds())
    close_equal(saved['gate'], gate, 'new prospective mechanism gate')
    ...
    for phase, directory in (('collect', collection), ('fit', fitted)):
        for name in ('dev.npz', 'sensor-laws.npz', 'summary.json', 'training-reference.json') if phase == 'collect' else (
    ...
    result = {'version': VERSION, 'agreement': True, 'technical_complete': False,
        ...
        'sensitivity': sensitivity, 'oracle_reconstruction': oracle_check, 'legacy_gate': legacy_gate,
        'training_reference': training_reference,
        'normalization': {'record': c.read(fitted / 'normalization.json'), 'train_variance_recomputed': False,
                          'scale_from_recorded_variance_checked': True, 'effect_floor_from_recorded_mean_checked': True,
                          'fit_metadata_scale_joins_checked': 12},
        ...
        'limitations': ['No model, fit or simulator is replayed; public Bayesian targets and sensor CDF laws are independently reconstructed.',
                        ...
                        'This new gate cannot change previous results or admit old TEST, confirmation or new execution.']}
    run.check()
