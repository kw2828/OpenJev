"""Render sealed pose-transport measurements without running any predictors."""
...
NAMES = {"transport": "Transported memory", "body": "Body GRU", "world": "World GRU",
         "history16": "History16 transport", "ridge16": "Torque ridge16", "hold": "Hold last",
...
def render(completion, out):
    ...
    colors = ["#086c94", "#c16b19", "#91633e", "#bd8b49"] + ["#477d66"] * 6
    ...
    for col, panel in enumerate(("test_sin", "test_zigzag")):
        for row, metric in enumerate(("position_rmse_m", "rotation_rmse_rad")):
            ...
            ax.set_xscale("log")
            ticks = { (0, 0): [.04, .06, .1, .2, .4, .6],
                      (0, 1): [.08, .1, .2, .4, .6],
                      (1, 0): [.03, .05, .1, .15],
                      (1, 1): [.1, .14, .18, .24]}[(row, col)]
            ax.xaxis.set_major_locator(FixedLocator(ticks))
            ax.xaxis.set_major_formatter(FixedFormatter([f"{x:g}" for x in ticks]))
    ...
    fig.suptitle("OpenJev: geometry and recurrent memory", fontsize=20, weight="bold")
