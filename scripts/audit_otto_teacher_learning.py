"""Bounded independent saved-record audit; no scientific component imports.
...
VERSION = 'otto-teacher-learning-saved-audit-v1'
SELF = 'scripts/audit_otto_teacher_learning.py'
    ...
    def targets(self):
        ...
        for kind, costs in (('analytic', analytic), ('continuation', self.label_means)):
            ...
            self.require(original.dtype == np.float64 and original.shape == (558, 4)
                         ...
                         and np.allclose(original, centered, rtol=1e-11, atol=1e-10), 'independent eligible centering')
            ...
            self.require(cast.dtype == np.float32 and cast.shape == (558, 4) and np.isfinite(cast).all()
                         and np.array_equal(cast, (original / self.data_info['scales'][kind]).astype(np.float32)),
