...
VERSION = "otto-sparse-query-render-v2"
...
class Render:
    ...
    def figure(self, summary, metrics):
        ...
        fig, axes = plt.subplots(1, 3, figsize=(16, 6.4), sharex=True, sharey=True)
        fig.subplots_adjust(left=.065, right=.985, bottom=.28, top=.78, wspace=.13)
        ...
        fig.text(.5, .15, "Lower and left is better. Every arm retained; common axes. Star: sole primary candidate, period2.\n"
        ...
        fig.text(.5, .075, f"Physical study {self.times['study_parent_seconds']:.2f}s; saved audit {self.times['audit_parent_seconds']:.2f}s. "
                 "Fixed schedules test a control opportunity, not learned recurrence or architecture efficacy.", ha="center", fontsize=9)
        fig.text(.5, .02, "Audit correction: original reader failed on a cost-field type check; separately frozen V2 reader passed.\n"
