"""Launch one frozen-dynamics position observer development study with an original terminal receipt."""
...
ENGINEERING = ROOT / 'output/robot-position-observer-engineering-v1'
REGISTRATION = ROOT / 'research/robot-position-observer-registration.json'
OUTPUT = ROOT / 'output/robot-position-observer-study-v1'
COMMAND = ['.venv/bin/python', '-u', 'scripts/robot_position_observer_study.py',
           '--registration', 'research/robot-position-observer-registration.json',
           '--output', 'output/robot-position-observer-study-v1']
...
def main():
    ...
    for name in (*plan['sources'], str(REGISTRATION.relative_to(ROOT)),
                 'scripts/launch_robot_position_observer_study.py'):
    ...
    if plan['config']['wall_cap_seconds'] != 7200.:
        raise ValueError('Fixed 7200-second native and 7260-second external caps required')
    if OUTPUT.exists():
    ...
    record = {'command': COMMAND, 'prefit_commit': commit,
              ...
              'scope': 'Six position-gain fits with frozen dynamics and seven no-update FIT probes, complete retained controls; four exposed development recordings; no restart or official TEST access'}
