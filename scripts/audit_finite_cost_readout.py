"""Independent saved-output audit for the three-arm cost-readout finite-world diagnostic.
...
VERSION = 'finite-cost-readout-audit-v1'
NAMESPACE = 424260924
...
def validate_head_metadata(row):
    ...
    scale = 1. if arm == 'fixed_exact' else .9
...
def readout_comparisons(rows, fits):
    ...
    contrasts = []
    for control in ('fixed_exact', 'fixed_softened'):
        for seed in SEEDS:
            for horizon in HORIZONS:
    ...
    return {'paired': contrasts, 'means': means, 'matrices': matrices,
            'interpretation': 'Learned versus softened isolates trainability from initialization; learned versus exact retains the privileged anchor. Neither identifies latent alignment.'}
...
def validate_metadata(folder, summary, *, check=lambda: None):
    ...
    require(summary['version'] == 'finite-cost-readout-v1', 'cost-readout producer version')
    ...
    require([(row['arm'], row['seed']) for row in fits] == expected_pairs, 'nine completed cost-readout fits')
...
def audit(folder, *, check=lambda: None):
    ...
    return {'version': VERSION, 'agreement': True, 'technical_complete': False,
            ...
            'limitations': [
                ...
                'This synthetic readout diagnostic establishes neither convergence, native transfer nor architectural novelty.']}
