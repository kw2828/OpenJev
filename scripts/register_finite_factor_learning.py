...
from finite_factor_worker import ROOT, closed_producer, descriptor, publish, require, runtime

SOURCES = (
    'scripts/register_finite_factor_learning.py',
    'scripts/finite_factor_worker.py',
    'scripts/run_finite_factor_learning.py',
    'scripts/audit_finite_factor_learning.py',
    ...
    'tests/test_finite_factor_models.py',
    'tests/test_finite_factor_learning.py',
    'tests/test_finite_factor_worker.py',
    'tests/test_finite_factor_runner.py',
    'research/finite-factor-learning-protocol.md',
    'pyproject.toml',
    'uv.lock',
)
TESTS = [name for name in SOURCES if name.startswith('tests/')]
LINT = [name for name in SOURCES if name.endswith('.py')]
CONFIG = {'seed_namespace': 421260924, 'train_attempts': 512, 'dev_attempts': 128,
          'epochs': 480, 'batch_size': 64, 'learning_rate': .003,
          'fit_seeds': [421261001, 421261002, 421261003],
...
def main():
    ...
    folder = ROOT / 'output/finite-factor-learning-v1'
    ...
    plan = {'version': 'finite-factor-learning-v1', 'mode': args.mode,
