...
VERSION = "dialogue-observation-report-v1"
STUDY = "dialogue-observation-scientific-v1"
...
DEV_ENDPOINTS = 62329
WATCHDOG = "output/dialogue-observation-learning-v1/study-watchdog-01.py"
PROTOCOL = "research/dialogue-observation-learning-protocol.md"
NEW_SOURCES = {"scripts/study_dialogue_observation.py", "tests/test_study_dialogue_observation.py",
               ...
               "research/dialogue-observation-learning-scoring.md", PROTOCOL, WATCHDOG}
...
def authenticate_lineage(run, done, freeze_path, freeze_pin):
    ...
    require(plan["version"] == STUDY and plan["config"] == CONFIG and plan["fit_order"] == FIT_ORDER
    ...
    require(plan["source_sha256"] == done["source_sha256"] == frozen["source_sha256"]
            and len(plan["source_sha256"]) == 53, "Complete source identity")
    ...
    require(set(spec) == {"version", "prepared_completed_sha256", "prepared_plan_sha256", "cost_completed", "cost_audit",
                          "protocol", "limits", "freeze_limits"}
            and spec["version"] == "dialogue-observation-allocation-v1", "External allocation schema")
...
def authenticate_supervision(args, plan, done):
    ...
    if tail and tail[0] == "-u":
        tail = tail[1:]
    require(len(tail) == 8 and (ROOT / tail[0]).resolve() == ROOT / "scripts/study_dialogue_observation.py"
            and tail[1] == "train", "Supervised scientific training command")
    flags = dict(zip(tail[2::2], tail[3::2], strict=True))
    require(set(flags) == {"--plan", "--plan-sha256", "--out"}
            and Path(flags["--plan"]).resolve() == args.plan.resolve()
            and flags["--plan-sha256"] == args.plan_sha256
            and Path(flags["--out"]).resolve() == Path(read(args.run / "started.json")["request"]["out"]).resolve(),
    ...
    require(finite(launch["started_unix"], positive=True) and finite(end["finished_unix"], positive=True)
            and finite(end["wall_seconds"], positive=True)
            and 0 <= end["finished_unix"] - launch["started_unix"] <= end["wall_seconds"]
            and done["wall_seconds"] <= end["wall_seconds"] <= launch["cap_seconds"], "Supervisor elapsed bounds")
...
def report_text(summary):
    ...
    cost = summary["costs"]
    text.extend(["", f"Whole execution: {cost['execution_wall_seconds']:.3f} s. Training: {cost['training_seconds']:.3f} s; final evaluation: {cost['evaluation_seconds']:.3f} s; checkpoint writing: {cost['checkpoint_seconds']:.3f} s. These phases are nested within execution, not added to it.", "",
...
def execute(args):
    ...
    sources = {}
    def check():
        require(time.perf_counter() - started <= CAPS["wall_seconds"] and rss() <= CAPS["rss_bytes"], "Report wall/RSS cap")
    ...
    try:
        ...
        sources = {name: sha(ROOT / name) for name in ("scripts/report_dialogue_observation.py", "tests/test_report_dialogue_observation.py",
                                                     "src/openjev/research/dialogue_observation_metrics.py", "tests/test_dialogue_observation_metrics.py")}
        ...
        costs = {"execution_wall_seconds": done["wall_seconds"], "process_lifetime_peak_rss_bytes": done["peak_rss_bytes"],
                 ...
                 "scope": "Fit/training/evaluation/checkpoint times are nested; RSS is process lifetime highwater and sampled MPS is not a transient peak or additive to RSS"}
