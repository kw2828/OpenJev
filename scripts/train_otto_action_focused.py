"""Matched auxiliary-MSE versus SPO+-augmented fitting; no teacher or environment calls.
...
SELF = "scripts/train_otto_action_focused.py"
TEST = "tests/test_train_otto_action_focused.py"
...
METRICS = "src/openjev/research/otto_action_focused_metrics.py"
WINDOWS = "src/openjev/research/otto_score_forecast_data.py"
SAMPLING = "src/openjev/research/otto_sampled_forecast_data.py"
PROTOCOL = "research/otto-action-focused-protocol.md"
AUDIT = "scripts/audit_otto_action_focused.py"
AUDIT_TEST = "tests/test_audit_otto_action_focused.py"
...
VERSION = "otto-action-focused-training-v1"
COLLECTION_VERSION = "otto-action-focused-collection-v1"
KINDS = ("innovation_aux", "innovation_spo", "gru_aux", "gru_spo")
CELLS = {"innovation_aux": ("innovation", "shared", "aux"),
         "innovation_spo": ("innovation", "shared", "spo"),
         "gru_aux": ("innovation_gru", "shared", "aux"),
         "gru_spo": ("innovation_gru", "shared", "spo")}
...
CONFIG = {"families": list(KINDS), "fit_seeds": list(SEEDS), "epochs": 80,
          ...
          "prior_coefficient": 1., "spo_coefficient": 1.,
...
COMPONENTS = {SELF, TEST, MODELS, DATA, LOSS, BASE_LOSS, SPO_LOSS, METRICS, AUDIT, AUDIT_TEST,
    ...
    "tests/test_otto_action_focused_loss.py", "tests/test_otto_action_focused_metrics.py"}
...
def check_paired_initialization(rows):
    """The two objectives use exactly the same shared-head tensors and seed."""
    require(len(rows) == 4 and {r["family"] for r in rows} == set(KINDS)
            and len({r["seed"] for r in rows}) == 1, "complete single-seed initialization panel")
    for architecture in ("innovation", "innovation_gru"):
        paired = [r for r in rows if r["architecture"] == architecture]
        require(len(paired) == 2 and {r["objective"] for r in paired} == {"aux", "spo"}
                and all(r["readout"] == "shared" for r in paired)
                and paired[0]["initial_tensors"] == paired[1]["initial_tensors"]
                and paired[0]["initial_sha256"] == paired[1]["initial_sha256"]
                and paired[0]["initial_core_sha256"] == paired[1]["initial_core_sha256"],
                "same architecture has exact paired initialization across objectives")
        require(all(not k.startswith("prior_output.") for r in paired for k in r["initial_tensors"]),
                "shared readout only")
...
def batch_update(torch, models, data_module, losses, model, optimizer, data, indices, *, objective, check, stage):
    ...
    for start in range(0, max(lengths), 32):
        ...
        nonzero = bool((weight > 0).any()) or bool(prior_mask.any())
        with torch.set_grad_enabled(nonzero):
            ...
            if nonzero:
                ...
                spo_total += float(terms["spo"].detach())
                if objective == "spo":
                    spo_calls += 1
                    spo_weighted_rows += int((weight > 0).sum())
...
class Run:
    ...
    def admit(self):
        ...
        self.clock = load(ROOT / CLOCK, "_action_focused_training_clock").SuspendClock()
    ...
    def fit(self, kind, seed, history):
        ...
        for epoch in range(80):
            ...
            for start in range(0, count, 6):
                ...
                spo_calls += result["spo_loss_calls"]; spo_weighted_rows += result["spo_weighted_rows"]
            if (epoch + 1) % 20 == 0:
        ...
        final_loss, nonquery_loss, prior_loss, spo_loss = (terms[key] for key in ("total", "nonquery", "prior", "spo"))
        ...
        fit = {"family": kind, "architecture": architecture, "readout": readout, "objective": objective,
            ...
            "spo_loss_calls": spo_calls, "spo_weighted_rows": spo_weighted_rows,
            "final_train_loss": final_loss, "final_nonquery_loss": nonquery_loss,
            "final_prior_loss": prior_loss,
            "final_prior_loss_scope": "trained objective component in every cell",
            "final_spo_loss": spo_loss,
            "final_spo_loss_scope": "post-fit diagnostic for every cell; trained only by spo cells",
            "final_objective_spo_loss": spo_loss if objective == "spo" else 0.,
            "final_rescore_spo_loss_calls": 1,
            "final_objective_prior_loss": prior_loss,
    ...
    def body(self):
        ...
        self.models = load(ROOT / MODELS, "_action_focused_training_models")
        self.data = load(ROOT / DATA, "_action_focused_training_data")
        self.losses = load(ROOT / LOSS, "_action_focused_training_loss")
        self.windows = load(ROOT / WINDOWS, "_action_focused_census_windows")
        self.metrics = load(ROOT / METRICS, "_action_focused_metrics")
        require(self.models.KINDS == ("innovation", "innovation_gru")
        ...
        summary = {"version": VERSION, "models": records, "hold": hold,
            ...
            "required_conditions": 41, "gates": self.metrics.gate_decisions(conditions),
            "technical_complete_pending_saved_audit": True,
            "scientific_conditions": 40, "scientific_passed": sum(c["passes"] for c in conditions[1:]),
    ...
    def execute(self):
        ...
        def interrupted(_signum, _frame):
            raise InterruptedError("original action-focused supervisor stopped worker")
        ...
        try:
            ...
            require({p.name for p in self.out.iterdir()} == PAYLOADS, "exact 49 payload closure")
