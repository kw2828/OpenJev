"""One frozen development comparison of adaptation-aware pose prediction."""
...
def make_model(variant, scale):
    return (PoseTransport("body", scale) if variant == "gru" else
            PoseAdaptation("public" if variant == "public" else "learned", scale))


def forecast(model, context_p, context_r, past_actions, future_actions, variant):
    """No future-pose argument exists. Past includes exactly31 action blocks."""
    ...
    if past_actions.shape != (len(context_p), 31, 40) or future_actions.shape != (len(context_p), 25, 40):
        raise ValueError("31 past and25 future action blocks required")
    if variant == "gru":
        state = model.assimilate(model.initial(context_p[:, 0], context_r[:, 0]),
                                 context_p[:, 0], context_r[:, 0])
        for t in range(31):
            state = model.advance(state, past_actions[:, t])
            state = model.assimilate(state, context_p[:, t + 1], context_r[:, t + 1])
        # Assimilation has already set the actual30->31 motion: preserve CV1.
        positions, rotations = [], []
        for t in range(25):
            state = model.advance(state, future_actions[:, t])
            positions.append(state["p"])
            rotations.append(state["R"])
        return torch.stack(positions, 1), torch.stack(rotations, 1)
    if variant not in VARIANTS:
        raise ValueError("unknown forecast variant")
    state = model.fit_context(context_p, context_r, past_actions,
                              adapt=variant in ("meta", "static_adapt", "public"))
    return model.rollout(state, future_actions)


def predict(model, p, rotation, actions, variant):
    return forecast(model, p[:, :CONTEXT], rotation[:, :CONTEXT], actions[:, :31], actions[:, 31:], variant)


def sources():
    return [Path(p) for p in (
        "scripts/train_pose_adaptation.py", "scripts/audit_pose_adaptation.py", "scripts/audit_pose_transport.py",
...
def freeze(data, out):
    ...
    protocol = {"study": "pose-adaptation-v1", "scope": "exposed-data development, not confirmation",
                ...
                "variants": VARIANTS, "train_variants": TRAIN_VARIANTS, "references": REFERENCES, "panels": PANELS,
                ...
                "gate": "meta at least10% better on each physical endpoint against every learned and classical control on both panels; all3 paired fit endpoints nonworse; at least8/10 parents nonworse for each endpoint/control/panel; strictly lower MSE after each leave-one-parent-out; median full-window latency<=1.5x gru. Failure stays failed.",
                "gate_grouping": "17 grouped requirements retain all661 elementary comparisons",
                "prior_failure": "residual-dynamics-v1 failed36/49 and pose-transport-v1 failed345/541 remain unchanged",
    ...
    try:
        ...
        for seed in SEEDS:
            ...
            for variant in TRAIN_VARIANTS:
                ...
                model = make_model(variant, scale)
                stem = f"{variant}-{seed}"
                ...
                for epoch, order in enumerate(orders):
                    ...
                    for s in range(0, len(p), BATCH):
                        idx = order[s:s + BATCH]
                        pp, rr = predict(model, p[idx], r[idx], a[idx], variant)
        ...
        for panel in PANELS:
            ...
            for variant in (*VARIANTS, *REFERENCES):
                for seed in SEEDS if variant in VARIANTS else (None,):
                    backbone = {"meta_prior": "meta", "static_adapt": "static"}.get(variant, variant)
                    stem = f"{backbone}-{seed}" if seed is not None else variant
                    if variant in VARIANTS:
                        fit = next(x for x in fits if x["variant"] == backbone and x["seed"] == seed)
                        if sha(dest / (stem + ".pt")) != fit["checkpoint_sha256"]:
                            raise ValueError("checkpoint changed: " + stem)
                        model = make_model(backbone, scale)
                    ...
                    rows.append(row)
                    prefix = panel + "-" + (f"{variant}-{seed}" if seed is not None else variant)
