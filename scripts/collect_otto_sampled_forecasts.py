...
until all of its journals have been flushed/fsynced. TRAIN scores deployed states and selected windows; VALID is a census. No fitting.
"""
...
VERSION = "otto-sampled-forecast-collection-v1"
SELF, TEST = "scripts/collect_otto_sampled_forecasts.py", "tests/test_collect_otto_sampled_forecasts.py"
...
FIRST = {"train": {"lambda3": 23100001, "lambda4": 23200001},
         "valid": {"lambda3": 23300001, "lambda4": 23400001}}
...
CALL_CAPS = {"tensorflow_construction": 1, "tensorflow_build": 1, "tensorflow_load": 1,
    "tensorflow_value": 129006, "native_reset": EPISODES, "native_step": EPISODES * HORIZON,
    "actor_construction": EPISODES, "backend_binding": EPISODES + 54, "analytic_score": EPISODES * HORIZON,
    "feature_build": EPISODES * HORIZON, "teacher_score": 129006,
...
CONFIGURATION = {"arms": list(ARMS), "first_seeds": FIRST, "cases": CASES, "horizon": HORIZON,
    ...
    "selection_seed_first": 23600001, "validation_annotation": "complete census",
...
def authenticate_inputs(inputs):
    ...
    expected_seeds = [first + case for split, regimes in FIRST.items()
                      for first in regimes.values() for case in range(CASES[split])] + [235001, 235002, 235003] + list(range(23600001, 23600055))
...
class Run:
    def __init__(self, args):
        ...
        self.receipt = {"version": VERSION, "status": "started", "training_updates": 0,
            "scope": "Full fixed paths; sampled TRAIN annotation and census VALID. No fitting."}
    ...
    def setup(self):
        ...
        write(self.out / "deployment.json", {"original_setup_wall_seconds": self.original_setup_wall,
            ...
            "scope": "All full paths; TRAIN samples at most eight windows after termination, VALID annotates every state. Deployed and deferred scores are distinct. No deployment-cost claim."})
    ...
    def annotate_train(self, identity, history, start_row, held):
        """Uniform selection follows the full path; annotations cannot affect actions."""
        np, runtime = self.runtime.np, self.runtime
        selection = self.sample_windows(len(history), 23600001 + identity["episode_index"])
        selected = set(selected_steps(selection))
        ...
        for step, entry in enumerate(history):
            ...
            if step in selected and not self.label_mask[row_index]:
                if teacher is None:
        ...
        require(all(self.label_mask[start_row + step] for step in selected), "all selected window labels present")
    ...
    def save_stage(self, stage):
        ...
        if stage == "train":
            ...
            for row, selection in zip(rows, self.selections, strict=True):
                require(all(mask[row["start_row"] + step] for step in selected_steps(selection)),
                        "all selected TRAIN labels exported")
