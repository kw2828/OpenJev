...
from finite_shared_filter_worker import ROOT, closed_producer, descriptor, publish, require, runtime

SOURCES = (
    'scripts/register_finite_shared_filter.py',
    'scripts/finite_shared_filter_worker.py',
    'scripts/run_finite_shared_filter.py',
    'scripts/audit_finite_shared_filter.py',
    ...
    'tests/test_finite_shared_filter_models.py',
    'tests/test_finite_shared_filter_learning.py',
    'tests/test_finite_shared_filter_worker.py',
    'tests/test_finite_shared_filter_runner.py',
    'research/finite-shared-filter-protocol.md',
    'pyproject.toml',
    'uv.lock',
)
TESTS = [name for name in SOURCES if name.startswith('tests/')]
LINT = [name for name in SOURCES if name.endswith('.py')]
CONFIG = {'seed_namespace': 422260924, 'train_attempts': 512, 'dev_attempts': 128,
          'epochs': 480, 'batch_size': 64, 'learning_rate': .003,
          'fit_seeds': [422261001, 422261002, 422261003],
...
def main():
    ...
    folder = ROOT / 'output/finite-shared-filter-v1'
    ...
    plan = {'version': 'finite-shared-filter-v1', 'mode': args.mode,
