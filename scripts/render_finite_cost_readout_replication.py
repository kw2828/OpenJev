...
VERSION = 'finite-cost-readout-replication-report-v1'
STUDY_NAME = 'finite-cost-readout-replication-v1'
ARMS = ('fixed_softened', 'learned_readout')
SEEDS = (425261001, 425261002, 425261003)
CONFIG = {'seed_namespace': 425260924, 'train_attempts': 512, 'dev_attempts': 128,
...
LABELS = ('Fixed softened C', 'Learned bounded C')
...
def closed_phase(plan, plan_path, phase):
    ...
    require(len(command) == 2 + 2 * len(expected_flags) and command[0] == plan['runtime']['executable']
            and (Path(terminal['cwd']) / command[1]).resolve() == ROOT / 'scripts/finite_cost_readout_replication_worker.py'
...
def authenticate(studyfolder):
    require(studyfolder == ROOT / 'output' / STUDY_NAME, 'exact replication study directory')
    plan_path = regular(studyfolder / 'registration-01.json')
    plan = read(plan_path)
    require(plan['version'] == STUDY_NAME and plan['root'] == str(ROOT)
            and plan['mode'] == 'study' and plan['config'] == CONFIG, 'registered fresh-seed replication')
    ...
    require(saved['version'] == 'finite-cost-readout-replication-v1' and report['version'] == 'finite-cost-readout-replication-audit-v1'
            ...
            and audit['receipt']['result'] == report['gates']
            and audit['receipt']['continuation'] == report['continuation'],
            'independent audit and separate continuation under explicit external closure boundary')
    ...
    require(len(expected_files) == 29 and set(saved['files']) == expected_files
    ...
    require(fit['receipt']['result'] == {'fits': 6, 'rows': 24}
            and saved['counts']['readout_snapshot_evaluations'] == 12
            and saved['counts']['readout_snapshot_softmax_evaluations'] == 6
            and report['counts'] == {'array_decodes': 15, 'checkpoint_decodes': 0, 'model_calls': 0,
                'optimizer_calls': 0, 'world_or_generator_calls': 0, 'native_calls': 0},
            'exact replication fit, snapshot and independent audit work')
    finite_tree(saved)
    finite_tree(report)
    return plan_path, plan, {'qualify': qualify, 'fit': fit, 'audit': audit}, saved, report


def continuation_record(report, keyed):
    """Validate saved continuation arithmetic without changing any criterion."""
    learned = report['gates']['learned_readout']
    ...
    for seed in SEEDS:
        for horizon in (4, 8):
            ...
            require(candidate >= 0 and reference >= 0, 'nonnegative audited paired regret')
    ...
    expected = {'passed': passed, 'status': 'REPLICATION_CONTINUATION_' + ('PASS' if passed else 'FAIL'),
    ...
    require(report['continuation'] == expected, 'all eight original continuation conditions and six paired contrasts')
...
def extract(plan, phases, saved, report):
    ...
    continuation = continuation_record(report, keyed)
    ...
    paired = [{'candidate': 'learned_readout', 'control': control, 'seed': seed,
              ...
              for control in ('fixed_softened',) for h in (4, 8) for seed in SEEDS]
    ...
    return {'version': VERSION, 'gates': report['gates'], 'continuation': continuation,
            'rows': rows, 'baseline_rows': baselines,
            'three_seed_means_h4_h8': means, 'data_cases': report['data_cases'],
            'paired_differences_h4_h8': paired,
            'paired_difference_scope': 'Same-seed learned_readout minus the fixed-softened control on the same cases. The six strict regret comparisons enter the separate prospective continuation rule; other contrasts are descriptive. No significance or confidence interval claim.',
...
def figure(numbers, path):
    ...
    colors, markers = ('#D55E00', '#009E73'), ('o', 's', '^')
    ...
    for ax, title, lookup, key in ((axes[1, 0], 'All-attempt DEV prefix NLL', prefix_rows, 'mean_nll'),
        ...
        ax.set_xticks((0, 1), ('Fixed softened', 'Learned'))
        ax.set_xlim(-.4, 1.4)
        ax.set_title(title)
    ...
    fig.suptitle('Synthetic cost-readout replication: fresh cases and fit seeds', fontsize=14, fontweight='bold')
    continuation = numbers['continuation']
    fig.text(.5, .935, f"{continuation['status']}: {sum(continuation['conditions'].values())}/8 conditions; "
             f"learned BLIND_EXTRAPOLATION: {'PASS' if continuation['blind_extrapolation_passed'] else 'FAIL'}",
...
def document(numbers):
    continuation = numbers['continuation']
    lines = ['# Fresh-seed replication of learned cost readouts', '',
        f"**{continuation['status']} ({sum(continuation['conditions'].values())}/8 conditions).**", '',
        ('This prospective continuation rule is separate from the three absolute learning criteria below. A continuation '
         'pass permits consideration of a separately registered follow-up; it does not admit execution or establish that '
         'blind extrapolation passes. All earlier study outcomes remain unchanged.'), '',
        ('Two arms share the same learned reset emission, recurrent branch operators and coefficient-one prefix loss. '
         '`fixed_softened` uses 0.9C; `learned_readout` uses '
        ...
        ('The learned head adds 32 logits, or 24 identifiable head degrees of freedom. Learned versus softened '
         'isolates trainability from initialization. This replication does not repeat the original fixed-exact-C learned '
         'arm, so gains here do not independently repeat that stronger anchor comparison and might partly undo softening. '
         'Both arms start with world-aligned readouts.'), '',
        ...
        ('[Frozen protocol](../finite-cost-readout-replication-protocol.md). Original qualification, fit and audit closures, source '
        ...
        '| Condition | Outcome |', '|---|---|']
    for name, passed in continuation['conditions'].items():
        lines.append(f"| {name} | {'PASS' if passed else 'FAIL'} |")
    lines += ['', ('All eight conditions are required: learned short-horizon and observed-filtering criteria, plus strictly '
        'lower H4/H8 blind regret than softened for each of the three paired seeds. A tie fails. Means and prefix NLL '
        'cannot rescue a condition. These are prospective rules for this replication, not revisions to earlier gates.'), '',
    ...
    lines += ['', 'Means describe three separately fitted policies, not an ensemble or confidence interval.', '',
        ...
        ('Every difference is learned minus the named control on the same seed and cases. Negative favors learned '
         'for that metric. The six strict regret comparisons enter the separately declared continuation rule. Other '
         'differences are descriptive arithmetic, not significance tests or additional gates.'), '',
    ...
    lines += ['', ('All six fits use 480 epochs, batch size 64, Adam 0.003, global gradient clip 5 and the same forecast '
    ...
    lines += [f"| Total of successful phases | {numbers['total_phase_seconds']:.3f} |", '',
        ...
        '24 endpoint rows, six prefix rows, conditions, per-seed contrasts, readout snapshots and declared work.'), '',
        '## Interpretation limits', '',
        ('The original exact-C anchor arm is not repeated here. The zero-parameter exact correctness reference is '
        'different: it receives the true posterior and operators, and is not a learned comparison arm. Both fitted arms '
        'retain privileged world-aligned initialization. Real-valued learned costs are interior to the '
