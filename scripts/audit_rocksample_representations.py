"""Independent saved-transition accounting for the RockSample representation screen.
...
Bayesian inference or proof of planner optimality. Raw return and all thirteen gates
...
FACTORIZED = ("factorized_full", "factorized_recent128", "factorized_latest")
MAPS = tuple(range(13001, 13009))
RESETS = tuple(range(23001, 23005))
CLOCK_PIN = "cac9077db3c66f5ef43d9412b732f5b869c1004c4bac98589816780c120f6124"
RUNTIME_PIN = "b233dbd09714ae574626cdd2f58b3efde80c0127759af8f399147845ece4bcb2"
INHERITED_PLAN_PIN = "846520f9405509a39b29ee34e2a362874e8e4d958a8080935af77b72c71f7fcc"
ADDED_SOURCES = {
    "src/openjev/research/rocksample_public_belief.py",
    "src/openjev/research/rocksample_factorized_planning.py",
    "tests/test_rocksample_factorized_planning.py",
    "scripts/study_rocksample_representations.py",
    "tests/test_rocksample_representations.py",
    "research/rocksample-representation-protocol.md",
}
...
SCOPE = ("Independent complete public trace/macro/depletion joins, reward and discount arithmetic, "
         "recorded work, ESS and current-decision occupancy/reward-bound aggregation, construction-cost "
         "joins, eight-map means and thirteen gates. Hypothetical sensing-leaf coherence is not "
         "audited. Posterior values, "
...
def authenticate(args, budget):
    ...
    inherited_path = ROOT / "output/rocksample-policy-value-v1/plan-01.json"
    require(sha(inherited_path) == INHERITED_PLAN_PIN, "original frozen plan pin")
    inherited_sources = read(inherited_path)["sources"]
    require(len(inherited_sources) == 16 and len(plan["sources"]) == 22
            and set(plan["sources"]) == set(inherited_sources) | ADDED_SOURCES, "exact inherited and new source closure")
    require(all(plan["sources"][name] == pin for name, pin in inherited_sources.items()), "unchanged inherited sixteen sources")
    ...
    require(plan["cohort"] == {"map_seeds": list(MAPS), "reset_seeds": list(RESETS),
                              "arms": list(ARMS), "episodes": 256, "transition_seed_base": 440000,
                              "transition_seed_rule": "base + map_index*4 + reset_index; shared across arms for each case",
                              "arm_order": "rotate by case index modulo eight", "particle_bank_seed": 530001,
    ...
    if command[1:2] == ["-u"]:
        command.pop(1)
    expected = [str(ROOT / "tmp/pobax-runtime-02/bin/python"), str(ROOT / "scripts/study_rocksample_representations.py"),
...
def empty_diagnostics():
    return {"planning_decisions": 0, "maximum_cell_occupancy": None,
            "overfull_cell_decisions": 0, "overfull_cells_total": 0,
            "minimum_expected_reward": None, "maximum_expected_reward": None}


def add_diagnostics(total, recorded):
    """Aggregate recorded actual-state witnesses, without recomputing a posterior."""
    require(set(recorded) == {"kind", "ess", "max_weight", "alive_particles", "particles",
                              "locations_per_rock", "maximum_cell_occupancy", "overfull_cells",
                              "occupancy_excess_mass", "minimum_expected_reward", "maximum_expected_reward"},
            "factorized diagnostic fields")
    require(recorded["kind"] == "factorized_full_support" and recorded["ess"] is None
            and recorded["max_weight"] is None and recorded["alive_particles"] is None
            and recorded["particles"] == 0 and recorded["locations_per_rock"] == 110,
            "factorized diagnostic identity")
    occupancy = finite(recorded["maximum_cell_occupancy"], "maximum occupancy", 0)
    excess = finite(recorded["occupancy_excess_mass"], "occupancy excess", 0)
    overfull = integer(recorded["overfull_cells"], "overfull cells")
    low = finite(recorded["minimum_expected_reward"], "minimum expected sample reward")
    high = finite(recorded["maximum_expected_reward"], "maximum expected sample reward")
    require(0.1 - 1e-12 <= occupancy <= 11 + 1e-12 and overfull <= 110, "marginal occupancy limits")
    require((overfull > 0) == (occupancy > 1), "strict overfull count and maximum agree")
    require(max(0, occupancy - 1) - 1e-12 <= excess <= overfull * max(0, occupancy - 1) + 1e-12,
            "occupancy excess witness range")
    require(low <= high and low >= -10 * occupancy - 1e-12 and high <= 10 * occupancy + 1e-12,
            "additive reward witness range")
    total["planning_decisions"] += 1
    total["overfull_cell_decisions"] += int(overfull > 0)
    total["overfull_cells_total"] += overfull
    for name, value, operation in (("maximum_cell_occupancy", occupancy, max),
                                   ("minimum_expected_reward", low, min),
                                   ("maximum_expected_reward", high, max)):
        total[name] = value if total[name] is None else operation(total[name], value)
...
def episode(key, traces, decisions, reported, compare):
    ...
    planning = []
    representation = empty_diagnostics()
    ...
    for plan_index, decision in enumerate(decisions):
        ...
        if arm in ("particle_full", "known_map", "privileged"):
            particles = 1 if arm in ("known_map", "privileged") else 256
        ...
        if arm in FACTORIZED:
            add_diagnostics(representation, diag)
    ...
    construction = finite(reported["representation_construction_seconds"], "construction time", 0)
    require(filter_time - construction >= 0, "filter time includes representation construction")
    expected = {"map_seed": key[0], "reset_seed": key[1], "arm": arm,
                ...
                "representation_construction_seconds": construction,
                "representation_diagnostics": representation,
                "information": ("true map and qualities" if arm == "privileged" else
                                "supplied true map; qualities inferred from public history" if arm == "known_map" else
                                "public history only")}
...
def aggregate(rows):
    ...
    for control in ("factorized_recent128", "factorized_latest", "quality", "particle_full"):
        differences = [panel["scores"]["factorized_full"]["raw_return"] - panel["scores"][control]["raw_return"] for panel in maps]
        gap = averages["factorized_full"]["raw_return"] - averages[control]["raw_return"]
    ...
    for arm, margin in (("factorized_full", 5), ("known_map", 20), ("privileged", 20)):
    ...
    representation = {}
    for arm in FACTORIZED:
        selected = [row["representation_diagnostics"] for row in rows if row["arm"] == arm]
        totals = {key: sum(item[key] for item in selected)
                  for key in ("planning_decisions", "overfull_cell_decisions", "overfull_cells_total")}
        totals["maximum_cell_occupancy"] = max(item["maximum_cell_occupancy"] for item in selected)
        totals["minimum_expected_reward"] = min(item["minimum_expected_reward"] for item in selected)
        totals["maximum_expected_reward"] = max(item["maximum_expected_reward"] for item in selected)
        representation[arm] = totals
    full = representation["factorized_full"]
    coherent_observed_bounds = (full["maximum_cell_occupancy"] <= 1 and full["overfull_cells_total"] == 0
                                and full["minimum_expected_reward"] >= -10
                                and full["maximum_expected_reward"] <= 10)
    decisions.extend([{"name": "complete_cohort", "value": len(rows), "passes": True},
                      {"name": "factorized_observed_forecast_bounds", "value": full,
                       "passes": coherent_observed_bounds}])
    require(len(decisions) == 13, "thirteen independent conditions")
    return {"scope": "state representation and information diagnostic; no learned architecture result",
            "episodes": len(rows), "scores_equal_map_means": averages, "by_map": maps,
            "representation_diagnostics": representation, "criteria": decisions,
...
def execute(args):
    ...
    try:
        ...
        for key in expected_order:
            ...
            require(eg is not None and tg is not None and dg is not None, "all 256 cases present")
        ...
        require(receipt["completed_episodes"] == len(rows) == 256, "root episode accounting")
        ...
        for name, pin in _plan["sources"].items():
            require(sha(ROOT / name) == pin, f"source unchanged: {name}")
        budget()
        ...
        result = {"status": "completed", "agreement": True, "source_sha256": own_source,
                  ...
                  "inherited_plan_sha256": INHERITED_PLAN_PIN,
