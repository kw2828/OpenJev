"""Package closed expected-count evidence with an opaque-byte roundtrip.
...
from render_finite_expected_count_learning import (
    REGISTRATION_SHA256,
    ROOT,
    STUDY_NAME,
    authenticate,
...
VERSION = 'finite-expected-count-learning-package-v1'
...
def package(study, report, output, *, registration_sha256, commit, publication):
    ...
    require(report == ROOT / 'research/finite-expected-count-learning-results',
    ...
    require(receipt['renderer'] == {'path': str(ROOT / 'scripts/render_finite_expected_count_learning.py'),
             **descriptor(ROOT / 'scripts/render_finite_expected_count_learning.py')},
            'renderer source matches receipt')
    require(receipt['version'] == 'finite-expected-count-learning-report-v1'
    ...
    named = {'README.md', 'LICENSE', 'scripts/render_finite_expected_count_learning.py',
             'scripts/package_finite_expected_count_learning.py', *publication,
    ...
    manifest = {
        ...
        'gates': auth['audit']['gates'],
        'paired_comparisons': auth['audit']['dynamics_comparisons']['paired'],
        ...
        'scope': 'Every authenticated study and numerical-prerequisite artifact, all 36 pinned sources, 27 checkpoints, training orders, histories, predictions, timings, update traces, original phase records and named publication files. No directory is included wholesale.',
