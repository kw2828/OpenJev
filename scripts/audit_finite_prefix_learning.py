"""Independent saved-output audit for the all-attempt prefix-supervision finite-world diagnostic.
...
VERSION = 'finite-prefix-learning-audit-v1'
NAMESPACE = 423260924
...
ENDPOINT_WORK_KEYS = CORE_WORK_KEYS | FILTER_WORK_KEYS | {'privileged_prefix_rows', 'event_probability_rows'}
PREFIX_WORK_KEYS = CORE_WORK_KEYS | FILTER_WORK_KEYS | {'prefix_probability_rows', 'prefix_nll_rows'}
...
def validate_metadata(folder, summary, *, check=lambda: None):
    ...
    require(summary['version'] == 'finite-prefix-learning-v1', 'prefix-learning producer version')
    compare(summary['config'], CONFIG, path='registered prefix-learning configuration')
    fits = summary['fits']
    expected_pairs = [(arm, seed) for index, seed in enumerate(SEEDS) for arm in ARMS[index % 2:] + ARMS[:index % 2]]
    require([(row['arm'], row['seed']) for row in fits] == expected_pairs, 'six completed prefix-learning fits')
    ...
    require(set(summary['files']) == expected_files, 'complete prefix-learning producer payload roster')
    ...
    for row in fits:
        ...
        weight = int(arm == 'endpoint_plus_prefix')
        ...
        pm = row['parameter_metadata']
        require(pm['parameter_count'] == 1088 and pm['parameter_bytes'] == 8704 and pm['buffer_bytes'] == 256
                and pm['parameters'] == {'reset_logits': {'count': 32, 'dtype': 'torch.float64', 'bytes': 256},
                    'observed_logits': {'count': 1056, 'dtype': 'torch.float64', 'bytes': 8448}},
                'identical shared-filter parameters and fixed readout in both arms')
    ...
    for seed in SEEDS:
        for key in ('initial_state_sha256', 'operator_initial_sha256', 'reset_initial_sha256', 'fixed_buffers_sha256'):
            require(fit_map[ARMS[0], seed][key] == fit_map[ARMS[1], seed][key], 'identical same-seed paired initialization')
    ...
    expected_counts = {'train_generation_count': 1, 'dev_generation_count': 1, 'model_constructions': 6,
        ...
        'training_attempt_exposures': 6 * 480 * 512, 'training_prefix_rollouts': 3 * updates,
        'training_prefix_event_exposures': 3 * 480 * events, 'zero_endpoint_batches': zero,
...
def audit(folder, *, check=lambda: None):
    ...
    return {'version': VERSION, 'agreement': True, 'technical_complete': False,
            ...
            'limitations': [
                ...
                'Both arms have equal attempts, endpoint examples and Adam updates; auxiliary prefix supervision adds likelihood terms and computation.',
                ...
                'The fixed cost readout distinguishes four decision signatures, not all eight posterior coordinates.',
                'Observed filtering has intervening observations and remains separate from blind extrapolation.',
                'This synthetic supervision diagnostic establishes neither convergence, native transfer nor architectural novelty.']}
