"""Independent saved-output audit for the two-arm fresh cost-readout replication.
...
VERSION = 'finite-cost-readout-replication-audit-v1'
NAMESPACE = 425260924
ARMS = ('fixed_softened', 'learned_readout')
SEEDS = (425261001, 425261002, 425261003)
HORIZONS = (1, 2, 4, 8)
...
ENDPOINT_WORK_KEYS = CORE_WORK_KEYS | FILTER_WORK_KEYS | HEAD_WORK_KEYS | {'privileged_prefix_rows', 'event_probability_rows'}
PREFIX_WORK_KEYS = CORE_WORK_KEYS | FILTER_WORK_KEYS | HEAD_WORK_KEYS | {'prefix_probability_rows', 'prefix_nll_rows'}
...
def validate_predictions(data, predictions, np):
    ...
    require(set(predictions) == expected, 'complete six-model prediction fields')
    n = len(data['case_ids'])
...
def gates(rows, baselines, counts):
    ...
    require(len(rows) == len(keyed) == 24 and set(keyed)
...
def replication_continuation(rows, baselines, counts):
    """Separate prospective continuation, with no means or tolerance for ties."""
    absolute = gates(rows, baselines, counts)
    learned = absolute['learned_readout']
    keyed = {(row['arm'], row['seed'], row['horizon']): row for row in rows}
    conditions = {'learned_short_horizon_learning': learned['SHORT_HORIZON_LEARNING']['passed'],
                  'learned_observed_filtering_extrapolation': learned['OBSERVED_FILTERING_EXTRAPOLATION']['passed']}
    paired = []
    for seed in SEEDS:
        for horizon in (4, 8):
            candidate = keyed['learned_readout', seed, horizon]['blind_regret']
            reference = keyed['fixed_softened', seed, horizon]['blind_regret']
            require(type(candidate) in (int, float) and type(reference) in (int, float)
                    and math.isfinite(candidate) and math.isfinite(reference) and candidate >= 0 and reference >= 0,
                    'finite nonnegative paired replication regret')
            passed = candidate < reference
            conditions[f'{seed}_h{horizon}_strictly_lower_regret'] = passed
            paired.append({'seed': seed, 'horizon': horizon, 'learned_regret': candidate,
                'softened_regret': reference, 'difference': candidate - reference,
                'relative_reduction': (reference - candidate) / reference if reference > 0 else None,
                'passed': passed})
    passed = all(conditions.values())
    return {'passed': passed, 'status': 'REPLICATION_CONTINUATION_' + ('PASS' if passed else 'FAIL'),
            'conditions': conditions, 'paired_regret': paired,
            'blind_extrapolation_passed': learned['BLIND_EXTRAPOLATION']['passed'],
            'exact_cost_anchor_repeated': False, 'requires_separate_followup_registration': True}
...
def validate_work_schema(work):
    require(type(work) is dict and set(work) == set(ARMS), 'all two-arm structural work records')
    for arm in ARMS:
...
def validate_work(work, datasets, prefixes, orders, fits, np, *, check=lambda: None):
    ...
    def add(arm, route, values):
        if arm == 'learned_readout' and not route.endswith('_prefix'):
    ...
    return {'arms': 2, 'routes': 14, 'zero_endpoint_batches': sum(empty_batches.values()),
...
def validate_head_metadata(row):
    ...
    learned = arm == 'learned_readout'
    parameters = {'reset_logits': {'count': 32, 'dtype': 'torch.float64', 'bytes': 256},
                  'observed_logits': {'count': 1056, 'dtype': 'torch.float64', 'bytes': 8448}}
    if learned:
        parameters['cost_logits'] = {'count': 32, 'dtype': 'torch.float64', 'bytes': 256}
    require(pm['parameter_count'] == (1120 if learned else 1088)
            and pm['parameter_bytes'] == (8960 if learned else 8704)
            and pm['buffer_bytes'] == (0 if learned else 256) and pm['parameters'] == parameters,
            'readout-specific parameter and buffer storage')
    initial, final = readout_matrix(row['readout_initial']), readout_matrix(row['readout_final'])
    scale = .9
    ...
    if not learned:
        require(initial == expected and final == initial and row['readout_initial']['sha256'] == row['readout_final']['sha256'],
                'fixed exact/softened readout unchanged including snapshot bytes')


def readout_comparisons(rows, fits):
    """Descriptive paired contrasts; these do not alter any absolute criterion."""
    metrics = ('blind_regret', 'blind_cost_mse', 'observed_cost_mse', 'observed_kl',
               'blind_survival_mae', 'observed_survival_mae')
    keyed = {(row['arm'], row['seed'], row['horizon']): row for row in rows}
    require(len(rows) == len(keyed) == 24
            and set(keyed) == {(arm, seed, h) for arm in ARMS for seed in SEEDS for h in HORIZONS},
            'complete paired readout contrast roster')
    contrasts = []
    for control in ('fixed_softened',):
        for seed in SEEDS:
            for horizon in HORIZONS:
                learned, baseline = keyed['learned_readout', seed, horizon], keyed[control, seed, horizon]
                contrasts.append({'control': control, 'seed': seed, 'horizon': horizon,
                    'metrics': {name: {'learned': learned[name], 'control': baseline[name],
    ...
    for row in fits:
        ...
        matrices.append({'arm': row['arm'], 'seed': row['seed'], 'initial': row['readout_initial'],
    ...
    return {'paired': contrasts, 'means': means, 'matrices': matrices,
            'interpretation': 'This replication repeats learned versus softened trainability only. The original exact-cost anchor comparison is not repeated, and neither result identifies latent alignment.'}
...
def validate_metadata(folder, summary, *, check=lambda: None):
    ...
    require(summary['version'] == 'finite-cost-readout-replication-v1', 'cost-readout producer version')
    compare(summary['config'], CONFIG, path='registered cost-readout configuration')
    fits = summary['fits']
    expected_pairs = [(arm, seed) for index, seed in enumerate(SEEDS) for arm in ARMS[index % 2:] + ARMS[:index % 2]]
    require([(row['arm'], row['seed']) for row in fits] == expected_pairs, 'six completed readout-replication fits')
    ...
    require(set(summary['files']) == expected_files, 'complete cost-readout producer payload roster')
    ...
    for row in fits:
        ...
        for key, present in (('prefix_initial_sha256', False),
                             ('operator_initial_sha256', True),
                             ('reset_initial_sha256', True),
                             ('prefix_operator_initial_sha256', True)):
    ...
    for seed in SEEDS:
        for key in ('operator_initial_sha256', 'reset_initial_sha256'):
            require(len({fit_map[arm, seed][key] for arm in ARMS}) == 1, 'identical same-seed reset/branch initialization')
        require(all(fit_map[arm, seed]['prefix_operator_initial_sha256']
                    == fit_map[arm, seed]['operator_initial_sha256'] for arm in ARMS), 'shared prefix/forecast operator value hash')
        learned, softened = (fit_map[arm, seed]['readout_initial']['matrix'] for arm in ('learned_readout', 'fixed_softened'))
        require(max(abs(learned[i][j] - softened[i][j]) for i in range(4) for j in range(8)) <= 1e-12,
                'paired initial learned/softened readout functions')
    ...
    for records in (orders, epochs):
        require([(r['arm'], r['seed'], r['epoch']) for r in records] == expected_epochs, 'complete 2880-epoch journal roster')
    ...
    require([(row['arm'], row['seed']) for row in prefix_times] == [(arm, seed) for arm in ARMS for seed in SEEDS],
            'six complete prefix diagnostic views')
    ...
    eval_batches = 6 * math.ceil(dev_n / 64)
    oracle_batches = math.ceil(n / 64) + math.ceil(dev_n / 64)
    expected_counts = {'train_generation_count': 1, 'dev_generation_count': 1, 'model_constructions': 6,
        'oracle_model_constructions': 1, 'fit_count': 6, 'checkpoint_writes': 6,
        'training_blind_rollouts': 6 * updates - zero, 'training_observed_rollouts': 6 * updates - zero,
        'optimizer_attempts': 6 * updates, 'optimizer_steps': 6 * updates, 'training_case_exposures': 6 * 480 * n,
        'training_attempt_exposures': 6 * 480 * 512, 'training_prefix_rollouts': 6 * updates,
        'training_prefix_event_exposures': 6 * 480 * events, 'zero_endpoint_batches': zero,
        'evaluation_blind_rollouts': eval_batches, 'evaluation_observed_rollouts': eval_batches,
        'evaluation_shuffled_rollouts': eval_batches, 'evaluation_case_views': 6 * dev_n,
        'evaluation_prefix_rollouts': 6 * 2,
        'evaluation_prefix_event_views': 6 * summary['dataset_counts']['base']['valid_prefix_events'],
        ...
        'readout_snapshot_evaluations': 12, 'readout_snapshot_softmax_evaluations': 6}
    ...
    return {'fits': 6, 'checkpoint_files_hashed': 6, 'epochs': 2880, 'evaluation_views': 6,
            'prefix_evaluation_views': 6, 'retained': retained, 'optimizer_steps_attested': 6 * updates,
            'training_case_exposures_attested': 6 * 480 * n, 'training_attempt_exposures_attested': 6 * 480 * 512,
...
def audit(folder, *, check=lambda: None):
    ...
    return {'version': VERSION, 'agreement': True, 'technical_complete': False,
            ...
            'readout_comparisons': readout_comparisons(rows, summary['fits']),
            'continuation': replication_continuation(rows, baselines, counts),
            ...
            'limitations': [
                ...
                'The exact control receives a privileged posterior and known operators; both learned arms receive public inputs only.',
                'All arms have equal attempts, endpoint/prefix targets and Adam updates; the learned head adds parameters and softmax computation.',
                ...
                'Readout snapshots and matrix hashes are independently validated, but checkpoint/readout correspondence remains source-attested without checkpoint decoding.',
                'This fresh replication excludes the original exact-cost anchor arm and cannot establish native transfer or novelty.',
                'Continuation success permits only a separately registered follow-up, not automatic longer-horizon training.']}
