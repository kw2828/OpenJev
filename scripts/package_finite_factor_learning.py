"""Package an authenticated closed factor study by opaque, deterministic bytes.
...
from render_finite_factor_learning import (
    ROOT,
    authenticate,
    descriptor,
    inventory,
...
VERSION = 'finite-factor-learning-package-v1'
STUDY_NAME = 'finite-factor-learning-v1'
...
def package(studyfolder, reportfolder, outputfolder, *, registration_sha256, commit=None, publication_paths=()):
    ...
    require(render_receipt['version'] == 'finite-factor-learning-report-v1'
    ...
    renderer_path = regular(ROOT / 'scripts/render_finite_factor_learning.py')
    ...
    require(sorted(studyfolder.glob('engineering-registration-[0-9][0-9].json')) == [qualification_plan],
    ...
    for name in ('README.md', 'LICENSE', 'scripts/render_finite_factor_learning.py', 'scripts/package_finite_factor_learning.py'):
    ...
    manifest = {'version': VERSION, 'study': STUDY_NAME, 'registration': descriptor(plan_path),
        ...
        'scope': 'All registered sources, original closed phase payloads including twelve checkpoints and the fresh TRAIN/DEV arrays, oracle controls, journals, original receipts/launches/terminals/logs, the original first qualification and its source snapshot, and completed report. No prior scientific datasets or checkpoints.',
