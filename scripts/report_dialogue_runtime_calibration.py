...
All four completed phase terminals precede evaluator or NPZ decoding here.
The original qualification's failed cost gate remains explicitly failed.
...
VERSION = "dialogue-calibration-report-v2"
LIMITS = {"wall_seconds": 300, "rss_bytes": 4 * 1024**3, "output_bytes": 128 * 1024**2}
PLAN_PATH = "output/dialogue-calibration-runtime-v2/report-plan-01.json"
OUTPUT_PATH = "output/dialogue-calibration-runtime-v2/report-01"
SOURCES = {"scripts/report_dialogue_runtime_calibration.py", "tests/test_report_dialogue_runtime_calibration.py",
           "scripts/report_dialogue_calibration.py", "tests/test_report_dialogue_calibration.py",
           "src/openjev/research/dialogue_calibration_metrics.py", "tests/test_dialogue_calibration_metrics.py"}
FIT_ORDER = list(common.FIT_ORDER)
DEV_ROWS, CALIBRATION_ROWS, CALIBRATION_DIALOGUES = 62329, 13333, 512
PHASES = ("prepare", "qualify", "pilot", "infer")
SCOPE = ("Saved-only development control, not untouched confirmation. Temperatures use calibration TRAIN endpoints only. "
         ...
         "the numerical scorer reuses the qualified original group definitions. Original raw FAIL6/7 and the V1 failed cost gate remain unchanged.")
...
def validate_analysis_plan(args, budget):
    ...
    for name in ("scripts/dialogue_runtime_common.py", "src/openjev/research/suspend_clock.py"):
    ...
    require(control["version"] == common.VERSION and control["inputs"][common.CONTROL_PLAN] == common.PINS[common.CONTROL_PLAN],
            "Pinned runtime control and original control lineage")
    legacy_path = ROOT / common.CONTROL_PLAN
    require(common.sha(legacy_path) == control["inputs"][common.CONTROL_PLAN], "Original control plan pin")
    legacy_plan = common.read(legacy_path)
    legacy_helper = "scripts/dialogue_calibration_common.py"
    require(common.sha(ROOT / legacy_helper) == legacy_plan["sources"][legacy_helper], "Direct original reader source pin")
    paths = {"prepare": common.PREPARED, "qualify": common.QUALIFIED,
             "pilot": control["outputs"]["pilot"], "infer": control["outputs"]["infer"]}
    require(set(plan["phases"]) == set(PHASES), "All four phase bindings required")
    for phase, binding in plan["phases"].items():
        require(set(binding) == {"path", "completed_sha256", "terminal"}
                ...
                and absolute(binding["path"]) == absolute(paths[phase]), "Exact phase binding")
    for phase, terminal_name in (("prepare", common.PREP_TERMINAL), ("qualify", common.QUAL_TERMINAL)):
        binding = plan["phases"][phase]
        require(binding["completed_sha256"] == control["inputs"][paths[phase] + "/completed.json"]
                and absolute(binding["terminal"]["path"]) == absolute(terminal_name)
                and binding["terminal"]["sha256"] == control["inputs"][terminal_name], "Immutable original phase binding")
...
def check_phase_links(plan, receipts, terminals):
    prepared, qualified, pilot, inferred = (receipts[p] for p in PHASES)
    ...
    for receipt in (qualified, inferred):
        require(receipt["prepared_sha256"] == prep_pin and receipt["prepared_terminal_sha256"] == prep_terminal_pin,
                "Unchanged original calibration preparation identity")
    for receipt in (qualified, pilot, inferred):
        require(receipt["fit_order"] == FIT_ORDER and [f["fit_id"] for f in receipt["fits"]] == FIT_ORDER
    ...
    require(qualified["parity_passed"] is True and qualified["completed_forwards"] == 24
            and qualified["projection"]["admitted"] is False
            and qualified["projection"]["threshold_seconds"] == 1800
            and math.isfinite(qualified["projection"]["total_seconds"])
            and qualified["projection"]["total_seconds"] > 1800
            and qualified["projection"]["preparation_seconds"] == terminals["prepare"]["wall_seconds"],
            "Original completed parity and failed V1 cost admission preserved")
    screening = pilot["projection"]
    require(pilot["parity_passed"] is True and pilot["completed_forwards"] == 12 * 130
            and pilot["warmup_forwards"] == 24 and pilot["timed_forwards"] == 12 * 128
            and screening["admitted"] is True and screening["all_verifications_passed"] is True
            and screening["verifications_passed"] == screening["verifications_total"] == 12
            and screening["threshold_seconds"] == 1800 and math.isfinite(screening["total_seconds"])
            and 0 < screening["total_seconds"] <= 1800
            and screening["components"]["preparation_parent_wall_seconds"] == terminals["prepare"]["wall_seconds"],
            "Complete V2 pilot parity and all-twelve fixed runtime admission")
    require(inferred["pilot_sha256"] == plan["phases"]["pilot"]["completed_sha256"]
            and inferred["pilot_terminal_sha256"] == plan["phases"]["pilot"]["terminal"]["sha256"]
            and inferred["calibration_work"] == pilot["calibration_work"]
...
def authenticate(args, budget):
    ...
    historical = SimpleNamespace(command="infer", plan=control_path,
                                 ...
                                 pilot_sha256=phases["pilot"]["completed_sha256"],
                                 pilot_terminal=absolute(phases["pilot"]["terminal"]["path"]),
                                 pilot_terminal_sha256=phases["pilot"]["terminal"]["sha256"])
    ctx = common.authenticate(historical, budget)
    require(ctx["runtime_plan_path"] == control_path, "Exact authenticated runtime control plan path")
    receipts, terminals = {}, {}
    for phase in PHASES:
        binding = phases[phase]
        budget.check()
        reader = common.legacy if phase in ("prepare", "qualify") else common
        receipts[phase] = reader.authenticate_output(absolute(binding["path"]), binding["completed_sha256"], ctx, phase)
        terminals[phase] = reader.authenticate_terminal(absolute(binding["terminal"]["path"]),
    ...
    for phase in ("qualify", "pilot", "infer"):
        ...
        for record in receipts[phase]["fits"]:
            ...
            witness = fit["witness"]
            count = {"qualify": 2, "pilot": 130, "infer": CALIBRATION_DIALOGUES}[phase]
            ...
            if phase == "infer":
                ...
                require(len(fit["warmup"]["dialogue_ids"]) == 2
                        and len(fit["estimator"]["dialogue_ids"]) == len(fit["verification"]["dialogue_ids"]) == 64
                        and fit["verification_result"]["passed"] is True,
                        "Complete prior-predicted V2 timing blocks")
    ...
    require(common.read(absolute(phases["pilot"]["path"]) / "projection.json") == receipts["pilot"]["projection"],
            "Saved V2 pilot projection agrees")
    ...
    require(published["technical_validity_passed"] is True and published["technical_complete_fits"] == 12
            ...
            and published["continuation"]["checks_total"] == 7,
...
def compute(args, budget, plan, ctx, receipts, terminals, published):
    ...
    summary.update(version=VERSION, status="completed", technical_validity_passed=True, complete_fits=12,
                   ...
                   legacy_v1_cost_failure={"admitted": False, "preserved": True,
                                           "projection": receipts["qualify"]["projection"],
                                           "completed_sha256": plan["phases"]["qualify"]["completed_sha256"]},
                   runtime_v2_admission=receipts["pilot"]["projection"],
                   calibration_service_exposure=receipts["prepare"]["service_exposure"],
                   costs={"separate_phases": separate_costs,
                          ...
                          "scope": "Parent prepare/V1-qualify/V2-pilot/V2-infer intervals are separate and summed once, including the failed V1 cost gate. "
                                   "Original training is separate, not added. "
...
def report_text(summary):
    ...
    lines = ["# Complete output-temperature control after runtime V2 admission", "",
             ...
             ("V1 cost admission remains failed. The separate V2 pilot passed all twelve runtime verification blocks; "
              "its success does not change any scientific condition."), "",
    ...
    lines += ["", "| Phase | Actual parent wall seconds |", "|---|---:|"]
    lines.extend(f"| {phase} | {summary['costs']['separate_phases'][phase]['parent_wall_seconds']:.8g} |" for phase in PHASES)
    lines.append(f"| Total of four separate phases | {summary['costs']['nonnested_prior_phase_seconds']:.8g} |")
...
def execute(args):
    ...
    try:
        ...
        receipt = {"version": VERSION, "status": "completed", "technical_validity_passed": True,
                   ...
                   "legacy_v1_cost_failure_preserved": True, "runtime_v2_admission_passed": True,
