...
ROOT = cost.ROOT
VERSION = "dialogue-observation-scientific-v1"
ALLOCATION_VERSION = "dialogue-observation-allocation-v1"
...
NEW_SOURCES = ["scripts/study_dialogue_observation.py", "tests/test_study_dialogue_observation.py",
               ...
               "research/dialogue-observation-learning-scoring.md", PROTOCOL,
...
ALLOCATION_KEYS = {"version", "prepared_completed_sha256", "prepared_plan_sha256", "cost_completed",
                   "cost_audit", "protocol", "limits", "freeze_limits"}
...
def allocation(path, pin):
    ...
    for field in ("cost_completed", "cost_audit", "protocol"):
        ref = value[field]
...
class Budget:
    def __init__(self, out, start, limits):
        self.out, self.start, self.limits = out, start, limits
        self.max_driver = self.max_current = 0

    def check(self):
        require(time.perf_counter() - self.start <= self.limits["wall_seconds"], "Whole-phase wall cap")
...
def source_map(cost_plan):
    ...
    require(len(result) == 53, "Exact scientific 53-source closure")
    return result
...
def execute(args):
    ...
    try:
        ...
        budget = Budget(args.out, started, limits)
        budget.storage()

        def timeout(*_):
            raise TimeoutError("Whole-phase allocated wall cap")
        handler = signal.signal(signal.SIGALRM, timeout)
        signal.setitimer(signal.ITIMER_REAL, max(.001, limits["wall_seconds"] - (time.perf_counter() - started)))
        ...
        result.update({"version": VERSION, "status": "completed", "phase": args.command,
                       "files": members(args.out), "wall_seconds": time.perf_counter() - started,
                       "peak_rss_bytes": cost.qualified.rss(),
                       "wall_scope": "Through manifest construction; allocated cap checked through completion hash"})
        ...
        try:
            ...
            write(args.out / "failed.json", {"version": VERSION, "status": "failed", "request": request,
                  ...
                  "wall_seconds": time.perf_counter() - started, "peak_rss_bytes": cost.qualified.rss(), "no_retry": True})
