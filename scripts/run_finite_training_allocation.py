"""Fresh-data recurrent learning under one eligibility window per fit."""
...
from openjev.research.finite_expected_count_bridge import DYNAMICS, torch_objective
...
from openjev.research.finite_training_allocation import ARMS, Hooks, Snapshot, run_allocation

VERSION = 'finite-training-allocation-v1'
DEFAULT_CONFIG = {'seed_namespace': 430260924, 'train_attempts': 512, 'dev_attempts': 128,
    'batch_size': 64, 'learning_rate': .003, 'fit_seeds': [430261001, 430261002, 430261003],
...
def train(arm, seed, data, prefixes, config, folder, counts, structural_work, check):
    ...
    def validate():
        ...
        context['model'] = learned_model('factorized', seed)
    ...
    try:
        allocation = run_allocation(arm, hooks, stage1_seconds=config['stage1_seconds'],
...
def run(folder: Path, config: dict, check):
    ...
    try:
        ...
        for seed in config['fit_seeds']:
            starts = [row['initial_state_sha256'] for row in fits if row['seed'] == seed]
        ...
        summary = {'version': VERSION, 'config': config, 'counts': counts, 'rows': rows,
            ...
            'allocation_scope': 'One overall eligibility clock, including model construction and boundary saves. Both stages use deadlines relative to that original start. Actual overshoot, final summaries and trace serialization are charged in fit seconds. This is not exact FLOP or actual wall-time equality.',
