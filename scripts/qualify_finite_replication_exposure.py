...
from finite_reuse_replication_worker import descriptor, files, publish, require
from register_finite_reuse_replication import EXPOSURE
...
def main():
    ...
    record = {'version': 'finite-replication-exposure-v1', 'status': 'FAILED', **EXPOSURE,
    ...
    try:
        ...
        verified = audit(output / 'run', check=check, profile='engineering-945201')
        publish(output / 'audit.json', verified)
        require(files(output / 'run') == original_files, 'engineering producer unchanged by audit')
        require(verified['agreement'] is True and verified['exact_oracle_agreement'] is True,
                'independent engineering saved-output audit agrees')
