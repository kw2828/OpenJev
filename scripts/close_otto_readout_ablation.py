...
def main():
    ...
    spec = importlib.util.spec_from_file_location('_readout_closure_producer', ROOT / 'scripts/run_otto_readout_ablation.py')
    ...
    run.require(audit['agreement'] is True and audit['counts'] == audit_receipt['audit_counts']
                and audit['counts']['array_decodes'] == audit_receipt['array_decodes'] == 28
                and audit['counts']['checkpoint_decodes'] == audit_receipt['checkpoint_decodes'] == 12
                and audit['counts']['views_completed'] == audit_receipt['views_completed'] == 12
                and audit['counts']['journal_updates_checked'] == 3240
    ...
    run.require(result['total_cells'] == 6 and result['recurrent_weight_hypothesis_continues']
                == (result['passed_cells'] == 6) == all(x['passed'] for x in result['cells']), 'saved fixed diagnostic identity')
    run.require(not any(k in sys.modules for k in ('numpy', 'torch', 'tensorflow', 'jax', 'mlx')), 'metadata-only closure')
    status = 'MECHANISM_DEV_PASS' if result['recurrent_weight_hypothesis_continues'] else 'MECHANISM_DEV_FAIL'
    run.write(args.output, {'version': 'otto-readout-state-ablation-closure-v1', 'status': status,
        **{k: run.descriptor(getattr(args, k)) for k in ('plan', 'producer_receipt', 'producer_terminal', 'audit_receipt', 'audit_terminal')},
        ...
        'diagnostic': result, 'counts': audit['counts'], 'dev_reused': True,
