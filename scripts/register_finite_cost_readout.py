...
from finite_cost_readout_worker import ROOT, closed_producer, descriptor, publish, require, runtime

SOURCES = (
    'scripts/register_finite_cost_readout.py',
    'scripts/finite_cost_readout_worker.py',
    'scripts/run_finite_cost_readout.py',
    'scripts/audit_finite_cost_readout.py',
    ...
    'tests/test_finite_cost_readout_audit.py',
    'tests/test_finite_cost_readout_worker.py',
    'tests/test_finite_cost_readout_runner.py',
    'research/finite-cost-readout-protocol.md',
    'pyproject.toml',
    'uv.lock',
)
TESTS = [name for name in SOURCES if name.startswith('tests/')]
LINT = [name for name in SOURCES if name.endswith('.py')]
CONFIG = {'seed_namespace': 424260924, 'train_attempts': 512, 'dev_attempts': 128,
          'epochs': 480, 'batch_size': 64, 'learning_rate': .003,
          'fit_seeds': [424261001, 424261002, 424261003],
...
def main():
    ...
    folder = ROOT / 'output/finite-cost-readout-v1'
    ...
    plan = {'version': 'finite-cost-readout-v1', 'mode': args.mode,
