"""Independent saved-output audit for fixed-time allocation between prefix and joint learning.
...
VERSION = 'finite-training-allocation-audit-v1'
NAMESPACE = 430260924
...
PROFILES = MappingProxyType({'science': SCIENCE,
    'engineering-939001': StudyProfile('engineering-939001', 939001, (939101,), 8, 8, 3, 2., 4.)})
...
def allocation_comparisons(rows, fits, original_gates, *, spec=SCIENCE):
    ...
    paired, means, times = [], [], []
    for control in ('joint_continuous', 'joint_restart'):
        for horizon in (4, 8):
    ...
    return {'paired': paired, 'mean_regret': means, 'mean_fit_time': times,
            'advance': {'name': 'ALLOCATION_ADVANCE', 'passed': passed,
                        'status': 'ALLOCATION_ADVANCE_PASS' if passed else 'ALLOCATION_ADVANCE_FAIL',
                        'conditions': conditions},
            'interpretation': 'Fixed eligibility time compares learning allocation strategies. Independent stage-one timing may produce different control boundary states, so this is not an isolated Adam-reset intervention. No mean rescues an original gate or a worse paired regret.'}
...
def validate_allocation(result, arm, *, spec=SCIENCE):
    ...
    require(result['version'] == 'finite-training-allocation-v1' and result['arm'] == arm and arm in ARMS
    ...
    for index, stage in enumerate(stages, 1):
        kind = 'prefix' if arm == 'prefix_then_joint' and index == 1 else 'joint'
        ...
        if index == 2:
            ...
            require(boundary['joint_cursor'] == cursor and boundary['model_sha256'] == model_hash
                    ...
                    and boundary['stage1_optimizer_kind'] == ('prefix' if arm == 'prefix_then_joint' else 'joint')
                    and boundary['stage2_optimizer_kind'] == 'joint'
                    and boundary['optimizer_reset'] is (arm != 'joint_continuous')
            ...
            if arm == 'joint_continuous':
                require(boundary['optimizer_after_sha256'] == optimizer_hash, 'continuous Adam moments retained')
        ...
        optimizer_updates = cursor if arm == 'joint_continuous' and index == 2 else 0
    ...
    total_work.update(validation_calls=1, optimizer_constructions=1 if arm == 'joint_continuous' else 2, checkpoint_calls=3)
...
def verify_boundaries(allocations, checkpoints, optimizer_payloads, np):
    """Join three decoded parameter/optimizer boundaries per fit without replay."""
    result, initial_by_seed = [], {}
    joint_empty = {}
    for (arm, seed), allocation in allocations.items():
        if arm == 'joint_continuous':
            joint_empty[seed] = optimizer_state(optimizer_payloads['initial', arm, seed], 'joint', 0)['sha256']
    for (arm, seed), allocation in allocations.items():
        initial = checkpoints['initial', arm, seed]
        first_kind = 'prefix' if arm == 'prefix_then_joint' else 'joint'
        if seed in initial_by_seed:
            require(all(initial[name].tobytes() == initial_by_seed[seed][name].tobytes() for name in PARAMETER_SHAPES),
                    'all three conditions share exact original parameter bytes')
        initial_by_seed[seed] = initial
        checked = []
        for index, label in enumerate(('initial', 'boundary', 'final')):
            arrays = checkpoints[label, arm, seed]
            hashes = boundary_hashes(arrays, np)
            stage = None if index == 0 else allocation['stages'][index - 1]
            steps = 0 if stage is None else (stage['accepted_updates'] if index == 1 or arm != 'joint_continuous'
                                             else allocation['accepted_joint_updates'])
            kind = first_kind if index < 2 else 'joint'
        ...
        if arm == 'prefix_then_joint':
            require(initial['cost_logits'].tobytes() == checkpoints['boundary', arm, seed]['cost_logits'].tobytes(),
                    'prefix-only stage preserves actual cost-head bytes')
        if arm != 'joint_continuous':
            require(allocation['boundary']['optimizer_after_sha256'] == joint_empty[seed],
                    'restart uses fresh empty joint Adam state with registered parameter group')
...
def validate_metadata(folder, summary, *, check=lambda: None, spec=SCIENCE):
    ...
    require(summary['version'] == 'finite-training-allocation-v1', 'registered allocation producer')
    ...
    for row in fits:
        ...
        compare(row['construction_work'], {**dict.fromkeys(FACTOR_WORK_KEYS, 0),
            'matching_log_calls': 0, 'matching_log_entries': 0}, path='unchanged factorized constructor work')
    ...
    for seed in spec.seeds:
        require(len({allocations[arm, seed]['initial_model_sha256'] for arm in ARMS}) == 1,
                'paired complete original initialization')
...
def validate_execution(summary, allocations, datasets, prefixes, orders, np, *, check=lambda: None, spec=SCIENCE):
    ...
    def add(arm, route, values):
        ...
        values = {**values, **{key: count * factors for key, count in FACTORIZATION_WORK.items()},
            'reset_emission_softmax_calls': 0, 'prefix_operator_softmax_calls': 0, 'operator_observed_softmax_calls': 0}
    ...
    for (arm, seed), allocation in allocations.items():
        for row in allocation['trace']:
            ...
            if kind == 'prefix':
                ...
                add(arm, 'training_prefix', prefix_work(prefixes['train'], np.arange(spec.train_attempts), np))
            else:
...
def audit(folder, *, check=lambda: None, profile='science'):
    ...
    return {'version': VERSION, 'profile': profile, 'agreement': True, 'technical_complete': False,
            ...
            'limitations': [
                ...
                'Independent clocks can produce distinct joint-control boundary states. Their contrast does not isolate optimizer restart alone.',
                ...
                'This synthetic allocation diagnostic establishes neither latent identification, native transfer, statistical significance nor architectural novelty.']}
