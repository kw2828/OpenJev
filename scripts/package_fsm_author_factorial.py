...
# Exact --basetemp directories named by the retained qualification definitions.
# Prune only these ephemeral trees; qualification logs/receipts/snapshots stay.
ENGINEERING_FIXTURE_TREES = {
    'output/fsm-author-nllfr-budget-engineering-v1/producer-qualification-01/fabricated-fixtures',
    'output/fsm-author-factorial-plot-engineering-v1/qualification-01/fabricated-tmp',
}
...
def tree(folder, *, omit_transient=False):
    ...
    found, excluded = [], []
    scoped_engineering = folder in {ROOT/name for name in (*ENGINEERING, *OPTIONAL_ENGINEERING)}
    for directory, dirs, names in os.walk(folder, followlinks=False):
        dirs.sort()
        for name in dirs[:]:
            path = Path(directory)/name
            require(not path.is_symlink(), 'symlink directory')
            fixture = scoped_engineering and str(path.relative_to(ROOT)) in ENGINEERING_FIXTURE_TREES
            if omit_transient and (name in TRANSIENT or fixture):
                dirs.remove(name)
