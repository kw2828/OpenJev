"""Package an authenticated closed cost-readout study by opaque, deterministic bytes.
...
from render_finite_cost_readout_replication import (
    ROOT,
    authenticate,
    descriptor,
    inventory,
...
VERSION = 'finite-cost-readout-replication-package-v1'
...
def package(studyfolder, reportfolder, outputfolder, *, registration_sha256, commit=None, publication_paths=()):
    ...
    require(render_receipt['version'] == 'finite-cost-readout-replication-report-v1'
    ...
    renderer_path = regular(ROOT / 'scripts/render_finite_cost_readout_replication.py')
    ...
    for engineering_plan in engineering_plans:
        ...
        if engineering_plan == qualification_plan:
            ...
            require(terminal['cwd'] == str(ROOT) and command[0] == record['runtime']['executable']
                    and (ROOT / command[1]).resolve() == ROOT / 'scripts/finite_cost_readout_replication_worker.py'
    ...
    for name in ('README.md', 'LICENSE', 'scripts/render_finite_cost_readout_replication.py', 'scripts/package_finite_cost_readout_replication.py'):
    ...
    manifest = {'version': VERSION, 'study': STUDY_NAME, 'registration': descriptor(plan_path),
        ...
        'files': members, 'gates': audit['gates'], 'continuation': audit['continuation'],
        'recorded_scientific_work': saved['counts'],
        ...
        'scope': 'All registered sources, original closed phase payloads including six checkpoints, fresh endpoint and all-attempt prefix arrays, oracle controls, journals, original receipts/launches/terminals/logs, every named engineering attempt with its source snapshot, and completed report. Failed attempts remain failures. No prior scientific datasets or checkpoints.',
    ...
    receipt = {'version': VERSION, 'status': 'PASS', 'registration': descriptor(plan_path),
        'continuation': audit['continuation'], 'gates': audit['gates'],
