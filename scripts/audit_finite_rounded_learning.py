"""Independent saved-output audit for rounded transport under one shared training schedule.
...
VERSION = 'finite-rounded-learning-audit-v1'
NAMESPACE = 432260924
...
PROFILES = MappingProxyType({'science': SCIENCE,
    'engineering-941001': StudyProfile('engineering-941001', 941001, (941101,), 8, 8, 3, 2., 4.)})
...
def allocation_comparisons(rows, fits, original_gates, *, spec=SCIENCE):
    ...
    return {'paired': paired, 'mean_regret': means, 'mean_fit_time': times,
            'advance': {'name': 'ROUNDED_ADVANCE', 'passed': passed,
                        'status': 'ROUNDED_ADVANCE_PASS' if passed else 'ROUNDED_ADVANCE_FAIL',
...
def validate_metadata(folder, summary, *, check=lambda: None, spec=SCIENCE):
    ...
    require(summary['version'] == 'finite-rounded-learning-v1', 'registered allocation producer')
...
def audit(folder, *, check=lambda: None, profile='science'):
    ...
    return {'version': VERSION, 'profile': profile, 'agreement': True, 'technical_complete': False,
            ...
            'limitations': [
                ...
                'This synthetic rounded-transition diagnostic establishes neither latent identification, native transfer, statistical significance nor architectural novelty.']}
