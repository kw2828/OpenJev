...
def validate_registered_plan(plan_path, plan):
    from register_finite_reuse_replication import (
        CONFIG,
        EXPOSURE,
        SOURCES,
        TESTS,
    ...
    require(Path(plan_path).resolve() == expected_path and plan['version'] == 'finite-reuse-replication-v1'
    ...
    require(authenticate_parent() == plan['replication_prerequisite'],
...
def qualification_commands(plan):
    ...
    return [[str(root / '.venv/bin/ruff'), 'check', *plan['lint_sources']],
            ...
            [plan['runtime']['executable'], str(root / 'scripts/qualify_finite_replication_exposure.py'),
...
def validate_launch_binding(plan, receipt):
    ...
    expected = [plan['runtime']['executable'],
                str(Path(plan['root']) / 'scripts/finite_reuse_replication_worker.py'),
...
def admit_qualification(plan, receipt_path):
    ...
    for key in ('version', 'root', 'runtime', 'sources', 'config', 'phases', 'tests',
                'lint_sources', 'rss_limit_bytes', 'output_limit_bytes', 'replication_prerequisite', 'exposure_probe'):
    ...
    require(probe['audit'] == descriptor(probe_folder / 'audit.json')
            and probe_audit['version'] == 'finite-reuse-replication-audit-v1'
            and probe_audit['profile'] == 'engineering-945201'
    ...
    require(probe['status'] == 'PASS' and probe['config'] == plan['exposure_probe']['config']
            ...
            and {(row['arm'], row['seed']) for row in probe['projections']} ==
                {(arm, 945301) for arm in ('original_free', 'matched_free', 'rounded')}
...
def main():
    ...
    try:
        ...
        require(plan['version'] == 'finite-reuse-replication-v1', 'registered study version')
        ...
        if args.phase == 'qualify':
            ...
            from run_finite_reuse_replication import run
