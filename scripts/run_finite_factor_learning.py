"""Known/learned prefix and transition factors; admission belongs to the worker."""
...
from openjev.research.finite_observation_models import make_model as previous_model
...
VERSION = 'finite-factor-learning-v1'
ARMS = ('learned_exact', 'exact_learned', 'learned_learned', 'gru')
ORACLE_FIELDS = FIELDS[:-1]
DEFAULT_CONFIG = {'seed_namespace': 421260924, 'train_attempts': 512,
    'dev_attempts': 128, 'epochs': 480, 'batch_size': 64, 'learning_rate': .003,
    'fit_seeds': [421261001, 421261002, 421261003], 'gradient_clip': 5.,
    'train_horizon': 2, 'dev_horizon': 8}


def tensors(data, oracle):
    return {**_tensor_data(data), 'oracle_prefix': torch.from_numpy(oracle.copy())}


def call(model, arm, batch, observed=False):
    kwargs = {'oracle_prefix': batch['oracle_prefix']} if arm in ('exact_learned', 'exact_exact') else {}
...
def train(model, arm, seed, data, config, folder, counts, check):
    n = len(data['prefix'])
    initial = _state_hash(_state(model))
    prefix_hash = initial_group_hash(model, ('assimilation.', 'projection.')) if arm != 'gru' else None
    operator_hash = initial_group_hash(model, ('observed_logits',)) if arm != 'gru' else None
...
def predict(model, arm, data, oracle, config, counts, check, *, shuffled):
        ...
        with torch.no_grad():
            for start in range(0, n, config['batch_size']):
                ...
                if shuffled:
                    ...
                    # The exact-prefix diagnostic gets the posterior associated
                    # with the same shifted prefix, never the original target case.
                    moved = {**batch, **{name: public[name][indices]
                                        for name in ('prefix', 'lengths', 'oracle_prefix')}}
                    counts['evaluation_shuffled_rollouts'] += 1
                    outputs['shuffled_blind_costs'] = call(model, arm, moved)['cost_contrasts']
...
def run(folder: Path, config: dict, check):
    ...
    try:
        ...
        train_data, train_oracle = generate(0, 'train')
        training = tensors(train_data, train_oracle)
        for seed_index, seed in enumerate(config['fit_seeds']):
            ...
            for arm in ARMS[offset:] + ARMS[:offset]:
                ...
                model = previous_model('gru', seed) if arm == 'gru' else factor_model(arm, seed)
                fits.append(train(model, arm, seed, training, config, folder, counts, check))
        ...
        for arm in ARMS:
            for seed in config['fit_seeds']:
                ...
                start = time.perf_counter()
                arrays = predict(model, arm, data, oracle, config, counts, check, shuffled=True)
