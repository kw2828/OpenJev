...
SELF = "scripts/qualify_otto_prequery_capacity.py"
TEST = "tests/test_qualify_otto_prequery_capacity.py"
TRAINER = "scripts/train_otto_prequery_calibration.py"
TRAINER_TEST = "tests/test_train_otto_prequery_calibration.py"
...
COMPONENTS = {SELF, TEST, TRAINER, TRAINER_TEST, MODEL, MODEL_TEST, CHRONOLOGY, LOSS,
...
SOURCES = COMPONENTS | {SAMPLING, DATA, CLOCK, SUPERVISOR, PROTOCOL,
           "scripts/train_otto_cross_query_forecasts.py", "src/openjev/research/otto_prequery_metrics.py",
...
VERSION = "otto-prequery-capacity-v1"
...
CONFIG = {"families": list(KINDS), "batch": 6, "length": 2188, "chunk": 32, "period": 4,
          ...
          "feature_seed": 0x50514331, "model_seed": 0x50514D31,
          "selection_seeds": [0x50515700 + i for i in range(6)],
          "learning_rate": .003, "clip_norm": 5., "weight_decay": 0., "scale": 64.,
          "fit_seeds": 3, "updates_per_fit": 720, "projection_multiplier": 1.5,
          "fixed_overhead_seconds": 120., "admission_seconds": 8100., "training_cap_seconds": 10800,
...
class Run:
    ...
    def admit(self):
        ...
        self.clock = load(CLOCK, "_prequery_capacity_clock").SuspendClock()
    ...
    def one_family(self, kind, data):
        ...
        row = {"family": kind, "architecture": architecture, "objective": objective,
    ...
    def body(self):
        ...
        self.models = load(MODEL, "_prequery_capacity_models")
        self.data = load(CHRONOLOGY, "_prequery_capacity_data")
        self.losses = load(LOSS, "_prequery_capacity_loss")
        self.trainer = load(TRAINER, "_prequery_capacity_trainer")
        sampling = load(SAMPLING, "_prequery_capacity_sampling")
        require(self.models.KINDS == ("innovation", "innovation_gru") and self.trainer.KINDS == KINDS
                and self.trainer.CELLS == CELLS and self.trainer.LIMITS["seconds"] == 10800
        ...
        summary = {"version": VERSION, "technical_complete": True, "families": rows,
            "sum_batch_seconds": total, "projected_seconds": projected, "threshold_seconds": 8100.,
            "admitted": projected <= 8100., "formula": "1.5 * 3 * 720 * sum(batch_seconds) + 120 <= 8100",
