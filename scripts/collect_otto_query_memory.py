"""Fresh TRAIN/DEV/TEST census collection for query-written memory research.

Metadata admission precedes numerical imports. Every retained physical state
receives exactly one teacher score, including states where the fixed collector
skips its deployed query. Annotation cannot change the selected action or held
cache. All collectors and saved feature clocks remain period four. A separate
prospective projector owns period-eight observations on these same fixed paths.

Compressed journal returns remain provisional until episode flush/fsync and
both durable completion records. There is no deferred replay, window sampling,
learner import, fitting, evaluation selection or autonomous saving claim here.
"""
...
VERSION = "otto-query-memory-collection-v1"
SELF, TEST = "scripts/collect_otto_query_memory.py", "tests/test_collect_otto_query_memory.py"
...
FIRST = {"train": {"lambda3": 303000001, "lambda4": 304000001},
         "dev": {"lambda3": 305000001, "lambda4": 306000001},
         "test": {"lambda3": 307000001, "lambda4": 308000001}}
CASES = {"train": 9, "dev": 3, "test": 6}
STAGE_EPISODES = {"train": 54, "dev": 18, "test": 36}
HORIZON, EPISODES = 2188, 108
FIT_SEEDS = (309000001, 309000002, 309000003)
LIMITS = {"native_seconds": 7200, "rss_bytes": 4 * 1024**3, "output_bytes": 2 * 1024**3}
# Exactly one teacher score per retained state, reused for deployed queries.
# 108 complete paths, each bounded by2188 steps:236304 teacher/native calls.
...
CONFIGURATION = {"arms": list(ARMS), "first_seeds": FIRST, "cases": CASES, "horizon": HORIZON,
    "train_episodes": 54, "dev_episodes": 18, "test_episodes": 36,
    "rotation": "global case index modulo three", "initial_hit": "1 + case % 3",
    "Ngrid": 53, "Nhits": 4, "R_dt": 2., "norm_Poisson": "Euclidean",
    "collector_period": 4, "annotation": "complete census in every split; one score per retained state",
    ...
    "schedule_shift": "future period-eight observations on fixed period-four collector paths; no autonomous saving claim",
    "deferred_annotation": False, "window_sampling": False,
...
NEW_SOURCES = NEW_COMPONENTS | {PROTOCOL,
    "scripts/collect_otto_protected_readout.py", "tests/test_collect_otto_protected_readout.py",
    "src/openjev/__init__.py", "src/openjev/research/__init__.py",
    "src/openjev/research/otto_query_memory.py", "tests/test_otto_query_memory.py",
    "src/openjev/research/otto_scheduled_predictor.py", "tests/test_otto_scheduled_predictor.py",
    "src/openjev/research/otto_query_memory_model.py", "tests/test_otto_query_memory_model.py",
    "src/openjev/research/otto_query_memory_data.py", "tests/test_otto_query_memory_data.py",
    "tests/test_otto_query_memory_integration.py",
    "src/openjev/research/otto_query_memory_metrics.py", "tests/test_otto_query_memory_metrics.py"}
...
PAYLOADS = {f"{name}.gz" for name in JOURNALS} | {"started.json", "runtime.json", "setup.json",
    ...
    "dev.npz", "test.npz", "costs.json", "summary.json"}
...
def validate_seed_review(seeds):
    expected_seeds = [first + case for split, regimes in FIRST.items()
                      for first in regimes.values() for case in range(CASES[split])] + list(FIT_SEEDS)
    allocations = {split: {regime: [first, first + CASES[split] - 1]
                          for regime, first in regimes.items()} for split, regimes in FIRST.items()}
    allocations["fits"] = list(FIT_SEEDS)
    require(seeds["status"] == "reserved_before_run" and seeds["hits"] == []
            and seeds["block_hits"] == [] and isinstance(seeds["files"], dict) and seeds["files"]
            and seeds["seeds"] == expected_seeds and len(set(expected_seeds)) == 39
            and seeds["allocations"] == allocations, "scoped exact fresh seed reservation")
...
def authenticate_inputs(inputs):
    ...
    validate_seed_review(read(paths["seed_review"]))
...
class Run:
    def __init__(self, args):
        ...
        self.receipt = {"version": VERSION, "status": "started", "training_updates": 0,
            "scope": "Full fixed paths; complete TRAIN/DEV/TEST teacher census. No fitting or autonomous efficacy."}
    ...
    def setup(self):
        ...
        write(self.out / "deployment.json", {"original_setup_wall_seconds": self.original_setup_wall,
            ...
            "scope": "All full paths and every teacher score in TRAIN/DEV/TEST. Deployed and annotation-only scores are distinguished. All collector schedules remain period four. No deployment-cost claim."})
    ...
    def body(self):
        ...
        costs = {"original_setup_wall_seconds": self.original_setup_wall, "feature_module_seconds": self.feature_module_seconds,
            **timings, "journal_io_seconds": self.ledger.io_seconds, "operation_seconds": self.ledger.calls,
            "scope": "Setup and each TRAIN/DEV/TEST collection and serialization are disjoint physical intervals. "
    ...
    def execute(self):
        ...
        try:
            ...
            self.receipt.update(status="completed", complete=True, files=files, calls=self.ledger.calls,
                completed_episodes=len(self.rows), train_episodes=54, dev_episodes=18, test_episodes=36,
