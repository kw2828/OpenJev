"""Present completed, independently audited scalar-return outcomes and fixed paths.
...
VERSION = 'otto-return-value-presentation-v1'
...
REGIMES = {'lambda3': {'seed': 11100001, 'lambda': 3, 'scope': 'Training-supported'},
           'lambda4': {'seed': 11200001, 'lambda': 4, 'scope': 'Training-supported'},
           'lambda5': {'seed': 11300001, 'lambda': 5, 'scope': 'Unseen supplied kernel'}}
LABELS = {'min8': 'Min-linear', 'mlp8': 'MLP', 'homogeneous8': 'Homogeneous MLP', 'analytic_inbounds': 'Analytic'}
COLORS = {'min8': '#8b47b8', 'mlp8': '#2563eb', 'homogeneous8': '#0f8b73', 'analytic_inbounds': '#c76a15'}
...
SCOPE = ('Saved independently audited aggregates, all ten arms and all three settings, with every fit seed retained. '
         'All 54 conditions apply to the minimum-linear candidate. Fixed first-case replays use saved public positions; '
...
class Render:
    ...
    def authenticate(self):
        ...
        require(result['version'] == 'otto-return-value-saved-audit-v1' and result['agreement'] is True,
    ...
    def rules(summary):
        ...
        for group, total in (('competence', 18), ('compression', 12), ('architecture', 24)):
            ...
            result[group] = {'candidate': 'min8', 'passed': passed, 'total': total,
        ...
        result['overall'] = {'candidate': 'min8', 'passed': passed, 'total': 54, 'passes': summary['pilot_continuation']}
        return result
    ...
    def chart(self, summary):
        ...
        rule_labels = {'compression': 'Utility/compute'}
        status = [f"{rule_labels.get(key, key.title())}: {'PASS' if record['passes'] else 'FAIL'} {record['passed']}/{record['total']}"
                  for key, record in rules.items()]
        fig.suptitle('OTTO: scalar values with explicit observation branches', x=.06, y=.99, ha='left', fontsize=23, weight='bold')
        fig.text(.06, .952, 'All 720 autonomous evaluations. Same analytic-policy return targets, three value families and analytic reference.', fontsize=13)
        ...
        fig.text(.06, .014, 'Move dots: eight paired blocks, no confidence intervals. Complete controller cost includes setup, branches, features and updates. One CPU run; no novelty claim.',
                 fontsize=10, color='#405168')
        fig.subplots_adjust(left=.075, right=.987, bottom=.13, top=.865, hspace=.45, wspace=.29)
        fig.savefig(self.out / 'return-value-comparison.png', dpi=180, facecolor='white')
        fig.savefig(self.out / 'return-value-comparison.svg', facecolor='white', metadata={'Date': None})
        plt.close(fig)
        write(self.out / 'plotted-values.json', {'scope': SCOPE, 'points': points, 'rules': rules,
              'input_receipts': {k: self.receipt['inputs'][k] for k in ('worker', 'audit')},
              'audited_summary': self.receipt['inputs']['audit_summary'],
              'family_means': {r: summary['regimes'][r]['family_means'] for r in REGIMES},
              'candidate': 'min8', 'fit_seeds': SEEDS, 'pilot_continuation': summary['pilot_continuation'],
    ...
    def replay_cases(self):
            ...
            for line in stream:
                ...
                if key in selected:
                    require('episode' not in selected[key] and row['initial_hit'] == 1 and row['block'] == 0
                            and row['episode_id'] == f"eval:{key[0]}:{key[1]}:{key[2]}", 'unique first case in evaluation order')
        ...
        for regime, spec in REGIMES.items():
            ...
            result[regime] = {'seed': spec['seed'], 'case_index': 0, 'initial_hit': 1, 'lambda': spec['lambda'],
                              'selection': 'First case in declared evaluation order; no outcome-based selection.',
                              'frame_steps': frame_steps, 'arms': arms}
        write(self.out / 'replay-cases.json', {'scope': SCOPE, 'cases': result,
              'source_payloads': {k: self.receipt['inputs'][k] for k in ('evaluation', 'transitions')},
              'input_receipts': {k: self.receipt['inputs'][k] for k in ('worker', 'audit')}})
        return result
    ...
    def execute(self):
        ...
        try:
            ...
            self.clock = load(ROOT / CLOCK, '_return_value_presentation_clock').SuspendClock()
