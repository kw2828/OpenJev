"""Launch exactly one frozen reflection-capacity campaign with a terminal receipt."""
...
OUTPUT = ROOT / 'output/robot-reflection-capacity-study-v1'
COMMAND = ['.venv/bin/python', '-u', 'scripts/robot_reflection_capacity_study.py',
           '--registration', 'research/robot-reflection-capacity-registration.json',
           '--output', 'output/robot-reflection-capacity-study-v1']
...
def main():
    ...
    for name in (*plan['sources'], str(REGISTRATION.relative_to(ROOT)),
        ...
        if subprocess.check_output(['git', 'show', f'{commit}:{name}'], cwd=ROOT) != (ROOT / name).read_bytes():
            raise ValueError('Source not frozen in pre-fit commit: ' + name)
    if OUTPUT.exists():
    ...
    record = {'command': COMMAND, 'prefit_commit': commit,
              ...
              'scope': 'Six fresh attempts and 36 cached fits; fixed 69-condition rule; no restart or CONFIRM/TEST access'}
