...
def main():
    ...
    result = {"version": "otto-readout-compute-capacity-v1", "status": "started", "episodes": 54, "seed": SEED,
    ...
    try:
        ...
        result["projected_seconds"] = 2 * 3 * sum(EPOCHS[row["arm"]] * row["seconds"] for row in result["arms"]) + 240
        require(result["total_updates"] == 18 and result["episode_exposures"] == 108, "complete capacity roster")
        result.update(status="passed" if result["projected_seconds"] <= 4050 else "failed_capacity", pending=None,
                      ...
                      projection_rule="2 * 3 * (74 * residual_epoch_seconds + 40 * joint_epoch_seconds) + 240 <= 4050")
        check()
