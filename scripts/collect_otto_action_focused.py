"""Complete public collection for the matched action-focused study.
...
VERSION = "otto-action-focused-collection-v1"
SELF, TEST = "scripts/collect_otto_action_focused.py", "tests/test_collect_otto_action_focused.py"
...
FIRST = {"train": {"lambda3": 291000001, "lambda4": 292000001},
         "valid": {"lambda3": 293000001, "lambda4": 294000001}}
...
CONFIGURATION = {"arms": list(ARMS), "first_seeds": FIRST, "cases": CASES, "horizon": HORIZON,
    ...
    "selection_seed_first": 296000001, "validation_annotation": "complete census",
...
NEW_COMPONENTS = {SELF, TEST,
    ...
    'src/openjev/research/otto_sampled_forecast_data.py', 'src/openjev/research/otto_action_focused_metrics.py',
    ...
    'tests/test_otto_sampled_forecast_data.py', 'tests/test_otto_action_focused_metrics.py',
    'scripts/train_otto_action_focused.py', 'tests/test_train_otto_action_focused.py',
    'scripts/audit_otto_action_focused.py', 'tests/test_audit_otto_action_focused.py',
...
def authenticate_inputs(inputs):
    ...
    expected_seeds = [first + case for split, regimes in FIRST.items()
                      for first in regimes.values() for case in range(CASES[split])] + [295000001, 295000002, 295000003] + list(range(296000001, 296000055))
...
class Run:
    ...
    def annotate_train(self, identity, history, start_row, held):
        ...
        selection = self.sample_windows(len(history), 296000001 + identity["episode_index"])
