...
SELF = "scripts/run_otto_residual_reanalysis.py"
...
PROTOCOL = "research/otto-residual-reanalysis-protocol.md"
...
CLOSED_SCREEN = "output/otto-residual-estimator-v1/dev-technical-closure-01.json"
CLOSED_SCREEN_PIN = "e5c1e13eea4c25a24a311cde34fa3cece49b80f170556079c0f638ac64c4bcb5"
ROLES = {"evaluate": ("collection_plan", "collection_receipt", "collection_terminal", "engineering", "bridge_receipt", "bridge_terminal"),
         "audit": ("producer_plan", "producer_receipt", "producer_terminal")}
COMPONENTS = {SELF, "tests/test_run_otto_residual_reanalysis.py", COLLECTOR, "tests/test_collect_otto_residual.py",
              LINEAGE, "tests/test_otto_residual_lineage.py", "scripts/qualify_otto_residual_reanalysis_capacity.py",
...
COMPONENTS |= {BRIDGE, "tests/test_otto_residual_native_bridge.py", CLOSED_SCREEN,
               "tests/test_otto_residual_reanalysis_admission.py", "scripts/qualify_otto_residual_handoff.py"}
COMPONENTS |= {f"tests/test_otto_residual_{name}.py" for name in ("contract", "features", "replay", "gate")}
CONFIG = {"data_origin": "Prior development paths from closed technical failure; separately registered reanalysis",
          "prior_confirmation_admitted": False, "phase_scope": "dev only", "episodes": 18, "master_episodes": 54, "fit_seeds": list(SEEDS),
...
def authenticate_collection(inputs, sources):
    """Consume a completed native proof without requiring its runtime here."""
    require(set(inputs) == set(ROLES["evaluate"]), "exact reanalysis evidence roles")
    verify_records(inputs)
    require(descriptor(CLOSED_SCREEN)["sha256"] == CLOSED_SCREEN_PIN,
            "preserved original technical failure")
    closed = read(CLOSED_SCREEN)
    require(closed["status"] == "DEV_TECHNICAL_FAIL" and closed["scientific_decision"] is None
            and closed["evaluation_views_completed"] == 0 and closed["old_test_access"] is False
            and same_record(closed["collection"]["receipt"], inputs["collection_receipt"])
            and same_record(closed["collection"]["original_supervisor"], inputs["collection_terminal"]),
            "explicit prior development data, not a retry of the stopped screen")
    require(sources.get(BRIDGE) == descriptor(BRIDGE)["sha256"], "qualified metadata bridge source")
    bridge = load(BRIDGE, "_residual_reanalysis_bridge")
    bridge_inputs = {role: record for role, record in inputs.items() if role != "engineering"}
    collection = bridge.verify_bridge(bridge_inputs)
    proof = read(inputs["bridge_receipt"]["path"])
    require(all(sources.get(name) == pin for name, pin in proof["sources"].items()),
            "native proof source closure contained in reanalysis qualification")
    return collection
...
def authenticate(phase, inputs):
    ...
    require(read(producer_plan["inputs"]["bridge_terminal"]["path"])["finished_ns"] <= parent["started_ns"],
            "evaluation starts after original native metadata bridge closure")
...
def close(args):
    ...
    result = {"version": VERSION, "status": "DEV_PASS" if decision["passed"] else "DEV_FAIL",
              ...
              "scope": "Reanalysis of prior development paths; no held-out, autonomous, calibration or novelty claim",
              "prior_screen_remains_closed": True, "prior_confirmation_admitted": False}
...
class Run:
    ...
    def bind(self):
        ...
        if phase == "evaluate":
            require(read(self.plan["inputs"]["bridge_terminal"]["path"])["finished_ns"]
                    <= self.launch["started_ns"], "native metadata bridge closes before evaluation starts")
...
def main():
    ...
    if args.mode in ("plan", "close"):
        class DenyNumericalImports:
            def find_spec(self, fullname, _path=None, _target=None):
                if fullname.split(".")[0] in {"numpy", "torch", "tensorflow", "jax", "mlx", "scipy"}:
                    raise ImportError("Numerical import forbidden during metadata admission: " + fullname)
        require(not ({"numpy", "torch", "tensorflow", "jax", "mlx", "scipy"} & set(sys.modules)),
                "fresh metadata-only process")
        sys.meta_path.insert(0, DenyNumericalImports())
    if args.mode == "plan":
        freeze(args)
