...
VERSION = "otto-sparse-query-render-v1"
PRODUCER = "scripts/study_otto_sparse_query.py"
AUDITOR = "scripts/audit_otto_sparse_query.py"
PRODUCER_VERSION = "otto-sparse-query-study-v1"
AUDIT_VERSION = "otto-sparse-query-saved-audit-v1"
...
class Render:
    ...
    def authenticate(self):
        ...
        require(audit["version"] == AUDIT_VERSION and audit["status"] == "completed"
            ...
            and audit["sources"] == {name: plan["sources"][name] for name in (AUDITOR, CLOCK, SUPERVISOR)}
    ...
    def figure(self, summary, metrics):
        ...
        fig.subplots_adjust(left=.065, right=.985, bottom=.25, top=.78, wspace=.13)
        ...
        offsets = {"analytic": (6, 8), "neural": (6, 8), "period2": (6, -17),
                   "random_pair": (6, 9), "entropy": (6, -16)}
        for regime, ax in zip(REGIMES, axes, strict=True):
            for row in (r for r in metrics if r["regime"] == regime):
                ...
                ax.annotate(LABELS[arm], (row["paid_controller_seconds"], y), xytext=offsets[arm],
                            textcoords="offset points", fontsize=8, color=COLORS[arm])
            if log_x:
                ax.set_xscale("log")
        ...
        fig.suptitle(f"Sparse query control: {status}, {summary['required_passed']}/16 primary conditions", fontsize=16, y=.95)
        fig.text(.5, .86, "360 fresh EVAL paths; five arms in every setting; 24 paired cases per arm/setting; full horizon 2,188", ha="center")
        fig.text(.5, .13, "Lower and left is better. Every arm retained; common axes. Star: sole primary candidate, period2.\n"
        ...
        fig.text(.5, .025, f"Physical study {self.times['study_parent_seconds']:.2f}s; saved audit {self.times['audit_parent_seconds']:.2f}s. "
    ...
    def execute(self):
        ...
        try:
            ...
            self.receipt.update(status="completed", files=files, inputs=self.bound, plotted_cells=15, block_rows=120,
