...
SELF = 'scripts/qualify_otto_action_effect.py'
SOURCE_REVIEW = 'output/otto-action-effect-v1/source-review-01.json'
SOURCE_REVIEW_PIN = '54af9f53634457d3c420c569c67e8eda9f7b91f29c11869bf5160c47c3ae5d39'
SEED_REVIEW = 'output/otto-action-effect-v1/seed-review-01.json'
SEED_REVIEW_PIN = 'c73c9f4fa5ae906506a9c481582ece094015ef85de5b2a464c96afa63037f757'
...
def source_evidence():
    ...
    c.require(evidence['version'] == 'otto-action-effect-source-review-v1'
    ...
    c.require(seeds['passed'] is True and seeds['proposed_unique_seeds'] == 259
...
def capacity(path):
    ...
    import audit_otto_action_effect as audit
    import run_otto_action_effect as runner
    ...
    keys = ('prefix', 'prefix_lengths', 'actions', 'continuation', 'outcomes', 'raw_costs', 'gap_oracle', 'normal_oracle', 'opposite_oracle')
    tensors = {k: torch.from_numpy(train[k]) for k in keys}
    times, inference, scoring, independent, updates = {}, {}, {}, {}, {}
    reports, sensitivities, independent_reports, independent_sensitivities = [], [], [], []
    ...
    for family in runner.FAMILIES:
        ...
        for start in range(0, n, batch_size):
            ...
            loss = runner.paired_loss(gap, normal, opposite, batch, torch, .01,
                                      effect_variance=1e-6, family=family)
        ...
        for condition in ('gap', 'normal'):
            report = metrics.score(dev['outcomes'], predictions[condition], costs[condition], dev['raw_costs'], dev['legal'],
            ...
            reports.extend({**report, 'fit_seed': seed} for seed in c.CONFIG['fit_seeds'])
        report = metrics.action_sensitivity(dev['gap_oracle'], dev['opposite_oracle'],
        ...
        sensitivities.extend({'family': family, 'fit_seed': seed, 'report': report} for seed in c.CONFIG['fit_seeds'])
        scoring[family] = time.perf_counter() - tick
        tick = time.perf_counter()
        for condition in ('gap', 'normal'):
            report = audit.scalar_report(np, dev, predictions[condition], costs[condition], family=family,
                                         fit_seed=c.CONFIG['fit_seeds'][0], condition=condition, check=resource_check)
            independent_reports.extend({**report, 'fit_seed': seed} for seed in c.CONFIG['fit_seeds'])
        report = audit.sensitivity_report(np, dev, predictions['gap'], predictions['opposite'], check=resource_check)
        independent_sensitivities.extend({'family': family, 'fit_seed': seed, 'report': report} for seed in c.CONFIG['fit_seeds'])
        independent[family] = time.perf_counter() - tick
    options = {'candidate': c.CONFIG['candidate'], 'controls': c.CONFIG['controls'],
               'fit_seeds': c.CONFIG['fit_seeds'], 'regimes': ('lambda3', 'lambda4'), 'thresholds': runner.THRESHOLDS}
    tick = time.perf_counter()
    gate = metrics.evaluate_reports(reports, sensitivities, **options)
    producer_gate_seconds = time.perf_counter() - tick
    tick = time.perf_counter()
    independent_gate = audit.mechanism_gate(independent_reports, independent_sensitivities, **options)
    independent_gate_seconds = time.perf_counter() - tick
    audit.close_equal(gate, independent_gate, 'fabricated full-size gate agreement')
    ...
    fit_projection = 2 * (seeds * (epochs * sum(times.values()) + sum(inference.values()) + sum(scoring.values())) + producer_gate_seconds) + 180
    audit_projection = 2 * (seeds * sum(independent.values()) + oracle_seconds + independent_gate_seconds) + 180
    ...
    c.write(path, {'status': 'passed' if passed else 'failed', 'one_epoch_seconds': times,
        ...
        'producer_gate_seconds': producer_gate_seconds, 'independent_gate_seconds': independent_gate_seconds,
        'fabricated_gate_reports_replicated_across_seed_ids': True,
