...
from render_finite_convex_readout import (
    ROOT,
    authenticate,
    descriptor,
    inventory,
...
VERSION = 'finite-convex-readout-package-v1'
...
def package(studyfolder, reportfolder, outputfolder, *, registration_sha256, commit=None, publication_paths=()):
    ...
    require(render_receipt['version'] == 'finite-convex-readout-report-v1'
    ...
    renderer_path = regular(ROOT / 'scripts/render_finite_convex_readout.py')
    ...
    for engineering_plan in engineering_plans:
        ...
        if engineering_plan == qualification_plan:
            ...
            require(terminal['cwd'] == str(ROOT) and command[0] == record['runtime']['executable']
                    and (ROOT / command[1]).resolve() == ROOT / 'scripts/finite_convex_readout_worker.py'
    ...
    for name in ('README.md', 'LICENSE', 'scripts/render_finite_convex_readout.py', 'scripts/package_finite_convex_readout.py'):
    ...
    manifest = {'version': VERSION, 'study': STUDY_NAME, 'registration': descriptor(plan_path),
        ...
        'scope': 'All registered sources; all current closed producer/audit payloads including nine complete solves and state caches, original TRAIN copy, fresh development endpoint/prefix arrays, oracle controls, predictions and journals; original receipts/launches/terminals/logs; every named engineering attempt with source snapshots; complete pinned parent-study historical evidence; completed report. No new recurrent checkpoint is trained, and failed attempts remain failures.',
        'member_mapping': 'study/ preserves current study-relative names; upstream/finite-factorized-dynamics-v1/ preserves explicitly pinned parent-study names; sources/ preserves repository source paths; publication/ contains named presentation artifacts. Absolute original paths remain provenance metadata only.',
