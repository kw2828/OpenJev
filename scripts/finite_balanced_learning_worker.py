...
def validate_launch_binding(plan, receipt):
    ...
    expected = [plan['runtime']['executable'],
                str(Path(plan['root']) / 'scripts/finite_balanced_learning_worker.py'),
...
def main():
    ...
    try:
        ...
        require(plan['version'] == 'finite-balanced-learning-v1', 'registered study version')
        ...
        if args.phase == 'qualify':
            ...
            from run_finite_balanced_learning import run
