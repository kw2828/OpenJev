"""Prepare and package a closed cross-query release using opaque saved bytes.
...
SELF = "scripts/package_otto_cross_query_forecasts.py"
...
PLOTTER = "scripts/plot_otto_cross_query_forecasts.py"
STUDY = ROOT / "output/otto-cross-query-forecast-v1"
PACKAGE = ROOT / "output/otto-cross-query-package-v1"
VERSION = "otto-cross-query-package-v1"
LIMITS = {"seconds": 300, "rss_bytes": 1024**3, "output_bytes": 6 * 1024**3}
NAME = "openjev-otto-cross-query-forecast-v1.tar.gz"
...
REQUIRED_PUBLICATIONS = {"research/otto-cross-query-forecast-results.md",
                         "docs/assets/otto-cross-query-forecast.png"}
...
def prepare(args):
    ...
    try:
        ...
        def closed(role):
            record, path = records[role], roles[role]
        ...
        for phase in ("collection", "training"):
            ...
            require(worker["complete"] is True and worker["sources"] == plan["sources"]
            ...
            closed(phase + "_receipt")
            executable = ".venv-otto-released-native/bin/python" if phase == "collection" else ".venv/bin/python"
            script = "collect_otto_cross_query_forecasts.py" if phase == "collection" else "train_otto_cross_query_forecasts.py"
        ...
        require(audit["agreement"] is True and audit["version"] == "otto-cross-query-forecast-saved-audit-v1", "independent audit agreement")
        closed("audit_receipt")
        ...
        options = parent_join(audit, records["audit_terminal"], [str(ROOT / ".venv/bin/python"),
            str(ROOT / "scripts/audit_otto_cross_query_forecasts.py")], roles["training_plan"],
        ...
        require(figure["status"] == "completed", "finished publication figure")
        closed("figure_receipt")
        ...
        for phase in ("collection_plan", "training_plan"):
            for source, pin in records[phase]["sources"].items():
        ...
        for name in publications:
            add(name)
        for artifact in ("forecast", "conditions"):
            for suffix in ("png", "svg"):
                canonical = f"docs/assets/otto-cross-query-{artifact}.{suffix}"
                if canonical in {relative(p) for p in publications}:
                    item = figure["files"][f"cross-query-{artifact}.{suffix}"]
        ...
        restore = ("# Restoring the cross-query forecast evidence\n\n"
            ...
            "The external inventory and packaging receipt are outside the tar to avoid self-reference. Technical closure is separate from the scientific 53-condition result.\n")
...
def archive(args):
    ...
    spec = importlib.util.spec_from_file_location("_cross_query_opaque_packager", ROOT / ENGINE)
