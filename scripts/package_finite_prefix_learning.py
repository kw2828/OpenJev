"""Package an authenticated closed prefix-supervision study by opaque, deterministic bytes.
...
from render_finite_prefix_learning import (
    ROOT,
    authenticate,
    descriptor,
    inventory,
...
VERSION = 'finite-prefix-learning-package-v1'
STUDY_NAME = 'finite-prefix-learning-v1'
...
def package(studyfolder, reportfolder, outputfolder, *, registration_sha256, commit=None, publication_paths=()):
    ...
    require(render_receipt['version'] == 'finite-prefix-learning-report-v1'
    ...
    renderer_path = regular(ROOT / 'scripts/render_finite_prefix_learning.py')
    ...
    for engineering_plan in engineering_plans:
        ...
        if engineering_plan == qualification_plan:
            ...
            require(terminal['cwd'] == str(ROOT) and command[0] == record['runtime']['executable']
                    and (ROOT / command[1]).resolve() == ROOT / 'scripts/finite_prefix_learning_worker.py'
    ...
    for name in ('README.md', 'LICENSE', 'scripts/render_finite_prefix_learning.py', 'scripts/package_finite_prefix_learning.py'):
