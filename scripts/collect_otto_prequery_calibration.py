"""Complete public collection for the matched prequery-calibration study.
...
VERSION = "otto-prequery-calibration-collection-v1"
SELF, TEST = "scripts/collect_otto_prequery_calibration.py", "tests/test_collect_otto_prequery_calibration.py"
...
FIRST = {"train": {"lambda3": 271000001, "lambda4": 272000001},
         "valid": {"lambda3": 273000001, "lambda4": 274000001}}
...
CONFIGURATION = {"arms": list(ARMS), "first_seeds": FIRST, "cases": CASES, "horizon": HORIZON,
    ...
    "selection_seed_first": 276000001, "validation_annotation": "complete census",
...
NEW_COMPONENTS = {SELF, TEST, "src/openjev/research/otto_prequery_scores.py",
...
def authenticate_inputs(inputs):
    ...
    expected_seeds = [first + case for split, regimes in FIRST.items()
                      for first in regimes.values() for case in range(CASES[split])] + [275000001, 275000002, 275000003] + list(range(276000001, 276000055))
...
class Run:
    ...
    def annotate_train(self, identity, history, start_row, held):
        ...
        selection = self.sample_windows(len(history), 276000001 + identity["episode_index"])
