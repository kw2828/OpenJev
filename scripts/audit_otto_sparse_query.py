...
SELF = "scripts/audit_otto_sparse_query.py"
...
VERSION = "otto-sparse-query-saved-audit-v1"
...
class Audit:
    ...
    def admit(self):
        ...
        self.require(self.plan["version"] == PRODUCER_VERSION and self.plan["status"] == "frozen_before_validation"
            and self.plan["sources"][SELF] == self.digest(SELF)["sha256"]
        ...
        self.receipt.update(plan_sha256=self.args.plan_sha256, producer_inputs=self.inputs,
            ...
            sources={name: self.plan["sources"][name] for name in (SELF, CLOCK, SUPERVISOR)})
    ...
    def costs(self, rows):
        ...
        self.require(all(type(c[k]) in (int, float) and math.isfinite(c[k]) and c[k] >= 0 for k in c if k.endswith("seconds")), "finite physical stage costs")
...
def main():
    ...
    if not all(getattr(args, key).is_absolute() for key in ("plan", "worker", "terminal", "supervision", "output")):
